...
# DISPLAY_ROTATION, HOSTNAME, ENABLE_FB_SPLASH, ENABLE_BOOT_SPLASH). Label printing is OFF by default (ENABLE_PRINTING
...
# An explicit HOSTNAME= in the environment still wins; the provisioner also
# appends -2, -3, ... if pr.local is already taken on the network.
HOSTNAME_CHOICE="${HOSTNAME:-pr}"
