...
# Total system RAM in whole MB (0 if unreadable). FORCE_MEM_MB overrides it for
# tests and manual runs. Used to pick the Mealie default per board: a 2 GB Pi
# runs Pantry Raider + Grocy comfortably, but Mealie pushes the floor to 4 GB.
detect_ram_mb() {
  if [ -n "${FORCE_MEM_MB:-}" ]; then printf '%s' "$FORCE_MEM_MB"; return 0; fi
  local kb=""
  [ -r /proc/meminfo ] && kb="$(awk '/^MemTotal:/ {print $2; exit}' /proc/meminfo 2>/dev/null)"
  if [ -n "$kb" ] && [ "$kb" -gt 0 ] 2>/dev/null; then printf '%s' "$((kb / 1024))"; else printf '0'; fi
}
# RAM floor (MB) below which Mealie defaults OFF. See firstboot.sh MEALIE_MIN_MB.
  ...
  if [ -z "$ENABLE_MEALIE" ]; then
    if [ "$DEPLOYMENT_MODE" = "pi_remote" ]; then
      ...
      if [ "$_ram_mb" -gt 0 ] 2>/dev/null && [ "$_ram_mb" -lt "$MEALIE_MIN_MB" ]; then
        # Low-RAM board (2 GB Pi): ship Grocy only. Mealie can be turned on later
        # on a 4 GB+ board from Settings or 'docker compose --profile with-mealie up -d'.
        ENABLE_MEALIE=false
        say "Low RAM (${_ram_mb} MB): installing Pantry Raider + Grocy only. Add Mealie later on a 4 GB+ board."
      else
        ENABLE_MEALIE=true
      fi
    fi
