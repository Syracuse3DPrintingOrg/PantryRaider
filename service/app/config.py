...
def persistent_submenus_default(deployment_mode: str, ui_scale: str) -> bool:
    """Resolve the default for keeping a nav section's submenus open on its pages.

    When on, the section you are in (say Kitchen Guide) keeps all of its
    sub-items in view while you are on any of its pages, instead of tucking them
    back into a collapsed dropdown. On the web there is room for that, so it
    defaults on. On a Pi appliance it defaults on only at the small interface
    scale, where more fits on screen; larger kiosk scales keep the tidy
    collapsed menus so the big buttons stay uncluttered. A per-device choice in
    the browser overrides this either way. Pure, like the helpers above, so it
    is unit-testable and reused by the template context."""
    if deployment_mode in ("pi_hosted", "pi_remote"):
        return ui_scale == "small"
    return True
