...
class Settings(BaseSettings):
    ...
    # cheesecake is not filed as a room-temperature pantry item. Default off;
    # when off, shelf life and storage come from the built-in category rules as
    # before. Needs a configured provider (does nothing without one).
    ...
    # _start_first(). start_page_mode picks what /ui/start renders: "glance" is an
    # auto-generated home (large square buttons to the top-level nav pages plus a
    # row of notification pills), "custom" is the arrange-your-own launcher built
    # from start_page_layout. Existing installs keep whatever they saved; only the
    # fresh-install default changed.
    ...
    # width and height equal for them; round also draws a circular safe-area
    # guide on the design canvas so text and the logo stay inside the printable
    # circle. The rendered raster stays rectangular either way (thermal printers
    # need that); the shape guides layout, it does not mask the image.
    ...
    # space with the name and dates. Device-local, like the label size and
    # design above (a satellite prints through the server; see FoodAssistant-
    # eml9), so this is deliberately NOT in SATELLITE_PULL_FIELDS.
    ...
    # set, the food label renders through the layout engine instead. Device-local
    # on purpose (a printer and its stock live on one device), so it is NOT in
    # SATELLITE_PULL_FIELDS. Stored as a string so a malformed value can never
    # break settings loading; the renderer validates and falls back on parse.
