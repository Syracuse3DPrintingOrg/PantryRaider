...
async def redirect_if_unconfigured(request: Request, call_next):
    ...
    if not settings.is_configured():
        # First-boot readiness gate (FoodAssistant-0m61): on a brand-new Pi
        # appliance the app serves minutes before its co-hosted Grocy does, so
        # instead of landing on a wizard that cannot finish, browsers land on a
        # live progress page until the inventory service answers once. The gate
        # is sticky-off: services/readiness remembers the first answer (and a
        # user dismissal), so it can never reappear later.
        from .services import readiness
        if request.url.path == "/setup" and request.method == "GET":
            if request.query_params.get("skip_ready") == "1":
                # The getting-ready page's escape hatch: remember the choice
                # so the wizard is not bounced straight back.
                readiness.dismiss()
...
async def require_auth(request: Request, call_next):
    ...
    if (request.url.path.startswith("/cub/firmware/") and request.method == "GET"
            and request.client
            and pairing_svc.is_private_address(request.client.host)):
        return await call_next(request)
