...
from .routers import analyze, defaults, inventory, expiring, ui, setup, pending, mealie, admin, qr, tunnel, grocy, satellite, proxy, devices, current_recipe, events, action_items, nutrition, audit, affiliate, printing, cook_wizard, recipes, gadgets
...
def _satellite_recipe_upstream(path: str) -> str | None:
    """The server base URL when this request is native-recipe data a satellite
    must forward, else None."""
    ...
    try:
        ...
        return JSONResponse(
            {"detail": "The main server is not reachable. "
                       "Recipes will return when it is."},
            status_code=502,
        )
