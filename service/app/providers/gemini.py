...
from .base import (VisionProvider, format_recipe_for_prompt, parse_json_response,
                   _OPTIMIZE_RECIPE_PROMPT, _PARSE_INGREDIENTS_PROMPT,
                   format_ingredient_lines)
...
When the source groups its ingredients under headings (for example "Meat sauce",
"For the topping", "Pasta"), return "ingredients" instead as an ordered list of
section objects, each heading with its own lines:
  "ingredients": [
    {{"section": "Meat sauce", "items": ["1 lb ground beef", "..."]}},
    {{"section": "Topping", "items": ["1 cup breadcrumbs", "..."]}}
  ]
Use the flat string list when the source has no such headings, and never invent a
heading that is not written on the source.
When the handwriting or print is ambiguous, prefer the most common, plausible
cooking ingredient or term over a literal misreading (for example, read an
unclear "sappron sheds" as "saffron threads"). Spell well-known ingredients the
standard way, and spell each one the same way every time it appears, in both the
ingredients and the instructions. If part is still illegible, make your best
...
class GeminiProvider(VisionProvider):
    ...
    async def extract_receipt_prices(self, image_data: bytes,
        ...
        image_part = {"mime_type": mime_type, "data": image_data}
        response = await self._gen([build_prices_prompt(), image_part])
    ...
    async def identify_barcode(self, barcode: str) -> dict | None:
        ...
        response = await self._gen([BARCODE_IDENTIFY_PROMPT.format(barcode=barcode)])
        return parse_json_response(response.text)
    ...
    async def parse_ingredients(self, lines: list[str]) -> list[dict] | None:
        ...
        response = await self._gen([prompt])
        return parse_json_response(response.text).get("ingredients", [])
