...
(function () {
  ...
  // Prefer the consolidated kiosk poll: read presence from its status.activity
  // slice (the same host-bridge shape this fetched directly). When the field is
  // absent (the server omits the admin-gated activity for a remote viewer
  // session, exactly as /setup/kiosk/activity 403s that caller today) apply()
  // unmounts, so the icon stays hidden just as before. Fall back to the
  // dedicated activity poll on a page or cached copy without the shared loop.
  if (window.PRKioskStatus) {
    // While the sensor has not fired (nothing mounted), ask the shared loop for
    // no more than the slow backoff cadence, so a sensor-less kiosk never pins
    // the whole poll at 2s. apply() returns true once a mountable answer lights
    // the icon, which pulls the cadence back to POLL_MS, mirroring the old
    // standalone poll's snap-back on a mountable answer.
    var wantFast = false;
    window.PRKioskStatus.subscribe(
      function (s) { wantFast = apply(s && s.activity); },
      { interval: function () { return wantFast ? POLL_MS : BACKOFF_MAX_MS; } });
  } else {
