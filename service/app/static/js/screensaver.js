...
(function () {
  ...
  // The old drift was a fixed scale(1.12) with a tiny 6% pan over the whole
  // photo, which read as nearly still on a small panel across a kitchen, so
  // "normal" is deliberately larger now and the control lets it go bolder.
  var KEN_BURNS_MAG = { slow: 0.55, normal: 1, fast: 1.7 };
  var KEN_BURNS_SPEED =
  ...
  function kenBurnsFrame(speed) {
    ...
    var pan = 5 * mag;                 // max offset from centre, percent
