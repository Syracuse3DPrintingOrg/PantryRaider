...
(function () {
  ...
  // Inside the split view's panes (any same-origin frame) the WRAPPER page
  // owns the screensaver, so its overlay spans the whole display: a framed
  // page never starts its own idle saver over just one pane, and never runs
  // its own external-wake poll (FoodAssistant-dqpq). The test hook still
  // works everywhere the script loads.
  var FRAMED = false;
  ...
  if (!FRAMED && (kiosk || ALL_CLIENTS) && IDLE_MS > 0) {
  ...
  if (!FRAMED && (kiosk || ALL_CLIENTS)) {
    ...
    // The shared poll only fetches the host-bridge activity slice when a
    // subscriber asks for it, so riding along means claiming it. This
    // registers no interval (the saver keeps its own EXTERNAL_POLL_MS
    // cadence) and no real callback: it exists purely to turn the field on.
    // Without it the ride-along below would read undefined forever and the
    // saver would stop noticing activity on other surfaces, quietly.
    var claimed = false;
    var claimActivity = function () {
      if (claimed || !window.PRKioskStatus || !window.PRKioskStatus.subscribe) return;
      window.PRKioskStatus.subscribe(function () {}, { wants: ['activity'] });
      claimed = true;
    };

    var pollExternalActivity = function () {
      // Ride the consolidated kiosk poll's activity slice (same host-bridge
      // shape) whenever the shared loop exists on the page. The claim above
      // makes that loop fetch the slice, so it WILL arrive; until its first
      // poll lands, last() is empty and the right move is to wait one tick.
      //
      // This used to fall back to a direct /setup/kiosk/activity fetch when
      // last() was empty, and a fresh document ALWAYS starts with an empty
      // last(), so every navigation on every kiosk paid a host-bridge round
      // trip (measured at 460 ms on a Pi 4) for a value the shared poll was
      // about to deliver anyway. The direct fetch is now only for pages that
      // have no shared loop at all.
      claimActivity();
      if (window.PRKioskStatus) {
        var shared = window.PRKioskStatus.last();
        if (shared) processExternal(shared.activity || null);
        return;
      }
