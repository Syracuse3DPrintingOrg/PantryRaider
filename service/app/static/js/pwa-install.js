...
(function () {
  ...
  // A kiosk (the wall display) is not where anyone installs the app. On the
  // Pi's own screen we still register the beforeinstallprompt handler so it can
  // call preventDefault() and SUPPRESS the browser's native install prompt,
  // which was popping up over the kiosk as a modal and swallowing touch/scroll
  // (FoodAssistant-xq5d follow-up). We just never show our own install bar
  // there. Gate on kiosk mode, not the URL: every app page lives under /ui/, so
  // a path check would suppress the prompt everywhere it should appear.
  var suppressOnly = false;
  try {
    suppressOnly = localStorage.getItem("kioskMode") === "true";
  } catch (e) { /* private mode: fall through and offer the install */ }

  // Belt and braces (FoodAssistant-r4kt): the latch above is a browser-storage
  // flag, and a panel whose storage was cleared once came up unlatched and
  // offered to install the app onto the appliance running it. A Pi serving its
  // own display over loopback is that display, latch or no latch, and nobody
  // installs a home-screen shortcut there. Only loopback counts: a phone
  // browsing the same Pi is a real install candidate and must keep the offer.
  try {
    var _host = window.location.hostname;
    var _loopback = _host === "localhost" || _host === "127.0.0.1" || _host === "::1";
    if (_loopback && document.documentElement.getAttribute("data-is-pi") === "1") {
      suppressOnly = true;
    }
  } catch (e) { /* no DOM attribute: keep the latch's answer */ }

  window.addEventListener("beforeinstallprompt", function (e) {
    e.preventDefault();          // never let the browser show its own prompt
    if (suppressOnly) return;    // on the kiosk, suppress and stop here
    deferredPrompt = e;
    build("prompt");
  });
  ...
  if (suppressOnly) return;      // no manual/iOS install bar on the kiosk either
