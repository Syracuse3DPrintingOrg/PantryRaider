...
//     wants is an optional list of opt-in expensive fields:
//     'expiring' (the Grocy-backed count the Start page needs),
//     'activity' (a host-bridge read that costs a connection per tick, so
//     only the presence indicator and the screensaver ask for it) and
//     'scanner_mode' (which on a satellite is a forward to the main server,
//     so only Manage Pantry asks for it). Returns an
...
(function () {
  ...
  function buildUrl() {
    ...
    if (wantFlags().scanner_mode) q += '&scanner=1';
    if (wantFlags().activity) q += '&activity=1';
