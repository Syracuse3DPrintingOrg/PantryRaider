...
(function () {
  ...
  if (!shouldRun) {
    ...
    clearBlackout();
    // Mark the intro as spent for this tab session even though it did not
    // play: any page load means the boot moment has passed, so a kiosk flag
    // latched later in the session must not replay the intro on the next
    // navigation (it read as a splash between menus, FoodAssistant-9dhh).
