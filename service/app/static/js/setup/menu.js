...
// The Stream Deck menu entry (Devices group, Pi only): the deck editor is a
// sub-area of the Start Page pill with no pill of its own, so this opens that
// pill and lands on the deck side. Setting the hash first lets the pill's
// shown handler pick the deck too, so the two paths agree.
function openStreamDeckSettings() {
  ...
  if (typeof _sdLoadProfiles === 'function') _sdLoadProfiles();
  if (typeof showDeckStart === 'function') showDeckStart('deck');
