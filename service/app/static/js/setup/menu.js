...
function _showAllMenuItems() {
  document.querySelectorAll('.side-menu [data-bs-toggle="pill"], .side-menu .menu-heading, .side-menu .menu-static-link')
    .forEach(function (el) { el.classList.remove('d-none'); });
}

// The Stream Deck menu entry (Devices group, Pi only, FoodAssistant-gn6g):
// the ONE way into the deck editor, which lives as a sub-area of the Start
// Page pane with no pill of its own. Setting the hash first lets the pill's
// shown handler land on the deck side once the pane transition completes;
// flipping the sub-area directly here mid-transition raced Bootstrap's own
// class changes, which could leave the click looking like a dead end. When
// the pill is already active no shown event fires, so flip directly then.
// Works with no deck attached: the pane's connected toggle still renders.
function openStreamDeckSettings() {
  // Path spelled out on purpose; see the note at the hash update below.
  const here = window.location.pathname + window.location.search;
  try { history.replaceState(null, '', here + '#pane-streamdeck'); } catch (e) { }
  const pill = document.getElementById('pill-deckstart');
  if (!pill) return;
  // The editor's script lazy-loads on first use (FoodAssistant-0hwez), so
  // everything it defines is called through loadPaneScript's then.
  if (pill.classList.contains('active')) {
    loadPaneScript('deck-editor.js', function () {
      try { showDeckStart('deck'); } catch (e) { }
    });
  } else {
    try { bootstrap.Tab.getOrCreateInstance(pill).show(); }
      ...
      pill.click();
      loadPaneScript('deck-editor.js', function () {
        try { showDeckStart('deck'); } catch (e2) { }
      });
    }
  }
  loadPaneScript('deck-editor.js', function () {
    if (typeof _sdLoadProfiles === 'function') {
      try { _sdLoadProfiles(); } catch (e) { }
    }
  });
...
function settingsSearch(q) {
  ...
  // The static shortcut entries (Stream Deck) have no pane of their own to
  // match, so they hide with the headings.
  document.querySelectorAll('.side-menu .menu-heading, .side-menu .menu-static-link')
...
// Panes whose script lazy-loads on first open (FoodAssistant-0hwez); see
// loadPaneScript in helpers.js. Keyed by the pill's pane target. The shown
// handler below loads the file however the pane was reached (pill click, a
// #pane-* hash, a search hit, openSettingsPane), so the pane's buttons always
// have their functions by the time they can be pressed. cameras-ha.js serves
// two panes: the camera tools live on Connections and the Home Assistant
// helpers on its own pane.
const PANE_LAZY_SCRIPTS = {
  'pane-start-page': 'deck-editor.js',
...
(function init() {
  ...
  document.querySelectorAll('.side-menu [data-bs-toggle="pill"]').forEach(p => {
    p.addEventListener('shown.bs.tab', () => {
      const lazy = PANE_LAZY_SCRIPTS[(p.getAttribute('data-bs-target') || '').slice(1)];
      if (lazy) loadPaneScript(lazy);
      if (p.id !== 'pill-deckstart') {
        const deck = document.getElementById('pane-streamdeck');
        if (deck) deck.classList.remove('active', 'show');
      }
      // Keep the URL hash on the open pane (FoodAssistant-wcc4). Several saves
      // reload the page (theme, display, background, start page, navigation);
      // without a hash the reload landed back on the Overview pane. With the
      // hash always current, the activation below reopens the same pane after
      // any reload. replaceState avoids piling history entries and page jumps.
      // A raw #pane-streamdeck hash is kept while its host pill activates so
      // the deck-editor bookmark below still lands on the deck.
      const target = p.getAttribute('data-bs-target') || '';
      if (target.startsWith('#pane-')
          && !(p.id === 'pill-deckstart' && window.location.hash === '#pane-streamdeck')
          && window.location.hash !== target) {
        // Spell the path out (FoodAssistant-6upk). A bare "#pane-x" is a
        // relative URL, so replaceState resolves it against this page's
        // <base href="<ingress path>/"> and rewrites the address to the site
        // root. The saves that reload (theme, display, background) then came
        // back on the home page instead of the pane the user was editing.
        const here = window.location.pathname + window.location.search;
        try { history.replaceState(null, '', here + target); } catch (e) { }
      }
    });
  });
  // Opening the Start Page pill programmatically (a hash or a
  ...
  // Each initializer below serves its own card and none depends on the ones
  // after it, so run them independently: a throw in one (an element missing
  // on the first-run wizard page, say) must not silence the rest. The
  // wizard's "Continue with Google" button was once hidden exactly this way,
  // by an earlier step failing before _initCloudMeta() ran.
  var _step = function (fn) { try { fn(); } catch (e) { } };
  _step(showProvider);
  _step(showRecipeSource);
  _step(scannerTypeChanged);
  _step(renderNavEditor);
  if (!IS_SATELLITE) {
    _step(tunnelModeChanged);
    _step(_initTunnelStatus);
  }
...
document.addEventListener('DOMContentLoaded', loadApConfig);
