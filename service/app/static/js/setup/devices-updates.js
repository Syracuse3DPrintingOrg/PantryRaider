  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (r.ok) {
        ...
        if (v) {
          if (outEl) { outEl.className = 'test-result text-secondary'; outEl.textContent = 'The app is back on v' + v + ' (it may already have been up to date).'; }
          return;
        }
      }
