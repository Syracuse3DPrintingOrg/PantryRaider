...
// Bandit Cubs (docs/design/bandit-cub.md): the paired-Cub cards and the
// fleet-wide content settings in the Devices pane. A Cub registers itself on
// its first summary poll, so the list needs no manual add.
// Last-fetched Cubs by device id, so the per-Cub view select can merge its
// change into the device's stored overrides instead of clobbering them.
let _cubDevices = {};

function cubCardHtml(c) {
  const id = escapeHtml(c.device_id || '').replace(/'/g, "\\'");
  ...
  const fw = (c.firmware_version ? escapeHtml(c.firmware_version) : '--') + vbadge;
  const profile = c.hardware_profile ? escapeHtml(c.hardware_profile) : '--';
  const seen = c.last_seen ? escapeHtml(c.last_seen) : '--';
  const view = (c.overrides && c.overrides.default_view) || '';
  const opt = (v, label) =>
    `<option value="${v}"${view === v ? ' selected' : ''}>${label}</option>`;
  // The firmware override is a bool, so its "same as every Cub" empty state is
  // the override key simply not being there.
  const auto = (c.overrides && typeof c.overrides.auto_update === 'boolean')
    ? (c.overrides.auto_update ? 'yes' : 'no') : '';
  const autoOpt = (v, label) =>
    `<option value="${v}"${auto === v ? ' selected' : ''}>${label}</option>`;
  return `<div class="border rounded p-2 mb-2">
    <div class="d-flex flex-wrap align-items-center gap-2">
      ${online}
      <input type="text" class="form-control form-control-sm" style="max-width:200px"
        id="cub-name-${id}" value="${escapeHtml(c.name || '')}" placeholder="Name this Cub"
        onchange="renameCub('${id}')">
      <span class="small text-secondary">${profile}</span>
      <span class="small">${fw}</span>
      <span class="small text-secondary ms-auto">Last seen: ${seen}</span>
      <button class="btn btn-outline-danger btn-sm" onclick="removeCub('${id}')" title="Remove this Cub"><i class="bi bi-trash"></i></button>
    </div>
    <div class="d-flex align-items-center gap-2 mt-2">
      <label class="small text-secondary mb-0" for="cub-view-${id}">Idle view</label>
      <select class="form-select form-select-sm" style="max-width:220px"
        id="cub-view-${id}" onchange="setCubView('${id}', this)">
        ${opt('', 'Same as every Cub')}
        ${opt('expiring', 'Expiring items')}
        ${opt('rotation', 'Rotate through views')}
        ${opt('clock', 'Clock')}
      </select>
      <label class="small text-secondary mb-0 ms-2" for="cub-auto-${id}">Firmware updates</label>
      <select class="form-select form-select-sm" style="max-width:200px"
        id="cub-auto-${id}" onchange="setCubAutoUpdate('${id}', this)">
        ${autoOpt('', 'Same as every Cub')}
        ${autoOpt('yes', 'Keep up to date')}
        ${autoOpt('no', 'Leave alone')}
      </select>
      <span class="test-result small" id="cub-edit-result-${id}"></span>
    </div>
  </div>`;
}

async function loadCubs() {
  const out = document.getElementById('cubs-list');
  if (!out) return;
  try {
    const r = await fetch('cub/devices');
    const d = await r.json();
    const cubs = d.devices || [];
    _cubDevices = {};
    cubs.forEach((c) => { _cubDevices[c.device_id] = c; });
    if (!cubs.length) {
      out.innerHTML = '<span class="text-secondary small">No Cubs paired yet. <a href="ui/cubs">Flash a new Cub</a> to set one up: plug it in over USB, flash from the browser, then approve it here.</span>';
      return;
    }
    out.innerHTML = cubs.map(cubCardHtml).join('');
  } catch (e) {
    out.innerHTML = `<span class="text-danger small">${escapeHtml(String(e))}</span>`;
  }
}

async function _editCub(id, body, resultId) {
  const el = document.getElementById(resultId);
  try {
    const r = await fetch(`cub/devices/${encodeURIComponent(id)}`, {
      method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify(body),
    });
    const d = await readJsonResponse(r);
    if (el) {
      el.className = 'test-result small ' + (d.ok ? 'text-success' : 'text-danger');
      el.textContent = d.ok ? 'Saved.' : (d.error || 'Could not save.');
    }
    return !!d.ok;
  } catch (e) {
    if (el) { el.className = 'test-result small text-danger'; el.textContent = e.message || 'Could not save.'; }
    return false;
  }
}

function renameCub(id) {
  const inp = document.getElementById(`cub-name-${id}`);
  if (!inp) return;
  _editCub(id, { name: inp.value.trim() }, `cub-edit-result-${id}`);
}

function setCubView(id, sel) {
  // Merge into the stored overrides so future per-Cub settings survive; an
  // empty choice removes the override and the Cub follows the global view.
  const overrides = Object.assign({}, (_cubDevices[id] && _cubDevices[id].overrides) || {});
  if (sel.value) overrides.default_view = sel.value;
  else delete overrides.default_view;
  if (_cubDevices[id]) _cubDevices[id].overrides = overrides;
  _editCub(id, { overrides }, `cub-edit-result-${id}`);
}

function setCubAutoUpdate(id, sel) {
  // Same merge as the view above: an empty choice drops the override and this
  // Cub follows whatever the fleet-wide switch says.
  const overrides = Object.assign({}, (_cubDevices[id] && _cubDevices[id].overrides) || {});
  if (sel.value) overrides.auto_update = sel.value === 'yes';
  else delete overrides.auto_update;
  if (_cubDevices[id]) _cubDevices[id].overrides = overrides;
  _editCub(id, { overrides }, `cub-edit-result-${id}`);
}

async function removeCub(id) {
  if (!confirm('Remove this Cub? It reappears here on its next check-in.')) return;
  try {
    await fetch(`cub/devices/${encodeURIComponent(id)}`, { method: 'DELETE' });
  } catch (e) { /* best effort */ }
  loadCubs();
}

function saveCubSettings(btn) {
  return savePane({
    cub_default_view: optVal('cub_default_view'),
    cub_timers_take_over: chk('cub_timers_take_over'),
    cub_probes_take_over: chk('cub_probes_take_over'),
    cub_alerts_take_over: chk('cub_alerts_take_over'),
    cub_rotate_seconds: num('cub_rotate_seconds', 12),
    cub_poll_seconds: num('cub_poll_seconds', 15),
    cub_auto_update: chk('cub_auto_update'),
    cub_ble_advertise: chk('cub_ble_advertise'),
  }, btn, 'cub-settings-result');
}

// The satellite sensor relay toggle (FoodAssistant-me3t), on the Bandit
// Remotes pane of a satellite only. Saves on the switch itself, like the
// automatic-updates toggle, because it is a single choice with nothing else
// in its panel to batch with.
async function saveRelayGadgets(box) {
  const out = document.getElementById('relay-gadgets-result');
  if (out) { out.className = 'test-result ms-1 text-secondary'; out.textContent = 'Saving...'; }
  box.disabled = true;
  try {
    const r = await fetch('setup/save', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ relay_gadgets_upstream: box.checked }),
    });
    const d = await readJsonResponse(r);
    if (!d.ok) throw new Error(d.detail || 'Could not save.');
    if (out) {
      out.className = 'test-result ms-1 text-success';
      out.textContent = box.checked
        ? 'Readings go to the main server.'
        : 'Readings stay on this device.';
    }
    const pill = document.getElementById('relay-gadgets-pill');
    if (pill) {
      pill.textContent = box.checked ? 'On' : 'Off';
      pill.classList.toggle('set-pill-good', box.checked);
      pill.classList.toggle('set-pill-neutral', !box.checked);
    }
  } catch (e) {
    box.checked = !box.checked;   // the save failed; show what is stored
    if (out) {
      out.className = 'test-result ms-1 text-danger';
...
// Named distinctly from wizard.js's _pairingPollTimer: classic scripts share
// one global lexical scope, so two files declaring the same top-level `let`
// throw a SyntaxError at parse time and silently kill every function defined
// after it (this broke the wizard's Next button until they were split apart).
let _devicePairingPollTimer = null;
function startPairingPoll() {
  if (_devicePairingPollTimer) return;
  _devicePairingPollTimer = setInterval(() => {
    ...
    if (!out || out.offsetParent === null) {
      clearInterval(_devicePairingPollTimer);
...
async function checkApStatus() {
  try {
    ...
    if (banner && d.active) {
      // The hotspot name is user-settable; show the one actually on the air
      // when the status reply carries it (it does when the app holds the
      // bridge token). textContent, so the name is never parsed as markup.
      const ssidEl = document.getElementById('ap-banner-ssid');
      if (ssidEl && d.ssid) ssidEl.textContent = d.ssid;
...
// -- Recovery hotspot card (settable name + password) ------------------------

function _apStatusHtml(active) {
  if (active) {
    return '<span class="badge bg-warning text-dark"><i class="bi bi-broadcast me-1"></i>Broadcasting now</span>'
      + '<span class="text-secondary small ms-2">This device has no network, so the hotspot is on the air.</span>';
  }
  return '<span class="badge bg-secondary"><i class="bi bi-wifi me-1"></i>Standing by</span>'
    + '<span class="text-secondary small ms-2">Turns itself on if this device loses your network for a few minutes, and off again when the network returns.</span>';
}

// Client-side mirror of the server's hotspot validation: SSIDs are limited to
// 32 BYTES (not characters) with no control characters; passwords are WPA2's
// 8 to 63 printable ASCII characters. Returns '' when everything is fine.
function _apValidate(ssid, passphrase) {
  if (ssid) {
    ...
    for (const c of ssid) {
      ...
      if (n < 32 || n === 127) return 'The hotspot name cannot contain control characters.';
    }
  }
  if (passphrase) {
    ...
    for (const c of passphrase) {
      const n = c.codePointAt(0);
      if (n < 32 || n > 126) return 'The hotspot password can only use letters, digits, spaces, and punctuation.';
    }
  }
  return '';
}
...
async function loadApConfig() {
  const statusEl = document.getElementById('ap-config-status');
  const ssidEl = document.getElementById('ap_ssid');
  const passEl = document.getElementById('ap_passphrase');
  if (!statusEl && !ssidEl) return;
  try {
    const d = await fetch('setup/ap/config').then(r => r.json());
    if (!d.ok) {
      if (statusEl) statusEl.innerHTML = `<span class="text-secondary small">Unavailable: ${escapeHtml(d.error || 'the host bridge did not answer')}</span>`;
      return;
    }
    if (statusEl) statusEl.innerHTML = _apStatusHtml(d.active);
    if (ssidEl && !ssidEl.value) ssidEl.value = d.ssid || '';
    if (passEl && !passEl.value && d.passphrase) passEl.value = d.passphrase;
  } catch (e) {
    if (statusEl) statusEl.innerHTML = '<span class="text-secondary small">Could not reach the host bridge.</span>';
  }
}

async function saveApConfig(btn) {
  const ssid = document.getElementById('ap_ssid')?.value.trim() || '';
  const passphrase = document.getElementById('ap_passphrase')?.value || '';
  const out = document.getElementById('ap-config-result');
  const err = _apValidate(ssid, passphrase);
  if (err) {
    if (out) { out.className = 'test-result text-danger'; out.textContent = err; }
    return;
  }
  const body = {ssid};
  if (passphrase) body.passphrase = passphrase;
  if (btn) btn.disabled = true;
  if (out) { out.className = 'test-result text-secondary'; out.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Saving...'; }
  try {
    const r = await fetch('setup/ap/config', {
      method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify(body),
    });
    const d = await r.json();
    if (!d.ok) {
      if (out) { out.className = 'test-result text-danger'; out.textContent = d.error || 'Save failed.'; }
      return;
    }
    const ssidEl = document.getElementById('ap_ssid');
    ...
    if (out) {
      ...
      out.innerHTML = `<i class="bi bi-check-circle me-1"></i>Saved. The hotspot will broadcast as <strong>${escapeHtml(d.ssid || '')}</strong>${d.active ? ' and is restarting with the new details now' : ''}.`;
    }
    loadApConfig();
  } catch (e) {
    if (out) { out.className = 'test-result text-danger'; out.textContent = String(e); }
...
async function regenerateApPassphrase(btn) {
  const out = document.getElementById('ap-config-result');
  if (btn) btn.disabled = true;
  if (out) { out.className = 'test-result text-secondary'; out.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Generating...'; }
  try {
    const r = await fetch('setup/ap/config', {
      method: 'POST', headers: {'Content-Type': 'application/json'},
    ...
    const d = await r.json();
    if (!d.ok) {
      if (out) { out.className = 'test-result text-danger'; out.textContent = d.error || 'Could not generate a password.'; }
      return;
    }
    const passEl = document.getElementById('ap_passphrase');
    if (passEl && d.passphrase) passEl.value = d.passphrase;
    if (out) {
      out.className = 'test-result text-success';
      out.innerHTML = '<i class="bi bi-check-circle me-1"></i>New password generated and saved. Click the eye to see it, and update anything you wrote down.';
    }
    loadApConfig();
  } catch (e) {
    if (out) { out.className = 'test-result text-danger'; out.textContent = String(e); }
