...
function wifiStatusHtml(d) {
  ...
  switch (state) {
    ...
    case 'no-adapter':
      return ethBadge || `<span class="badge bg-secondary me-2"><i class="bi bi-wifi-off me-1"></i>No Wi-Fi adapter</span><span class="text-secondary small">${detail || 'This device has no Wi-Fi hardware. Use Ethernet.'}</span>`;
    ...
    case 'unmanaged':
      return ethBadge || `<span class="badge bg-warning text-dark me-2"><i class="bi bi-exclamation-triangle me-1"></i>Unmanaged</span><span class="text-secondary small">${detail || 'Wi-Fi device is not managed by NetworkManager.'}</span>`;
    default:
      // Disconnected Wi-Fi: if wired is up, lead with that; otherwise show the
      // Wi-Fi "not connected" state, optionally noting Ethernet alongside.
      if (ethBadge) return ethBadge + '<span class="badge bg-secondary ms-2"><i class="bi bi-wifi-off me-1"></i>Wi-Fi off</span>';
