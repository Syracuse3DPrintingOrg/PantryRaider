...
async function applyTheme(value) {
  // The custom theme builder only shows for a custom theme ("custom" or a
  // saved "custom:<id>"). Flip it before the save so the panel reacts at once
  // even though a successful save reloads the page anyway.
  const builder = document.getElementById('custom-theme-builder');
  if (builder) builder.classList.toggle('d-none', !String(value || '').startsWith('custom'));
