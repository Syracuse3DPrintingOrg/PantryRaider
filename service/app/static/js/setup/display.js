...
// Advanced-display margins are read by the kiosk from <html> data attributes at
// load, so a change only shows once the panel reloads. Snapshot them at load the
// same way as the scale, and restart the kiosk when any of them actually change.
function _faMarginSnapshot() {
  return ['display_margin_top', 'display_margin_right',
          'display_margin_bottom', 'display_margin_left']
    .map((id) => document.getElementById(id)?.value ?? '')
    .join(',');
}
let _faLoadedMargins = _faMarginSnapshot();
document.addEventListener('DOMContentLoaded', function () {
  if (_faLoadedUiScale === null) {
    const el = document.getElementById('ui_scale');
    if (el) _faLoadedUiScale = el.value;
  }
  _faLoadedMargins = _faMarginSnapshot();
});
...
async function saveDisplaySettings() {
  ...
  const wake_on_presence = document.getElementById('wake_on_presence')?.value || 'auto';
  // On-glass presence indicator (FoodAssistant-99vy); the toggle only renders
  // on a Pi, and ?? keeps the default true when the row is absent.
  const presence_indicator_enabled = document.getElementById('presence_indicator_enabled')?.checked ?? true;
  // Auto-return to the home page after inactivity on a kiosk (FoodAssistant-6e5m).
  const kiosk_auto_home_enabled = document.getElementById('kiosk_auto_home_enabled')?.checked || false;
  const kiosk_auto_home_seconds = parseInt(document.getElementById('kiosk_auto_home_seconds')?.value || '120', 10);
  // Advanced display: per-edge safe-area insets (px). The server clamps to
  // 0-200; send whatever the fields hold (absent fields stay undefined and are
  // dropped from the JSON, leaving the saved value untouched).
  const marginPx = (id) => {
    const el = document.getElementById(id);
    if (!el) return undefined;
    const n = parseInt(el.value, 10);
    return isFinite(n) ? Math.max(0, Math.min(200, n)) : 0;
  };
  const display_margin_top = marginPx('display_margin_top');
  const display_margin_right = marginPx('display_margin_right');
  const display_margin_bottom = marginPx('display_margin_bottom');
  const display_margin_left = marginPx('display_margin_left');
  try {
    ...
    const saveResp = await (await fetch('setup/save', {
      method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({display_touch, display_type, display_idle_timeout, screensaver_minutes, screensaver_speed, screensaver_pill_scale, screensaver_mode, screensaver_photo_seconds, screensaver_ken_burns, screensaver_ken_burns_speed, screensaver_all_clients, osk_enabled, wake_on_motion, wake_on_presence, presence_indicator_enabled, kiosk_auto_home_enabled, kiosk_auto_home_seconds, display_margin_top, display_margin_right, display_margin_bottom, display_margin_left, ...photoSourceFields()}),
    })).json();
    ...
    const marginsChanged = _faMarginSnapshot() !== _faLoadedMargins;
    if (_faLoadedUiScale === null || ui_scale !== _faLoadedUiScale || marginsChanged) {
...
// Split screen (FoodAssistant-dqpq): persist just the split-view fields, so
// this save can never clobber unrelated display settings.
async function saveSplitView() {
  const el = document.getElementById('split-save-result');
  if (el) el.innerHTML = '<span class="text-secondary">Saving...</span>';
  const body = {
    kiosk_split_enabled: document.getElementById('kiosk_split_enabled')?.checked || false,
    kiosk_split_primary: document.getElementById('kiosk_split_primary')?.value || 'start',
    kiosk_split_secondary: document.getElementById('kiosk_split_secondary')?.value || 'add',
    kiosk_split_lock_primary: document.getElementById('kiosk_split_lock_primary')?.checked || false,
  };
  try {
    const r = await fetch('setup/save', {
      method: 'POST', headers: {'Content-Type': 'application/json'},
    ...
    if (!r.ok) throw new Error('HTTP ' + r.status);
    if (el) el.innerHTML = '<span class="text-success"><i class="bi bi-check-circle me-1"></i>Split screen saved.</span>';
  } catch (e) {
    if (el) el.innerHTML = '<span class="text-danger"><i class="bi bi-x-circle me-1"></i>Could not save: ' + e + '</span>';
  }
}
