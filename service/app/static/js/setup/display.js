...
// Per-device persistent submenus (FoodAssistant-ohro): persist this device's
// choice in localStorage so nav-submenus.js can honour it. 'default' clears the
// override and follows the server-resolved default (on for a browser and small
// panel, off for a larger kiosk).
function _navSubmenusSetDevice(val) {
  try {
    if (val === 'on' || val === 'off') {
      localStorage.setItem('navPersistentSubmenus', val);
    } else {
      localStorage.removeItem('navPersistentSubmenus');
    }
  } catch (e) { }
}

function _navSubmenusInit() {
  const sel = document.getElementById('nav_persistent_device');
  if (!sel) return;
  let v = null;
  try { v = localStorage.getItem('navPersistentSubmenus'); } catch (e) { }
  sel.value = (v === 'on' || v === 'off') ? v : 'default';
}
