...
function showDeckStart(which) {
  ...
  if (which === 'deck' && d) {
    ...
    _sdRefreshCustom();
    _renderDeckSuggested();
    // Load the saved layout, rotation, and brightness the first time the deck
    // editor is shown. Without this the editor opens with an empty grid and a
    // 0-rotation default, so a save (even just changing the key style) would
    // post that blank state and wipe the real keys and reset the orientation
    // (FoodAssistant-gbou). Guarded so re-opening the pane keeps in-progress edits.
    if (!_sdLoaded) { _sdLoaded = true; _sdLoadFromConfig(); }
  } else {
    which = 'start';
  ...
  // Keep the URL hash on the visible sub-area, so a save that reloads the page
  // (Save Start Page does) comes back to this same editor (FoodAssistant-wcc4).
  const hash = which === 'deck' ? '#pane-streamdeck' : '#pane-start-page';
  if (window.location.hash !== hash) {
    // Keep the path: a bare hash resolves against the page's <base> and would
    // send the reload to the site root (FoodAssistant-6upk).
    const here = window.location.pathname + window.location.search;
    try { history.replaceState(null, '', here + hash); } catch (e) { }
  }
}
...
async function _startEditorInit() {
  ...
  if (_startState) { _startEditorRender(); _renderStartSuggested(); return; }   // keep edits across pane switches
  const n = _startKeyCount();
  _startState = [];
  const saved = window.START_LAYOUT || [];
  for (let i = 0; i < n; i++) _startState.push(saved[i] || '');
  // No saved arrangement yet: seed the grid from the Glance preset so a switch
  // to Custom layout starts from the current home instead of a blank grid
  // (FoodAssistant-7598). Pages with no deck equivalent stay blank. A fetch
  // failure just leaves the blank grid, same as before.
  if (!saved.some(t => t)) {
    try {
      const r = await fetch('ui/start/glance-seed').then(x => x.json());
      if (r && r.ok && Array.isArray(r.layout)) {
        for (let i = 0; i < n; i++) { if (r.layout[i]) _startState[i] = r.layout[i]; }
      }
    } catch (e) { }
  }
...
function _startRenderPalette() {
  ...
  const groups = {};
  (Array.isArray(_sdCatalog) ? _sdCatalog : []).forEach(a => { if (_sdIsRetiredAction(a)) return; (groups[a.group || 'Other'] = groups[a.group || 'Other'] || []).push(a); });
...
// A "Suggested" section on both editors: quick-action buttons learned from
// what the user actually buys (Grocy purchase history) and cooks (the cook
// count tally), served by GET ui/start/suggested and shared by both editors
// (one list, one dismissed set, so an add or dismiss in either place sticks).
let _suggestedCache = null;

async function _loadSuggested(force) {
  if (_suggestedCache && !force) return _suggestedCache;
  try {
    const r = await fetch('ui/start/suggested').then(x => x.json());
    _suggestedCache = (r && r.ok && Array.isArray(r.suggestions)) ? r.suggestions : [];
  } catch (e) { _suggestedCache = []; }
  return _suggestedCache;
}

function _dismissSuggested(id) {
  if (!id) return;
  fetch('ui/start/suggested/dismiss', {
    method: 'POST', headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({id}),
  }).catch(() => {});
  if (_suggestedCache) _suggestedCache = _suggestedCache.filter(s => s.id !== id);
}

// Renders one suggestion list into `containerId`: a label + reason, a + to
// accept it (onAdd) and an x to dismiss it (removed from view either way).
function _renderSuggestions(containerId, suggestions, onAdd) {
  ...
  suggestions.forEach(sug => {
    ...
    info.innerHTML =
      '<div class="fw-semibold small">' + escapeHtml(sug.label) + '</div>' +
      '<div class="text-secondary" style="font-size:0.72rem;">' + escapeHtml(sug.reason) + '</div>';
    card.appendChild(info);
    const addBtn = document.createElement('button');
    addBtn.type = 'button';
    addBtn.className = 'btn btn-outline-success btn-sm';
    addBtn.title = 'Add this button';
    addBtn.innerHTML = '<i class="bi bi-plus-lg"></i>';
    addBtn.onclick = () => { onAdd(sug); card.remove(); };
    card.appendChild(addBtn);
    const xBtn = document.createElement('button');
    xBtn.type = 'button';
    xBtn.className = 'btn btn-outline-secondary btn-sm';
    xBtn.title = 'Dismiss this suggestion';
    xBtn.innerHTML = '<i class="bi bi-x-lg"></i>';
    xBtn.onclick = () => { _dismissSuggested(sug.id); card.remove(); };
    card.appendChild(xBtn);
    wrap.appendChild(card);
  });
}

// Accept a suggestion on the Start Page: drop it into the first empty key. A
// shopping_add suggestion becomes a new custom key (same builder the "Add
// custom key" button uses, just prefilled); a cook_recipe suggestion places
// the existing built-in "Cook" action, same as dragging its palette chip.
function _startAcceptSuggestion(sug) {
  if (!_startState) return;
  const idx = _startState.findIndex(t => !t);
  if (idx === -1) {
    alert('No empty keys on the Start Page. Clear one first, or choose a bigger key count.');
    return;
  }
  if (sug.kind === 'shopping_add') {
    const wrap = document.getElementById('start-overrides');
    if (!wrap) return;
    const row = _startBuildRow(sug.payload || {});
    wrap.appendChild(row);
    _startState[idx] = row.dataset.cid;
  } else if (sug.kind === 'cook_recipe' && sug.payload && sug.payload.token) {
    _startState[idx] = sug.payload.token;
  } else {
    return;
  }
  _startRenderPalette();
...
// Accept a suggestion on the Stream Deck: same idea, placed into _sdKeys
// (growing the list by one page slot if every existing key is filled).
function _sdAcceptSuggestion(sug) {
  let idx = _sdKeys.findIndex(t => !t || t === 'blank');
  if (idx === -1) { idx = _sdKeys.length; _sdKeys.push('blank'); }
  if (sug.kind === 'shopping_add') {
    const wrap = document.getElementById('streamdeck-overrides');
    if (!wrap) return;
    const row = _sdOverrideRow(sug.payload || {});
    wrap.appendChild(row);
    _sdKeys[idx] = row.dataset.cid;
  } else if (sug.kind === 'cook_recipe' && sug.payload && sug.payload.token) {
    _sdKeys[idx] = sug.payload.token;
  } else {
    return;
  }
  _sdRefreshCustom();
...
async function _renderStartSuggested() {
  const suggestions = await _loadSuggested();
  _renderSuggestions('start-suggested-list', suggestions, _startAcceptSuggestion);
}

async function _renderDeckSuggested() {
  const suggestions = await _loadSuggested();
  _renderSuggestions('streamdeck-suggested-list', suggestions, _sdAcceptSuggestion);
}

// Home style toggle (FoodAssistant-6oew): Glance builds the home screen on
// its own, so the arrange-keys editor only shows in Custom layout mode. The
// editor nodes stay in the DOM (savePaneStartPage still reads them); only the
// visibility flips, live, when the select changes.
function _startModeChanged() {
  const mode = optVal('start_page_mode') || 'glance';
  const custom = mode === 'custom';
  const editor = document.getElementById('start-layout-editor');
  const note = document.getElementById('start-glance-note');
  if (editor) editor.classList.toggle('d-none', !custom);
  if (note) note.classList.toggle('d-none', custom);
  // The grid may not have rendered while hidden; make sure it is live now.
  if (custom) _startEditorRender();
}
...
function savePaneStartPage(btn) {
  ...
  return savePane({
    ...
    // Glance (auto home) vs Custom (arrange the grid below). See _pane_start_page.
    start_page_mode:    optVal('start_page_mode') || 'glance',
...
let _sdLoaded = false;       // whether the editor has loaded the saved config
...
async function _sdLoadCatalog() {
  ...
  _sdCatalog = [
    ...
    // Legacy fixed HA slot keys: kept so a saved layout still renders its
    // face, but hidden from the palette (build a custom Home Assistant
    // action key instead).
    {name:'ha_1',label:'HA 1',kind:'ha_entity',group:'Home Assistant',color:'#475569',legacy:true},
    {name:'ha_2',label:'HA 2',kind:'ha_entity',group:'Home Assistant',color:'#475569',legacy:true},
    {name:'ha_3',label:'HA 3',kind:'ha_entity',group:'Home Assistant',color:'#475569',legacy:true},
    {name:'ha_4',label:'HA 4',kind:'ha_entity',group:'Home Assistant',color:'#475569',legacy:true},
    {name:'ha_5',label:'HA 5',kind:'ha_entity',group:'Home Assistant',color:'#475569',legacy:true},
...
// Actions hidden from the palettes: the retired ha_1..ha_5 fixed slot keys
// (replaced by custom Home Assistant action keys with the entity picker).
// A saved layout that still holds one keeps rendering and firing; users just
// cannot place NEW ones. The name test covers a Pi whose host bridge still
// serves a catalog without the legacy flag.
function _sdIsRetiredAction(a) {
  return !!(a && (a.legacy || /^ha_[1-5]$/.test(a.name)));
}
...
// Load the editor's grid, rotation, and brightness from the saved deck config
// so it reflects what is actually on the deck. Without this the editor opens
// blank (empty grid, rotation 0) and a save would overwrite the real config
// (FoodAssistant-gbou). Best-effort: on a failed fetch the grid falls back to
// the default layout rather than blanking, and the rotation stays as the
// server rendered it.
async function _sdLoadFromConfig() {
  let cfg = null;
  try {
    const r = await fetch('setup/streamdeck/config').then(x => x.json());
    if (r && r.ok && r.config && typeof r.config === 'object') cfg = r.config;
  } catch (e) { cfg = null; }
  if (cfg) {
    const rot = document.getElementById('streamdeck_rotation');
    if (rot && [0, 90, 180, 270].includes(cfg.rotation)) rot.value = String(cfg.rotation);
    const bri = document.getElementById('streamdeck_brightness');
    if (bri && Number.isFinite(cfg.brightness)) bri.value = String(cfg.brightness);
    if (Array.isArray(cfg.keys) && cfg.keys.length > 0) {
      _sdKeys = cfg.keys.slice();
    }
  }
  if (_sdKeys.length === 0) _sdKeys = SD_DEFAULT_ORDER.slice();
  _sdPage = 0;
  await _sdRenderGrid();
}
...
async function _sdRenderGrid() {
  ...
  if (palette) {
    ...
    for (const a of _sdCatalog) {
      if (_sdIsRetiredAction(a)) continue;
...
function _sdOverrideRow(o) {
  ...
  const macroOpts = (Array.isArray(_sdCatalog) ? _sdCatalog : [])
    .filter(a => a.name !== 'blank' && a.kind !== 'macro' && !_sdIsRetiredAction(a))
  ...
  row.innerHTML = `
    ...
    <div class="row g-2 mt-1 sd-ov-fields-ha_action">
      <div class="col-md-6">
        ...
        <input type="text" class="form-control form-control-sm sd-ov-entity" list="${uid}-ha-ents" placeholder="Search by name or entity id" value="${escapeHtml(o.entity_id || '')}">
        <datalist id="${uid}-ha-ents"></datalist>
        <div class="form-text sd-ov-ha-hint d-none"></div>
      </div>
      <div class="col-md-6">
        <label class="form-label small mb-1">Service</label>
        <input type="text" class="form-control form-control-sm sd-ov-service" list="${uid}-ha-svcs" placeholder="homeassistant.toggle" value="${escapeHtml(o.service || '')}">
        <datalist id="${uid}-ha-svcs"></datalist>
      </div>
  ...
  sync();
  _sdWireHaPicker(row, uid);
  return row;
}

// -- Home Assistant entity picker for custom HA action keys (FoodAssistant-yzjr)
// One fetch of setup/ha/entities per page load, shared by every builder row
// (deck and Start Page). The server groups the actionable entities by domain
// and includes each one's friendly name, current state, and default service.
let _haEntitiesPromise = null;
function _haEntitiesData() {
  if (!_haEntitiesPromise) {
    _haEntitiesPromise = fetch('setup/ha/entities').then(r => r.json())
      .catch(() => ({ok: false, groups: []}));
  }
  return _haEntitiesPromise;
}

// Turn an ha_action row's plain Entity/Service inputs into a discovery picker:
// the entity datalist offers every actionable HA entity (friendly name,
// domain, state), and picking one prefills the domain's default service (and
// the label, from the friendly name) so the key works with one click. The
// stored shape is unchanged: the same .sd-ov-entity / .sd-ov-service values
// _sdDefFromRow already reads. With no HA connection the datalists stay empty
// and a hint points at Settings; the fields still accept free text.
async function _sdWireHaPicker(row, uid) {
  const entInput = row.querySelector('.sd-ov-entity');
  const svcInput = row.querySelector('.sd-ov-service');
  const labelInput = row.querySelector('.sd-ov-label');
  const hint = row.querySelector('.sd-ov-ha-hint');
  const entList = document.getElementById(uid + '-ha-ents') || row.querySelector('datalist[id$="-ha-ents"]');
  const svcList = document.getElementById(uid + '-ha-svcs') || row.querySelector('datalist[id$="-ha-svcs"]');
  if (!entInput || !svcInput || !entList) return;
  const data = await _haEntitiesData();
  ...
  if (!groups.length) {
    if (data && data.configured === false) setHint(data.hint || 'Connect Home Assistant under Settings to pick entities here.');
    return;
  }
  const byId = {};
  ...
  for (const g of groups) {
    servicesByDomain[g.domain] = Array.isArray(g.services) ? g.services : [];
    for (const e of (g.entities || [])) {
      byId[e.entity_id] = e;
      opts += `<option value="${escapeHtml(e.entity_id)}">${escapeHtml(e.name)} (${escapeHtml(e.domain)}${e.state ? ': ' + escapeHtml(e.state) : ''})</option>`;
    }
  }
  entList.innerHTML = opts;
  const fillServices = (domain) => {
    if (svcList) svcList.innerHTML = (servicesByDomain[domain] || [])
      .map(s => `<option value="${escapeHtml(s)}">`).join('');
  };
  const applyEntity = (initial) => {
    const e = byId[entInput.value.trim()];
    if (!e) return;
    fillServices(e.domain);
    // Prefill the default service and a label from the friendly name, but
    // only over an empty field or a previous auto-fill, never over something
    // the user typed or a saved value.
    if (!initial) {
      if (!svcInput.value.trim() || svcInput.value === svcInput.dataset.auto) {
        svcInput.value = e.default_service || '';
        svcInput.dataset.auto = svcInput.value;
      }
      if (labelInput && (!labelInput.value.trim() || labelInput.value === labelInput.dataset.auto)) {
        labelInput.value = e.name || '';
        labelInput.dataset.auto = labelInput.value;
      }
      _sdRefreshCustom();
    }
    setHint(e.name + ' is ' + (e.state || 'unknown') + '.');
  };
  entInput.addEventListener('change', () => applyEntity(false));
  entInput.addEventListener('input', () => applyEntity(false));
  // A row loaded with a saved entity gets its services list and state hint,
  // with the saved service and label left exactly as stored.
  applyEntity(true);
}
...
// The fixed ha_1..ha_5 slot editor is retired (FoodAssistant-yzjr): new Home
// Assistant keys are custom ha_action keys built with the entity picker. Any
// streamdeck_ha_slots already saved keep working end to end (settings,
// satellite sync, the deck's slot binding, and the Start Page fire path all
// still honour them); there is just no UI to create or edit them any more.

// Per-camera "pop up on" detection toggles (FoodAssistant-akd0), shared by the
// manual/HA and Reolink row builders below. A camera that reports one of these
// (a Reolink AI detection, or a Home Assistant automation posting to
// /events/camera-detect) pops up on screen through the existing camera pop-up
// channel. Kept as a small class-matched checkbox group (no per-row ids) so it
// works for any number of camera rows without id collisions.
const _CAM_POPUP_TYPES = [['person', 'Person'], ['vehicle', 'Vehicle'], ['animal', 'Animal'], ['visitor', 'Doorbell visitor']];

function _camPopupTypesHtml() {
  return '<div class="col-12 mt-1"><label class="form-label small mb-1">Pop up this camera on</label>' +
    '<div class="d-flex gap-3 flex-wrap">' +
    _CAM_POPUP_TYPES.map(([t, label]) =>
      `<div class="form-check form-check-inline mb-0">` +
      `<input class="form-check-input sd-cam-popup-type" type="checkbox" value="${t}">` +
      `<label class="form-check-label small">${label}</label></div>`
    ).join('') + '</div></div>';
}

// Check the boxes matching a stored popup_types list; called after a row (with
// _camPopupTypesHtml already in its innerHTML) is attached to the document.
function _camSetPopupTypes(div, types) {
  const set = new Set((types || []).map(t => String(t).toLowerCase()));
  div.querySelectorAll('.sd-cam-popup-type').forEach(cb => { cb.checked = set.has(cb.value); });
}

// Read the checked types from a camera row, in the order offered.
function _camReadPopupTypes(row) {
  return Array.from(row.querySelectorAll('.sd-cam-popup-type:checked')).map(cb => cb.value);
}
...
function _camPreview(cam) {
  ...
  const showError = (text) => {
    ...
    msg.textContent = text || 'Could not load a snapshot from this camera.';
    msg.classList.remove('d-none');
  };
  if (cam.postUrl) {
    // Credentialed preview (Reolink): POST the login so it never rides a URL or
    // the page source. The server signs in and streams back the image bytes,
    // which we show through a blob so the secret stays off the page.
    img.removeAttribute('src');
    fetch(cam.postUrl, {
      method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify(cam.postBody || {}),
    }).then(async r => {
      ...
      let text = 'Could not load a snapshot from this camera.';
      try { const j = await r.json(); if (j && (j.error || j.detail)) text = j.error || j.detail; } catch (e) {}
      showError(text);
    }).catch(e => showError('Could not reach the camera: ' + e));
  ...
  img.onerror = () => showError('Could not load a snapshot from this camera.');
  new bootstrap.Modal(modal).show();
}
  ...
  for (const row of wrap.querySelectorAll('.sd-camera-row')) {
    ...
    if (row.querySelector('.sd-cam-source')?.value === 'reolink') {
      const entry = {
        ...
        popup_types: _camReadPopupTypes(row),
      };
      // device_type/two_way_talk (FoodAssistant-qft4) are set by the server when
      // the camera was added (a best-effort Reolink device-info probe); they
      // ride hidden fields on the row purely so a re-save does not lose them.
      const devType = row.querySelector('.sd-cam-device-type')?.value.trim() || '';
      if (devType) entry.device_type = devType;
      if (row.querySelector('.sd-cam-two-way-talk')?.value) entry.two_way_talk = true;
      out.push(entry);
      continue;
    }
    ...
    const entry = {name, stream_url, snapshot_url, popup_types: _camReadPopupTypes(row)};
...
// Migrate HA settings from a legacy deck config into the form. Credentials are
// server-rendered from app settings (the source of truth), so this only fills
// a field that the server left blank: a deck configured before the credentials
// moved to the server still surfaces its base URL here for one save. The old
// per-slot (ha_1..ha_5) rows are gone; saved ha_slots keep working unchanged.
...
async function saveStreamDeckSettings() {
  ...
  // The HA URL/token live in the Home Assistant pane and are stamped into the
  // deck config by the server, so they are not posted here. Cameras are owned
  // by the Cameras pane. streamdeck_ha_slots is deliberately NOT posted: the
  // slot editor is retired, and leaving the key out of the save preserves any
  // slots a device already has (setup/save only applies fields present).
