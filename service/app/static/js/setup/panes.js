  ...
  try {
    ...
    if (list) list.innerHTML = printers.map((p, i) => {
      ...
      const badge = advanced
        ...
        : '';
      return `<div class="d-flex align-items-center justify-content-between border rounded p-2 mb-1" style="max-width:520px">
        <div class="me-2">
          <div>${label}${badge}</div>
          <div class="small text-secondary">${p.uri}</div>
        </div>
        <button type="button" class="btn btn-outline-info btn-sm" onclick='addDiscoveredPrinter(this, ${JSON.stringify(p)})'>
...
// Add a printer found by the search. Uses the suggested name, the device URI,
// and the driver the discovery classified (everywhere for driverless, raw for a
// socket printer).
async function addDiscoveredPrinter(btn, printer) {
  const orig = btn.innerHTML;
  btn.disabled = true;
  btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Adding…';
  try {
    ...
    }, document.getElementById('printer-discover-result'));
  } finally {
    btn.disabled = false;
    btn.innerHTML = orig;
  }
...
async function loadBluetoothPrintStatus() {
  ...
  const btn = document.getElementById('bt-print-setup-btn');
  try {
    ...
    if (btn) btn.disabled = status === 'installing';
    if (!result) return;
    if (status === 'installing') {
      result.innerHTML = '<span class="text-secondary"><span class="spinner-border spinner-border-sm me-1"></span>Setting up the Bluetooth printer. The first setup can take several minutes while it prepares the printer software.</span>';
    } else if (status === 'ready') {
      result.innerHTML = '<span class="text-success"><i class="bi bi-check-circle me-1"></i>Ready. Turn the printer on, then use Find printers above; it shows up as a network printer.</span>';
    } else if (status === 'failed') {
      const tail = d.log_tail ? `<pre class="small mt-1 mb-0 text-wrap">${_gEsc(String(d.log_tail).slice(-1500))}</pre>` : '';
      result.innerHTML = `<span class="text-danger"><i class="bi bi-x-circle-fill me-1"></i>Setup did not finish. Try Set up again.</span>${tail}`;
    } else {
      result.innerHTML = '';
    }
...
async function bluetoothPrintSetup(btn) {
  ...
  try {
    ...
    const d = await r.json().catch(() => ({}));
    if (!r.ok || d.ok === false) {
      if (result) result.innerHTML = `<span class="text-warning"><i class="bi bi-info-circle me-1"></i>${_gEsc(d.message || 'Could not start setup.')}</span>`;
      btn.disabled = false;
      btn.innerHTML = '<i class="bi bi-bluetooth me-1"></i>Set up';
      return;
    }
    if (result) result.innerHTML = `<span class="text-secondary"><span class="spinner-border spinner-border-sm me-1"></span>${_gEsc(d.message || 'Setting up…')}</span>`;
    loadBluetoothPrintStatus();
  } catch (e) {
    if (result) result.innerHTML = `<span class="text-danger">Could not start setup: ${e.message}</span>`;
    btn.disabled = false;
    btn.innerHTML = '<i class="bi bi-bluetooth me-1"></i>Set up';
  }
}
...
function _gadgetsDeviceCard(dev, unit) {
  ...
  if (dev.battery != null) meta.push(`battery ${dev.battery}%`);
