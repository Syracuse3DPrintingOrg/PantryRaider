...
// (the host bridge does the installing there), so without this the spinner
// never resolved on a server. Try to connect a detected local inventory
// service right away; when none answers, replace the spinner with honest
// guidance instead of letting it spin forever.
let _wizServerInvChecked = false;
async function _wizServerInventoryStatus() {
  if (_installMode !== 'server') return;
  if (_wizServerInvChecked || window._firstRunGrocyDone) return;
  if (!document.getElementById('grocy-install-result')) return;  // already connected
  _wizServerInvChecked = true;
  const url = val('grocy_base_url');
  if (url) {
    let d = null;
    try {
      d = await postJson('setup/first-run/grocy', {base_url: url});
  ...
  setResult('grocy-install-result', false,
    'The inventory service has not answered yet. If it is still starting, '
    + 'give it a minute; connecting finishes by itself. Running your own '
    + 'Grocy? Open Advanced below and enter its address.');
}
