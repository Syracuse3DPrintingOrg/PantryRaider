...
def active_section(active_key: str, tree: list[dict] | None = None) -> str:
    """Key of the top-level nav section whose own page or one of its submenu
    pages is the current page, or '' when the current page is a plain top-level
    link with no submenu.

    Drives the persistent-submenu option: while you are on any page under a
    section (say a Kitchen Guide sub-item), that section stays expanded so all
    of its sub-items remain in view. Pure: pass a tree to test it.
    """
    if tree is None:
        tree = build_nav_tree()
    for node in tree:
        ...
        if not children:
            continue
        if node.get("key") == active_key or any(
                c.get("key") == active_key for c in children):
            return node.get("key", "")
    return ""
