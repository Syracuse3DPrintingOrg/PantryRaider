...
# Jinja's default is to stat every template file on every render to see
# whether it changed, and base.html pulls in a couple of dozen partials, so
# that is a couple of dozen stat calls per page on an SD card, for a change
# that in production never comes: the image bakes the templates in. Compile
# once and keep them. The dev compose sets PR_DEV=1 so an edit still shows
# without a restart there.
templates.env.auto_reload = os.environ.get("PR_DEV", "").strip() == "1"
