...
from .config import settings, theme_info, ui_scale_factor, resolve_custom_colors, nav_chrome_hidden, timer_chips_hidden, persistent_submenus_default, APP_NAME, APP_VERSION
from .hardware import is_raspberry_pi
from .ingress import template_globals
from .navigation import visible_tabs, auto_hidden_groups, build_nav_tree, active_section
...
def theme_context(request: Request) -> dict:
    ...
    return {
        ...
        # Persistent submenus (FoodAssistant-ohro): keep the section you are in
        # expanded so all its sub-items stay in view. This is the server-resolved
        # default; nav-submenus.js layers a per-device browser choice over it.
        "persistent_submenus_default": persistent_submenus_default(
            settings.deployment_mode, settings.ui_scale),
...
templates.env.globals["active_section"] = active_section
