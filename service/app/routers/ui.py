...
async def camera_snapshot(idx: int):
    ...
    from ..services.cameras import proxied_snapshot
    cams = settings.streamdeck_cameras or []
    if idx < 0 or idx >= len(cams):
        return JSONResponse({"detail": "Unknown camera."}, status_code=404)
    entry = cams[idx]
    # HA (bearer header) and Reolink (login in the URL) feeds are fetched
    # server-side so no credential ever reaches the browser; a plain manual /
    # Frigate camera carries no secret and is redirected to below.
