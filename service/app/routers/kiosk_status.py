...
async def _device_fields(out: dict, admin: bool, kiosk: bool,
                         want_activity: bool = False) -> None:
    ...
    ``want_activity`` is an opt-in because the answer costs a fresh connection
    to the host bridge on every tick, and only two surfaces read it: the
    presence indicator (when that setting is on) and the screensaver's
    external-activity loop. Every other open tab was paying for a field it
    threw away, several times a minute, for as long as it stayed open.
    """
    if not admin:
        return
...
async def _gather_local(request: Request, since: int, want_expiring: bool,
                        want_scanner: bool, admin: bool, kiosk: bool,
                        db: Session, want_activity: bool = False) -> dict:
    ...
    # Only the page that shows the scan mode asks for it (Manage Pantry and
    # the deck). base.html and the Glance page never send scanner=1, so they
    # no longer pay for a field they never render.
    if want_scanner:
        try:
    ...
    await _device_fields(out, admin, kiosk, want_activity)
    return out
...
async def _gather_satellite(request: Request, since: int, want_expiring: bool,
    ...
    # These used to be awaited one after another, which on a satellite meant
    # up to five Wi-Fi round trips to the main server in a row every time the
    # micro-caches were cold. They are independent (each is its own try, each
    # lands in its own key), so they go out together and the poll costs one
    # round trip instead of five. Two of them take the request's db session
    # but never touch it on this path: both check _upstream() and return the
    # forward (or a micro-cache hit) before any query, which is what makes
    # sharing one session across the gather safe here. Omit-on-failure is
    # kept exactly: a failed field is left out, the rest still arrive.
    jobs: list[tuple[str, object]] = [("timers", cr_router.get_timers(request))]
    # On a satellite this is a forward to the main server, so gating it saves
    # a round trip on every tick of every kiosk page that never shows the mode.
    if want_scanner:
        jobs.append(("scanner_mode", pending_router.scanner_mode_get(request)))
    jobs.append(("pending", pending_router.pending_count(request, db)))
    jobs.append(("actions", ai_router.count_items(request, db)))
    ...
    if want_expiring:
        jobs.append(("expiring", expiring_router.get_expiring_count(days=7)))
...
async def kiosk_status(
    ...
    and ``scanner=1`` into the scanner mode (Manage Pantry and the deck), which
    on a satellite is a forward to the main server. ``activity=1`` opts into
    the host-bridge activity read, which only the presence indicator and the
    screensaver's external-activity loop consume.
    """
    admin = _caller_is_admin(request)
    if scanner:
        # Manage is the only screen that asks for the scanner mode, so this
        # poll is also how the keys learn someone is standing at it and should
        # be wearing the mode colors rather than resting pink.
        from ..services import stemma
        stemma.note_manage_open()
    if _upstream():
        return await _gather_satellite(
            request, since, bool(expiring), bool(scanner), admin, bool(kiosk), db,
            bool(activity))
