...
async def get_expiring_count(days: int = Query(default=7, ge=0, le=365)):
    ...
    shows a stale or crashing value."""
    from ..services.start_page import expiring_soon_count
    grocy = GrocyClient()
    try:
        # Pull the 30-day window once, then bucket it the same way /summary does
        # so the shared count helper sees the exact fields the deck relies on.
