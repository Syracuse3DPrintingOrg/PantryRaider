...
async def scan_lan(body: ScanBody = Body(default=ScanBody())):
    ...
    # No caller passes ports, so the sweep stays on the two the app answers on:
    # an arbitrary port list would turn this into a general port scanner.
    results = await run_in_threadpool(lan_scan.scan_for_instances, cidr,
                                      list(lan_scan.DEFAULT_PORTS))
