...
def cub_firmware_manifest(profile: str = ""):
    """The ESP Web Tools manifest for one profile. The firmware part path is
    same-origin-relative to this URL, so ESP Web Tools fetches it back through
    the .bin proxy below."""
    manifest = cub_svc.firmware_manifest(profile, APP_VERSION)
    if manifest is None:
