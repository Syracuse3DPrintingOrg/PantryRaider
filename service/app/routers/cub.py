...
async def _fetch_release_ota(profile: str, version: str) -> bytes | None:
    """The published app image for a profile at a version, straight off the
    release (bandit-cub-<profile>-<version>.ota.bin). None when the release
    does not carry one, which is the normal answer for anything published
    before CI started attaching it. Never raises."""
    try:
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            r = await client.get(cub_svc.ota_release_download_url(GITHUB_REPO, version, profile))
            if r.status_code == 200 and r.content:
                return cub_svc.valid_ota_image(r.content)
    except Exception:
        pass
    return None


async def _ota_bytes(profile: str) -> bytes | None:
    """The app image a Cub flashes over the air.

    Preference order, published image first: a locally built .ota.bin, a
    locally built factory image (a dev build must still win over the release),
    the cached published .ota.bin, a fresh fetch of the published .ota.bin
    (cached for next time), and finally the app sliced out of the factory
    image. That last step is the fallback for releases published before CI
    attached the app image; it is the only path that assumes where the app
    sits in flash, so it is the one taken last. Never raises.
    """
    cached = None
    try:
        local_ota = cub_svc.local_ota_override_path(settings.data_dir, profile)
        if local_ota.exists():
            return cub_svc.valid_ota_image(local_ota.read_bytes())
        local_factory = cub_svc.local_override_path(settings.data_dir, profile)
        if local_factory.exists():
            return cub_svc.ota_image_from_factory(local_factory.read_bytes())
        cached = cub_svc.cached_ota_path(settings.data_dir, profile, APP_VERSION)
        if cached.exists():
            return cub_svc.valid_ota_image(cached.read_bytes())
    except Exception:
        cached = None

    data = await _fetch_release_ota(profile, APP_VERSION)
    if data is None:
        # No published app image for this release: slice the factory one.
        return cub_svc.ota_image_from_factory(await _firmware_bytes(profile))
...
async def cub_firmware_manifest(profile: str = ""):
    ...
    if cub_svc.CUB_PROFILES.get(profile) is None:
        return JSONResponse({"detail": "Unknown Cub hardware profile."},
                            status_code=404)
    ota = cub_svc.firmware_ota_block_for_app(profile, await _ota_bytes(profile))
...
async def cub_firmware_ota_bin(profile: str):
    """The app image a Cub flashes over the air, which is the only part an
    update replaces: the one published with the release when there is one, else
    the factory image above with the bootloader and partition table trimmed
    off. Declared before the factory route below because both match this path
    and the first one declared wins."""
    if profile not in cub_svc.CUB_PROFILES:
        return JSONResponse({"detail": "Unknown Cub hardware profile."},
                            status_code=404)
    app = await _ota_bytes(profile)
    if app is None:
        return JSONResponse(
