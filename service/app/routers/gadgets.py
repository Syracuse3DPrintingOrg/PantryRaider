...
from ..services import (gadgets, gadgets_ha, gadgets_esp, gadgets_buttons,
                        gadgets_relay, scan_session, stemma)
...
async def reader_config():
    ...
    On a satellite whose gadget relay is on (FoodAssistant-me3t), this runs
    locally (the /gadgets proxy exempts it) and each list is the union of
    this device's own settings and the main server's, mirrored on every
    satellite sync. A sensor added on the server is therefore scanned for by
    the satellite's radio, and the server's entry wins when both define the
    same id. The Cub broadcast fields come from the server too, so the
    packet keeps carrying the server's flag and install tag."""
    gadgets.mark_reader_seen()
    upstream = gadgets_relay.upstream_config()
    return {
        "enabled": (bool(settings.gadgets_enabled)
                    or bool(upstream.get("gadgets_enabled"))),
        "devices": gadgets_relay.merge_device_lists(
            gadgets.configured_devices(), upstream.get("gadget_devices")),
        ...
        "hygrometers_enabled": (bool(settings.hygrometers_enabled)
                                or bool(upstream.get("hygrometers_enabled"))),
        "hygrometers": gadgets_relay.merge_device_lists(
            gadgets.configured_hygrometers(), upstream.get("hygrometer_devices")),
        ...
        "buttons_enabled": (bool(settings.buttons_enabled)
                            or bool(upstream.get("buttons_enabled"))),
        "buttons": gadgets_relay.merge_device_lists(
            gadgets_buttons.configured_buttons(), upstream.get("button_devices")),
        ...
        "contacts_enabled": (bool(settings.contacts_enabled)
                             or bool(upstream.get("contacts_enabled"))),
        "contacts": gadgets_relay.merge_device_lists(
            gadgets.configured_contacts(), upstream.get("contact_devices")),
        # BLE status broadcast (FoodAssistant-yl6u): when on, the reader
        # advertises the Cub summary packet over Bluetooth for battery
        # displays. device_id seeds the packet's install tag so a receiver
        # locks onto this install and not the neighbor's.
        "cub_ble_advertise": (bool(upstream["cub_ble_advertise"])
                              if "cub_ble_advertise" in upstream
                              else bool(settings.cub_ble_advertise)),
        "device_id": upstream.get("device_id") or settings.device_id,
        # Plug-in STEMMA QT / Qwiic accessories (FoodAssistant-etsc). Its own
        # block because it is not a Bluetooth class: the agent's I2C module
        # reads it, and the list is deliberately NOT merged with the server's.
        # A QT board is plugged into one device, so its registry is that
        # device's, even on a satellite that manages its BLE sensors upstream.
        "stemma": {
            "enabled": bool(settings.stemma_enabled),
            "devices": stemma.configured_devices(),
        },
        # UART barcode scanner (FoodAssistant-x61t). The scanner_uart block is
        # the hardware config for the host reader (enable flag, serial port,
        # baud). Like the STEMMA block it belongs to THIS host and is never
        # merged with an upstream list: a UART scanner is wired to one device.
        "scanner_uart": {
            "enabled": bool(settings.scanner_uart_enabled),
            "port": settings.scanner_uart_port,
            "baud": int(settings.scanner_uart_baud),
        },
        # scan_active and scanner_mode are top-level because they are the app's
        # shared state, not host-local hardware. The reader opens the port and
        # reads continuously ONLY while scan_active is true (a scan page reported
        # itself open within the scan-session TTL), then POSTs each decode to
        # /pending/scan. scanner_mode is the effective fleet mode (fetched
        # upstream on a satellite, cached) so the reader can show what mode its
        # scans land in; the scan endpoint still routes by the server's own mode
        # state, so a stale value here never misroutes. scan_active stays local
        # to this device (a scan page open here), never merged upstream.
        "scan_active": scan_session.is_active(),
        "scanner_mode": (await stemma.current_mode_state()).get("mode"),
    }


@router.get("/outputs")
async def outputs(on_manage: int | None = None,
                  rest_color: str | None = None,
                  timer_color: str | None = None):
    """What the agent's output devices should show (FoodAssistant-kh1m).

    Small and local on purpose: a NeoKey's LEDs poll this every couple of
    seconds to stay honest about the scanner mode, so it is built from state
    already in memory (the mode, live alarms, the timer registry) and never
    touches Grocy or the network. That is what makes a 2 second poll free.

    The one non-local case is a satellite, where the scanner mode belongs to
    the main server (POST /pending/scanner-mode forwards there). Reading the
    local file would light the wrong key on every Bandit, so the mode is
    fetched upstream behind a few seconds of cache: still about one request
    per poll interval, and the NeoKey on a Bandit shows the fleet's real
    mode."""
    from ..services import timers as timers_svc
    mode_state = await stemma.current_mode_state()
    # ``on_manage`` arrives only from a satellite, which forwards this whole
    # request and answers for its own screen (see the forwarding middleware in
    # main.py). A device asking for itself just reads its own state.
    at_manage = stemma.manage_open() if on_manage is None else bool(on_manage)
    # Same for the colors: a satellite sends the look chosen on the device the
    # pad is actually plugged into, and anything unusable falls back here.
    resting = (stemma.rest_color() if rest_color is None else
               stemma.parse_hex_color(rest_color, stemma.BRAND_PINK))
    timer_rgb = (stemma.timer_color() if timer_color is None else
                 stemma.parse_hex_color(timer_color, stemma.ALARM_RED))
    return stemma.build_outputs(mode_state, gadgets.active_alarms(),
                                timers_svc.list_timers(),
                                key_test=gadgets.stemma_key_test(),
                                on_manage=at_manage,
                                resting=resting, timer_rgb=timer_rgb)
...
async def post_readings(payload: dict):
    ...
    Two relay-shaped cases (FoodAssistant-me3t): a push CARRYING a "source"
    tag was relayed from a satellite, so it must not count as this install's
    own reader heartbeat; and on a satellite whose relay is on, every push
    is also queued for the main server (fire-and-forget, never blocking this
    ingest) and mapped button actions are left for the server to execute."""
    payload = payload if isinstance(payload, dict) else {}
    relayed = bool(str(payload.get("source") or "").strip())
    relaying = gadgets_relay.relay_active()
    result = gadgets.ingest(payload, mark_reader=not relayed)
    buttons = await gadgets_buttons.handle_payload(payload, execute=not relaying)
    result["button_events"] = buttons.get("events", 0)
    if relaying:
        gadgets_relay.enqueue(payload)
    if relayed:
        # This push came from a satellite, which left the alarm call to us. Hand
        # our live alarms back on the reply so its kitchen screen can show the
        # same warning ours does, seconds after the reading raised it
        # (FoodAssistant-me3t). Only on a relayed push: a local reader's own app
        # already has them.
        result["alarms"] = gadgets.active_alarms()
    return result


# The keys of the state snapshot that describe THIS device rather than the
# fleet: the plug-in accessory registry and what the local agent's bus sweep
# found, the I2C bus health, and the local reader's heartbeat. On a satellite
# they are laid over the server's snapshot (FoodAssistant-t0vp5).
_LOCAL_ACCESSORY_KEYS = (
    "stemma_enabled", "stemma", "stemma_discovered", "stemma_choices",
    "i2c_available", "i2c_detail", "reader_age_seconds",
)


def overlay_local_accessories(upstream: dict, local: dict) -> dict:
    """The server's state snapshot with this device's own accessory section
    in place of the server's. Pure. A satellite's thermometers, hygrometers,
    door sensors, and buttons are fleet state and stay the server's; a QT
    board is plugged into ONE device, so the pane on that device must show
    its own registered pads, its own discoveries, and the truth about its
    own bus, none of which the server can know."""
    merged = dict(upstream)
    for key in _LOCAL_ACCESSORY_KEYS:
        if key in local:
...
async def state():
    ...
    Answered locally on every deployment mode. A satellite fetches the
    server's snapshot itself (the fleet picture its kiosk shows) and lays its
    own accessory section over it (FoodAssistant-t0vp5); when the server
    does not answer, the local snapshot stands on its own so the pad keeps
    working."""
    data = gadgets.get_state()
    data.update(gadgets_buttons.state_snapshot())
    if settings.is_satellite():
        from ..main import satellite_upstream_get
        upstream = await satellite_upstream_get("/gadgets/state")
        if upstream is not None:
            return overlay_local_accessories(upstream, data)
    return data
...
#
# I2C boards on the Pi's QT connector, read by the host agent's I2C module.
# The registry lives in settings.stemma_devices, keyed by a bus-plus-address
# id like i2c:1:0x30. The NeoKey 1x4 is the first device: four keys mapped to
# the scanner modes, LEDs following whatever the mode is.


class StemmaDeviceIn(BaseModel):
    id: str
    kind: str = ""
    name: str = ""


class StemmaEditIn(BaseModel):
    device_id: str
    name: str | None = None
    keymap: list[str] | None = None
    brightness: int | None = None


class StemmaTestIn(BaseModel):
    device_id: str
    key: int = 0


@router.post("/stemma")
async def add_stemma(payload: StemmaDeviceIn):
    """Add a plug-in accessory (usually one the agent's bus sweep found).
    Adding the first one is the class's opt-in, so it also flips
    stemma_enabled. A NeoKey arrives with the default key order already
    mapped, so it works before the user touches the editor."""
    dev_id = stemma.norm_id(payload.id)
    if not stemma.parse_device_id(dev_id):
        return {"ok": False,
                "error": "a device id like i2c:1:0x30 is required"}
    kind = str(payload.kind or "").strip().lower()
    if kind not in stemma.STEMMA_KINDS:
        return {"ok": False, "error": "that accessory is not supported yet"}
    devices = [dict(d) for d in stemma.configured_devices()]
    for dev in devices:
        if stemma.norm_id(dev.get("id")) == dev_id:
            if payload.name:
                dev["name"] = payload.name[:60]
            dev["kind"] = kind
            break
    else:
        devices.append({"id": dev_id, "kind": kind,
                        "name": payload.name[:60] or stemma.kind_label(kind),
                        "options": stemma.normalize_options(kind, {})})
    settings.save({"stemma_devices": devices, "stemma_enabled": True})
    return {"ok": True, "stemma": devices}


@router.post("/stemma/edit")
async def edit_stemma(payload: StemmaEditIn):
    """Rename an accessory, remap its keys, or set its LED brightness. Only
    the fields present in the request are applied, so a rename never disturbs
    a mapping. A key mapped to "" does nothing and stays dark."""
    dev_id = stemma.norm_id(payload.device_id)
    provided = payload.model_dump(exclude_unset=True)
    devices = [dict(d) for d in stemma.configured_devices()]
    for dev in devices:
        if stemma.norm_id(dev.get("id")) != dev_id:
        ...
        if "name" in provided:
            dev["name"] = str(provided["name"] or "").strip()[:60]
        options = dict(dev.get("options") or {})
        if "keymap" in provided:
            options["keymap"] = stemma.normalize_keymap(provided["keymap"])
        if "brightness" in provided:
            options["brightness"] = stemma.normalize_brightness(
                provided["brightness"])
        dev["options"] = stemma.normalize_options(dev.get("kind"), options)
        settings.save({"stemma_devices": devices})
        return {"ok": True, "stemma": devices}
    return {"ok": False, "error": "unknown device"}


@router.delete("/stemma/{device_id:path}")
async def remove_stemma(device_id: str):
    dev_id = stemma.norm_id(device_id)
    devices = [dict(d) for d in stemma.configured_devices()
               if stemma.norm_id(d.get("id")) != dev_id]
    settings.save({"stemma_devices": devices})
    return {"ok": True, "stemma": devices}


class KeyActionIn(BaseModel):
    action: str = ""
    # Which surface fired it ("neokey" for the physical pad). A physical
    # control has no screen of its own, so a timer action's outcome gets a
    # local confirmation toast; other callers stay quiet.
    source: str = ""


# Forwarding client for the satellite timer-action hop (see key_action).
_key_fwd_client = None


def _key_action_upstream() -> str | None:
    """The main server base URL when this device is a satellite, else None."""
    if (settings.is_satellite() and settings.remote_server_url
            and settings.upstream_api_key):
        return settings.remote_server_url.rstrip("/")
    return None


async def _forward_key_action(base: str, action: str) -> dict:
    """Run a timer action on the main server, where the shared registry
    lives. Sent without the source (like the scanner-mode forward) so the
    server acts on its timers without also toasting its own screens."""
    global _key_fwd_client
    if _key_fwd_client is None:
        import httpx
        _key_fwd_client = httpx.AsyncClient(timeout=10.0)
    try:
        r = await _key_fwd_client.post(
            f"{base}/gadgets/key-action",
            json={"action": action},
    ...
    except Exception:  # noqa: BLE001 - a key press must fail politely
        return {"ok": False, "error": "The main server is not reachable. "
                                      "This will work again when it is."}


def _run_timer_plan(plan: dict, timers_svc) -> str:
    """Execute a stemma.timer_key_plan against the shared registry and
    return the message to show. Kept out of the endpoint so the plan's
    truth table stays pure in stemma.py and this stays four obvious lines."""
    op = plan.get("op")
    if op == "create":
        timers_svc.create_timer(plan["label"], plan["seconds"])
    elif op == "extend":
        if timers_svc.extend_timer(plan["id"], plan["seconds"]) is None:
            # It expired between the plan and the press landing: nothing to
            # extend anymore, and saying "added 5 minutes" would be a lie.
            return "That timer just finished."
    elif op == "cancel":
        for timer_id in plan.get("ids", []):
            timers_svc.cancel_timer(timer_id)
    return str(plan.get("message") or "")


@router.post("/key-action")
async def key_action(payload: KeyActionIn):
    """One NeoKey press, delivered as the slot's action string
    (FoodAssistant-bo9v).

    The agent stays dumb: a key mapped to anything beyond a plain scanner
    mode posts its action string here verbatim (mode keys still post to
    /pending/scanner-mode exactly as they always have). This path answers
    LOCALLY on every deployment mode (the satellite forwarding middleware
    exempts it) because locality differs per action, mirroring the
    _announce_mode split in routers/pending.py:

    * a mode action is fleet state: set locally or forwarded upstream, with
      the kiosk jump announced on THIS device (apply_scanner_mode).
    * a navigate action is pure local feedback: whoever pressed the key is
      standing at THIS device's kiosk, so the navigate event fires on this
      device's ring and never forwards.
    * a timer action acts on the fleet's shared registry: local on a server,
      forwarded upstream from a satellite (where /timers itself forwards),
      with the confirmation toast shown locally either way.
    """
    from ..services import ha_events
    from ..services import timers as timers_svc
    from .pending import apply_scanner_mode

    action = stemma.normalize_key_action(payload.action)
    if not action:
        return {"ok": False,
                "error": "That key is not set to anything this app understands."}
    physical = str(payload.source or "").strip().lower() == "neokey"

    if stemma.is_mode_action(action):
        # The identical path a direct POST /pending/scanner-mode takes,
        # source included, so a mode routed here behaves exactly like the
        # agent's legacy post.
        result = await apply_scanner_mode(action, payload.source)
        if isinstance(result, dict):
            return {"ok": True, "action": action, **result}
        return result   # a satellite's forwarded Response, passed through

    if action.startswith(stemma.NAV_ACTION_PREFIX):
        # always=True: a physical press is local feedback, so the kiosk
        # follows it even where on-screen HA events are turned off (the same
        # rule the scanner-mode jump uses). Navigation is its own feedback,
        # so no toast rides along.
        ha_events.add_navigate(stemma.nav_action_path(action), always=True)
        return {"ok": True, "action": action,
                "message": f"Opening {stemma.key_action_label(action)}."}

    # A timer action. The registry is fleet state: a satellite runs it on
    # the main server, everything else runs it here.
    base = _key_action_upstream()
    if base:
        result = await _forward_key_action(base, action)
        message = str(result.get("message") or "")
    else:
        plan = stemma.timer_key_plan(action, timers_svc.list_timers())
        message = _run_timer_plan(plan, timers_svc)
        result = {"ok": True, "action": action, "message": message}
    if physical and result.get("ok") and message:
        # The pad has no screen, so the outcome shows on the kiosk beside it.
        try:
            ha_events.add_confirmation(message)
        except Exception:  # noqa: BLE001 - feedback must never fail the press
            pass
    return result


@router.post("/stemma/test")
async def test_stemma_key(payload: StemmaTestIn):
    """Light one key for a moment so the user can tell which physical key is
    which. Queued for the agent, which flashes it on its next outputs poll;
    the agent is the only thing that can talk to the bus, so the app asks
    rather than writes."""
    dev = stemma.find_device(payload.device_id)
    if not dev:
        return {"ok": False, "error": "unknown device"}
    if not 0 <= int(payload.key) < stemma.NEOKEY_KEYS:
        return {"ok": False, "error": "unknown key"}
    stemma.request_key_test(dev["id"], int(payload.key))
    return {"ok": True, "message": "Watch the keys: that one lights up now."}
