...
async def save_setup(payload: SetupPayload):
    ...
    # The two device toggles that ride the same "None means the field was not
    # submitted" contract: the Cub Bluetooth beacon and the satellite sensor
    # relay. Dropping a null keeps the stored value instead of writing None
    # over a boolean setting.
    for f in ("cub_ble_advertise", "relay_gadgets_upstream"):
        if f in data and data[f] is None:
            data.pop(f, None)
