...
async def add_reolink_camera(payload: ReolinkAddPayload):
    ...
    server-side by the camera proxy, so no login ever reaches the page. Reolink
    doorbells work the same way; their two-way audio is not part of this feed.
    Optionally verifies the still image before saving so a wrong login is caught
    up front. Returns the stored entry without the password."""
    ...
    if payload.test:
        url = cam_svc.reolink_snapshot_from_entry(entry)
        try:
            async with httpx.AsyncClient(timeout=8.0, verify=False) as c:
                r = await c.get(url, follow_redirects=True)
        except Exception as e:
            return {"ok": False, "error": f"Could not reach the camera: {e}"}
        ctype = r.headers.get("content-type", "").lower()
        looks_image = ctype.startswith("image/") or r.content[:2] == b"\xff\xd8"
        if r.status_code in (401, 403) or (r.status_code == 200 and not looks_image
                                           and b"login" in r.content[:200].lower()):
            return {"ok": False, "error": "The camera rejected that username or password."}
        if r.status_code != 200 or not looks_image:
            return {"ok": False,
                    "error": f"The camera did not return a snapshot (HTTP {r.status_code})."}
