...
def _mealie_parsed(detail: dict) -> tuple[dict, list]:
    ...
    # Carry Mealie's ingredient groups across the migration (zq7k), but only when
    # the recipe actually had headings, so an ungrouped recipe migrates unchanged.
    if any(sections):
        parsed["ingredient_sections"] = sections
