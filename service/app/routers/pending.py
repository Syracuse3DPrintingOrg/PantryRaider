...
from ..services import best_by_provenance, expiry_learning, scanner_mode, shopping_source
...
async def scan_barcode(body: ScanRequest, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Never fabricate a product for a store-assigned code (the deli-meat
        # scan that came back as "bananas"): queue a clearly-labeled
        # placeholder instead, the pending card prompts for a photo, and the
        # LLM fallback never even runs (lookup_barcode short-circuits it).
        lookup_failed = True
        item = apply_defaults(
            FoodItem(name=f"Store barcode ({barcode}) - take a photo instead"), db
        )
    except (BarcodeNotFound, BarcodeServiceError):
        lookup_failed = True
    ...
    row = PendingItem(
        ...
        # Where that date came from ("llm" from barcode enrichment, "default"
        # from a category rule), kept on the row so the commit can record
        # provenance and the printed label badges honestly (FoodAssistant-vb60).
...
async def scanner_mode_set(body: ScannerModePayload, request: Request):
    if _upstream():
        # A satellite forwards the whole body, source included, so the toast
        # fires on the server where the kitchen screens are looking.
        return await _forward(request, "/scanner-mode")
...
# Physical controls that deserve an on-screen confirmation when they change
# the mode: they have no display of their own, so without this a press is
# invisible to anyone standing at the kiosk.
_PHYSICAL_MODE_SOURCES = ("neokey",)


def _announce_mode(state: dict, source: str) -> None:
    """Toast the new scanner mode when a physical control selected it.

    Best-effort: a mode change must never fail because the toast channel is
    ...
    try:
        ...
        ha_events.add_confirmation(
            f"Scanner mode: {state.get('label') or state.get('mode')}",
