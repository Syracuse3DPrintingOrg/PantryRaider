...
async def import_recipe_url(payload: ImportUrlPayload, db: Session = Depends(get_db)):
    ...
    # Never fetch loopback or link-local targets: no recipe lives there, and a
    # crafted URL would otherwise turn this fetch into a probe of same-host
    # services (the host bridge answers on 127.0.0.1:9299; its token still
    # gates every action, but there is no reason to let the request leave the
    # building at all). Defense in depth for the SSRF surface.
    import ipaddress
    from urllib.parse import urlsplit
    host = (urlsplit(url).hostname or "").strip("[]").lower()
    _blocked = host in ("localhost",) or host.endswith(".localhost")
    if not _blocked:
        try:
            ip = ipaddress.ip_address(host)
            _blocked = ip.is_loopback or ip.is_link_local
        except ValueError:
            pass  # a normal hostname; DNS decides
    if _blocked:
...
def _public_image_url(image: str) -> str:
    ...
    # public photo), so a base like http://192.168.1.170:9284 would fail the
