...
from ..services import cook_counts, recipe_source, recipes_external, recipes_forager
...
async def import_recipe_pdf(file: UploadFile = File(...)):
    ...
    the picture. A scanned, image-only PDF has no readable text, so it gets a
    clear "try a photo instead" message rather than a blank draft.
    """
    ...
    # letters extracted as mojibake. Both go to the friendly photo path rather
    # than feeding nonsense to the AI (FoodAssistant-nw0k).
    if len(page_text) < MIN_RECIPE_TEXT or is_mostly_garbage(page_text):
        raise HTTPException(
            422, "This PDF has no readable text (it looks like a scan or images). "
...
async def suggest(
    ...
    # Made-before counts across every tier, one batch query (FoodAssistant-bjps).
...
async def mealie_recipe_detail(slug: str = ""):
    """Full detail of one saved Mealie recipe, normalized for the in-app quick
    view (FoodAssistant-az1s). Lets a user read the ingredients and steps
    without opening Mealie in another tab. Fails soft when the slug is missing
    or Mealie cannot be reached."""
    slug = (slug or "").strip()
    if not slug:
...
async def parse_recipe_ingredients(payload: ParseIngredientsPayload):
    ...
    try:
        await m.set_recipe_ingredients(slug, structured)
    except MealieError:
        # Some Mealie versions reject structured ingredients (FoodAssistant-ztjc);
        # the recipe is left untouched rather than erroring out with Mealie's
        # raw message.
        raise HTTPException(
            502,
    ...
    # Count the lines that came back with a food (the ones now parsed); a line
    # kept as a plain note does not count toward the tally shown to the user.
    count = sum(1 for s in structured if s.get("food"))
...
async def import_external_recipe(payload: ImportExternalPayload):
    ...
    if payload.source == recipes_forager.SOURCE:
        ...
        # into the saved Mealie recipe (FoodAssistant-l2hk).
...
async def share_recipe(payload: ShareRecipePayload):
    """Share a recipe the user has to the Forager community (FoodAssistant-l2hk).

    Accepts a Mealie slug (loaded from the recipe manager) or the recipe fields
    directly. Requires the install to be linked to a Forager account and an
    ...
    # Start from the Mealie recipe when a slug is given, then let any fields on
    # the payload override it (so a manual share needs no Mealie at all).
    recipe: dict = {}
    if payload.slug and settings.mealie_configured():
        try:
...
async def cooked_recipe(payload: CookedPayload, db: Session = Depends(get_db)):
    ...
    # Bump the made-before count for this Mealie recipe (FoodAssistant-bjps).
