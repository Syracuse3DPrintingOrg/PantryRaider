...
def _effective_label_queue() -> str:
    """The label queue this device prints to (FoodAssistant-7u7z): a label printer
    physically on THIS device wins (so the device with the printer prints to it
    directly, FoodAssistant-eml9 follow-up), then its own local choice, then the
    fleet default published by the main server and inherited by a satellite."""
    local = printing.local_label_queue(settings.label_printer_queue)
    if local:
        return local
...
# A Bluetooth label printer sleeps after a while and only wakes on its own
# power button; this is the one thing the app can tell the user to do about
# it (FoodAssistant-fu62). Distinct from a generic print error so the UI can
# show a targeted prompt instead of a confusing failure.
_PRINTER_OFFLINE_MESSAGE = "Turn your label printer on, then try again."


def _result_json(result: printing.PrintResult) -> dict:
    """A stable JSON body for a print attempt. When the printer looks powered
    off or unreachable (result.error_kind == "offline"), the error is
    replaced with a clear prompt to turn it on and ``printer_offline`` is set,
    so the UI can show that case distinctly from a generic failure."""
    offline = result.error_kind == "offline"
    error = _PRINTER_OFFLINE_MESSAGE if offline else result.error
    return {"ok": result.ok, "job_id": result.job_id, "error": error,
...
class LabelIn(BaseModel):
    ...
    # Grocy stock-entry code on the label (FoodAssistant-28f3). A caller that
    # knows the exact stock entry passes its id (with product_id), or a ready
    # grocycode string; a product_id alone gets the product's newest entry.
    # The label then carries a QR square that, scanned later, uses up that
    # exact entry.
    stock_entry_id: Optional[str] = None
    grocycode: str = ""
    # Label stock shape (FoodAssistant-bprm): "rectangle", "square", or
    # "round". Unset falls back to the configured stock shape; the settings
    # pane passes its current pick so the live preview follows the shape
    # dropdown before Save.
...
class DecorativeIn(BaseModel):
    ...
    icon: str = ""
    outline: bool = False
    # Label stock shape (FoodAssistant-bprm); unset falls back to the
    # configured stock shape.
    shape: Optional[str] = None
...
# -- Stock-entry code on labels (FoodAssistant-28f3) -------------------------


def newest_stock_entry_id(entries) -> str:
    """The stock id of the newest usable entry in a Grocy entries list, or "".

    "Newest" is the entry created last (row_created_timestamp, ties broken by
    the higher numeric row id), among entries that still hold stock and carry a
    stock id. A label printed right after an item was put away should code the
    entry that add just created, and that is the newest one. Pure and fully
    testable; any malformed row is simply skipped."""
    best_key = None
    best_sid = ""
    for entry in entries or []:
        if not isinstance(entry, dict):
            continue
        sid = str(entry.get("stock_id") or "").strip()
        if not sid:
            continue
        try:
        ...
        if amount <= 0:
            continue
        ts = str(entry.get("row_created_timestamp") or "")
        try:
...
async def _newest_entry_id(product_id: int, grocy: Optional[GrocyClient] = None) -> str:
    """The newest stock entry id for a product, or "" when Grocy cannot say.

    Best-effort by design: a label must always print, so an unreachable Grocy
    or a product with no entries just means the label goes out without the
    entry code, exactly as labels always did."""
    try:
        client = grocy or GrocyClient()
        entries = await client._get(f"/stock/products/{int(product_id)}/entries")
    except Exception:  # noqa: BLE001
        return ""
    return newest_stock_entry_id(entries)


async def _resolve_grocycode(body: LabelIn,
                             grocy: Optional[GrocyClient] = None) -> str:
    """The grocycode this label should carry, or "" for a plain label.

    A ready grocycode string wins; then an explicit product + entry pair; then
    a product id alone resolves to its newest stock entry (the one a just-added
    item landed in). A label built from free-typed fields, with no product
    behind it, has no entry to point at and prints without a code."""
    ready = (body.grocycode or "").strip()
    if ready:
        return ready
    if body.product_id is None:
        return ""
    entry_id = (body.stock_entry_id or "").strip()
    if not entry_id:
        entry_id = await _newest_entry_id(body.product_id, grocy)
    if not entry_id:
        return ""
    return label_render.build_grocycode(body.product_id, entry_id)
...
def _effective_shape(requested: Optional[str]) -> str:
    """The label-stock shape a render should use: the request's when given,
    else the configured stock shape. Always one of label_render.LABEL_SHAPES,
    so a stale stored value can never leak through to the renderer."""
    return label_render.normalize_shape(requested or settings.label_shape)


def _render_label_image(spec: label_render.LabelSpec, shape: str = "rectangle"):
    """Render a food label to a PIL image, honouring a saved custom layout
    and the label stock's shape.
    ...
    used unchanged, so an untouched install prints exactly as before. On round
    stock (FoodAssistant-bprm) the design, custom or default, is laid out in
    the cut circle's inscribed content box and composed onto the round canvas,
    so every field stays inside the circle."""
    shape = label_render.normalize_shape(shape)
    layout = _saved_layout()
    if layout is None:
        return label_render.render_shaped_label(spec, shape)
    render_spec = spec
    if shape == "round":
        render_spec = label_render.round_inner_spec(spec)
    layout.width_in = render_spec.width_in
    layout.height_in = render_spec.height_in
    layout.dpi = render_spec.dpi
    layout.margin_in = render_spec.margin_in
    img = label_render.render_layout(layout, _values_from_spec(spec))
    if shape == "round":
        return label_render.compose_round_label(spec, img)
    return img
...
async def print_label(body: LabelIn):
    ...
    spec.grocycode = await _resolve_grocycode(body)
    png = label_render.render_to_png_bytes(
        _render_label_image(spec, _effective_shape(body.shape)))
...
class LayoutPreviewIn(BaseModel):
    ...
    ``shape`` follows the label stock (FoodAssistant-bprm); unset falls back
    to the configured stock shape, so the designer preview matches what would
    print."""
    layout: dict[str, Any] = {}
    values: dict[str, Any] = {}
    shape: Optional[str] = None
...
def preview_layout(body: LayoutPreviewIn):
    ...
    shape = _effective_shape(body.shape)
    try:
        if shape == "round":
            # Round stock (FoodAssistant-bprm): lay the design out in the cut
            # circle's inscribed content box, then compose it onto the round
            # canvas, exactly as the print path does.
            outer = label_render.LabelSpec(
                name="", width_in=layout.width_in, height_in=layout.height_in,
                dpi=layout.dpi, margin_in=layout.margin_in)
            inner_spec = label_render.round_inner_spec(outer)
            layout.width_in = inner_spec.width_in
            layout.height_in = inner_spec.height_in
            layout.margin_in = 0.0
            img = label_render.compose_round_label(
                outer, label_render.render_layout(layout, values))
        else:
...
async def print_label_batch(body: BatchIn):
    ...
    grocy = GrocyClient()
    try:
        entries = await grocy.get_full_stock()
    except GrocyError as e:
    ...
    for pid in ids:
        ...
        if not item:
            missing += 1
            continue
        spec = _spec_from_fields(LabelIn(product_id=pid), item)
        # Each batch label codes its product's newest stock entry, the one the
        # just-finished import created. Best-effort: a miss just prints that
        # label plain (FoodAssistant-28f3).
        spec.grocycode = await _resolve_grocycode(LabelIn(product_id=pid), grocy)
        specs.append(spec)
    if not specs:
        raise HTTPException(404, "None of those items are in stock any more.")
    pdf = label_render.render_batch_pdf_bytes(specs, shape=_effective_shape(None))
