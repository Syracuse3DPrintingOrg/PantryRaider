...
500. There is no satellite forwarding here on purpose, a printer is attached to
the device the user is standing at, so each device prints to its own queues.
"""
...
def _label_queue_detail() -> Optional[str]:
    ...
    if off:
        return off
    if not settings.label_printer_queue:
...
def _document_queue_detail() -> Optional[str]:
    ...
    if off:
        return off
    if not settings.document_printer_queue:
...
async def print_label(body: LabelIn):
    ...
    result = printing.print_bytes(settings.label_printer_queue, png)
...
async def print_label_batch(body: BatchIn):
    ...
    result = printing.print_bytes(settings.label_printer_queue, pdf)
...
async def print_document_route(body: DocumentIn):
    ...
    result = printing.print_bytes(settings.document_printer_queue, pdf)
    return JSONResponse(_result_json(result), status_code=200 if result.ok else 502)
