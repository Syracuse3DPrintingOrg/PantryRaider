...
async def set_current_from_mealie(payload: FromMealieIn, db: Session = Depends(get_db)):
    """Load a saved recipe (by slug) as the active recipe (FoodAssistant-1g4l).

    Fetches the full recipe from the recipe library (the native store, or
    Mealie on the Mealie backend), normalizes its ingredients/steps/yield into
    the active-recipe shape, and makes it current. This is what the Recipes
    and Cook page 'Cook this' buttons call so a recipe can be launched without
    leaving the app. The route keeps its /from-mealie name for compatibility
    with existing pages and satellites."""
    slug = (payload.slug or "").strip()
    if not slug:
        return JSONResponse({"detail": "A recipe slug is required."}, status_code=400)
    from ..services import recipe_source, recipe_store
    if recipe_source.active_backend() == recipe_source.BACKEND_NATIVE:
        detail = recipe_store.detail(db, slug)
        if not detail:
            return JSONResponse({"detail": "Recipe not found in your library."},
                                status_code=404)
    else:
