...
from ..services.grocy import GrocyClient, GrocyError, error_payload, opened_amounts
...
async def get_dashboard(sort: str = "expiry_asc"):
    ...
    # The enriched rows do not carry Grocy's amount_opened, so it is merged
    # back in from the raw stock list; the Opened indicator hangs on it
    # (FoodAssistant-oyef). Best effort: a hiccup here must not lose the
    # dashboard that already loaded.
    try:
        opened = opened_amounts(await grocy.get_stock())
    except Exception:  # noqa: BLE001
        opened = {}
    for item in items:
        item["amount_opened"] = opened.get(item["product_id"], 0.0)
