...
def _require_backup_password(password: str) -> None:
    ...
    streaming. An open install (no auth_password configured) has nothing to
    verify against, so it streams as before rather than locking anyone out.
    The compare is constant-time via verify_secret and the entered password is
    never logged.
    """
    if not settings.auth_password:
        return
    if not verify_secret(password, settings.auth_password):
        raise HTTPException(403, "Enter your current password to download the backup.")
