...
# The app database, and the working files that are only meaningful next to a
# database mid-write or next to a half-finished copy. A journal or WAL captured
# out of step with the database it belongs to can roll a restored database
# back, so none of them are ever archived or restored.
_DB_NAME = "foodassistant.db"
_VOLATILE_SUFFIXES = ("-journal", "-wal", "-shm", ".part", ".tmp")


def _is_volatile(name: str) -> bool:
    return name.endswith(_VOLATILE_SUFFIXES)


def _add_db_snapshot(zf: zipfile.ZipFile, db: Path) -> None:
    """Add the app database to the archive as a consistent snapshot.

    Requests keep writing while a backup runs, so a byte-for-byte copy of the
    live file can catch a half-written page and produce an archive that will
    not open. SQLite's own backup API takes a consistent copy of a database
    that is in use. The temp copy lives outside data_dir, which is being walked
    for the rest of the archive. A raw copy is the fallback: a database that
    refuses to open for backup must not cost the user the whole backup.
    """
    arc = f"{_BACKUP_PREFIX}/{_DB_NAME}"
    tmp_dir = tempfile.mkdtemp(prefix="pantryraider-backup-")
    try:
        tmp = Path(tmp_dir) / _DB_NAME
        try:
            ...
            logger.warning("backup: live database snapshot failed (%s); "
                           "copying the file as-is", exc)
            zf.write(db, arc)
            return
        zf.write(tmp, arc)
    finally:
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        if data_dir.exists():
            for f in sorted(data_dir.rglob("*")):
                ...
                if _is_volatile(f.name):
                    continue  # journal/WAL/partial: never archived
                if rel.as_posix() == _DB_NAME:
                    continue  # added below through SQLite's own backup API
...
def _safe_members(zf: zipfile.ZipFile, data_dir: Path) -> list[tuple[str, Path]]:
    ...
    for info in zf.infolist():
        ...
        if _is_volatile(parts[-1]):
            continue  # an older archive's journal/WAL would roll the DB back
