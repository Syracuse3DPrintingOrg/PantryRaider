...
def humidity_from(value, state) -> float | None:
    """A 0-100 humidity percentage from an ESPHome humidity sensor's
    value/state, or None. Pure; rejects non-numbers, NaN, and out-of-range."""
    try:
        pct = float(value)
    except (TypeError, ValueError):
        return None
    if pct != pct:  # NaN, ESPHome's "no reading yet"
        return None
...
def hygro_sensor_reading(host: str, sensor: str, temp_data: dict,
                         humidity_data: dict | None, now: float,
                         battery=None) -> dict | None:
    """Turn an ESP temperature sensor body (plus an optional humidity
    companion body) into a gadgets hygrometer entry (kind="hygrometer").
    Pure. The temperature drives it: no usable temperature, no reading; a
    missing or unusable humidity companion just yields humidity None."""
    base = sensor_reading(host, sensor, temp_data, now, battery=battery)
    if base is None:
        return None
    humidity = None
    if isinstance(humidity_data, dict):
        humidity = humidity_from(humidity_data.get("value"),
                                 humidity_data.get("state"))
    return {
        "id": base["id"],
        "kind": "hygrometer",
        "name": base["name"],
...
def configured_devices() -> list[dict]:
    ...
    for dev in settings.gadget_esp_devices or []:
        ...
        # A hygrometer entry (FoodAssistant-q97i): kind routes the reading to
        # the hygrometer class, humidity names the companion sensor read
        # alongside the temperature.
        if dev.get("kind") == "hygrometer":
            clean["kind"] = "hygrometer"
        humidity = str(dev.get("humidity") or "").strip().lower()
        if humidity and valid_sensor(humidity):
            clean["humidity"] = humidity
        out.append(clean)
    return out
...
async def poll_once(client: httpx.AsyncClient | None = None,
    ...
    if own_client:
        # An ESP lives on the LAN, so private addresses are allowed, but the
        # host comes from saved settings and the guard still keeps a poll off
        # loopback and the metadata address.
        from . import egress
        client = egress.guarded_async_client(allow_private=True, timeout=8.0)
    readings = []
    try:
        for dev in devices:
            ...
            if dev.get("kind") == "hygrometer":
                hum_body = None
                if dev.get("humidity"):
                    hum_body = await _read_sensor(client, host, dev["humidity"],
                                                  dev.get("auth"))
                reading = hygro_sensor_reading(host, sensor, body, hum_body,
                                               now, battery=battery)
            else:
...
async def discover_sensors(host: str,
    ...
    if own_client:
        from . import egress
        client = egress.guarded_async_client(
            allow_private=True, timeout=DISCOVER_SECONDS + 3.0)
    chunks: list[str] = []
    try:
...
def _screen_mmss(seconds) -> str:
    """A compact clock string for a small screen: m:ss, or h:mm:ss past an
    hour. Pure; negatives clamp to 0:00."""
    try:
...
def compose_screen(timers: list[dict], recipe_title: str = "",
                   max_lines: int = 4) -> list[str]:
    """Build compact display lines for a small ESP screen from local state.

    Pure. ``timers`` is timers.list_timers() output; ``recipe_title`` is the
    active recipe name or "". A finished timer comes first (it needs
    attention), then running timers soonest-first, then the active recipe if
    there is room. Labels are trimmed to fit a narrow OLED. Always returns at
    least one line so a polling screen never goes blank."""
    try:
        cap = max(1, min(8, int(max_lines)))
    except (TypeError, ValueError):
        cap = 4
    lines: list[str] = []
    rows = [t for t in timers if isinstance(t, dict)]
    done = [t for t in rows if t.get("expired")]
    running = [t for t in rows if not t.get("expired")]
    running.sort(key=lambda t: t.get("remaining_seconds") or 0)
    for t in done:
        lines.append(f"{str(t.get('label') or 'Timer')[:14]} DONE")
    for t in running:
        label = str(t.get("label") or "Timer")[:14]
        lines.append(f"{label} {_screen_mmss(t.get('remaining_seconds'))}")
    if recipe_title and len(lines) < cap:
        lines.append(f"Cook: {str(recipe_title)[:14]}")
    if not lines:
        lines.append("Pantry Raider")
    return lines[:cap]
