...
# The catalog only changes when the deck software updates, but /ui/start is the
# kiosk HOME page and the bridge round trip cost ~330 ms per load on a Pi
# (measured on the Bandit, FoodAssistant-7dt9). Cache the resolved catalog for
# a few minutes so the home page renders in the same ~10 ms as every other page.
...
async def fetch_deck_catalog() -> list[dict]:
    ...
    the same face as the editor and the deck. Cached for a few minutes: the
    catalog is static between deck software updates."""
    import time
    now = time.monotonic()
    if _catalog_cache["value"] is not None and now - _catalog_cache["at"] < _CATALOG_TTL_SECONDS:
    ...
    if is_raspberry_pi():
        try:
            ...
            pass
    if result is None:
        result = bundled_catalog() or FALLBACK_CATALOG
...
# Glance used to be a separate hardcoded render branch; now it is a seed for the
# same layout engine Custom uses: the top-level nav pages become builtin keys in
# the resolved-layout shape, on the smallest deck grid that fits them.

# Rotation used when a nav page has no matching deck action to borrow a colour
# from. Drawn from the same palette the deck catalog uses so a mixed grid still
# reads as one set.
_GLANCE_COLOR_ROTATION = (
    "#0f766e", "#7e22ce", "#b45309", "#1d4ed8",
    "#0e7490", "#15803d", "#b54708", "#475569",
)

# Deck actions whose on-screen press fires server-side instead of navigating
# (the FIRE_KEYS set in start.html). A Glance seed token must navigate to its
# page, so the href-to-action match skips these.
_NON_NAVIGATING_ACTIONS = {
    "timer_1", "timer_2", "timer_3", "timer_eggs", "timer_pasta", "timer_rice",
}


def glance_key_count(page_count: int) -> int:
    """The smallest deck grid that fits ``page_count`` pages (largest if none do),
    so the Glance grid sizes itself to the nav instead of a fixed shape."""
    for n in sorted(VALID_KEY_COUNTS):
        if n >= page_count:
            return n
    return max(VALID_KEY_COUNTS)


def _glance_action_for(page: dict, cat: dict) -> str | None:
    """The deck action a nav page corresponds to, if one exists.

    Most top-level nav keys are also deck action names with the same page
    (inventory, cook, shopping, ...); a heading like Time & Temp lands on a
    child's href, so fall back to any catalog action that opens the same page.
    Press-fires-a-timer actions are skipped: a Glance key must navigate."""
    key = str(page.get("key") or "")
    href = str(page.get("href") or "")
    if key in cat and ACTION_HREF.get(key) == href:
        return key
    for name, h in ACTION_HREF.items():
        if h == href and name in cat and name not in _NON_NAVIGATING_ACTIONS:
            return name
    return None


def glance_layout(nav_pages: list[dict] | None, keys: int | None = None,
                  catalog: list | None = None) -> list[dict]:
    """The Glance home as a resolved Start Page layout.

    Turns the top-level nav pages (navigation.glance_pages() entries: key,
    label, icon, href) into the same render-ready key dicts resolve_layout
    produces, so /ui/start draws Glance through the one grid path. Every key
    is a navigating builtin; colours come from the matching deck action where
    one exists (so Stock is the deck's Stock colour) with a stable rotation
    otherwise; the grid is the smallest deck shape that fits the pages unless
    ``keys`` pins one. Padded with blanks to fill the grid."""
    cat = {a["name"]: a for a in (catalog or bundled_catalog() or FALLBACK_CATALOG)
           if isinstance(a, dict) and a.get("name")}
    pages = [p for p in (nav_pages or []) if isinstance(p, dict) and p.get("href")]
    key_count = glance_key_count(len(pages)) if keys is None else normalize_key_count(keys)
    out: list[dict] = []
    for i, p in enumerate(pages[:key_count]):
        action = _glance_action_for(p, cat)
        color = (cat[action].get("color") or "") if action else ""
        if not color:
            color = _GLANCE_COLOR_ROTATION[i % len(_GLANCE_COLOR_ROTATION)]
        out.append({"kind": "builtin", "key": str(p.get("key") or ""),
                    "label": str(p.get("label") or ""),
                    "icon": _norm_icon(p.get("icon")),
                    "color": color, "href": str(p["href"])})
    while len(out) < key_count:
        out.append({"kind": "blank"})
    return out


def glance_seed_tokens(nav_pages: list[dict] | None, keys: int | None = None,
                       catalog: list | None = None) -> list[str]:
    """The Glance arrangement as stored-layout tokens, to seed the Custom editor.

    Each nav page becomes its matching deck action token where one exists; a
    page with no deck equivalent seeds as a blank (the stored layout can only
    hold deck tokens). Same grid-size rule and padding as glance_layout, so the
    seeded editor mirrors the Glance grid the user is leaving."""
    cat = {a["name"]: a for a in (catalog or bundled_catalog() or FALLBACK_CATALOG)
           if isinstance(a, dict) and a.get("name")}
    pages = [p for p in (nav_pages or []) if isinstance(p, dict) and p.get("href")]
    key_count = glance_key_count(len(pages)) if keys is None else normalize_key_count(keys)
    toks = [(_glance_action_for(p, cat) or "") for p in pages[:key_count]]
    while len(toks) < key_count:
        toks.append("")
    return toks
