...
def ingest(payload: dict, now: float | None = None, *,
    ...
    with _lock, state_write_lock(_state_file()):
        ...
        stemma_state = _state.setdefault("stemma", {})
        stemma_seen = _state.setdefault("stemma_discovered", {})
        for entry in devices_in or []:
            ...
            if isinstance(entry, dict) and entry.get("kind") == "button":
                ...
                continue
            if isinstance(entry, dict) and entry.get("kind") == "stemma":
                # Plug-in QT accessories (FoodAssistant-etsc) are bus-powered
                # and carry no reading of their own; the push is a heartbeat,
                # so the card can say plugged in or unplugged.
                from . import stemma as stemma_svc
                beat = stemma_svc.normalize_heartbeat(entry, now)
                if beat:
                    stemma_state[beat["id"]] = beat
                continue
        ...
        for entry in discovered_in or []:
            ...
            if entry.get("kind") == "contact":
                ...
                if source:
                    contact_seen[dev_id]["via"] = source
                continue
            if entry.get("kind") == "button":
                continue  # routed to gadgets_buttons (FoodAssistant-771d)
            if entry.get("kind") == "stemma":
                # Plug-in QT accessories (FoodAssistant-etsc). Ids are
                # bus-plus-address, not MACs, so they keep their own case
                # rules; a board we can see but cannot drive rides along with
                # supported=False, the same courtesy the BLE scan extends.
                from . import stemma as stemma_svc
                found = stemma_svc.normalize_discovered(entry, now)
                if found and found["id"] not in stemma_ids:
                    stemma_seen[found["id"]] = found
                continue
        ...
        _state["stemma"] = {k: v for k, v in stemma_state.items()
                            if now - v.get("ts", 0) <= PRUNE_SECONDS}
        _state["stemma_discovered"] = {k: v for k, v in stemma_seen.items()
                                       if now - v.get("ts", 0) <= DISCOVERED_TTL
                                       and k not in stemma_ids}
