...
and every mutation's read-modify-write (a readings ingest, the reader
heartbeat, the protection sweep) additionally holds the shared cross-process
file lock (services/state_lock.py), so two workers ingesting a push or
sweeping for alarms at the same time can no longer lose one of the updates.
...
# How long a queued NeoKey test-light request stays live (FoodAssistant-kh1m).
# Long enough that the agent's 2-3 second outputs poll cannot miss it, short
# enough that a click never flashes a key a minute later.
STEMMA_TEST_TTL = 10.0
...
# What the Accessories pane says about the I2C bus before the host reader has
# ever checked in on this device (FoodAssistant-dxr22).
I2C_NOT_SET_UP_DETAIL = "The accessory reader is not set up on this device yet."


def i2c_status(i2c: dict | None, reader_seen: float) -> tuple[bool, str]:
    """The Accessories pane's bus line as (available, detail). Pure.

    Honest by default (FoodAssistant-dxr22): the old default reported the bus
    as fine before any reader had EVER checked in, so a fresh device showed
    "nothing new plugged in, check the cable" when the truth was that the
    reader was never installed. Until the first reader contact (reader_seen
    0) the bus reads unavailable with a plain "not set up yet" detail; once a
    reader has checked in, its own report keeps today's semantics unchanged.
    """
    if not reader_seen:
        return False, I2C_NOT_SET_UP_DETAIL
    i2c = i2c if isinstance(i2c, dict) else {}
    return i2c.get("available") is not False, str(i2c.get("detail") or "")
...
_state: dict = {"devices": {}, "discovered": {}, "alerts": {}, "reader_seen": 0.0,
                ...
                "stemma": {}, "stemma_discovered": {},
                "i2c": {"available": True, "detail": ""},
                "relay_seen": 0.0, "relay_source": ""}
...
def _load_locked() -> None:
    ...
    if isinstance(data, dict):
        ...
        _state = {
            ...
            # Plug-in QT accessories (FoodAssistant-etsc): heartbeats, the
            # bus sweep's finds, and the bus's own health.
            "stemma": data.get("stemma") if isinstance(data.get("stemma"), dict) else {},
            "stemma_discovered": data.get("stemma_discovered") if isinstance(data.get("stemma_discovered"), dict) else {},
            "i2c": data.get("i2c") if isinstance(data.get("i2c"), dict) else {"available": True, "detail": ""},
            # Satellite relay bookkeeping (FoodAssistant-me3t): when the
            # freshest push arrived through a satellite, and which one.
            "relay_seen": (float(data.get("relay_seen"))
                           if isinstance(data.get("relay_seen"), (int, float)) else 0.0),
            "relay_source": str(data.get("relay_source") or "")[:60],
        }
...
def _alarms_deferred_upstream() -> bool:
    """True when this install relays its gadget data to a main server
    (FoodAssistant-me3t), which then owns every alarm: probe target alerts
    and the protection sweep both stay quiet locally so one breach or
    crossing cannot page the kitchen from two devices at once."""
    try:
        from . import gadgets_relay
        return gadgets_relay.relay_active()
    except Exception:  # noqa: BLE001 - alarm evaluation must never crash
        return False
...
def set_stemma_key_test(device_id: str, key: int, now: float | None = None) -> dict:
    """Ask the agent to light one NeoKey key (FoodAssistant-kh1m).

    The app cannot touch the I2C bus (the agent owns it), so a Test click
    parks a request in the shared state file and the agent picks it up on its
    next outputs poll. It lives in this file rather than a new one because it
    is gadget state with the same multi-worker problem: the click may land on
    a different uvicorn worker than the poll.
    """
    now = time.time() if now is None else now
    entry = {"id": str(device_id or ""), "key": int(key), "ts": now}
    with _lock, state_write_lock(_state_file()):
        _load_locked()
        _state["stemma_test"] = entry
        _save_locked()
    return entry


def stemma_key_test(now: float | None = None) -> dict | None:
    """A pending key-test request, or None when there is none or it has aged
    out. Kept for a few seconds so a poll cannot miss it; the agent flashes a
    given ts once, so re-serving it is harmless."""
    now = time.time() if now is None else now
    with _lock:
        _load_locked()
        entry = _state.get("stemma_test")
    if not isinstance(entry, dict):
        return None
    if now - float(entry.get("ts") or 0) > STEMMA_TEST_TTL:
        return None
    return dict(entry)
...
def ingest(payload: dict, now: float | None = None, *,
    ...
    A push relayed from a satellite (FoodAssistant-me3t) carries a top-level
    "source" tag (the satellite's hostname). It is stored per device as
    "via", so the UI can say which satellite heard a sensor; a device also
    heard by this install's own radio simply keeps whichever ingest wrote
    last (every push carries only fresh readings), with "via" recording the
    reading's origin ("" means this device's own radio or pollers)."""
    now = time.time() if now is None else now
    ...
    i2c_in = payload.get("i2c") if isinstance(payload, dict) else None
    source = (str(payload.get("source") or "").strip()[:60]
              if isinstance(payload, dict) else "")

    configured = configured_devices()
    targets = _targets_map(configured)
    # Device-family routing lives on the SensorDecoder inhabitants of the
    # integrations registry (FoodAssistant-pjtq): each family ("thermometer"
    # the default, "hygrometer", "contact", "button", "stemma") owns how its
    # entries land in this state file and its prune windows, so adding a
    # family is registering a decoder, not editing this function. The id
    # sets are gathered before the lock, exactly as the inline branches
    # always did.
    from ..integrations import sensors as sensor_seam
    decoders = sensor_seam.ingest_decoders()
    by_kind = {d.wire_kind: d for d in decoders}
    fallback = sensor_seam.fallback_decoder(decoders)
    ids_by_kind = {d.wire_kind: d.configured_ids() for d in decoders}

    with _lock, state_write_lock(_state_file()):
        _load_locked()
        for entry in devices_in or []:
            kind = entry.get("kind") if isinstance(entry, dict) else None
            decoder = (by_kind.get(kind) if isinstance(kind, str) else None) or fallback
            decoder.store_reading(_state, entry, now, source)
        for entry in discovered_in or []:
            ...
            kind = entry.get("kind")
            decoder = (by_kind.get(kind) if isinstance(kind, str) else None) or fallback
            decoder.store_discovered(_state, entry, dev_id, protocol, now, source,
                                     ids_by_kind.get(decoder.wire_kind) or set())
        # Prune what is long gone so the file cannot grow without bound;
        # each family keeps its own windows.
        for decoder in decoders:
            decoder.prune(_state, now, ids_by_kind.get(decoder.wire_kind) or set())
        ...
        # A satellite that relays upstream leaves alarm evaluation to the
        # server (FoodAssistant-me3t): the server has the fleet's thresholds
        # and the warning channel, and one crossing must not page twice.
        deferred = _alarms_deferred_upstream()
        if deferred:
            fired = []
        else:
        ...
        if source:
            # Remember that (and through which satellite) relayed data is
            # arriving, so the Settings pane can tell "no reader installed"
            # apart from "readings arrive via a satellite".
            _state["relay_seen"] = now
            _state["relay_source"] = source
        if mark_reader:
            _state["reader_seen"] = now
            ...
            # I2C bus health rides the same push (FoodAssistant-etsc), so the
            # accessories card can say "no I2C bus here" or "permission
            # denied" rather than showing an empty list.
            if isinstance(i2c_in, dict):
                _state["i2c"] = {
                    "available": i2c_in.get("available") is not False,
                    "detail": str(i2c_in.get("detail") or "")[:200],
                }
        _save_locked()
...
def get_state(now: float | None = None) -> dict:
    ...
    from ..config import settings
    from . import stemma as stemma_svc
    now = time.time() if now is None else now
    configured = configured_devices()
    ...
    with _lock:
        ...
        relay_seen = float(_state.get("relay_seen") or 0.0)
        relay_source = str(_state.get("relay_source") or "")
        ...
        protection_raw = {k: dict(v) for k, v in (_state.get("protection") or {}).items()
                          if isinstance(v, dict)}
        i2c = _state.get("i2c") if isinstance(_state.get("i2c"), dict) else {}
        stemma_raw = {k: dict(v) for k, v in (_state.get("stemma") or {}).items()
                      if isinstance(v, dict)}
        stemma_ids = {stemma_svc.norm_id(d.get("id"))
                      for d in stemma_svc.configured_devices()}
        stemma_discovered_raw = [dict(v) for v in (_state.get("stemma_discovered") or {}).values()
                                 if isinstance(v, dict)
                                 and now - v.get("ts", 0) <= DISCOVERED_TTL
                                 and stemma_svc.norm_id(v.get("id")) not in stemma_ids]
    ...
    for dev in configured:
        ...
        devices.append({
            ...
            # Which satellite relayed the freshest reading ("" = heard by
            # this install's own radio or pollers), FoodAssistant-me3t.
    ...
    i2c_available, i2c_detail = i2c_status(i2c, reader_seen)
    return {
        ...
        # Plug-in STEMMA QT / Qwiic accessories (FoodAssistant-etsc): the
        # configured boards with their heartbeat, plus what the agent's bus
        # sweep found that is not added yet. i2c reports the bus itself, so
        # the pane can say "no I2C bus on this device" instead of showing an
        # empty list with no explanation.
        "stemma_enabled": bool(settings.stemma_enabled),
        "stemma": stemma_svc.device_cards(stemma_raw, now),
        "stemma_discovered": stemma_discovered_raw,
        # What a NeoKey key can be set to (grouped: modes, pages, timer
        # actions), so the Accessories pane renders the server's catalog
        # instead of keeping its own copy that drifts (FoodAssistant-bo9v).
        "stemma_choices": stemma_svc.keymap_choices(),
        "i2c_available": i2c_available,
        "i2c_detail": i2c_detail,
        ...
        # Satellite relay health (FoodAssistant-me3t): seconds since the last
        # relayed push (None = never) and which satellite it came from, so a
        # radio-less server can show "readings arrive via <satellite>".
        "relay_age_seconds": (round(now - relay_seen, 1)
                              if relay_seen > 0 else None),
...
def run_protection_sweep(now: float | None = None) -> list:
    ...
    A satellite whose gadget relay is on defers to its main server
    (FoodAssistant-me3t): the sweep is a no-op there, so the server alone
    evaluates the fleet's thresholds against the relayed readings."""
    if _alarms_deferred_upstream():
        return []
    now = time.time() if now is None else now
...
def alarm_level(breach: dict) -> str:
    """How loud one breach reads: "error" for a temperature out of range or a
    door left open, "warning" for humidity and a sensor gone quiet. Pure, and
    the single rule every surface shares, so a toast raised here and a toast
    mirrored on a satellite carry the same weight."""
    return ("error" if (breach or {}).get("kind") in ("temp_high", "temp_low", "open")
            else "warning")


def active_alarms(now: float | None = None) -> list[dict]:
    """The live protection alarms, for the Cub summary and any other surface:
    [{key, kind, device_id, name, location, message, level, started_epoch}],
    oldest first. Reads the shared state only; evaluation happens in the sweep.

    ``key`` is the protection key the sweep fires on (hygro / hygro-stale /
    contact plus the device id), so a consumer can tell two alarms on one
    sensor apart and show each one once (FoodAssistant-me3t)."""
    now = time.time() if now is None else now
    unit = display_unit()
    with _lock:
        _load_locked()
    ...
    for key, entry in prot.items():
        ...
        out.append({
            ...
            "level": alarm_level(entry.get("breach") or {}),
