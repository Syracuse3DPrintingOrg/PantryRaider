...
def parse_recipe_file(filename: str, raw: bytes) -> dict:
    ...
    ingredients, ingredient_sections = _normalize_ingredients(
    ...
    # Only carry the parallel sections when the file actually grouped its
    # ingredients, so a plain recipe's parsed dict is unchanged.
    if any(ingredient_sections):
...
def _normalize_ingredients(value) -> tuple[list[str], list[str | None]]:
    """Reduce ingredients (strings or objects) to plain lines plus a parallel list
    of the section heading each line sits under (FoodAssistant-zq7k).

    Mealie's export marks a section with a ``title`` on a recipeIngredient entry;
    following untitled entries belong to it, and a title-only entry (no content)
    is a pure heading that contributes no line. A file with no titles yields an
    all-``None`` sections list, so a plain recipe imports exactly as before."""
    lines: list[str] = []
    ...
    for item in value or []:
        ...
        if isinstance(item, str):
            ...
            title = _first_str(item.get("title")).strip()
            if title:
                current = title
