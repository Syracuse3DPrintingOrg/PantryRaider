...
def food_weather_note(temp, unit: str = "F", desc: str = "", wind=None) -> str:
    ...
    nothing stands out. Pure and deterministic so it is fully unit-testable;
    ``temp`` is read in Fahrenheit internally, converting from Celsius when
    ``unit`` is "C"."""
    try:
        t = float(temp)
    except (TypeError, ValueError):
    ...
    if is_stormy:
        return "Thunder in the forecast: bring the grill session indoors and let the oven do the work."
    if is_snowy:
        return "Snow day. Time for cocoa and something simmering on the stove."
    if t is not None and t >= 90:
        return "Hot enough to fry an egg on the sidewalk. Maybe skip the stove today."
    if is_rainy and t is not None and t >= 60:
        return "Fire up the grill, but you might get wet."
    if is_rainy:
        return "A little damp out there. Good day to keep something warm on the stove."
    if t is not None and t <= 35:
        return "Soup weather, no question."
    if w is not None and w >= 20:
        return "Windy enough to blow the napkins off the table. Weigh them down."
    if t is not None and 65 <= t <= 80:
        return "Kitchen weather is pretty good today too."
    return ""
