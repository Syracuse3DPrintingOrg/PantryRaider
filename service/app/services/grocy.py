...
def stock_sniff_candidates(stock: list[dict], days: int = 7,
    ...
    entry has nothing to extend. When a name appears more than once the
    earliest date wins, since that is the entry driving the expiring row.
    """
    if today is None:
        today = date.today()
    out: dict[str, dict] = {}
            ...
            if from_id:
                ...
                if propose_best_by is not None and old_iso:
                    ...
                    if new_date is not None:
                        ...
                        if entry_id:
                            await self._request(
                                "PUT", f"/objects/stock/{entry_id}",
                                {**entry, "best_before_date": new_date.isoformat()})
