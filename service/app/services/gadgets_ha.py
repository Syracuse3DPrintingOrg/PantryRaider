...
def hygro_device_id_for(temp_entity: str) -> str:
    """The gadgets hygrometer device id an HA pair maps to, keyed by its
    temperature entity. The HAH: prefix keeps it apart from a thermometer
    reading the same entity (ids are kept uppercase)."""
    return f"HAH:{str(temp_entity or '').strip().upper()}"


def configured_hygro_pairs() -> list[dict]:
    """The sanitized gadget_ha_hygrometers list from settings: each a dict
    with a valid temperature entity, an optional humidity companion, and a
    name. Malformed rows are dropped."""
    from ..config import settings
    out: list[dict] = []
    seen: set[str] = set()
    for pair in settings.gadget_ha_hygrometers or []:
        if not isinstance(pair, dict):
            continue
        temp_entity = str(pair.get("temperature") or "").strip().lower()
        if not valid_entity_id(temp_entity) or temp_entity in seen:
            continue
        seen.add(temp_entity)
        hum_entity = str(pair.get("humidity") or "").strip().lower()
        out.append({
            "temperature": temp_entity,
            "humidity": hum_entity if valid_entity_id(hum_entity) else "",
            "name": str(pair.get("name") or temp_entity)[:60],
        })
    return out


def hygro_reading(pair: dict, temp_data: dict, humidity_data: dict | None,
                  now: float, stale_after: float = ENTITY_STALE_SECONDS) -> dict | None:
    """Turn an HA temperature + humidity entity pair into a gadgets
    hygrometer entry (kind="hygrometer"). Pure. The temperature entity
    drives it: no usable temperature, no reading. A missing or unusable
    humidity companion just yields humidity None; battery comes from either
    entity's battery_level attribute. Fahrenheit is converted; downstream is
    always Celsius."""
    base = entity_reading(pair.get("temperature"), temp_data, now,
    ...
    if isinstance(humidity_data, dict):
        ...
        if state not in (None, "", "unavailable", "unknown"):
            ...
            if humidity is not None and not (0.0 <= humidity <= 100.0):
                humidity = None
        if battery is None:
            attrs = humidity_data.get("attributes")
            if isinstance(attrs, dict) and attrs.get("battery_level") is not None:
                try:
                    battery = max(0, min(100, int(attrs["battery_level"])))
                except (TypeError, ValueError):
                    battery = None
    return {
        "id": hygro_device_id_for(pair.get("temperature")),
        "kind": "hygrometer",
        "name": str(pair.get("name") or pair.get("temperature") or "")[:60],
        "protocol": "home_assistant",
        "temp_c": base["probes"][0]["temp_c"],
        "humidity": humidity,
        "battery": battery,
    }


def is_humidity_entity(entity: dict) -> bool:
    """Whether a GET /api/states row looks like a humidity sensor, for the
    hygrometer pair picker. Pure: device_class humidity, or a sensor with a
    percent unit whose id or name mentions humidity."""
    if not isinstance(entity, dict):
        return False
    ...
    return unit == "%" and ("humid" in entity_id.lower() or "humid" in name.lower())
...
def source_active() -> bool:
    """Whether the HA source has everything it needs to poll: the connection,
    the toggle, and at least one thermometer entity or hygrometer pair."""
    from ..config import settings
    base, token = ha_connection()
    return bool(settings.gadget_ha_enabled and base and token
                and (configured_entities() or configured_hygro_pairs()))
...
async def poll_once(client: httpx.AsyncClient | None = None,
    ...
    pairs = configured_hygro_pairs()
    if not (base and token and (entities or pairs)):
        return 0
    headers = {"Authorization": f"Bearer {token}"}
    ...
    try:
        for entity_id in entities:
            data = await _get_state(entity_id)
            if data is None:
                continue
            reading = entity_reading(entity_id, data, now)
            if reading:
                readings.append(reading)
        # Hygrometer pairs (FoodAssistant-q97i): the temperature entity
        # drives the reading, the humidity companion rides along.
        for pair in pairs:
            temp_data = await _get_state(pair["temperature"])
            if temp_data is None:
                continue
            hum_data = (await _get_state(pair["humidity"])
                        if pair.get("humidity") else None)
            reading = hygro_reading(pair, temp_data, hum_data, now)
            if reading:
                readings.append(reading)
    finally:
        if own_client:
...
async def list_hygro_entities(client: httpx.AsyncClient | None = None
                              ) -> tuple[list[dict], list[dict]]:
    """(temperature entities, humidity entities) from HA for the hygrometer
    pair picker, from one /api/states pull. Both lists are [] when HA is
    unconfigured or unreachable (the picker degrades to plain text fields)."""
    base, token = ha_connection()
    if not (base and token):
        return [], []
    own_client = client is None
    ...
    def _row(entity: dict) -> dict:
        ...
        return {
            ...
            "name": str(attrs.get("friendly_name") or entity.get("entity_id"))[:60],
            "state": entity.get("state"),
            "unit": attrs.get("unit_of_measurement") or "",
            "has_value": has_current_value(entity.get("state")),
        }

    temps, hums = [], []
    for row in rows if isinstance(rows, list) else []:
        if is_temperature_entity(row):
            temps.append(_row(row))
        elif is_humidity_entity(row):
            hums.append(_row(row))
    temps.sort(key=lambda e: str(e.get("name") or "").lower())
    hums.sort(key=lambda e: str(e.get("name") or "").lower())
    return temps, hums
