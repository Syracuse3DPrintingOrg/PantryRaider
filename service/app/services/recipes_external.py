...
# Brand/size/packaging words specific to retail product names; joined with
# the descriptor/stop-word sets already maintained in mealie.py by
# _noise_words() below.
_RETAIL_NOISE = (
    _PREP_MODIFIERS
    | {
...
@lru_cache(maxsize=1)
def _noise_words() -> frozenset:
    """The full noise-word set: the descriptor/stop-word sets already
    maintained in mealie.py plus the retail words above. Built on first use
    so this module does not import mealie at load time (the Mealie backend
    is optional; these two token sets are pure helpers it happens to keep)."""
    from .mealie import _PHRASE_MODIFIERS, _STOP_WORDS
    return frozenset(_STOP_WORDS | _PHRASE_MODIFIERS | _RETAIL_NOISE)
