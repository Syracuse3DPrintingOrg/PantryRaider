...
async def fire_key(name: str, long: bool = False) -> dict:
    ...
    if otype == "shopping_add":
        # Quick-add the key's item to the Mealie shopping list, mirroring the
        # physical deck's behavior (first list is the default one the Shopping
        # page shows). Suggested buttons (FoodAssistant-1a8h) place these keys
        # on the Start Page, so an on-screen tap must work too.
        ...
        try:
            ...
            if not lists:
                return {"ok": False, "detail": "Mealie has no shopping list yet."}
            await m.add_shopping_item(lists[0]["id"], item, 1.0)
