...
async def fire_key(name: str, long: bool = False) -> dict:
    ...
    A thin shim over the actions seam (FoodAssistant-pjtq): the registry
    asks each ActionProvider in turn, and this module's ``execute_token`` is
    the built-in provider's engine, so every existing caller (the Start
    Page, the ESP button route, the BLE button mappings) keeps this entry
    point and gets the same answers it always has."""
    from ..integrations import actions as action_seam
    return await action_seam.fire_key(name, long=long)


async def execute_token(name: str, long: bool = False) -> dict | None:
    """Execute a built-in vocabulary token, or return None for a token this
    provider does not own (the seam then reports the unknown key)."""
    from ..config import settings
    slot = resolve_ha_slot(name, settings.streamdeck_ha_slots)
    if slot:
        ok, detail = await call_ha_service(*slot)
    ...
    if otype == "shopping_add":
        # Quick-add the key's item to the shopping list through the shopping
        # seam (the same list the Shopping page shows, whichever backend holds
        # it). Suggested buttons (FoodAssistant-1a8h) place these keys on the
        # Start Page, so an on-screen tap must work too.
        item = str(override.get("item", "") or override.get("label", "")).strip()
        if not item:
            return {"ok": False, "detail": "This key has no item configured."}
        try:
            from . import shopping_source
            await shopping_source.quick_add(item)
            return {"ok": True, "detail": f"Added {item} to the shopping list."}
        except Exception as e:  # noqa: BLE001 - a key press never raises
            return {"ok": False, "detail": f"Could not add {item}: {e}"}
    if otype == "timer":
