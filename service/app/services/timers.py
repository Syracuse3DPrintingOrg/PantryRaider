...
Concurrency choice: writes are atomic (temp file + os.replace) but
read-modify-write across workers is not serialized, so two simultaneous
creates in different workers could race. This registry serves a single
kitchen (a handful of timers, created by a human tapping a button), so that
window is accepted rather than paying for a cross-process lock file.
"""
