...
def build_outputs(mode_state: dict, alarms: list, timers: list,
                  now: float | None = None, key_test: dict | None = None) -> dict:
    ...
    out = {
        ...
        "mode_colors": {m: list(c) for m, c in MODE_COLORS.items()},
        "alarm_active": bool(alarms),
        "timer": {
            "ringing": ringing,
            "running": len(remaining),
            "soonest_remaining": int(min(remaining)) if remaining else None,
        },
        "generated": int(now),
    }
...
# current_mode_state). The NeoKey polls outputs every 2 to 3 seconds and the
# kiosk may poll too; this collapses that into roughly one upstream request
# per interval, and a couple of seconds of lag on a mode someone changed on
# the server is invisible next to the optimistic local LED update.
