...
def record(name: str, barcode: Optional[str], storage_type: str,
    ...
    try:
        ...
        if point is None:
            return False
        # Cross-process lock too: appending to the queue is a read-modify-write
        # of the whole list, and a server runs several uvicorn workers, so
        # without it one worker's queued correction is silently dropped by
        # another's write (FoodAssistant-dxl0).
        with _lock, state_write_lock(_queue_file()):
            points = _load_locked()
            points.append(point)
            _save_locked(points)
        return True
...
def _remove_points(sent: list[dict]) -> None:
    ...
    # Same read-modify-write, same reason: without the cross-process lock a
    # concurrent record() can be lost when the uploaded batch is removed.
    with _lock, state_write_lock(_queue_file()):
        points = _load_locked()
        for p in sent:
            try:
