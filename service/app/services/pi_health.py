...
# Settings pane that owns the fix for a warning key, so its toast can deep-link
# straight there instead of dropping the user on the generic /setup landing
# page (FoodAssistant-44f6). All the device-health warnings surface on the
# Network pane's "This device needs attention" banner (see
# templates/setup/_pane_network.html); an unfamiliar key falls back to that
# same pane since that is where Device health status lives on Status too.
_DEFAULT_PANE = "pane-network"
_WARNING_PANE = {
    "undervoltage": "pane-network",
    "throttled": "pane-network",
    "freq_capped": "pane-network",
    "temp_limit": "pane-network",
    "temperature": "pane-network",
    "disk": "pane-network",
}
...
def warning_pane(key: str) -> str:
    """Settings pane id a warning's toast should deep-link to. Pure; falls
    back to the Network pane (where every device-health condition surfaces)
    for a key we don't have a bespoke mapping for."""
    return _WARNING_PANE.get(key, _DEFAULT_PANE)
...
async def poll_and_toast(fetch_warnings) -> int:
    ...
    for w in new_toasts:
        ...
        ha_events.add_warning(message, title=title, key=key,
                              level=warning_level(key), pane=warning_pane(key))
