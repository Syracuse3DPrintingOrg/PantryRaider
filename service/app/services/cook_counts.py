...
def top_counts(db: Session, min_count: int = 1, limit: int = 10) -> list[dict]:
    """The most-cooked recipes at or above ``min_count``, highest first.

    Backs the "frequently cooked recipes" signal for Suggested Custom Buttons
    (services/suggested_actions.py): a small ranked query rather than a full
    table scan, since the suggestions panel only ever needs the top few. Fails
    soft to [] so a database hiccup degrades to no suggestions rather than an
    error."""
    try:
        rows = (db.query(RecipeCookCount)
                .filter(RecipeCookCount.count >= min_count)
                .order_by(RecipeCookCount.count.desc())
                .limit(limit)
                .all())
    except Exception as exc:  # noqa: BLE001
        logger.info("cook count: top query failed: %s", exc)
        return []
    return [{"recipe_key": r.recipe_key, "title": r.title or r.recipe_key,
             "count": r.count or 0, "source": r.source} for r in rows]
