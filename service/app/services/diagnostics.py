...
def configure_console_logging(level: int = logging.INFO) -> None:
    ...
    if root.level == logging.NOTSET or root.level > level:
        root.setLevel(level)
    # Per-request access lines are off unless asked for. uvicorn logs one line
    # for every request, and a kitchen screen polls every few seconds on top
    # of every static asset and every Home Assistant sensor read, so on a Pi
    # that is a write to the SD card several times a second, forever, into a
    # container log that Docker never trims. The satellite's systemd unit
    # already passed --no-access-log for this reason; this makes the image do
    # the same. The app's own INFO lines still reach the console, and the
    # debug download still captures everything it did. Set PR_ACCESS_LOG=1 to
    # get the request lines back (the dev compose does, for the console).
    # This runs at app import, after uvicorn has set its access logger up,
    # so the level sticks.
    if os.environ.get("PR_ACCESS_LOG", "").strip().lower() not in ("1", "true", "yes", "on"):
