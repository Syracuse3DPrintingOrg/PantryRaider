...
cross-process file lock (services/state_lock.py), so two workers changing
the active recipe (or a course) at the same time can no longer lose one of
the updates. Reads stay lock-free: the mtime cache keeps a read at one stat
call.
"""
...
class Ingredient:
    ...
    section is the ingredient-group heading the line sits under (zq7k), or None
    for an ungrouped line, so the On the Line view can show the recipe's own
    headings (sz4h)."""
    name: str
...
def _from_stored(data: dict) -> ActiveRecipe:
    ...
    ings = [
        Ingredient(name=str(i.get("name", "")), quantity=i.get("quantity"),
                   unit=i.get("unit"),
                   section=(str(i.get("section")).strip() or None)
                   if i.get("section") else None)
...
def _normalize_ingredient(raw) -> Ingredient:
    ...
    return Ingredient(
        ...
        section=(str(raw.get("section")).strip() or None) if raw.get("section") else None,
    )
...
def set_active(recipe_dict: dict) -> dict:
    ...
    with _lock, state_write_lock(_recipe_path()):
...
def from_mealie_detail(detail: dict, slug: str = "") -> dict:
    ...
    Ingredient section headings carry through (sz4h): a ``title`` on an entry
    marks a section start (Mealie's own mechanism, which the native store's
    wire mirrors) and the following untitled entries belong to it, so the On
    the Line view can show the recipe's own headings. A recipe with no titles
    yields no section on any ingredient, exactly as before."""
    d = detail or {}
    ings: list[dict] = []
    ...
    for raw in d.get("recipeIngredient") or []:
        ...
        ing = _mealie_ingredient(raw)
        if not ing.get("name"):
            continue
        if current_section:
            ing["section"] = current_section
        ings.append(ing)
    steps = []
    ...
    return {
        ...
        # Carried through for the print document's quick-facts header
        # (FoodAssistant-gm4c); "performTime" is Mealie's raw field name for
        # cook time, the native store already writes cook time as "cookTime".
