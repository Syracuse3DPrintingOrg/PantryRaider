...
def firmware_manifest(profile: str, version: str) -> dict | None:
    """The ESP Web Tools manifest for one profile, or None for an unknown
    profile. The part path is same-origin-relative to the manifest URL
    (/cub/firmware/manifest.json), so ESP Web Tools fetches the binary back
    through our own proxy (a cross-origin part would be refused)."""
