...
def ota_asset_name(profile: str, version: str) -> str:
    """The release-asset / cache filename for a profile's app-only image, the
    one an over-the-air update flashes. CI attaches this alongside the factory
    image (bandit-cub-<profile>-<version>.ota.bin); releases built before it
    did carry only the factory image, which is why the slice below stays."""
    return f"bandit-cub-{profile}-{version}.ota.bin"
...
def local_ota_override_path(data_dir, profile: str) -> Path:
    """A locally built app image, the OTA twin of local_override_path:
    data_dir/cub-firmware/<profile>.ota.bin."""
    return firmware_dir(data_dir) / f"{profile}.ota.bin"


def cached_ota_path(data_dir, profile: str, version: str) -> Path:
    """The on-disk cache of a fetched .ota.bin release asset, keyed by
    version+profile like the factory cache beside it."""
    return firmware_dir(data_dir) / ota_asset_name(profile, version)
...
def ota_release_download_url(repo: str, version: str, profile: str) -> str:
    """The pinned release URL for a profile's published app image. Same
    constructed-from-constants github.com host as the factory URL above."""
    return (f"https://github.com/{repo}/releases/download/"
            f"v{version}/{ota_asset_name(profile, version)}")


# A factory image is the whole flash layout: bootloader, partition table, then
# the app. An update over the air replaces only the app. CI now attaches that
# app image (ESPHome's own firmware.ota.bin) to each release, so the server
# prefers it and trusts nothing about the flash layout. Releases published
# before that keep working through the slice below: the app image is the
# factory image from the app's flash offset on, and every board in the fleet
# uses ESPHome's default partition table, which puts the app at 0x10000. 0xE9
# is the magic byte every ESP app image starts with, which is what makes both
# paths checkable rather than hopeful.
CUB_APP_OFFSET = 0x10000
_ESP_IMAGE_MAGIC = 0xE9


def valid_ota_image(data: bytes | None) -> bytes | None:
    """The bytes back when they look like an ESP app image, else None. What a
    published .ota.bin is checked against before it reaches a device, the same
    check the slice below applies to what it cuts out. Pure."""
    if not data:
        return None
    return data if data[0] == _ESP_IMAGE_MAGIC else None
...
def firmware_ota_block_for_app(profile: str, app_image: bytes | None) -> dict | None:
    """The manifest's ota block for one profile, built from the app image
    itself: where a Cub fetches it and the md5 it must match. None when there
    is no usable image, in which case the manifest simply goes out without it.
    Pure.
...
def firmware_ota_block(profile: str, factory_image: bytes | None) -> dict | None:
    """The ota block for a profile whose only image is a factory one: slice the
    app out, then hash it. The fallback shape, for a release published before
    CI attached the app image on its own. Pure."""
    return firmware_ota_block_for_app(profile, ota_image_from_factory(factory_image))
...
# The decoders live in the reader package, which ships INSIDE the app image
# (service/Dockerfile copies gadgets/foodassistant_gadgets to /app), so the
# relay works on a plain Docker server with no radio of its own. The import is
# still guarded: a source checkout run outside the container, or a hand-built
# image, may not have it, and there the relay reports itself unavailable,
# publishes no allowlist, and accepts nothing rather than pretending to work.
# See ble_relay_available().
...
def ble_relay_available() -> bool:
    ...
    True in the shipped image, which carries the reader package. False where it
    is not importable, which is the one honest answer: the endpoint then
    declines and the summary publishes no allowlist, so no Cub ever burns radio
    time relaying into a void."""
