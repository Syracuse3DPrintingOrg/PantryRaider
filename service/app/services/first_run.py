        ...
        if need_mealie:
            ...
            for base in _mealie_candidates():
                ...
                if not report.get("retryable"):
                    need_mealie = False
                    break
    # Out of attempts with the inventory still unconnected (a Grocy that never
    # came up at all). Nothing more is coming from this task, so stop holding
    # the first-boot page: the wizard and its "Set up for me" button are more
    # use to the user now than a progress bar that will never fill.
    if need_grocy:
        logger.info("Grocy first run gave up after %s attempts", attempts)
        _release_readiness_gate()
