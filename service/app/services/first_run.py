...
async def ensure_mealie_shopping_list(client: httpx.AsyncClient, base: str,
    ...
    Deliberately isolated from the rest of provisioning: shopping is planned
    to move to Grocy as Mealie winds down to an optional connector, and this
    is the one Mealie step that will go with it. Never creates a second list.
    Returns the step entry for the report.
    """
...
async def provision_mealie(base_url: str = "") -> dict:
    ...
    async with httpx.AsyncClient(timeout=_TIMEOUT) as client:
        ...
        # 5. Make sure a shopping list exists so the Shopping page works on
        # day one. Kept as its own function: the shopping feature is planned
        # to move to Grocy, at which point this call is simply deleted.
...
async def startup_first_run(attempts: int = 20, delay: float = 30.0,
    ...
    touched. Retries with a fixed delay while a backend looks like it is
    still booting, then gives up silently: the settings pane's "Set up for
    me" button remains for later.
    """
