...
from ..config import _DEFAULT_GROCY_URL, settings
...
# One provisioning run at a time, across both backends. The wizard button and
# the background "provision when it comes up" loop can otherwise run at once:
# both sign in with the factory login, both generate and PUT a new admin
# password, and settings ends up holding whichever save landed last while the
# backend holds the other, so the stored sign-in no longer opens the backend.
# Every skip check is re-read inside the lock, so the second run sees the first
# one's saved key and reports "already connected" instead of redoing the work.
_PROVISION_LOCK = asyncio.Lock()


def provisioning_in_progress() -> bool:
    """True while a provisioning run holds the lock, so a request can be told
    to wait rather than queue behind a run of chained network timeouts."""
    return _PROVISION_LOCK.locked()
...
async def provision_grocy(base_url: str = "") -> dict:
    """Connect this install to Grocy with no manual steps. Serialized."""
    async with _PROVISION_LOCK:
        return await _provision_grocy(base_url)


async def _provision_grocy(base_url: str = "") -> dict:
    """The Grocy run itself. Caller holds _PROVISION_LOCK.
    ...
    async with httpx.AsyncClient(timeout=_TIMEOUT,
        ...
        if not api_key:
            # Retryable: a Grocy still finishing its first-boot init can answer
            # the login POST with a 302 (sign-in "succeeds") a beat before its
            # Manage API keys page renders the new key, so the parse comes back
            # empty. Treating that as a definite failure made the retry loop
            # give up for good AND release the readiness gate, dropping the user
            # into a Grocy-less app while Grocy was seconds from ready
            # (FoodAssistant-n2b4). Retrying rides it out; the backstop deadline
            # and the wizard's paste-a-key fallback cover a genuine parse break.
            steps.append(_step("create API key",
                               reason="Could not read the new key back yet."))
            return _report(ok=False, configured=False, retryable=True, steps=steps,
                           message="Grocy has not handed back the new API key "
                                   "yet; it will connect automatically once it "
                                   "is fully up, or try again in a moment.")
        ...
        secured = bool(settings.grocy_admin_password)
        if secured:
            # A generated sign-in is already stored, so changing the password
            # again would leave the stored one no longer opening Grocy.
            steps.append(_step("secure admin sign-in", skipped=True,
                               reason="A generated sign-in is already stored."))
        else:
            try:
                ...
                secured = False
        if secured and not settings.grocy_admin_password:
...
async def provision_mealie(base_url: str = "") -> dict:
    """Connect this install to Mealie with no manual steps. Serialized."""
    async with _PROVISION_LOCK:
        return await _provision_mealie(base_url)


async def _provision_mealie(base_url: str = "") -> dict:
    """The Mealie run itself. Caller holds _PROVISION_LOCK.
    ...
    async with httpx.AsyncClient(timeout=_TIMEOUT) as client:
        ...
        password = generate_password()
        secured = bool(settings.mealie_admin_password)
        if secured:
            # Already stored: a second change would orphan the stored sign-in.
            ...
            try:
                ...
                secured = False
        if secured and not settings.mealie_admin_password:
...
# Addresses a co-hosted Grocy answers on, in the order worth trying: loopback
# on the published port (a Pi appliance, whose app has host networking and
# cannot resolve compose service names) and the compose service name (a plain
# Docker stack, whose app is on the compose network). The same list the
# readiness gate and the setup page's "Grocy detected locally" probe use, so
# provisioning can never end up blind to a Grocy those two can see.
...
def _grocy_candidates() -> list[str]:
    ...
    The configured address is always tried first. When it is empty or still the
    compose default, nobody has chosen a Grocy yet, so the local addresses above
    are tried too: that covers an appliance whose seeded address never landed,
    and a host whose Pi detection reads false inside the container (only the
    appliance compose sets FOODASSISTANT_FORCE_MODEL, so the plain stack on a Pi
    reads as not-a-Pi and used to skip loopback entirely, leaving a perfectly
    healthy Grocy unprovisioned).

    An address the user DID choose gets no local fallback: falling back there
    could connect the box's own Grocy behind their back and overwrite the
    address they asked for. A candidate that does not resolve simply reports
    unreachable and is skipped.
    """
    configured = (settings.grocy_base_url or "").rstrip("/")
    candidates = [configured] if configured else []
    if not configured or configured == _DEFAULT_GROCY_URL:
...
def _release_readiness_gate() -> None:
    """Tell the first-boot readiness page that provisioning is finished.

    Best-effort and deliberately one-directional: this module decides when the
    inventory work is over, readiness.py decides what to show. A failure here
    must never take down the provisioning task, and the gate has its own
    deadline backstop if this never lands."""
    try:
        from . import readiness
        readiness.mark_provisioning_done()
    except Exception:
        logger.debug("Could not release the readiness gate", exc_info=True)


# ── Stillborn Grocy self-heal (pi_hosted first boot) ─────────────────────────
# Grocy builds and migrates its SQLite schema in exactly one place: while
# serving an HTTP GET of its root route. A fully hands-off appliance first
# boot never requests that route (the readiness probes and the provisioner
# only touch /login and /api paths), so the grocy.db created as a side effect
# of the first sign-in attempt stays at exactly 0 bytes forever and every
# sign-in answers HTTP 500. Retrying alone can never clear that wedge.
#
# The heal runs when that exact signature repeats on the appliance's own
# co-hosted Grocy, in two escalating steps:
#   1. A migration nudge: the same GET / every browser visit performs, which
#      makes Grocy build its schema in place. This is the actual cure for the
#      0-byte stub and needs no privileges (Grocy's root route bypasses auth
#      by design). It also heals devices whose host bridge predates step 2.
#   2. The host bridge's POST /grocy/repair: stop the container, move the
#      dead stub aside, start fresh. Only reached when the nudge could not
#      cure it (a database too broken to migrate), and guarded again on the
#      bridge side so a healthy database is untouchable by construction.

_SIGNIN_5XX_RE = re.compile(r"^Unexpected answer \(HTTP 5\d\d\)\.$")

# Heal only after this many consecutive 5xx sign-in answers, so a single
# hiccup mid-boot never triggers anything.
_GROCY_5XX_STREAK_THRESHOLD = 3

# The bridge repair restarts the inventory container, so it is requested at
# most this many times per process; the bridge's own cooldown is the real
# guard across processes and restarts.
_GROCY_REPAIR_MAX_REQUESTS = 2

# A full first migration on a Pi 4 SD card can take a couple of minutes and
# the nudge request blocks for it, so the read timeout is generous.
_MIGRATION_NUDGE_TIMEOUT = 300.0

_BRIDGE_REPAIR_URL = "http://127.0.0.1:9299/grocy/repair"

_grocy_5xx_streak = 0
_grocy_repairs_requested = 0
_grocy_repair_unavailable = False
_grocy_heal_in_flight = False


def _reset_grocy_heal_state() -> None:
    """Forget the heal counters (tests only)."""
    global _grocy_5xx_streak, _grocy_repairs_requested
    global _grocy_repair_unavailable, _grocy_heal_in_flight
    _grocy_5xx_streak = 0
    _grocy_repairs_requested = 0
    _grocy_repair_unavailable = False
    _grocy_heal_in_flight = False
...
def grocy_signin_answered_5xx(report: dict) -> bool:
    """Whether a provisioning report carries the stillborn-database signature:
    a retryable run whose sign-in step got an HTTP 5xx answer from Grocy.

    Deliberately narrow. Unreachable ("Grocy is not reachable."), a hands-off
    answer, and every later-step failure all read as normal booting, never as
    the wedge. Pure, for tests.
    """
    if not report.get("retryable"):
        return False
    return any(step.get("step") == "sign-in"
               and _SIGNIN_5XX_RE.match(step.get("reason") or "")
               for step in report.get("steps") or [])


def _grocy_heal_eligible(base: str) -> bool:
    """The heal only ever acts on a Pi appliance's own co-hosted Grocy."""
    if settings.deployment_mode != "pi_hosted":
        return False
    try:
...
async def _nudge_grocy_migrations(base: str) -> bool:
    """GET Grocy's root route so it runs its schema migrations.

    The root route is Grocy's only migration trigger, it bypasses auth by
    design, and on a healthy database it costs a handful of reads, so this is
    idempotent and safe to repeat. A brand-new Grocy answers its very first
    request with a one-shot redirect back to "/" instead of migrating, so a
    redirect that points at the root is followed once. Returns True when
    Grocy answered without a server error (the schema exists now or was just
    built), False when it is unreachable or still failing.
    """
    base = base.rstrip("/")
    try:
        async with httpx.AsyncClient(
                timeout=httpx.Timeout(_MIGRATION_NUDGE_TIMEOUT, connect=10.0),
                follow_redirects=False) as client:
            r = await client.get(f"{base}/")
            location = r.headers.get("location", "")
            if (r.status_code in (301, 302, 303, 307, 308)
                    and urlparse(location).path in ("", "/")):
                r = await client.get(f"{base}/")
    except httpx.HTTPError:
        logger.info("Grocy migration nudge at %s: not reachable yet", base)
        return False
    logger.info("Grocy migration nudge at %s answered HTTP %s",
                base, r.status_code)
    return r.status_code < 500


async def _request_grocy_repair() -> dict:
    """POST /grocy/repair on the host bridge.

    Returns the bridge's JSON report; {"ok": False, "unavailable": True} when
    this device's bridge predates the route (it gains it on the device's next
    update); {"ok": False, "error": ...} when the bridge cannot be reached.
    """
    from .bridge import bridge_client
    try:
        async with bridge_client(timeout=300.0) as client:
            r = await client.post(_BRIDGE_REPAIR_URL, json={})
        if r.status_code == 404:
...
async def _request_grocy_repair_capped() -> None:
    """Ask the bridge for the container-level repair, within the per-boot cap.

    A bridge without the route answers once and is never asked again; every
    other answer (repaired, refused, unreachable) is logged and counted, and
    the normal retry loop just continues either way.
    """
    global _grocy_repairs_requested, _grocy_repair_unavailable
    if _grocy_repair_unavailable:
        return
    if _grocy_repairs_requested >= _GROCY_REPAIR_MAX_REQUESTS:
        return
    _grocy_repairs_requested += 1
    answer = await _request_grocy_repair()
    if answer.get("unavailable"):
        _grocy_repair_unavailable = True
        logger.info("The device helper does not offer the Grocy repair yet "
                    "(it arrives with the device's next update); "
                    "provisioning keeps retrying normally")
        return
    logger.info("Grocy database repair request %s of %s answered: %s",
                _grocy_repairs_requested, _GROCY_REPAIR_MAX_REQUESTS, answer)


async def _maybe_heal_grocy(base: str, report: dict) -> None:
    """Track consecutive 5xx sign-ins on the local Grocy and heal the wedge.

    Called once per provisioning attempt from startup_first_run. Reports for
    a non-local base, or off a pi_hosted appliance, are ignored entirely; any
    answer that is not the 5xx signature breaks the streak. Never raises, and
    never touches need_grocy or the readiness gate: after a heal the normal
    retry loop simply finds a Grocy that can finish provisioning.
    """
    global _grocy_5xx_streak, _grocy_heal_in_flight
    if not _grocy_heal_eligible(base):
        return
    if not grocy_signin_answered_5xx(report):
        _grocy_5xx_streak = 0
        return
    _grocy_5xx_streak += 1
    if _grocy_5xx_streak < _GROCY_5XX_STREAK_THRESHOLD:
        return
    _grocy_5xx_streak = 0
    if _grocy_heal_in_flight:
        # Two tasks drive this loop (the startup task and the getting-ready
        # page's kicks); one heal at a time is plenty.
        return
    _grocy_heal_in_flight = True
    try:
        if await _nudge_grocy_migrations(base):
            return
        await _request_grocy_repair_capped()
    except Exception:
        logger.exception("Grocy self-heal attempt failed")
    finally:
        _grocy_heal_in_flight = False
        ...
        if need_grocy:
            ...
            for base in _grocy_candidates():
                ...
                if report.get("configured"):
                    ...
                    logger.info("Grocy first run at %s: %s", base,
                                report.get("message", ""))
                # A sign-in that keeps answering with a server error is the
                # stillborn-database wedge, which no amount of retrying can
                # clear on its own; see _maybe_heal_grocy above.
                await _maybe_heal_grocy(base, report)
                if not report.get("retryable"):
                    need_grocy = False
                    break
            if not need_grocy:
                # The first-boot readiness page waits on this: whether the
                # inventory just connected or this Grocy turned out to be
                # someone else's, provisioning is finished and the user must
                # not be held on a progress page any longer.
                _release_readiness_gate()
        if need_mealie:
            ...
            for base in _mealie_candidates():
                ...
                if not report.get("retryable"):
                    need_mealie = False
                    break
    # Out of attempts with the inventory still unconnected. This task is
    # bounded, but the appliance's Grocy is co-hosted and WILL come up, and the
    # getting-ready page keeps re-kicking a fresh run every few seconds while it
    # is shown (routers/ui.getting_ready_status), so one exhausted task is not
    # the end of provisioning. Deliberately do NOT release the readiness gate
    # here (FoodAssistant-n2b4): giving up after a bounded count once dropped
    # users into a Grocy-less app while Grocy was still finishing its first-boot
    # init. The gate now holds until the key actually lands, with the ~35-minute
    # backstop deadline and the user's "continue without waiting" as the only
    # escapes. A definite hands-off answer (someone else's Grocy) still releases
    # it, inside the loop above.
    if need_grocy:
        logger.info("Grocy first run gave up after %s attempts "
                    "(the readiness page will keep retrying)", attempts)
