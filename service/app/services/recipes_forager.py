...
Cloud endpoints (deployed as Stage 1/2, not changed here):
  ...
  POST /v1/recipes/shares                  -> create a private share link
  GET  /v1/recipes/shares/inbox            -> shares sent to this kitchen
  POST /v1/recipes/shares/{token}/revoke   -> turn a share link off
"""
...
def build_share_payload(recipe: dict, attribution: str, recipient: str = "",
                        email_to: str = "", message: str = "") -> dict:
    """Build the POST /v1/recipes/shares body: the recipe fields plus who to
    credit, and (for a person-to-person share) who to send it to. Pure, so the
    router can validate before any network call.

    Raises ValueError (user-facing message) when the recipe has no name or the
    attribution is missing, matching build_submit_payload so the two share
    flows never disagree on what is required."""
    payload = build_submit_payload(recipe, attribution)
    # A LAN-only image URL is useless on the cloud share page, so the router
    # only fills image_url when the kitchen has a public address; drop it here
    # when empty rather than sending a blank the cloud would have to ignore.
    if not payload.get("image_url"):
        payload.pop("image_url", None)
    for key, value in (("recipient", recipient), ("email_to", email_to),
                       ("message", message)):
...
async def create_share(payload: dict,
                       transport: httpx.AsyncBaseTransport | None = None) -> dict:
    """POST a recipe share (a private link, optionally emailed to someone).
    Mirrors submit_recipe's fail-soft shape; never raises on a network error:

        {"status": int, "url": str|None, "token": str|None, "error": str|None}

    ``status`` 0 means the cloud was unreachable. The router maps the statuses
    to friendly messages the same way the community share does."""
    if not settings.cloud_instance_token:
        return {"status": 401, "url": None, "token": None, "error": "not linked"}
    try:
        async with _client(_SUBMIT_TIMEOUT, transport) as client:
            resp = await client.post(f"{_base()}/v1/recipes/shares", json=payload,
                                     headers=_headers())
    except Exception as exc:
        logger.info("forager recipes: share unreachable: %s", exc)
        return {"status": 0, "url": None, "token": None, "error": str(exc)}
    try:
        body = resp.json()
    except Exception:
        body = {}
    body = body if isinstance(body, dict) else {}
    return {"status": resp.status_code, "url": body.get("url"),
            "token": body.get("token"),
            "error": body.get("detail") or body.get("error")}


async def upload_recipe_image(path: str, image: tuple[bytes, str] | None,
                              transport: httpx.AsyncBaseTransport | None = None) -> dict:
    """Upload a recipe photo's raw bytes to Forager so it hosts its own copy,
    independent of whether this kitchen is online or publicly reachable.

    ``path`` is the endpoint to post to (``/v1/recipes/{id}/image`` for a
    community share, ``/v1/recipes/shares/{token}/image`` for a private one).
    ``image`` is (bytes, content_type) or None. Fail-soft like the rest of this
    module: a photo is a nice-to-have, never a reason to fail the share, so any
    miss (not linked, no image, unreachable, rejected) resolves to a status dict
    the caller can ignore:

        {"status": int, "image_url": str|None, "error": str|None}

    ``status`` 0 means nothing was sent (no image or the cloud was unreachable)."""
    if not settings.cloud_instance_token:
        return {"status": 401, "image_url": None, "error": "not linked"}
    if not image or not image[0]:
        return {"status": 0, "image_url": None, "error": "no image"}
    data, content_type = image
    files = {"file": ("recipe-image", data, content_type or "application/octet-stream")}
    try:
        async with _client(_SUBMIT_TIMEOUT, transport) as client:
            resp = await client.post(f"{_base()}{path}", files=files,
                                     headers=_headers())
    except Exception as exc:  # unreachable: the share already went out
        logger.info("forager recipes: image upload unreachable: %s", exc)
        return {"status": 0, "image_url": None, "error": str(exc)}
    try:
        body = resp.json()
    except Exception:
        body = {}
    body = body if isinstance(body, dict) else {}
    return {"status": resp.status_code, "image_url": body.get("image_url"),
            "error": body.get("detail") or body.get("error")}


async def list_share_inbox(
        transport: httpx.AsyncBaseTransport | None = None) -> list[dict]:
    """Recipes other people have shared with this kitchen, newest-first as the
    cloud returns them. Empty list when not linked or on any failure, so the
    recipes page renders normally when the inbox cannot be fetched."""
    if not settings.cloud_instance_token:
        return []
    try:
        async with _client(_SEARCH_TIMEOUT, transport) as client:
            resp = await client.get(f"{_base()}/v1/recipes/shares/inbox",
                                    headers=_headers())
            resp.raise_for_status()
            body = resp.json()
    except Exception as exc:
        logger.info("forager recipes: share inbox failed: %s", exc)
        return []
    # The list may arrive wrapped ({"shares": [...]}) or bare; accept both.
    items = body.get("shares") if isinstance(body, dict) else body
...
async def revoke_share(token: str,
    ...
    """Turn off a share link. True only when the cloud confirms; any failure
    (unreachable, unlinked, unknown token) is False so callers can say the
    link may still work."""
    token = (token or "").strip()
    if not token or not settings.cloud_instance_token:
        return False
    try:
        async with _client(_SUBMIT_TIMEOUT, transport) as client:
            resp = await client.post(f"{_base()}/v1/recipes/shares/{token}/revoke",
                                     headers=_headers())
            return resp.status_code in (200, 204)
    except Exception as exc:
        # Log only a stub of the share token: app INFO lines reach the
        # container console now, and a pasted `docker logs` must never hand
        # out a possibly-still-active share link.
        logger.info("forager recipes: revoke %s... failed: %s", token[:8], exc)
        return False
