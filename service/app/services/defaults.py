...
def propose_review_best_by(old_best_by: date | None, hand_set: bool,
                           from_storage: str | None, to_storage: str | None,
                           dest_days: int | None,
                           today: date | None = None) -> date | None:
    """The new best-by a Review-screen storage change should set, or None for
    no change. Pure: every input is explicit, including today.

    A pending row's suggested date is only an estimate for the storage the
    scan guessed, so changing the storage recomputes it outright from the new
    area's rule: frozen to refrigerated pulls the date in, refrigerated to
    frozen pushes it out. A date the user typed by hand is real knowledge
    about this item, so it follows the transfer-hook semantics (a colder move
    may extend it) but is never shortened. A row with no date adopts the new
    area's rule, which is what the commit's apply_defaults would do anyway.
    """
    if dest_days is None or to_storage not in _COLDNESS:
        return None
    if (from_storage or "") == (to_storage or ""):
        return None
    if today is None:
        today = date.today()
    candidate = today + timedelta(days=dest_days)
    if old_best_by is None:
        return candidate
    if hand_set:
        proposal = propose_transfer_best_by(
            old_best_by, from_storage, to_storage, dest_days, today)
        # Never shorten a hand-typed date: a storage guess must not override
        # what the user actually knows about this item.
        if proposal is None or proposal < old_best_by:
            return None
        return proposal
    return candidate if candidate != old_best_by else None
