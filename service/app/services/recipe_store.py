...
def _detail_dict(db: Session, r: Recipe) -> dict:
    ...
    return {
        ...
        "image": f"/recipes/images/{r.id}" if r.image_path else None,
        "recipeIngredient": [_ingredient_wire(i) for i in ings],
...
def create_from_parsed(db: Session, parsed: dict, *, source: str = "manual",
    ...
    lines = [str(i).strip() for i in (parsed.get("ingredients") or [])
             if str(i or "").strip()]
...
def update_from_parsed(db: Session, slug: str, parsed: dict, *,
    ...
    so a rename must not orphan them. Name, description, servings, total/prep
    time, tags, and the ingredient and step lists are replaced from the parsed
    ...
    for pos, line in enumerate(lines):
        fields = _structured_fields(structured[pos] if pos < len(structured) else None)
        db.add(RecipeIngredient(recipe_id=r.id, position=pos, text=line, **fields))
    for pos, text in enumerate(steps):
