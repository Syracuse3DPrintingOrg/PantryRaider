...
def update_from_parsed(db: Session, slug: str, parsed: dict, *,
    ...
    so a rename must not orphan them. Name, description, servings, and the
    ingredient and step lists are replaced from the parsed dict; prep/cook/total
    time and tags are replaced only when the dict carries them (sz4h), so a
    caller that never mentions them leaves the stored values alone.
    ``structured`` optionally carries parsed ingredient entries aligned by
    ...
    # The time fields change only when the caller sends them (sz4h): a PUT that
    # never mentions a time keeps the stored one, while an explicit "" still
    # clears it. The edit form always posts all three, so it is unaffected.
    if "total_time" in parsed:
        r.total_time = str(parsed.get("total_time") or "").strip()
    if "prep_time" in parsed:
        r.prep_time = str(parsed.get("prep_time") or "").strip()
    if "cook_time" in parsed:
