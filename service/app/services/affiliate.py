...
DISCLOSURE = ("Some links here are affiliate links. If you buy through them the "
              ...
              "helps keep Pantry Raider free for personal and non-commercial use. "
              "Thanks for your support.")


# -- Our Picks: curated hardware for a Pantry Raider build ------------------
# (FoodAssistant-ztly). The picks themselves ship as data in
# ``app.data.our_picks`` so publishing new picks is just an app update; the
# grouping logic here stays pure so it is unit-testable, and every Amazon link
# is built through the same ``amazon_url`` helper as the rest of the app, so the
# affiliate tag is applied one way from config and never hand-appended.

def build_our_picks(picks, categories, tag="") -> list[dict]:
    """Group curated hardware picks by category, each with a tagged Amazon URL.

    Pure: pass the picks list, the category display order, and the affiliate
    tag. Returns ``[{category, products}, ...]`` in ``categories`` order,
    skipping empty categories. Each product is the pick dict plus ``url``, the
    Amazon link built by ``amazon_url`` from the pick's ``search`` term (a bare
    ASIN links to the product page; anything else is a live search). A pick
    whose category is not in ``categories`` falls into the last category so a
    typo never drops it.
    """
    order = list(categories or [])
    fallback = order[-1] if order else "Accessories"
    tagged: list[dict] = []
    for p in (picks or []):
        term = str(p.get("search") or p.get("name") or "").strip()
        tagged.append({**p, "url": amazon_url(term, tag)})
    by_cat: dict[str, list[dict]] = {}
    for p in tagged:
        cat = p.get("category")
        if cat not in order:
...
def our_picks(tag="") -> list[dict]:
    """Curated hardware picks grouped by category, tagged for the given tag.

    Loads the shipped data (``app.data.our_picks``) and delegates to
    ``build_our_picks``. Import is local so the pure helper above has no data
    dependency and stays trivially testable.
    """
    from ..data.our_picks import OUR_PICKS, CATEGORIES
    return build_our_picks(OUR_PICKS, CATEGORIES, tag)


def our_picks_disclosure() -> str:
    """The Our Picks affiliate disclosure line (lives with the picks data)."""
    from ..data.our_picks import DISCLOSURE as _D
    return _D
