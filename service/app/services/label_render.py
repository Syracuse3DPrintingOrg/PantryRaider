...
DateSource = Literal["manual", "default", "llm", "community"]

# Short, user-facing badge copy for each source. Deliberately plain: "est." for
# a rule-of-thumb estimate (a built-in rule or community shelf-life data, both
# estimates), "AI" for an AI guess, and nothing for a date the user set
# themselves (a typed date needs no caveat). No developer jargon.
...
# -- Grocy stock-entry code (FoodAssistant-28f3) ------------------------------
#
# A food label can carry the Grocy "grocycode" of the exact stock entry it was
# printed for, as a small QR square in the label's bottom-right corner. Scanning
# that label later uses up that entry, not just "one of" the product, so the
# right leftover container comes off stock with the right date. The string is
# Grocy's own format ("grcy:p:{product_id}:{stock_id}"), which Grocy's scanners
# accept too.

# How much of the smaller inner dimension the entry code occupies. Big enough
# to scan on a 2x1 inch thermal label, small enough that the text keeps most of
# the width.
GROCYCODE_FRACTION = 0.46

# Below this many pixels a QR square is not reliably scannable, so a very small
# label prints without the code rather than with a decorative smudge.
GROCYCODE_MIN_SIDE = 24


def build_grocycode(product_id, stock_entry_id: str = "") -> str:
    """The grocycode string for a product, or one exact stock entry of it.

    "grcy:p:{product_id}" names the product; adding the stock entry's own id
    ("grcy:p:{product_id}:{stock_id}") names one exact entry, which is what a
    leftover label wants. Returns "" for an unusable product id so a caller can
    simply skip the code. Pure and fully testable."""
    try:
        pid = int(product_id)
    ...
    if pid <= 0:
        return ""
    sid = str(stock_entry_id or "").strip()
    if not sid:
        return f"grcy:p:{pid}"
    return f"grcy:p:{pid}:{sid}"


def grocycode_qr_side(inner_w: int, inner_h: int,
                      fraction: float = GROCYCODE_FRACTION) -> int:
    """The entry-code QR's square side in pixels for an inner (inside the
    margins) area, or 0 when the label is too small to carry a scannable code.

    Scales off the smaller of the two dimensions, exactly like the brand mark,
    so a very wide or very tall label never gets an oversized square. Pure and
    fully testable."""
    inner_w = max(0, int(inner_w))
    inner_h = max(0, int(inner_h))
    side = int(min(inner_w, inner_h) * fraction)
    return side if side >= GROCYCODE_MIN_SIDE else 0
...
class LabelSpec:
    ...
    # Optional Grocy stock-entry code (FoodAssistant-28f3): when set, the label
    # carries this string as a QR square in the bottom-right corner so scanning
    # the label later uses up that exact stock entry. Empty means no code, and
    # the label renders exactly as it always has.
    grocycode: str = ""
...
def render_label(spec: LabelSpec | None = None, **kwargs) -> Image.Image:
    ...
    # Grocy stock-entry code (FoodAssistant-28f3): rendered up front so the
    # text below can keep clear of it. When the QR cannot be made (no code
    # given, or the label is too small for a scannable square) every measure
    # falls back to the full inner width and the layout is exactly the
    # no-grocycode layout, pixel for pixel.
    qr_side = 0
    qr_img = None
    if spec.grocycode:
        qr_side = grocycode_qr_side(inner_w, inner_h)
        qr_img = _render_qr_image(spec.grocycode, qr_side) if qr_side else None
        if qr_img is None:
            qr_side = 0
    # The right edge available to text: the full inner width normally, or up to
    # a small gap before the QR column when an entry code is on the label.
    text_right = right - (qr_side + max(2, int(inner_h * 0.04))) if qr_side else right
    text_w = max(1, text_right - m)
    ...
    name_size = _fit_font_size(draw, spec.name or "", text_w, int(name_band * 0.62),
                               bold=True, start=name_band)
    name_font = _load_font(name_size, bold=True)
    name_lines = _wrap_lines(draw, spec.name or "", name_font, text_w, max_lines=2)
    ...
    if has_hero:
        ...
        # (flush against the text column when an entry code holds the corner).
        ...
        if badge:
            ...
            _draw_badge_pill(draw, (text_right, y - 1), badge, badge_font,
        ...
        date_line = _ellipsize(draw, best_by, date_font, text_w)
    ...
    if footer:
        footer = _ellipsize(draw, footer, footer_font, text_w)
    ...
    if qr_img is not None:
        # The stock-entry code sits in the bottom-right corner, inside the
        # margins. The brand mark uses that same corner, and a scannable code
        # beats a decoration, so the logo is skipped on a coded label.
        img.paste(qr_img, (right - qr_side, m + inner_h - qr_side))
    elif spec.show_logo:
        draw_logo(img, m)
...
#
# Label stock comes in three shapes: rectangular (the default), square, and
# round (die-cut circles, the common stock for jar lids and spice-tin tops).
# A square label needs no special composition: it is just a stock whose sides
# are equal, and the layout already scales, so the shipped rectangular
# renderer is reused untouched and stays pixel-identical. A round label is
# composed inside its cut circle: the content is laid out in the largest
# square inscribed in that circle (shrunk by the margin and the cut-line
# width), so the name, dates, badge, and any entry code all keep clear of the
# curved edge, and a thin ring marks the cut line so the label is easy to
# line up on a rectangular backing sheet. The canvas itself stays rectangular
# either way (printers print rectangles); the corners around a round label
# stay plain background.

LABEL_SHAPES = ("rectangle", "square", "round")


def normalize_shape(value) -> str:
    """The canonical label-stock shape for a stored or requested value.

    "rect" and "rectangular" mean "rectangle"; case and whitespace are
    ignored; anything unknown (or empty) falls back to "rectangle" so a stale
    or hand-edited setting can never change how a label renders unexpectedly.
    Pure and fully testable."""
    s = str(value or "").strip().lower()
    if s in ("rect", "rectangular"):
        s = "rectangle"
    return s if s in LABEL_SHAPES else "rectangle"


def cut_line_width(dpi: int) -> int:
    """The cut-line ring's stroke width in pixels for a print resolution.

    About 0.008 inch: one pixel at low resolutions, a touch more at high dpi,
    so the ring reads as a hairline guide at any print density. Pure."""
    try:
        d = int(dpi)
    except (TypeError, ValueError):
        d = DEFAULT_DPI
    return max(1, round(max(1, d) * 0.008))


def inscribed_box(width_px: int, height_px: int,
                  inset_px: int = 0) -> tuple[int, int, int]:
    """The largest square that fits inside a round label's usable circle.

    The cut circle is the largest circle the stock holds (its diameter is the
    shorter side), centered on the canvas. ``inset_px`` shrinks the usable
    radius (the margin plus the cut-line width), and the returned (x, y,
    side) is the biggest square inscribed in what remains, centered on the
    canvas. A degenerate inset (larger than the radius) falls back to a small
    centered box instead of collapsing to nothing, so something always
    renders. Pure and fully testable."""
    w = max(1, int(width_px))
    h = max(1, int(height_px))
    radius = min(w, h) / 2.0 - max(0, int(inset_px))
    if radius <= 0:
        radius = min(w, h) / 4.0
    side = max(1, int(radius * math.sqrt(2)))
    x = int(round((w - side) / 2.0))
    y = int(round((h - side) / 2.0))
    return x, y, side


def round_inner_spec(spec: LabelSpec) -> LabelSpec:
    """The square spec a round label's content is actually rendered at.

    Its canvas is the inscribed content box of ``spec``'s cut circle, with no
    margin of its own (the circle geometry already provides the clearance),
    and every other field carried over unchanged. Pure."""
    inset = spec.margin_px + cut_line_width(spec.dpi)
    _, _, side = inscribed_box(spec.width_px, spec.height_px, inset)
    return replace(spec, width_in=side / spec.dpi, height_in=side / spec.dpi,
                   margin_in=0.0)


def compose_round_label(spec: LabelSpec, inner: Image.Image,
                        cut_line: bool = True) -> Image.Image:
    """Place an already-rendered content square onto a round label's canvas.

    The content is pasted centered (which puts an inscribed-box render inside
    the cut circle), the corners stay plain background, and ``cut_line``
    draws a thin ring on the cut circle so the printed label shows where the
    round edge falls on its rectangular backing sheet. Returns a mode "L"
    image sized to the spec."""
    w, h = spec.width_px, spec.height_px
    img = Image.new("L", (w, h), 255)
    img.paste(inner.convert("L"),
              (max(0, (w - inner.width) // 2), max(0, (h - inner.height) // 2)))
    if cut_line:
        d = min(w, h)
        left = (w - d) // 2
        top = (h - d) // 2
        draw = ImageDraw.Draw(img)
        draw.ellipse([left, top, left + d - 1, top + d - 1],
                     outline=0, width=cut_line_width(spec.dpi))
    return img


def render_shaped_label(spec: LabelSpec, shape: str = "rectangle", *,
                        cut_line: bool = True) -> Image.Image:
    """Render a food label honouring the label stock's shape.

    "rectangle" and "square" (and anything unknown) delegate straight to
    render_label, so the shipped rendering stays pixel-identical for them.
    "round" renders the same label into the cut circle's inscribed content
    box and composes it onto the round canvas (see compose_round_label)."""
    if normalize_shape(shape) != "round":
        return render_label(spec)
    return compose_round_label(spec, render_label(round_inner_spec(spec)),
                               cut_line=cut_line)
...
#
# A small curated set of kitchen-relevant symbols a user can drop onto a
# label as an "icon" element. Rendered as plain text characters (not images),
# so no extra asset or font is bundled: they are picked from Unicode blocks
# DejaVu Sans (the bundled label font) actually covers, so they print as a
# crisp glyph on a 1-bit thermal label rather than a blank box or a "tofu"
# missing-glyph square. Keys are stable identifiers stored in a layout; the
# glyph itself is what gets drawn.
ICON_GLYPHS: dict[str, str] = {
    "snowflake": "❄",   # freezer / keep frozen
    "warning": "⚠",     # caution / use care
    "star": "★",        # favorite / featured
    "check": "✓",       # verified / ready
    "hourglass": "⌛",   # use soon
    "coffee": "☕",      # hot beverage / cafe
    "sun": "☀",         # pantry / room temperature
    "scissors": "✂",    # cut / prep
}

ICON_KEYS = tuple(ICON_GLYPHS.keys())


def icon_glyph(key: str) -> str:
    """The printable glyph for an icon key, or "" for an unknown key.

    Unknown keys (a stale layout from a future version, a typo) draw nothing
    rather than raising. Pure."""
    return ICON_GLYPHS.get(key or "", "")


# -- QR payload builder (FoodAssistant-nxr8) ---------------------------------
#
# What a "qr" element encodes, beyond a plain URL/text value. Kept as a small
# pure string builder so it is fully unit-testable without the qrcode lib or
# any image rendering; render_layout calls this to get the string it hands to
# the QR encoder.

QR_PAYLOAD_KINDS = ("url", "text", "vcard")


def build_qr_payload(kind: str, *, text: str = "", url: str = "",
                     vcard_name: str = "", vcard_note: str = "",
                     vcard_org: str = "") -> str:
    """Build the string a QR element should encode, for the given payload kind.

    "url" and "text" both just pass their value through, trimmed (a URL is
    typed as-is; free text is typed as-is). "vcard" builds a minimal vCard 3.0
    record (name, optional organization, optional note) so a scanning phone
    offers to save a contact card instead of showing plain text, useful for a
    "return to" label on a shared container. An unrecognized kind falls back
    to "text" rather than raising. Pure: no I/O, no QR encoding here."""
    kind = kind if kind in QR_PAYLOAD_KINDS else "text"
    if kind == "url":
        return (url or text or "").strip()
    if kind == "vcard":
        name = (vcard_name or text or "").strip()
        if not name:
            return ""
        lines = ["BEGIN:VCARD", "VERSION:3.0", f"FN:{name}"]
        org = (vcard_org or "").strip()
        if org:
            lines.append(f"ORG:{org}")
        note = (vcard_note or "").strip()
        if note:
            lines.append(f"NOTE:{note}")
        lines.append("END:VCARD")
        return "\n".join(lines)
    return (text or url or "").strip()
...
class LabelElement:
    ...
    that); ``text`` supplies the content for the "static" field, the icon key
    for the "icon" field, and the primary QR/barcode value; ``uppercase``
    upper-cases the resolved text. ``outline`` draws a hairline border around
    the element's box, so a box/field can read as a card or a divider on the
    label. ``qr_kind`` picks what a "qr" element encodes (see
    ``build_qr_payload``); ``qr_extra`` is a secondary value for kinds that
    use one (the note line of a "vcard" QR).
    """
    field: str = "static"
    x: float = 0.0
    y: float = 0.0
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "qr_kind": self.qr_kind,
            "qr_extra": self.qr_extra,
        }
    ...
    def from_dict(cls, data: dict) -> "LabelElement | None":
        ...
        if scale <= 0:
            scale = 1.0
        qr_kind = data.get("qr_kind", "url")
        if qr_kind not in QR_PAYLOAD_KINDS:
            qr_kind = "url"
        return cls(
            field=field_name,
            ...
            qr_kind=qr_kind,
            qr_extra=str(data.get("qr_extra", "") or ""),
        )
...
def render_layout(layout: LabelLayout, values: dict) -> Image.Image:
    ...
    for el in layout.elements:
        ...
        if el.outline:
            # A hairline border around the element's own box, drawn first so
            # any content painted below sits on top of it rather than under.
            ow = max(1, int(round(inner_h * 0.008)))
            draw.rectangle([bx, by, bx + bw - 1, by + bh - 1], outline=0, width=ow)

        if el.field == "icon":
            glyph = icon_glyph(el.text)
            _draw_aligned_text(draw, glyph, bx, by, bw, bh,
        ...
        if el.field == "qr":
            fallback = el.text or str(values.get("name", "") or "")
            qr_text = build_qr_payload(
                el.qr_kind, text=fallback, url=fallback,
                vcard_name=el.text or str(values.get("name", "") or ""),
                vcard_note=el.qr_extra) or fallback
            side = min(bw, bh)
...
#
# A small user-maintained library of named label designs, distinct from the
# built-in stock-size LABEL_PRESETS above: a saved preset here is the user's
# own layout (elements the designer built), kept under a name so they can
# switch between a few label designs without redoing the drag-and-drop each
# time. Stored as JSON in the ``label_layout_presets`` setting; this section
# is the pure validate/normalize layer both the save endpoint and the loader
# share, so a hand-edited or partially corrupt list degrades gracefully
# instead of raising.

MAX_LAYOUT_PRESET_NAME = 60


def _clean_preset_name(name) -> str:
    """A preset name trimmed and length-capped, or "" if unusable. Pure."""
    name = str(name or "").strip()
    return name[:MAX_LAYOUT_PRESET_NAME]


def validate_layout_presets(raw) -> list[dict]:
    """Normalize a layout-presets payload into a clean list of {name, layout}.

    Accepts a JSON string (as stored in settings) or an already-parsed list.
    Each entry needs a non-empty, non-duplicate name and a layout with at
    least one valid element; anything else is dropped rather than raising, so
    a malformed or hand-edited value never breaks loading. Later entries with
    a name already seen are dropped (first one wins), so saving never
    silently produces duplicates. Pure."""
    ...
    for item in raw:
        if not isinstance(item, dict):
            continue
        name = _clean_preset_name(item.get("name"))
        if not name or name in seen:
            continue
        layout_data = item.get("layout")
        if not isinstance(layout_data, dict):
            continue
        layout = LabelLayout.from_dict(layout_data)
        if not layout.elements:
            continue
        seen.add(name)
        out.append({"name": name, "layout": layout.to_dict()})
    return out


def layout_presets_to_json(presets: list[dict]) -> str:
    """Serialize a (already-validated) preset list for storage. Pure."""
    return json.dumps(validate_layout_presets(presets))


def layout_presets_from_json(raw: str) -> list[dict]:
    """The saved preset list from its stored JSON string. Never raises;
    unusable JSON or structure yields an empty list. Pure."""
    return validate_layout_presets(raw)
...
_DECORATIVE_ICON_BAND = 0.30  # fraction of the inner height the icon band claims


def render_decorative_label(text: str, *, width_in: float = DEFAULT_WIDTH_IN,
                            height_in: float = DEFAULT_HEIGHT_IN,
                            dpi: int = DEFAULT_DPI, margin_in: float = 0.06,
                            bold: bool = True, icon: str = "",
                            outline: bool = False, shape: str = "rectangle",
                            cut_line: bool = True) -> Image.Image:
    """A dateless decorative label (spice jars, canisters, storage bins).

    The text is centered and set as large as the stock allows, wrapping onto
    up to three lines. No dates and no badges: this variant is for naming a
    container, not tracking spoilage.

    Two optional, simple layout touches (FoodAssistant-nxr8) go beyond plain
    centered text without needing the full field designer: ``icon`` places one
    of the curated ICON_GLYPHS above the text (an unknown key or "" omits it),
    and ``outline`` draws a hairline frame just inside the margin. Both are
    off by default, so a plain call renders exactly as before.

    ``shape`` follows the label stock (FoodAssistant-bprm): "round" (the
    natural stock for the jar lids and spice tops this label is for) composes
    the same centered text inside the cut circle with a thin cut-line ring
    (see compose_round_label); "rectangle" and "square" render exactly as
    before. Returns a mode "L" image."""
    if normalize_shape(shape) == "round":
        outer = LabelSpec(name=text, width_in=width_in, height_in=height_in,
                          dpi=dpi, margin_in=margin_in)
        inner_spec = round_inner_spec(outer)
        inner = render_decorative_label(
            text, width_in=inner_spec.width_in, height_in=inner_spec.height_in,
            dpi=dpi, margin_in=0.0, bold=bold, icon=icon, outline=outline)
        return compose_round_label(outer, inner, cut_line=cut_line)
    ...
    pad = 0
    if outline:
        ow = max(1, int(round(inner_h * 0.012)))
        draw.rectangle([m, m, m + inner_w - 1, m + inner_h - 1], outline=0, width=ow)
        pad = ow + max(1, int(inner_h * 0.03))

    glyph = icon_glyph(icon)
    icon_band_h = int(inner_h * _DECORATIVE_ICON_BAND) if glyph else 0

    text_x = m + pad
    text_y = m + pad + icon_band_h
    text_w = max(1, inner_w - 2 * pad)
    text_h = max(1, inner_h - 2 * pad - icon_band_h)

    if glyph:
        icon_font_size = max(8, int(icon_band_h * 0.8))
        icon_font = _load_font(icon_font_size, bold=True)
        iw, ih = _text_size(draw, glyph, icon_font)
        ix = m + max(0, (inner_w - iw) // 2)
        iy = m + pad + max(0, (icon_band_h - ih) // 2)
        draw.text((ix, iy), glyph, fill=0, font=icon_font)

    # Grow the type until wrapping to <= 3 lines no longer fits the space.
    best_size = 8
    best_lines = [text or ""]
    size = 8
    while size <= text_h:
        font = _load_font(size, bold=bold)
        lines = _wrap_lines(draw, text or "", font, text_w, max_lines=3)
        total_h = sum(_text_size(draw, ln, font)[1] for ln in lines)
        total_h += max(0, len(lines) - 1) * int(size * 0.15)
        widest = max((_text_size(draw, ln, font)[0] for ln in lines), default=0)
        if total_h <= text_h and widest <= text_w:
...
def render_to_pdf_bytes(img_or_specs, label_size: tuple[float, float] | None = None,
                        shape: str = "rectangle") -> bytes:
    ...
    ``shape`` applies to specs (bare images are used as-is): round stock
    composes each label inside its cut circle (see render_shaped_label), and
    the default keeps today's rectangular rendering untouched. Each page is
    set to the label's physical dimensions so a printer driver lays it out at
    true size. Returns the PDF as bytes."""
    ...
    for item in items:
        if isinstance(item, LabelSpec):
            pages.append(render_shaped_label(item, shape).convert("L"))
...
def render_batch_pdf_bytes(specs: list[LabelSpec], shape: str = "rectangle") -> bytes:
    ...
    single multi-page PDF, each page sized to the label stock. ``shape``
    follows the configured label stock, so a batch onto round stock keeps
    every label inside its cut circle. The page count equals the number of
    specs."""
    if not specs:
        raise ValueError("no labels to render")
    return render_to_pdf_bytes(list(specs), shape=shape)
