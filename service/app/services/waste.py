...
async def load_waste(grocy, limit: int = 500, top: int = 5) -> list[dict]:
    ...
    """
    # The log and the product names are independent reads, so the Expiring
    # page waits for one round trip rather than two.
    rows, product_rows = await asyncio.gather(
        grocy._get(
    ...
    if isinstance(rows, BaseException) or isinstance(product_rows, BaseException):
        return []
    try:
        products = {str(p["id"]): p["name"] for p in product_rows}
