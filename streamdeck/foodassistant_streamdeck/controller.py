...
class Controller:
    ...
    async def _shopping_check_press(self, name: str) -> str:
        ...
        if face == "Checked":
            # The quick-check page rebuilds and the item drops out, but the
            # kitchen display does not change, so confirm the check-off there
            # too, naming the item (FoodAssistant-rdlo).
            label = actions.shopping_item_name(item)
            await actions.post_deck_confirmation(
                self.client, self.config.base_url,
                f"Checked off {label}" if label else "Checked off shopping item",
            )
    ...
    async def _timer_press(self, name: str, long_press: bool = False) -> None:
        ...
        or the first cycle stage); a short press while running adds a minute
        to a preset countdown (FoodAssistant-p9vd), advances a plain key to
        its next cycle stage (stopping after the last), or, on a recipe key,
        jumps the kiosk to the recipe (FoodAssistant-y7ud). A long press
        cancels; a press on an expired
        ...
        try:
            if long_press or timer.alert_active():
                ...
                if recipe_spec is not None:
                    ...
                    return
                if preset > 0:
                    # Press again to ADD a minute to the running countdown
                    # (FoodAssistant-p9vd): extend the remaining time rather
                    # than restarting the preset from the top.
                    await self._extend_deck_timer(timer, name, 60)
                else:
    ...
    async def _extend_deck_timer(self, timer: TimerState, name: str,
                                 seconds: float = 60) -> None:
        """Add time to a running key's countdown (FoodAssistant-p9vd).

        Pressing a running preset timer adds a minute rather than restarting
        it. When the key is bound to a shared server timer we extend it on the
        server (POST /timers/{id}/extend), so the moved deadline lands on the
        web UI and satellites too; the key rebinds to the returned state. If
        that fails, or the countdown is a deck-local offline run, we extend the
        local deadline so the kitchen timer still gains the minute. Either way
        the already-elapsed time is preserved, never reset to the full preset.
        """
        extended = None
        if timer.timer_id is not None and self.client is not None:
            extended = await actions.extend_server_timer(
                self.client, self.config.base_url, timer.timer_id, seconds
            )
        if extended is not None:
            timer.bind(extended)
        else:
            timer.extend_local(seconds)
        # The screen does not change on a timer press, so confirm the extension
        # on the kiosk the same way a start does (FoodAssistant-rdlo).
        await self._confirm_timer_extended(self._timer_label(name), seconds)

    async def _confirm_timer_extended(self, label: str, seconds: float) -> None:
        """Queue a brief "+1 min" toast on the kiosk. Best-effort."""
        minutes = max(1, round(seconds / 60))
        clean = str(label or "").strip()
        suffix = f"+{minutes} min"
        msg = (f"{clean} {suffix}"
               if clean and clean.lower() != "timer" else f"Timer {suffix}")
        await actions.post_deck_confirmation(
            self.client, self.config.base_url, msg
        )
