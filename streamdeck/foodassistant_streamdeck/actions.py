...
class TimerState:
    ...
    def extend_local(self, seconds: float, now: Optional[float] = None) -> None:
        """Offline fallback for "add a minute": push a deck-local countdown out
        by ``seconds`` without resetting it (FoodAssistant-p9vd).

        Only a live, non-alerting countdown is extended; an idle or finished
        key is left alone (a press there starts or dismisses, it does not
        extend). The elapsed time is preserved because we move the existing
        deadline forward rather than starting over from a full duration."""
        now = time.time() if now is None else now
        if self.alerting or self.deadline_epoch <= 0:
            return
        self.deadline_epoch = max(self.deadline_epoch, now) + max(0.0, float(seconds))
...
def shopping_item_name(item: dict) -> str:
    """Best-effort display name for a shopping-list item.
...
class ActionSpec:
    ...
    item: str = ""           # for kind=="shopping_add" overrides: the product
                             # name to quick-add to the shopping list.
...
ACTIONS: dict[str, ActionSpec] = {
    ...
    "inventory": ActionSpec(
        ...
        color="#0f766e",
        kind="nav",
        # The explicit inventory route, not the bare "ui/" root: the root
        # redirects to whatever leads the nav (the Glance home when enabled), so
        # "ui/" sent the Stock key to Glance instead of stock (FoodAssistant-2e0f).
        target_path="ui/inventory",
    ...
    # ha_1..ha_5 are the LEGACY fixed Home Assistant slot keys (bound to
    # config.toml ha_slots in order). Retired from the editors in favour of
    # custom ha_action keys built with the entity picker (FoodAssistant-yzjr),
    # but kept registered so a deck.toml on a deployed device that still
    # references ha_1 keeps firing. catalog() marks them legacy so the
    # editors hide them from the palette while still rendering their faces.
    ...
    "guide": ActionSpec(
        ...
        color="#0f766e",
        kind="nav",
        target_path="ui/kitchen-guide",
        description="Open the Kitchen Guide page on the attached display.",
    ),
...
ACTION_EMOJI: dict[str, str] = {
    ...
    "convert": "abacus", "guide": "book", "timers_view": "stopwatch",
...
# Registered actions that are no longer offered for NEW placements: the
# fixed ha_1..ha_5 slot keys, replaced by custom ha_action keys with the
# entity picker (FoodAssistant-yzjr). They stay in ACTIONS (and in the
# catalog, flagged) so already-placed keys keep their face and fire path.
_LEGACY_ACTIONS = frozenset({"ha_1", "ha_2", "ha_3", "ha_4", "ha_5"})


def catalog() -> list[dict]:
    """Describe every assignable action for the web grid editor.

    Legacy actions carry ``"legacy": True``: the editors keep rendering them
    on a saved layout but leave them out of the palette, so no new ones get
    placed."""
    items = [{
        "name": spec.name,
        "label": spec.label,
        ...
        **({"legacy": True} if spec.name in _LEGACY_ACTIONS else {}),
...
async def add_shopping_item(client: Any, base_url: str, item: str) -> str:
    """Quick-add a favourite item to the shopping list. Returns a face.
    ...
    web UI shows, wherever the app keeps it), then posts the item note onto it.
    Returns "Added" on success, "No list" when there is no shopping list, and
    "Failed" on any error, so a press always degrades to a readable face rather
...
async def extend_server_timer(
    client: Any, base_url: str, timer_id: Any, seconds: float = 60,
) -> Optional[dict]:
    """Add time to a running shared timer (POST /timers/{id}/extend).

    Returns the updated timer dict (its deadline_epoch has moved out by
    ``seconds`` and total_seconds grew to match) or None. This is the "add a
    minute" path: the shared deadline shifts on the server, so the web UI,
    satellites, and this deck all show the extended countdown on their next
    read. Best-effort: any failure (including an expired timer, which 404s)
    returns None so the press can fall back to a deck-local extension."""
    try:
        tid = int(timer_id)
    except (TypeError, ValueError):
        return None
    base = base_url.rstrip("/")
    try:
        r = await client.post(f"{base}/timers/{tid}/extend", json={"seconds": seconds})
        return _timer_from_response(r)
    except Exception:  # noqa: BLE001 - shared timer is best-effort
        return None
