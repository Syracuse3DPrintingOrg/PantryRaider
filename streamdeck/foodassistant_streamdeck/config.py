...
# Clock key reading. "12" renders a compact 12-hour face (3:42P), "24" a
# 24-hour face (15:42), and "auto" the app's keep-current-behaviour value that
# reads 24-hour like the other surfaces. The default matches how the clock key
# has always rendered, so an older config.toml without the field stays 24-hour.
ALLOWED_CLOCK_FORMATS: tuple[str, ...] = ("auto", "12", "24")
DEFAULT_CLOCK_FORMAT = "24"
...
class Config:
    ...
    # 12/24-hour reading for the clock key, stamped by the app from the
    # clock_format setting so the deck agrees with the kiosk and app pages.
    # See ALLOWED_CLOCK_FORMATS; validated() clamps an unknown value to the
    # default, and an absent value keeps the historical 24-hour face.
    ...
    def validated(self) -> "Config":
        ...
        if self.clock_format not in ALLOWED_CLOCK_FORMATS:
...
def _apply(cfg: Config, data: dict) -> None:
    for name in ("base_url", "api_key", "kiosk_cdp_url", "weather_location", "weather_units",
                 ...
                 "bridge_token_path",
                 "key_style", "icon_color", "clock_format"):
