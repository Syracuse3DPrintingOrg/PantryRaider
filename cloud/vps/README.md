...
The shipped `cloud/Caddyfile` proxies the apex to `127.0.0.1:8000`, caps the
request body at 64MB (comfortably above the ~50MB cloud-backup limit, so raise
both together if `CLOUD_BACKUP_MAX_BYTES` ever moves), adds the on-demand TLS
block (`ask http://127.0.0.1:8000/v1/tunnel/tls-check` plus an issuance rate
guard), and imports `/etc/caddy/forager-kitchens.caddy`. Set
