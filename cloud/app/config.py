...
# The Stripe webhook signing-secret placeholder. It keeps the signature check
# real in tests and local dev, but the webhook handler refuses to process while
# this is still the configured value, so a deploy that forgets to override it
# fails loudly instead of trusting a secret published in this repo
# (FoodAssistant-spg1).
PLACEHOLDER_WEBHOOK_SECRET = "whsec_placeholder"
...
TRIAL_PLAN = "trial"
# The top plan. Cloud backup is gated to it (usage.premium_active): the trial
# and the smaller "basic" plan do not qualify.
...
class CloudSettings(BaseSettings):
    ...
    # The webhook handler refuses to run while this is still the placeholder,
    # so a missed override fails loudly rather than trusting a public value
    # (FoodAssistant-spg1).
    stripe_webhook_secret: str = PLACEHOLDER_WEBHOOK_SECRET

    # Stripe API secret key ("sk_..."), used for the few outbound Stripe
    # calls the portal makes on the owner's behalf: opening a Stripe
    # Customer Portal session (manage or cancel a plan) and cancelling a
    # subscription directly (the in-app cancel fallback and account
    # deletion). Empty keeps every outbound Stripe call dark, the same
    # all-or-nothing gating the other integrations use.
    ...
    # Passkey (WebAuthn) ceremonies. The begin/finish endpoints for both
    # registering a passkey and signing in with one share this window, applied
    # per IP (and per account where one is known), so neither the sign-in
    # button nor the add-a-passkey button can be hammered.
    passkey_rate_per_minute: int = 20
    ...
    # Flagging a recipe is a write anyone can call, so it gets the same tight
    # per-account and per-IP window as sharing, capping how fast one account or
    # one network can fire off reports.
    recipe_report_rate_per_minute: int = 5
    ...
    # Creating a share link is a cheap write anyone signed in can call, so it
    # gets the same tight per-account and per-IP window as community sharing.
    share_create_rate_per_minute: int = 5
    # Share emails put mail on the wire addressed by the caller, the classic
    # spam vector, so sends get a much slower budget: this many per rolling
    # hour per account, on top of the per-minute create limit above.
    share_email_per_hour: int = 5
    # Anonymous expiry-learning submissions (POST /api/learn/expiry). Fully
    # unauthenticated by design (the app's opt-in sharing works without an
    # account), so the per-IP window is the only throttle; nothing about the
    # caller is stored. An app uploads at most a batch or two per hour, so a
    # small budget costs nothing legitimate.
    learn_rate_per_minute: int = 6
    # How many observations a (barcode or product name, storage) group needs
    # before it appears in the published overrides feed: the k-threshold that
    # keeps a lone submission from ever being individually visible.
    learn_k_threshold: int = 5
    ...
    # Encryption key for two-factor authenticator secrets at rest (Fernet: 32
    # url-safe base64 bytes, from Fernet.generate_key()). The base32 TOTP seed
    # is the one stored credential that is directly usable, so it is encrypted
    # under a key kept out of the database rather than written in the clear (see
    # app/totp_crypto.py). REQUIRED once any account enables 2FA: the app
    # refuses to start when enrolled accounts exist without it, and never falls
    # back to storing a seed as plaintext. A deploy with no 2FA users can leave
    # it empty until the first enrollment.
    totp_secret_key: str = ""
    ...
    # --- Privacy: anonymous-reporter hashing and data retention ---
    # Pepper mixed into the hash of an anonymous reporter's address before it
    # is stored (see security.hash_ip): the database never holds a raw client
    # IP, and without this server-side value the short hash cannot be brute
    # forced back to an address. Set it once and leave it; changing it only
    # resets anonymous-report dedupe, nothing else.
    report_ip_pepper: str = ""
    # How often the retention sweep runs (hours). It purges expired and used
    # single-use credentials (email tokens, sign-in challenges, pairing
    # codes), expired sessions, and old share reports, so stale personal data
    # never just accumulates. 0 turns the background sweep off.
    retention_sweep_hours: float = 24.0
    # How long a share report is kept before the sweep deletes it. The
    # report's moderation effect (report_count, auto-revoke) is applied the
    # moment it is filed and is not undone by the purge.
    report_retention_days: int = 90

    # --- Cloud backup (Premium only) ---
    # Where uploaded kitchen backups are stored on the VPS filesystem, one
    # subdirectory per account. Kept on disk, never in Postgres (the database
    # holds only the small CloudBackup metadata row). A path under the app's
    # data directory in production; tests point it at a temp dir.
    backup_storage_dir: str = "/app/data/backups"
    # The largest single backup the upload accepts, in bytes. Anything larger
    # gets a clear "that backup is too large" message rather than being stored.
    backup_max_bytes: int = 50_000_000
    # How many backups to keep per account. A new upload past this count evicts
    # the oldest (its file and its row), so storage stays bounded per kitchen.
    backup_retention_count: int = 3

    # --- Shared-recipe image hosting (afx1) ---
    # Where the raw bytes of a shared recipe's photo live on the VPS filesystem,
    # one file per recipe or share keyed by its id or token. Forager hosts its
    # own copy so the community page and the website never depend on the origin
    # kitchen being online or publicly reachable. Empty points at a
    # "recipe-images" directory beside the backup storage dir; tests point it at
    # a temp dir.
    recipe_image_dir: str = ""
    # The largest recipe photo the share upload accepts, in bytes. A few MB
    # covers a real phone photo; anything larger is refused rather than stored.
    recipe_image_max_bytes: int = 5_000_000
    # The public site that shows community recipes and owns their readable
    # per-recipe URL (<slug>-<token>). Forager returns this URL on share so the
    # app and the website agree on where a shared recipe lives.
    community_site_base_url: str = "https://pantryraider.app"
...
# --- Anonymous-reporter pepper -------------------------------------------------
# security.hash_ip refuses an empty pepper, because an unsalted hash of an IPv4
# address is trivially reversible from a database dump (FoodAssistant-1k1b). If
# an operator sets CLOUD_REPORT_IP_PEPPER we use it; otherwise we generate a
# strong random pepper once and persist it next to the app's data, so the hash
# is never unsalted and an operator is never blocked by a missing setting.
_generated_pepper: str | None = None


def _pepper_file() -> Path:
    """Where a generated pepper is persisted: alongside the app data directory
    (the parent of the backup storage dir), so it survives restarts and stays
    stable, which keeps anonymous-report dedupe working across a reboot."""
    return Path(settings.backup_storage_dir).parent / "report_ip_pepper.key"


def report_ip_pepper() -> str:
    """The active anonymous-reporter pepper: the configured value if set, else
    a persisted random one, generating and saving it on first use.

    Never returns empty. If the value cannot be written to disk (a read-only or
    missing data dir, as in tests), it is still generated and cached in memory
    for the life of the process, so reporting keeps working and the hash is
    still salted; only cross-restart dedupe is affected, which is exactly what
    the setting's own comment says changing the pepper resets."""
    global _generated_pepper
    if settings.report_ip_pepper:
        return settings.report_ip_pepper
    if _generated_pepper:
        return _generated_pepper
    path = _pepper_file()
    try:
        if path.exists():
            existing = path.read_text(encoding="utf-8").strip()
            if existing:
                _generated_pepper = existing
                return _generated_pepper
    except OSError:
        pass
    _generated_pepper = secrets.token_urlsafe(32)
    try:
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(_generated_pepper, encoding="utf-8")
    except OSError:
        logger.warning(
            "CLOUD_REPORT_IP_PEPPER is unset and the generated pepper could not "
            "be persisted (%s); using an in-memory pepper for this run. Set "
            "CLOUD_REPORT_IP_PEPPER to a fixed random value to keep anonymous-"
            "report dedupe stable across restarts.", path)
    return _generated_pepper
