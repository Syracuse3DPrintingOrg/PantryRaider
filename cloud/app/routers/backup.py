...
async def upload_backup(file: UploadFile = File(...),
    ...
    # read_capped stops at the cap instead of buffering the whole body first,
    # so an over-sized push costs one chunk rather than its full size in memory.
    try:
        data = await read_capped(file, settings.backup_max_bytes)
    except UploadTooLarge as exc:
        cap_mb = exc.max_bytes // 1_000_000
