...
A member with a linked Pantry Raider kitchen (or one an admin has
...
from .. import ratelimit, recipe_audit, turnstile
...
from ..recipe_format import (NO_PDF_TEXT_MESSAGE, clean_text, extract_pdf_text,
                             format_recipe_draft)
from ..uploads import UploadTooLarge, read_capped
...
GATE_MESSAGE = ("Recipe upload is for members with a linked Pantry Raider "
                "kitchen. Link a kitchen and uploads unlock on their own. "
                "This gate exists only to keep spam and abuse out: if you "
                "would like uploads enabled without a kitchen, send us a "
                "short personal note from the email you signed up with and "
                "we will turn it on for you.")
...
def upload_page(request: Request,
    ...
    if not can_upload(account, _account_instances(db, account.id)):
        return templates.TemplateResponse(request, "recipe_upload.html", {
            "signed_in": True,
            "is_admin": is_admin(account),
            "gate_blocked": True,
    ...
    if request.query_params.get("shared"):
        notice = ("Thanks for sharing! Your recipe is in and headed to the "
                  "community.")
        if request.query_params.get("nudge"):
            notice += (" One tip: the title looks like a brand name on its "
                       'own. A "Copycat" or "-style" title (say, "Copycat '
                       'Oreo Truffles") makes clear the recipe is your own '
                       "take.")
...
async def upload_submit(
    ...
    # The gate: a linked kitchen that has checked in, or a hand-authorized
    # account, and never an admin-blocked one.
    if not can_upload(account, _account_instances(db, account.id)):
        return templates.TemplateResponse(request, "recipe_upload.html", {
            "signed_in": True,
            "is_admin": is_admin(account),
...
async def _read_upload(upload: UploadFile | None) -> tuple[bytes, str]:
    ...
    read_capped stops at the cap rather than buffering the whole file first, so
    an over-sized upload never costs its full size in memory."""
    if upload is None:
        return b"", ""
    try:
        return await read_capped(upload, settings.recipe_upload_max_bytes), ""
...
async def _draft_from_pdf(forwarder, pdf: UploadFile | None) -> tuple[dict, str]:
    ...
    if size_err:
        return {}, size_err
    # PDF parsing is synchronous CPU work on a file a stranger chose, so it runs
    # on a worker thread: a slow or hostile document stalls only its own upload,
    # never every other request the process is serving.
...
def upload_confirm(request: Request,
    ...
    # The same share audit the app's JSON submit runs, so nothing slips past
    # by posting here: an explicit publisher-copy line is refused with the
    # reason on the review form; a brand-name title saves and earns a tip on
    # the thank-you page.
    audit = recipe_audit.audit_recipe(title=draft["title"],
                                      steps=draft["steps"])
    if audit["level"] == recipe_audit.LEVEL_BLOCK:
        return _render_review(request, account, draft, attribution,
                              error=recipe_audit.block_message(audit),
                              status_code=400)
    ...
    recipe.audit_level = audit["level"]
    recipe.audit_findings = json.dumps(audit["findings"])
    db.add(recipe)
    db.commit()
    target = "/recipes/upload?shared=1"
    if audit["level"] == recipe_audit.LEVEL_ADVISE:
        target += "&nudge=1"
