...
def consume_totp(db: Session, account: Account, code: str) -> bool:
    ...
    if account.totp_secret:
        # The seed is stored encrypted at rest (totp_crypto); decrypt only here,
        # at the verification step, never holding the raw seed anywhere else.
        step = totp_matched_step(decrypt_secret(account.totp_secret), code)
        if step is not None:
