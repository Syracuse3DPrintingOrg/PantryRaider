...
from .. import ratelimit, recipe_audit, recipe_images, turnstile
...
def submit_recipe(payload: RecipeSubmission, request: Request,
    ...
    # The share audit (see recipe_audit): only an explicit publisher-copy
    # line is refused. A brand-name title is shared as-is and the naming
    # guidance rides back with the success reply.
    audit = recipe_audit.audit_recipe(title=title,
                                      description=payload.description,
                                      steps=steps)
    if audit["level"] == recipe_audit.LEVEL_BLOCK:
        raise HTTPException(400, detail=recipe_audit.block_message(audit))
    ...
    db.add(recipe)
    db.commit()
    reply = {"id": recipe.id, "slug": recipe.slug,
             "share_token": recipe.share_token, "url": canonical_url(recipe)}
    tips = recipe_audit.guidance(audit)
    if tips:
        reply["guidance"] = tips
    return reply
