...
def can_upload(account: Account | None, instances, now: datetime,
               active_days: int | None = None) -> bool:
    """Whether this account may upload its own recipes from the portal.

    Pure and testable. True when the account is signed in and either carries
    the manual authorization flag (set by an admin) or owns at least one
    kitchen that has checked in within the active window. A disabled account,
    or one with no recent kitchen and no authorization, is False. instances is
    the account's Instance rows; last_seen_at is a UTC ISO-8601 string, so a
    plain string compare against the cutoff is correct."""
    if account is None or getattr(account, "disabled", 0):
        return False
    if getattr(account, "recipe_upload_authorized", 0):
        return True
    if active_days is None:
        active_days = settings.recipe_active_kitchen_days
    cutoff = (now - timedelta(days=active_days)).isoformat(timespec="seconds")
    for inst in instances:
        seen = getattr(inst, "last_seen_at", "") or ""
