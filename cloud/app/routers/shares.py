...
from .. import ratelimit, recipe_images
...
def share_email_bodies(*, title: str, url: str, message: str,
    ...
    parts.append("Shared through Pantry Raider (https://pantryraider.app), "
                 "the food tracker that is free for personal use.")
    text = "\n\n".join(parts)
    ...
    html_parts.append(
        ...
        'tracker that is free for personal use.</p>')
    return subject, text, "\n".join(html_parts)
...
@router.post("/v1/recipes/shares/{token}/image")
async def upload_share_image(token: str, request: Request,
                             file: UploadFile = File(...),
                             db: Session = Depends(get_db)):
    """Store the shared recipe's photo on Forager so the share page shows it
    without depending on the origin kitchen being reachable. Owner only, and
    only a real, size-capped image is accepted."""
    account, _ = require_actor(request, db)
    share = db.query(SharedRecipe).filter_by(
        token=token, owner_account_id=account.id).first()
    if not share:
        raise HTTPException(404, detail=NOT_FOUND)
    # Rate limit per member and per address, matching the share-create window:
    # an image write is cheap but must not become a storage-abuse vector.
    limit = settings.share_create_rate_per_minute
    if (not ratelimit.allow(f"share-image-acct:{account.id}", limit)
            or not ratelimit.allow(f"share-image-ip:{client_ip(request)}", limit)):
        raise HTTPException(429, detail=RATE_LIMITED)

    data, ext = await recipe_images.read_image_upload(file)
    recipe_images.store_image(f"share-{share.token}", data, ext)
    share.image_url = (f"{settings.public_base_url.rstrip('/')}"
                       f"/v1/recipes/shares/{share.token}/image")
    share.updated_at = utc_now_iso()
    db.commit()
    return {"ok": True, "image_url": share.image_url}


@router.get("/v1/recipes/shares/{token}/image", include_in_schema=False)
def share_image(token: str, db: Session = Depends(get_db)):
    """Serve a live share's stored photo. A revoked or unknown token answers
    404, exactly like the share page itself."""
    share = _live_share(db, token)
    if not share:
        raise HTTPException(404, detail=NOT_FOUND)
    path = recipe_images.find_image(f"share-{share.token}")
    if path is None:
