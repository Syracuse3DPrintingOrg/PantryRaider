...
def image_url_ok(value: str) -> bool:
    """Whether the image link is empty or a plain web address. Anything else
    (javascript:, data:, a bare path) is refused: this string lands in an
    <img src> on a public page and in the JSON-LD download."""
    if not value:
        return True
...
def share_report(token: str, request: Request,
    ...
    reporter_key = (f"acct:{viewer.id}" if viewer
                    else f"ip:{client_ip(request)}")[:120]
