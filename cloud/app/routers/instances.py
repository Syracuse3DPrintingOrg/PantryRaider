...
from ..deps import (ACCOUNT_DISABLED_MESSAGE, current_account,
                    current_instance, instance_even_if_disabled, get_db,
                    utc_now_iso, client_ip)
...
def verify_unlock(payload: VerifyUnlockRequest, request: Request,
                  inst: Instance = Depends(instance_even_if_disabled),
                  db: Session = Depends(get_db)):
...
def instance_me(inst: Instance = Depends(instance_even_if_disabled),
                db: Session = Depends(get_db)):
...
def revoke_instance(inst: Instance = Depends(instance_even_if_disabled),
                    db: Session = Depends(get_db)):
