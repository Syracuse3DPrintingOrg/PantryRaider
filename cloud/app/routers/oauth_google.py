...
Three flows share the same Google round-trip, told apart by what was
...
- flow=unlock: an already-linked device's login screen sends its owner
  here so a member whose Forager account has no password (it was created
  with Google) can still unlock the device. Same round-trip and the same
  return_url guards as flow=app, but the minted code carries
  purpose="unlock": it never creates an account, never mints an instance,
  and only answers POST /v1/instance/verify-unlock, where the device's own
  bearer token proves which account the code must match.
...
from ..models import Account, PairingCode, TunnelPeer
...
def _return_url_ok_for_account(db: Session, url: str, account_id: int) -> bool:
    """Whether THIS account's provision code may be delivered to ``url``.

    _safe_return_url is only a syntactic gate: it accepts any Forager tunnel
    subdomain, but a tunnel subdomain serves some kitchen OWNER's device, and
    that owner is not necessarily the person who just signed in. Anyone can
    stand up a kitchen at their-name.forager.pantryraider.app, lure a victim
    through flow=app with return_url pointing at it, and read the victim's
    single-use code off their own inbound request (FoodAssistant-5g4l). So the
    callback re-checks with the authenticated account in hand:

    - a loopback / private-LAN / ".local" address is the app talking to itself
      on a network the signer already controls, and is allowed;
    - a tunnel subdomain is allowed only when it is THIS account's own kitchen
      (its TunnelPeer belongs to account_id), never another member's;
    - everything else, including the bare portal host, is refused.

    Used at the two points that actually mint and hand out a code (the Google
    callback and the 2FA submit), so a tampered return_url pointing at someone
    else's kitchen never receives the victim's code."""
    parts = urlsplit(url)
    if parts.scheme not in ("http", "https"):
        return False
    host = (parts.hostname or "").lower()
    ...
    if _is_local_host(host):
        return True
    if host.endswith(_TUNNEL_SUFFIX):
        subdomain = host[:-len(_TUNNEL_SUFFIX)]
        peer = (db.query(TunnelPeer)
                .filter_by(subdomain=subdomain).first())
        return bool(peer) and peer.account_id == account_id
    # The bare portal host and every other public host are never the app
    # talking to itself, so they never receive a code.
    return False
...
def _mint_provision_code(db: Session, account_id: int,
                         purpose: str = "link") -> str:
    ...
    pairing codes. purpose="unlock" mints the device-unlock flavour instead,
    spendable only at /v1/instance/verify-unlock."""
    ...
    db.add(PairingCode(code_hash=token_hash(code), account_id=account_id,
                       ...
                       created_at=utc_now_iso(), purpose=purpose))
    db.commit()
    return code
...
def google_callback(request: Request, code: str = "", state: str = "",
    ...
    flow = stashed.get("flow", "")
    # A sign-in from the login page must not create an account: offer signup
    # instead. The signup button and the app onboarding button create. A
    # device unlock (flow=unlock) NEVER creates, whatever intent rode along:
    # it only proves that an existing account owns the email.
    if flow == "unlock":
        create = False
    else:
        create = stashed.get("intent") == "signup" or flow == "app"
    account = _account_for_email(db, email, create=create)
    if account is None:
        if flow == "unlock":
            # No Forager account uses this Google address. Send the browser
            # back to the device's own sign-in screen with a friendly flag;
            # no code exists, so the syntactic return_url check is enough for
            # this hop (there is no account to bind to).
            return_url = stashed.get("return_url", "")
            if not _safe_return_url(return_url):
                raise HTTPException(
                    400, detail="This sign-in link is not valid. Start again "
                                "from your kitchen's own screen.")
            sep = "&" if "?" in return_url else "?"
            resp = RedirectResponse(f"{return_url}{sep}error=no-account",
                                    status_code=303)
    ...
    if flow in ("app", "unlock"):
        # flow=app mints a linking code (redeemable for an instance token);
        # flow=unlock mints an unlock code (spendable only at verify-unlock).
        purpose = "link" if flow == "app" else "unlock"
        return_url = stashed.get("return_url", "")
        if not _return_url_ok_for_account(db, return_url, account.id):
            # Fail safe: no provision code is minted or delivered to a return_url
            # that is not the signing account talking to itself. A tampered
            # return_url pointing at another member's kitchen subdomain lands
            # here and gets nothing (FoodAssistant-5g4l).
            raise HTTPException(
                400, detail="This sign-in link is not valid. Start again from "
                            "your kitchen's own screen.")
        ...
        # the portal login uses, carrying the app's return address (and this
        # flow's purpose) so a correct code sends the browser back to the app
        # with the code (portal login_2fa_submit does the mint and redirect).
