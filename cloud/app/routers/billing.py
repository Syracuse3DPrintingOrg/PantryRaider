...
from .accounts import (clear_totp_failures, consume_totp, register_totp_failure,
                       totp_locked)
...
def delete_account_submit(request: Request,
    ...
    # The second factor here is always a code (the password was already checked
    # above), so it is the same brute-force surface the login paths harden:
    # refuse while the second factor is locked, count each wrong code toward the
    # per-account TOTP lockout, and clear the run on success (zf1t). The
    # delete-acct rate limit above is per-minute only and counted nothing
    # durable toward that lockout.
    if account.totp_enabled:
        if totp_locked(account):
            return _delete_page(
                request, account,
                "Too many two-factor attempts. Wait a few minutes and try "
                "again. Your account was not deleted.", 429)
        if not consume_totp(db, account, totp):
