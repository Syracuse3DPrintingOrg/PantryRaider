...
from ..deps import (ACCOUNT_DISABLED_MESSAGE, SESSION_COOKIE, client_ip,
                    cookie_account, get_db, is_admin, require_same_origin,
                    utc_now_iso)
from ..email import base_url, email_configured, send_email
from ..models import (Account, AuthSession, CloudBackup, CommunityRecipe,
                      EmailToken, Instance, RecoveryCode, SharedRecipe,
                      TotpChallenge, UsageLedger, WebAuthnCredential)
...
from .accounts import (DISPOSABLE_EMAIL_MESSAGE, _issue_session, _valid_email,
                       authenticate, clear_totp_failures, consume_totp,
                       register_totp_failure, replace_recovery_codes,
                       totp_locked)
...
def _format_bytes(n: int) -> str:
    """A short human size for the account page's backup card ("12.3 MB")."""
    size = float(max(0, n))
...
_NOTICES = {
    ...
    "passkey-removed": "Passkey removed. It can no longer sign in to your account.",
    "passkey-added": "Passkey added. You can now sign in with your device.",
    "share-revoked": "Share link turned off. Anyone opening it now sees "
                     "nothing.",
    "cancel-scheduled": "Your subscription is cancelled. Your plan stays "
                        "active until the end of the period you already paid "
                        "for and will not renew.",
}
_ACCOUNT_ERRORS = {
    ...
    "twofa-throttled": "Too many attempts. Wait a few minutes and try again. "
                       "Two-factor authentication is still on.",
}
...
def _issue_totp_challenge(db: Session, account_id: int, return_url: str = "",
                          device_name: str = "",
                          purpose: str = "link") -> RedirectResponse:
    ...
    return_url and device_name are set only for the Google app-return flows
    (flow=app and flow=unlock): when present, redeeming the code sends the
    browser back to the app with a provision code instead of starting a
    portal session. purpose rides along so the minted code keeps the flavour
    the flow started with ("link" pairs a new install, "unlock" only proves
    identity to an already-linked device)."""
    token = new_token("prt")
...
def landing(request: Request, account: Account | None = Depends(cookie_account)):
    return templates.TemplateResponse(request, "landing.html",
                                      {"signed_in": account is not None,
                                       ...
                                       # Set right after a self-serve account
                                       # deletion, so the goodbye is said here.
...
def login_2fa_submit(request: Request, code: str = Form(""),
    ...
    if (not ratelimit.allow(f"login2fa-ip:{_client(request)}",
            ...
            or not ratelimit.allow(f"login2fa-acct:{account.id}",
                                   settings.login_rate_per_minute)):
        return retry("Too many attempts. Wait a minute and try again.", 429)
    if totp_locked(account):
        # Too many wrong codes: the second factor is in its lockout window, so
        # refuse without checking, the same cap the JSON paths enforce.
        return retry("Too many attempts. Wait a few minutes and try again.", 429)
    if not consume_totp(db, account, code):
        # One generic failure for a wrong TOTP code or a wrong/spent recovery
        # code; it never says which, or whether the account exists. Count it
        # toward the per-account second-factor lockout.
        register_totp_failure(db, account)
        return retry("That code did not match. Try again.")
    clear_totp_failures(db, account)
    # Correct. An app-return challenge (Google flow=app or flow=unlock on a
    # 2FA account) sends the browser back to the app with a fresh provision
    # code, not into a portal session. A plain portal login gets the real
    # session. device_name rides along in the challenge for symmetry with the
    # cookie state; the app still names its own kitchen when it redeems the
    # code. The challenge's purpose carries through to the minted code, so an
    # unlock sign-in can never end in a linking code.
    return_url = (challenge.return_url or "").strip()
    purpose = (challenge.purpose or "link").strip() or "link"
    db.delete(challenge)
    db.commit()
    from .oauth_google import (_mint_provision_code,  # avoids a cycle
                               _return_url_ok_for_account)
    if return_url and _return_url_ok_for_account(db, return_url, account.id):
        provision_code = _mint_provision_code(db, account.id, purpose=purpose)
...
def account_page(request: Request,
    ...
    # Share links this member created (for the revoke list) and live shares
    # addressed to them (their "Shared with you" inbox).
    my_shares = [
        {"token": s.token, "title": s.title, "created_at": s.created_at,
         "revoked": bool(s.revoked), "view_count": s.view_count}
        for s in (db.query(SharedRecipe)
                  .filter_by(owner_account_id=account.id)
                  .order_by(SharedRecipe.id.desc()).all())]
    inbox_shares = [
        {"token": s.token, "title": s.title, "attribution": s.attribution,
         "created_at": s.created_at}
        for s in (db.query(SharedRecipe)
                  .filter_by(recipient_account_id=account.id, revoked=0)
                  .order_by(SharedRecipe.id.desc()).all())]
    passkeys = [
        {"id": p.id, "nickname": p.nickname or "Passkey",
         "created_at": p.created_at, "last_used_at": p.last_used_at}
        for p in (db.query(WebAuthnCredential)
                  .filter_by(account_id=account.id)
                  .order_by(WebAuthnCredential.created_at).all())]
    # The account's cloud backups, newest first, for a read-only card. Premium
    # only, but shown to everyone (an empty state for non-Premium accounts).
    backup_rows = (db.query(CloudBackup).filter_by(account_id=account.id)
                   .order_by(CloudBackup.id.desc()).all())
    backups_total_bytes = sum(int(r.size_bytes or 0) for r in backup_rows)
    cloud_backups = {
        "count": len(backup_rows),
        "latest_at": backup_rows[0].created_at[:16].replace("T", " ") if backup_rows else "",
        "total_size": _format_bytes(backups_total_bytes),
        "premium": usage.premium_active(db, account.id),
    }
    percent = 0
    if state["quota"] > 0:
        percent = min(100, round(state["used"] * 100 / state["quota"]))
    # The Stripe subscription behind a paid plan, for the manage/cancel
    # button and the "cancels on <date>" line. None for a free account, a
    # trial, or an admin comp: those have nothing to cancel.
    from .billing import live_subscription, period_end_label
    ...
    return templates.TemplateResponse(request, "account.html", {
        ...
        "sub_cancelling": bool(sub.cancel_at_period_end) if sub else False,
        "sub_period_end": period_end_label(sub),
...
def change_password(request: Request,
    ...
    # People change a password because they think someone else has it. A
    # session token is a separate credential from the password, so without
    # this the other side keeps full access until the session simply ages
    # out. Every OTHER session for this account dies here; the one making the
    # change survives, so the owner is not logged out of the browser they are
    # standing at. Paired kitchens are untouched on purpose: those are device
    # credentials with their own Unlink, and dropping them would knock a
    # kitchen offline for a routine password change.
    keep = token_hash(request.cookies.get(SESSION_COOKIE, ""))
    (db.query(AuthSession)
       .filter(AuthSession.account_id == account.id,
               AuthSession.token_hash != keep)
...
def totp_enable(request: Request, secret: str = Form(""), code: str = Form(""),
    ...
    if step is None:
        return retry("That code did not match. Check your authenticator app "
                     "and try again.")
    # Store the seed encrypted at rest (totp_crypto), never as plaintext.
    account.totp_secret = encrypt_secret(secret)
    account.totp_enabled = 1
    # Record the step this enrollment code matched so the very same code cannot
    # be replayed at the next login: the replay guard (consume_totp) only accepts
    # a step newer than the last one used, and enable is the first use (w33x).
...
def totp_disable(request: Request, credential: str = Form(""),
    ...
    # A correct account password is legitimate proof of the owner and is not
    # counted against the second-factor lockout, matching how authenticate and
    # totp_gate keep the password and code counters apart.
    if bool(account.password_hash) and verify_password(
            credential, account.password_hash):
        pass
    else:
        # Otherwise the credential is a TOTP code (or a recovery code), and this
        # is the same brute-force surface the login 2FA page hardens: throttle
        # per IP and per account, refuse while the second factor is locked, and
        # count each wrong code toward that per-account lockout, so a session
        # holder without the password cannot grind the six-digit code to tear
        # down 2FA (zf1t).
        if (not ratelimit.allow(f"disable2fa-ip:{_client(request)}",
                                settings.login_rate_per_minute)
                or not ratelimit.allow(f"disable2fa-acct:{account.id}",
                                       settings.login_rate_per_minute)):
            return RedirectResponse("/account?e=twofa-throttled",
                                    status_code=303)
        if totp_locked(account):
            return RedirectResponse("/account?e=twofa-throttled",
                                    status_code=303)
        if not consume_totp(db, account, credential):
            register_totp_failure(db, account)
