...
def recipe_admin_row(recipe: CommunityRecipe, submitter_email: str,
    ...
    return {
        "id": recipe.id,
        "title": recipe.title,
        "submitter": submitter_email or f"#{recipe.submitter_account_id}",
...
def set_recipe_upload(account_id: int, authorize: str = Form(""),
                      admin: Account = Depends(require_admin),
                      db: Session = Depends(get_db)):
    """Turn recipe uploads on or off for an account by hand.

    Uploading recipes from the portal normally needs a kitchen that has checked
    in recently; this flag lets a trusted contributor upload without one (or
    takes that permission away). authorize is "1" to grant, anything else to
    revoke."""
    account = _target(db, account_id)
    account.recipe_upload_authorized = 1 if authorize == "1" else 0
    db.commit()
    _log(db, admin, "recipe-upload-authorize" if account.recipe_upload_authorized
         else "recipe-upload-revoke", account.id)
    return _back(account.id)
...
def moderate_recipe(recipe_id: int, action: str = Form(...),
    ...
    db.commit()
    _log(db, admin, f"recipe-{action}", recipe.submitter_account_id,
...
def delete_recipe(recipe_id: int, status: str = Form("all"),
    ...
    db.delete(recipe)
    db.commit()
    _log(db, admin, "recipe-delete", submitter,
