...
def recipe_admin_row(recipe: CommunityRecipe, submitter_email: str,
                     ...
                     takedown_open: bool = False) -> dict:
    """The row a moderator sees: everything the public card hides (submitter,
    report count, status) plus the flag reasons when there are any, the
    share-time audit verdict, and whether a takedown notice stands on it."""
    return {
        "id": recipe.id,
        "title": recipe.title,
        ...
        "audit_level": getattr(recipe, "audit_level", "") or "ok",
...
def admin_recipes(request: Request, status: str = "all", page: int = 1,
    ...
    # Submitter emails, flag reasons, and standing takedown notices for just
    # the recipes on this page, fetched in bulk the same way the account
    # overview batches its lookups.
    emails, reasons_by, taken_down = {}, {}, set()
    if ids:
        ...
        taken_down = {n.recipe_id for n in
                      db.query(TakedownNotice)
                      .filter(TakedownNotice.recipe_id.in_(ids),
                              TakedownNotice.restored_at == "").all()}

    rows = [recipe_admin_row(r, emails.get(r.submitter_account_id, ""),
                             reasons_by.get(r.id), r.id in taken_down)
            for r in recipes]
...
@router.post("/admin/recipes/{recipe_id}/takedown")
def takedown_recipe(recipe_id: int, contact: str = Form(""),
                    details: str = Form(""), status: str = Form("all"),
                    page: int = Form(1),
                    admin: Account = Depends(require_admin_write),
                    db: Session = Depends(get_db)):
    """Unpublish a recipe over a copyright (DMCA) takedown notice, keeping the
    record the process asks for: who sent the notice and what it claimed. The
    recipe is hidden, not deleted, so a valid counter notice can put it back.
    Intake arrives by email (support@pantryraider.app, the same address every
    account request uses); this is the operator acting on one."""
    recipe = _recipe_target(db, recipe_id)
    db.add(TakedownNotice(recipe_id=recipe.id, contact=contact.strip()[:300],
                          details=details.strip()[:2000],
                          created_at=utc_now_iso()))
    recipe.status = "hidden"
    recipe.updated_at = utc_now_iso()
    db.commit()
    _log(db, admin, "recipe-takedown", recipe.submitter_account_id or 0,
         f"recipe #{recipe.id} {recipe.title}"[:255])
    return _back_to_recipes(normalize_recipe_filter(status), max(1, page))


@router.post("/admin/recipes/{recipe_id}/counter-notice")
def counter_notice_restore(recipe_id: int, note: str = Form(""),
                           status: str = Form("all"), page: int = Form(1),
                           admin: Account = Depends(require_admin_write),
                           db: Session = Depends(get_db)):
    """Put a taken-down recipe back after a counter notice. The note lands on
    the standing takedown row (counter_note, restored_at), so the whole paper
    trail for one notice lives in one place."""
    recipe = _recipe_target(db, recipe_id)
    notice = (db.query(TakedownNotice)
              .filter_by(recipe_id=recipe.id, restored_at="")
              .order_by(TakedownNotice.id.desc()).first())
    if not notice:
        raise HTTPException(400, detail="No standing takedown notice on this "
                                        "recipe")
    notice.counter_note = note.strip()[:2000]
    notice.restored_at = utc_now_iso()
    recipe.status = "approved"
    recipe.updated_at = utc_now_iso()
    db.commit()
    _log(db, admin, "recipe-counter-notice", recipe.submitter_account_id or 0,
         f"recipe #{recipe.id} {recipe.title}"[:255])
    return _back_to_recipes(normalize_recipe_filter(status), max(1, page))
