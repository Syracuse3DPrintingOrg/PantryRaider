...
def hash_ip(ip: str, pepper: str) -> str:
    ...
    Refuses an empty pepper: an IPv4 address is only 2^32 inputs, so an
    unsalted hash is trivially reversible from a database dump, the exact
    de-anonymization this is meant to prevent. Callers resolve a non-empty
    pepper first (config.report_ip_pepper), so an empty one here is a bug,
    not a runtime input."""
    if not pepper:
        raise ValueError("hash_ip requires a non-empty pepper")
...
def totp_matched_step(secret: str, code: str, now: float | None = None,
                      window: int = 1) -> int | None:
    """The 30-second time-step counter ``code`` matches for ``secret``, or None
    if it matches none in the window.

    Returning the matched step (not just a yes/no) is what lets a caller block
    replay: a code is only accepted while its step is newer than the last one
    the account already used, so a captured code cannot be re-presented inside
    the +/- one-step verification window. Constant-time compare, and it rejects
    anything that is not six digits before doing any HMAC work."""
    code = (code or "").strip()
...
def totp_verify(secret: str, code: str, now: float | None = None,
    ...
    before doing any HMAC work. Use ``totp_matched_step`` where replay must be
    blocked (login and provisioning); this stays for enrollment and the
    re-authentication checks, where the code proves possession once."""
    return totp_matched_step(secret, code, now, window) is not None
