...
class Account(Base):
    ...
    # Lets this account upload its own recipes from the portal even without an
    # actively-used kitchen. Off by default; an admin turns it on from the
    # moderation panel for a trusted contributor. The normal path to upload is
    # simply having a kitchen that has checked in recently, so this stays 0 for
    # almost everyone.
...
class SharedRecipeReport(Base):
    ...
    anonymous visitor; reporter_key names whichever it was ("acct:<id>" or
    "ip:<address>"). The unique pair (share_id, reporter_key) means each
    ...
    # "acct:<account id>" for a signed-in reporter, "ip:<address>" otherwise.
