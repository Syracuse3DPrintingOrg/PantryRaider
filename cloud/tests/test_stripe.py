...
def test_a_failed_apply_leaves_no_processed_marker(client, session_token,
                                                   monkeypatch):
    """A delivery that cannot be applied must leave nothing behind.

    The "already processed" marker is what stops Stripe retrying, so writing it
    before the entitlement lands would mean a crash or database error in
    between turned a paid subscription into a 200 and silence: the retry sees
    the marker, gets acknowledged, and the customer is never upgraded. The
    marker goes in with the work, in one transaction, or not at all."""
    from app.models import StripeEvent
    from app.routers import stripe_webhook

    account_id = _account_id(client, session_token)
    real_set = stripe_webhook._set_entitlement

    def failing(db, acct_id, plan, status):
        raise RuntimeError("the database went away mid-delivery")

    monkeypatch.setattr(stripe_webhook, "_set_entitlement", failing)
    with pytest.raises(RuntimeError):
        _post_event(client, _checkout_event(account_id, event_id="evt_retry"))

    db = SessionLocal()
    try:
        # Nothing committed: no marker, no half-written subscription.
        assert db.query(StripeEvent).filter_by(event_id="evt_retry").count() == 0
        assert db.query(Subscription).count() == 0
        assert db.query(Entitlement).filter_by(account_id=account_id,
                                               source="stripe").count() == 0
    finally:
        db.close()

    # Stripe redelivers the same event and it is processed for real this time.
    monkeypatch.setattr(stripe_webhook, "_set_entitlement", real_set)
    resp = _post_event(client, _checkout_event(account_id, event_id="evt_retry"))
    assert resp.status_code == 200
    assert resp.json().get("duplicate") is None

    db = SessionLocal()
    try:
        ent = db.query(Entitlement).filter_by(account_id=account_id,
                                              source="stripe").first()
        assert ent is not None and ent.status == "active"
        assert db.query(StripeEvent).filter_by(event_id="evt_retry").count() == 1
    finally:
        db.close()
