...
def test_purpose_migration_backfills_existing_rows_as_link(tmp_path):
    """The purpose migration marks every pre-existing pairing code and TOTP
    challenge as "link", which is exactly what they were minted for, so an
    in-flight link keeps working across the deploy."""
    engine = create_engine(f"sqlite:///{tmp_path / 'purpose.db'}")
    cfg = _alembic_config()

    # Schema up to just before the purpose columns, then seed live rows the
    # way the app wrote them at that revision.
    with engine.begin() as connection:
        cfg.attributes["connection"] = connection
    ...
    with engine.begin() as c:
        c.execute(text(
            ...
            "redeemed, created_at) VALUES ('h1', 1, '2099-01-01T00:00:00', "
            "0, 't')"))
        c.execute(text(
            "INSERT INTO totp_challenges (token_hash, account_id, expires_at, "
            "created_at, return_url, device_name) VALUES ('h2', 1, "
            "'2099-01-01T00:00:00', 't', 'http://127.0.0.1:9284/x', 'Pi')"))
    ...
    with engine.begin() as c:
        assert c.execute(text(
            "SELECT purpose FROM pairing_codes")).scalar() == "link"
        assert c.execute(text(
            "SELECT purpose FROM totp_challenges")).scalar() == "link"


def test_encrypt_totp_secret_migration_encrypts_and_is_idempotent(
        tmp_path, monkeypatch):
    """The TOTP-encryption migration widens the column and rewrites every
    enrolled account's plaintext base32 seed as a Fernet token under
    CLOUD_TOTP_SECRET_KEY, and re-running it never double-encrypts (an already
    encrypted value is detected and left alone)."""
    from cryptography.fernet import Fernet
    key = Fernet.generate_key().decode()
    monkeypatch.setenv("CLOUD_TOTP_SECRET_KEY", key)
    engine = create_engine(f"sqlite:///{tmp_path / 'totpenc.db'}")
    cfg = _alembic_config()

    # Schema up to just before the encryption migration, then seed an enrolled
    # account whose totp_secret is a raw base32 seed (the old plaintext form).
    with engine.begin() as connection:
        cfg.attributes["connection"] = connection
        command.upgrade(cfg, "4e5f6a7b8c9d")
    seed = "JBSWY3DPEHPK3PXPJBSWY3DPEHPK3PXP"  # 32-char base32, like a real seed
    with engine.begin() as c:
        c.execute(text(
            "INSERT INTO accounts (id, email, password_hash, auth_provider, "
            "email_verified, disabled, failed_logins, locked_until, "
            "totp_secret, totp_enabled, totp_failures, totp_locked_until, "
            "totp_last_step, recipe_upload_authorized, recipe_upload_blocked, "
            "created_at) VALUES (1, 'a@x', '', 'password', 0, 0, 0, '', "
            ":s, 1, 0, '', 0, 0, 0, 't')"), {"s": seed})
        # A second account with 2FA off keeps its empty secret untouched.
        c.execute(text(
            "INSERT INTO accounts (id, email, password_hash, auth_provider, "
            "email_verified, disabled, failed_logins, locked_until, "
            "totp_secret, totp_enabled, totp_failures, totp_locked_until, "
            "totp_last_step, recipe_upload_authorized, recipe_upload_blocked, "
            "created_at) VALUES (2, 'b@x', '', 'password', 0, 0, 0, '', "
            "'', 0, 0, '', 0, 0, 0, 't')"))
    ...
    with engine.begin() as c:
        stored = c.execute(text(
            "SELECT totp_secret FROM accounts WHERE id = 1")).scalar()
        off = c.execute(text(
            "SELECT totp_secret FROM accounts WHERE id = 2")).scalar()
    assert stored != seed                                  # rewritten
    assert fernet.decrypt(stored.encode()).decode() == seed  # to real ciphertext
    assert off == ""                                       # off account untouched

    # Re-run tolerance: downgrade the encryption step and re-apply. The already
    # encrypted value is detected and left as a single token, not wrapped twice.
    with engine.begin() as connection:
        cfg.attributes["connection"] = connection
        command.downgrade(cfg, "4e5f6a7b8c9d")
    with engine.begin() as connection:
        cfg.attributes["connection"] = connection
        command.upgrade(cfg, "head")
    with engine.begin() as c:
        stored2 = c.execute(text(
            "SELECT totp_secret FROM accounts WHERE id = 1")).scalar()
    assert fernet.decrypt(stored2.encode()).decode() == seed  # still single seed


def test_community_slug_token_migration_backfills_and_is_idempotent(tmp_path):
    """The slug/token migration gives every pre-existing community recipe a
    title-derived slug and a fresh unique token, so its old ?id= link still
    resolves and its canonical URL exists. Re-running the step (downgrade then
    upgrade) keeps each recipe on a single, unique token."""
    engine = create_engine(f"sqlite:///{tmp_path / 'slugtoken.db'}")
    cfg = _alembic_config()

    # Schema up to just before the slug/token columns, then seed two live
    # recipes the way the app wrote them at that revision (no slug, no token).
    with engine.begin() as connection:
        cfg.attributes["connection"] = connection
        command.upgrade(cfg, "5f6a7b8c9d0e")
    with engine.begin() as c:
        c.execute(text(
            "INSERT INTO community_recipes (id, title, description, ingredients, "
            "steps, image_url, attribution, submitter_account_id, status, "
            "rating_count, rating_sum, report_count, created_at, updated_at) "
            "VALUES (1, 'Weeknight Chili', '', '[]', '[]', '', 'a', 1, "
            "'approved', 0, 0, 0, 't', 't'),"
            "(2, 'Lemon Cake!!', '', '[]', '[]', '', 'b', 1, 'approved', 0, 0, "
            "0, 't', 't')"))
    ...
    with engine.begin() as c:
        rows = c.execute(text(
            "SELECT id, slug, share_token FROM community_recipes "
            "ORDER BY id")).fetchall()
    by_id = {r[0]: (r[1], r[2]) for r in rows}
    assert by_id[1][0] == "weeknight-chili"      # slug from the title
    assert by_id[2][0] == "lemon-cake"           # punctuation collapsed
    tok1, tok2 = by_id[1][1], by_id[2][1]
    assert tok1 and tok2 and tok1 != tok2        # a fresh token each, distinct
    for tok in (tok1, tok2):
        assert len(tok) == 10 and tok.isalnum()  # base36, unguessable length

    # Re-entrancy: downgrade the step and re-apply. Each recipe keeps exactly
    # one token (a fresh one is fine), and the unique index holds.
    with engine.begin() as connection:
        cfg.attributes["connection"] = connection
        command.downgrade(cfg, "5f6a7b8c9d0e")
    with engine.begin() as connection:
        cfg.attributes["connection"] = connection
        command.upgrade(cfg, "head")
    with engine.begin() as c:
        again = c.execute(text(
            "SELECT share_token FROM community_recipes")).fetchall()
    tokens = [r[0] for r in again]
...
def test_alembic_cli_needs_no_pythonpath_crutch():
    """migrations/env.py imports the app to reach the database URL and the
    model metadata, but the alembic console script does not put the working
    directory on sys.path the way `python -m` does. Without prepend_sys_path
    every plain `alembic ...` dies with "No module named 'app'", and the next
    person works around it with a PYTHONPATH env var instead of fixing it.

    This bit once already: stamping the production database after a deploy
    failed on exactly that, mid-incident, which is the worst time to discover
    your migration tool will not start.
    """
    ...
    cfg.read(ini)
    assert cfg.get("alembic", "prepend_sys_path", fallback="").strip() == ".", \
        "alembic.ini must set prepend_sys_path = . so the plain CLI can import the app"
