...
def _comp_account(email="dan@example.com", expires_at="2099-12-31T23:59:59+00:00"):
    """Grant an admin comp directly, as /admin/accounts/{id}/comp writes it."""
    from app.config import PLAN_QUOTAS
    from app.models import Account, Entitlement
    db = SessionLocal()
    try:
        account = db.query(Account).filter_by(email=email).first()
        db.add(Entitlement(account_id=account.id, plan="premium",
                           status="active",
                           monthly_token_quota=PLAN_QUOTAS["premium"],
                           source="comp", expires_at=expires_at,
...
def test_comped_account_passes_the_gate(client, instance_token):
    # An admin-granted comp with no Stripe subscription behind it spends
    # exactly like a paid plan: the gate must not treat subscription as
    # "Stripe row exists".
    expire_trial()
    _comp_account()
    resp = _analyze(client, instance_token)
...
def test_expired_comp_is_402(client, instance_token):
    expire_trial()
    _comp_account(expires_at="2020-01-01T00:00:00+00:00")
    resp = _analyze(client, instance_token)
    assert resp.status_code == 402
