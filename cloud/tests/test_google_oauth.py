...
def _enroll_2fa(client, email="gina@example.com"):
    ...
    # Enabling records the step of the confirmation code (w33x), so that exact
    # code is a spent replay for the rest of its window. A real sign-in happens
    # later, in a new window, with a fresh code; clear the recorded step to stand
    # in for that (the replay guard itself is covered in test_totp.py).
    from app.database import SessionLocal
    from app.models import Account
    db = SessionLocal()
    try:
...
def test_safe_return_url_allows_local_and_first_party():
    from app.routers.oauth_google import _safe_return_url
    # Loopback, private LAN, mDNS ".local", localhost, and the Forager tunnel
    # subdomain are all the app talking to itself; each is accepted.
    for good in (
        "http://127.0.0.1:9284/setup/cloud/oauth-return",
        "http://[::1]:9284/setup/cloud/oauth-return",
        "http://192.168.1.170:9284/setup/cloud/oauth-return",
        "http://10.0.0.5:9284/setup/cloud/oauth-return",
        "http://172.16.4.2:9284/setup/cloud/oauth-return",
        "http://localhost:9284/setup/cloud/oauth-return",
        "http://foodassistant.local:9284/setup/cloud/oauth-return",
        "https://kitchen-pi.forager.pantryraider.app/setup/cloud/oauth-return",
    ):
        assert _safe_return_url(good) is True, good


def test_safe_return_url_rejects_public_hosts():
    from app.routers.oauth_google import _safe_return_url
    # A public host (attacker site), a look-alike domain, a bare public IP, and
    # non-http schemes never receive a code.
    for bad in (
        "https://evil.example.com/steal",
        "https://forager.pantryraider.app.evil.com/steal",
        "https://8.8.8.8/steal",
        "http://93.184.216.34/steal",
        "javascript:alert(1)",
        "ftp://host/x",
        "not-a-url",
        "",
    ):
        assert _safe_return_url(bad) is False, bad


def test_app_flow_rejects_public_return_url_end_to_end(client, google):
    # A successful Google login must not hand a provision code to a public
    # return_url: the start refuses it before Google is ever reached.
    resp = client.get("/auth/google/start",
                      params={"flow": "app",
                              "return_url": "https://evil.example.com/grab"},
                      follow_redirects=False)
    assert resp.status_code == 400


def test_browser_portal_flow_still_works(client, google):
    # The portal ("Continue with Google") flow does not carry an app return_url
    # and must keep landing a logged-in session on /account.
    resp = start_and_callback(client, "/auth/google/start?intent=signup")
    assert resp.status_code == 303
    assert resp.headers["location"] == "/account"
...
def _make_kitchen(email, subdomain):
    """An account with a tunnel-published kitchen at <subdomain>, so a return_url
    can point at its Forager subdomain."""
    from app.database import SessionLocal
    from app.models import Account, Instance, TunnelPeer
    db = SessionLocal()
    try:
        acct = db.query(Account).filter_by(email=email).first()
        if not acct:
            acct = Account(email=email, password_hash="", auth_provider="password",
                           email_verified=1, created_at="2026-01-01T00:00:00+00:00")
            db.add(acct)
            db.commit()
        inst = Instance(token_hash=f"hash-{subdomain}", account_id=acct.id,
                        name=subdomain, created_at="2026-01-01T00:00:00+00:00")
        db.add(inst)
        db.commit()
        db.add(TunnelPeer(instance_id=inst.id, account_id=acct.id,
                          tunnel_ip="10.99.0.9", subdomain=subdomain,
...
def test_app_flow_refuses_a_foreign_kitchen_subdomain(client, google):
    # FoodAssistant-5g4l: an attacker owns evilkitchen.forager.pantryraider.app
    # and lures the victim (gina, from the google fixture) through flow=app with
    # a return_url pointing at it. A tunnel subdomain passes the syntactic gate,
    # but it belongs to the ATTACKER's account, not the signing victim, so no
    # code is minted or delivered.
    _make_kitchen("attacker@example.com", "evilkitchen")
    ret = "https://evilkitchen.forager.pantryraider.app/setup/cloud/oauth-return"
    resp = start_and_callback(
        client, f"/auth/google/start?flow=app&return_url={ret}")
    assert resp.status_code == 400
    assert "code=" not in resp.headers.get("location", "")
    # Nothing was handed out: no pairing code exists for the victim's account.
    from app.database import SessionLocal
    from app.models import Account, PairingCode
    db = SessionLocal()
    try:
        gina = db.query(Account).filter_by(email="gina@example.com").first()
        assert gina is not None  # the victim's account was resolved
        assert db.query(PairingCode).filter_by(account_id=gina.id).count() == 0
    finally:
        db.close()


def test_app_flow_allows_the_signers_own_kitchen_subdomain(client, google):
    # The legitimate remote-setup path is unbroken: when the return_url is the
    # SIGNER's own kitchen subdomain, the provision code is delivered as before.
    _make_kitchen("gina@example.com", "gina-pi")
    ret = "https://gina-pi.forager.pantryraider.app/setup/cloud/oauth-return"
    resp = start_and_callback(
        client, f"/auth/google/start?flow=app&return_url={ret}")
    assert resp.status_code == 303
    target = urlsplit(resp.headers["location"])
    assert target.netloc == "gina-pi.forager.pantryraider.app"
    assert "code" in parse_qs(target.query)
