...
def test_signup_loads_turnstile_script_when_configured(client, monkeypatch):
    # The Turnstile api.js must actually load (in <head>), not sit inert inside
    # <title> where it never executes and blocks every signup (FoodAssistant-5q15).
    from app import config
    monkeypatch.setattr(config.settings, "turnstile_site_key", "test-site-key", raising=False)
    monkeypatch.setattr(config.settings, "turnstile_secret", "test-secret", raising=False)
    html = client.get("/signup").text
    assert 'turnstile/v0/api.js' in html
    # It must be outside the <title>...</title> element.
    title = html.split("<title>", 1)[1].split("</title>", 1)[0]
    assert 'turnstile' not in title
