...
def test_null_origin_post_is_rejected(client):
    """A sandboxed iframe sends `Origin: null`, not its real origin.

    This was a live bypass of the guard above. An attacker-controlled kitchen
    page could frame its own auto-posting form with
    sandbox="allow-forms allow-scripts"; the browser then sent Origin: null,
    which parsed to no host, so the mismatch check could not fire and the
    request was let through. The portal cookie is SameSite=Lax and a kitchen
    subdomain is SAME-site, so the cookie went with it: cancel a plan, remove
    a kitchen or a passkey, or drive a lured admin into disabling accounts.

    A present header now has to name a host that matches. Only an absent one
    is allowed through.
    """
    _portal_session(client)
    resp = client.post("/logout", follow_redirects=False,
...
def test_opaque_referer_is_rejected(client):
    """Same hole, reached through the other header."""
    _portal_session(client)
    resp = client.post("/logout", follow_redirects=False,
