...
def test_comp_grant_and_expire(admin_client, user_setup):
    ...
    # A comped account is fully active as far as the app is concerned: the
    # settings pane and the vision test both read "active", so the comp must
    # not be told it has no subscription.
    assert ent["plan"] == "premium" and ent["entitled"] is True and ent["active"] is True
    assert ent["plan_label"] == "Complimentary"
    assert ent["source"] == "comp"
    ...
    assert ent["plan_label"] == "No active plan"
