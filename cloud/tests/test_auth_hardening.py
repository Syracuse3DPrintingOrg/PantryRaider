...
def test_lockout_throttles_wrong_guesses_but_lets_the_owner_back_in(client, monkeypatch):
    # FoodAssistant-gszf: the lock throttles WRONG guesses so an online brute
    # force is capped, but the correct password still succeeds while locked, so
    # a pre-auth attacker who only knows the email cannot deny the owner access.
    ...
    db = SessionLocal()
    try:
        # Wrong passwords below the threshold: no lock yet.
        for _ in range(2):
        ...
        acct, locked = authenticate(db, "lock@example.com", "wrong-guess")
        assert acct is None and locked is None
        # A further wrong guess while locked is refused outright (throttled).
        acct, locked = authenticate(db, "lock@example.com", "still-wrong")
        assert acct is None and locked == ACCOUNT_LOCKED_MESSAGE
        # But the real owner's correct password gets in even during the lock,
        # and clears the counter behind them.
        ...
        assert db.query(Account).filter_by(
            email="lock@example.com").first().failed_logins == 0
    finally:
        db.close()


def test_lockout_window_expires_for_wrong_guesses(client, monkeypatch):
    from app.routers.accounts import (ACCOUNT_LOCKED_MESSAGE, authenticate)
    from app.database import SessionLocal
    ...
    try:
        ...
        # Locked right now: a wrong guess is refused as locked.
        _, locked = authenticate(db, "lock@example.com", "wrong-guess")
        assert locked == ACCOUNT_LOCKED_MESSAGE
        # Past the window the lock has lifted on its own (time-based, no cron):
        # a wrong guess is counted again rather than refused as locked.
...
def test_locked_account_login_endpoint_throttles_wrong_but_admits_owner(client, monkeypatch):
    monkeypatch.setattr(settings, "account_lockout_threshold", 2)
    _account(client)
    for _ in range(2):
        client.post("/v1/accounts/login",
                    json={"email": "lock@example.com", "password": "wrong-guess"})
    # A further wrong guess while locked is refused with 429.
    resp = client.post("/v1/accounts/login",
                       json={"email": "lock@example.com", "password": "wrong-guess"})
    assert resp.status_code == 429
    assert "failed attempts" in resp.json()["detail"].lower()
    # The owner's correct password still gets in: the lock throttles guessing,
    # it does not lock the owner out (FoodAssistant-gszf).
...
# --------------------------------------------------------------------------- #
# a password change has to end the other sessions, not just set a new password
# --------------------------------------------------------------------------- #

def test_changing_the_password_revokes_other_sessions(client):
    """The reason people change a password is that they think someone else has
    it. A session token is a separate credential, so without this the other
    side keeps full access until the session simply ages out.
    """
    ...
    client.post("/signup",
                data={"email": "revoke@example.com", "password": "hunter2222",
                      "confirm_password": "hunter2222"},
                follow_redirects=False)

    # A second signed-in browser: the one that should lose access.
    other = TestClient(app)
    assert other.post("/login",
                      data={"email": "revoke@example.com", "password": "hunter2222"},
                      follow_redirects=False).status_code == 303
    assert other.get("/account", follow_redirects=False).status_code == 200

    resp = client.post("/account/password",
                       data={"current_password": "hunter2222",
                             "new_password": "Different3333",
                             "confirm_password": "Different3333"},
                       follow_redirects=False)
    assert resp.status_code == 303 and "password-changed" in resp.headers["location"]

    # The other browser is signed out (a page route redirects to login).
    assert other.get("/account", follow_redirects=False).status_code == 303
    # The browser that made the change is still signed in, so a routine
    # password change does not log you out of the machine you are using.
...
def test_a_password_change_leaves_paired_kitchens_alone(client, instance_token):
    """Device credentials are managed separately (Unlink), so a routine
    password change must not knock a kitchen offline."""
    client.post("/account/password",
                data={"current_password": "hunter2222",
                      "new_password": "Different3333",
                      "confirm_password": "Different3333"},
                follow_redirects=False)
    r = client.get("/v1/instance/me",
