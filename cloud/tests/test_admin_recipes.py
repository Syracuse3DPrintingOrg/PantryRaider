...
def test_takedown_unpublishes_and_records_the_notice(admin_client, client):
    from app.models import AdminAction, TakedownNotice
    token = _member_token(client)
    rid = _submit(client, token, title="Contested Dish").json()["id"]

    resp = admin_client.post(
        f"/admin/recipes/{rid}/takedown",
        data={"contact": "Jane Author <jane@publisher.example>",
              "details": "Steps copied from my 2019 cookbook, page 44."},
        follow_redirects=False)
    assert resp.status_code == 303

    _set_status_check(rid, "hidden")
    titles = {c["title"] for c in client.get("/v1/recipes").json()["recipes"]}
    assert "Contested Dish" not in titles

    db = SessionLocal()
    try:
        notice = db.query(TakedownNotice).filter_by(recipe_id=rid).one()
        assert notice.contact == "Jane Author <jane@publisher.example>"
        assert "page 44" in notice.details
        assert notice.restored_at == ""
        assert (db.query(AdminAction)
                .filter_by(action="recipe-takedown").count() == 1)
    finally:
        db.close()

    # The moderation page shows the standing takedown.
    page = admin_client.get("/admin/recipes?status=hidden").text
    assert "takedown" in page
    assert "Counter notice" in page


def test_counter_notice_restores_and_records_the_note(admin_client, client):
    from app.models import TakedownNotice
    token = _member_token(client)
    rid = _submit(client, token, title="Vindicated Dish").json()["id"]
    admin_client.post(f"/admin/recipes/{rid}/takedown",
                      data={"contact": "someone", "details": "claim"})

    resp = admin_client.post(
        f"/admin/recipes/{rid}/counter-notice",
        data={"note": "Sharer wrote it themselves; counter notice on file."},
        follow_redirects=False)
    assert resp.status_code == 303

    _set_status_check(rid, "approved")
    titles = {c["title"] for c in client.get("/v1/recipes").json()["recipes"]}
    assert "Vindicated Dish" in titles

    db = SessionLocal()
    try:
        notice = db.query(TakedownNotice).filter_by(recipe_id=rid).one()
        assert "counter notice on file" in notice.counter_note
        assert notice.restored_at != ""
    finally:
        db.close()


def test_counter_notice_without_a_standing_takedown_is_refused(admin_client,
                                                               client):
    token = _member_token(client)
    rid = _submit(client, token).json()["id"]
    resp = admin_client.post(f"/admin/recipes/{rid}/counter-notice",
                             data={"note": "nothing to counter"})
    assert resp.status_code == 400


def test_takedown_is_admin_only(client, monkeypatch):
    monkeypatch.setattr(settings, "admin_emails", "someone-else@example.com")
    assert client.post("/admin/recipes/1/takedown",
                       data={"contact": "x", "details": "y"}).status_code == 404
    assert client.post("/admin/recipes/1/counter-notice",
                       data={"note": "z"}).status_code == 404
