...
def test_oversized_pdf_is_refused_before_it_is_parsed(portal_client, monkeypatch):
    """Past the cap the upload is turned away and never reaches the parser or
    the AI, so an over-sized file costs neither memory nor a provider call."""
    _authorize()
    monkeypatch.setattr(settings, "recipe_upload_max_bytes", 32)
    fake = _fake_ai(monkeypatch, {"title": "x", "ingredients": ["a"],
                                  "steps": ["b"]})
    parsed = []
    monkeypatch.setattr(recipe_upload, "extract_pdf_text",
                        lambda data: parsed.append(data) or "text")
    resp = portal_client.post(
        "/recipes/upload",
        data={"method": "pdf", "attribution": "A cookbook"},
        files={"pdf": ("big.pdf", b"%PDF-1.4 " + b"x" * 5_000,
                       "application/pdf")})
    assert resp.status_code == 400
...
def test_pdf_parsing_runs_off_the_request_loop(portal_client, monkeypatch):
    """Reading a PDF is synchronous work on a file a stranger picked, so it has
    to happen on a worker thread. Parsing it inline would let one crafted
    document freeze every other request the service is answering, not just its
    own upload."""
    _authorize()
    seen = {}

    class _ThreadNoting(FakeForwarder):
        async def forward(self, kind, image_data, mime_type, text):
            # An async call inside the endpoint: this is the request's own loop.
            seen["loop"] = threading.get_ident()
            return await super().forward(kind, image_data, mime_type, text)

    fake = _ThreadNoting({"title": "PDF Chili", "ingredients": ["beans"],
                          "steps": ["cook"]})
    monkeypatch.setattr(recipe_upload, "get_forwarder", lambda: fake)
    ...
    monkeypatch.setattr(recipe_upload, "extract_pdf_text", note_parse)
    resp = portal_client.post(
        "/recipes/upload",
        data={"method": "pdf", "attribution": "A cookbook"},
        files={"pdf": ("chili.pdf", b"%PDF-1.4 fake", "application/pdf")})
