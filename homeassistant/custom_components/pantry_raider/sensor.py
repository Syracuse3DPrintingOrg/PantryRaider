...
_SENSOR_META: dict[str, tuple[str, str, str | None, bool]] = {
    ...
    "timers_summary": ("Timers", "mdi:timer-play-outline", None, False),
...
class PantryRaiderSensor(PantryRaiderEntity, SensorEntity):
    ...
    def extra_state_attributes(self):
        # The two running-timer sensors expose the full list of active timers so
        # a card can render every countdown, not just the count or the next one.
        # Older servers omit timers.active, which shapes to an empty list.
        if self._key in ("timers_running", "timers_summary"):
            return {"timers": active_timers(self.coordinator.data)}
