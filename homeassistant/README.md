...
- `YOUR_HOST`: your Pantry Raider address (e.g. `https://food.yourdomain.com` for Pangolin, or `http://192.168.1.50:9284` for LAN-only)
- `YOUR_GROCY_HOST`: your Grocy address (e.g. `https://grocy.yourdomain.com` or `http://192.168.1.50:9383`); set **Grocy Public URL** in `/setup` so Pantry Raider itself also uses this for Grocy browser links
