...
Pin a specific version instead of latest by setting `FOODASSISTANT_TAG=0.18.99` in `.env`. Published tags carry no leading `v`: use the plain release number (`0.18.99`) to freeze one build, or the minor line (`0.18`) to keep picking up patch releases.
