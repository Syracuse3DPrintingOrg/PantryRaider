...
- **Pantry audit**: a read-only, location-scoped stock count, reached from the Manage page. Lock the scanner to one storage location and scan the items there; each scan is compared against Grocy's recorded stock so missing and unexpected items stand out, and nothing is written back to Grocy
- **Nutrition tracker**: log what you eat with calories and macros (protein, carbs, fat) on a Nutrition page under Kitchen Guide, with daily and recent-day totals; an optional AI estimate fills in macros from a food name when a provider is configured
