...
> [!WARNING]
> **Pre-release software.** Pantry Raider is under heavy active development and
> has not reached 1.0. Expect rough edges, fast-moving releases, and occasional
> breaking changes; back up your data before updating. Bug reports are very
> welcome in the issues tab.
...
- **Bandit Cubs** -- small ESP32 kitchen displays you flash from your browser and pair to your server; they show expiring items, running timers, and probe temperatures, and keep their own firmware up to date over the air
...
Prefer to do it by hand? Grab [`docker-compose.prod.yml`](docker-compose.prod.yml), save it as `docker-compose.yml`, and run `docker compose --profile with-grocy up -d`. The `with-grocy` profile is what starts the bundled inventory backend; leave it off only if you already run Grocy elsewhere.
...
Pin a specific version instead of latest by setting `PANTRYRAIDER_TAG=0.18.101` in `.env`. The older name `FOODASSISTANT_TAG` still works, so an existing `.env` keeps doing what it always did. Published tags carry no leading `v`: use the plain release number (`0.18.101`) to freeze one build, or the minor line (`0.18`) to keep picking up patch releases.
...
**Grocy in particular:** a Grocy release can rename a class that its saved `config.php` points at. Grocy 4.7.0 did exactly that (`AUTH_CLASS` moved to `Grocy\Middleware\Auth\DefaultAuthMiddleware`), and a `config.php` written by an older Grocy makes every API call answer with an error page, so the inventory stops loading. The compose files therefore mount a small start-up script into the Grocy container (`docker/grocy-init`) that fixes that line on its own, backup first. Before bumping the Grocy pin, compare the new version's `config-dist.php` against yours, and only bump in a release that carries that script. If Grocy moved and the inventory stopped loading, see [Grocy updated and inventory stopped loading](docs/platforms.md#grocy-updated-and-inventory-stopped-loading).
