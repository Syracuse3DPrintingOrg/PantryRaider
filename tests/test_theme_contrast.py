...
def test_base_template_has_contrast_hardening():
    ...
    # The shared page styles moved from an inline <style> in base.html to
    # static/css/base.css (cacheable across navigations). What the page
    # delivers is the two together, so that is what the rules are checked in;
    # and the page has to actually link the file for the rules to apply.
    assert 'href="static/css/base.css?v=' in base_html
    base = base_html + (_TEMPLATES.parent / "static" / "css" / "base.css").read_text(encoding="utf-8")
