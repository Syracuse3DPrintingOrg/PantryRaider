...
No serial port, no hardware, no network. The byte vectors here are the actual
frames on the wire, checked against the Waveshare module's documented scan
command and its fixed write-ack, so a refactor that drifts off the wire format
fails a test rather than a scanner on the counter. The passive path
(FoodAssistant-g4t5) is pinned too: a SEENGREAT engine ignores the GM trigger
and just emits plain ASCII codes on its own, so the reader must work with no
ack and no reply at all.
"""
...
# -- The passive path: a module that emits on its own (FoodAssistant-g4t5) -----

def test_parse_stream_plain_ascii_code_with_terminator():
    # What a passive engine in continuous mode emits: no ack, no frame, just
    # the code and its ending characters. Must decode with or without a flush.
    for flush in (False, True):
        codes, leftover = su.parse_stream(b"0123456789\r\n", flush=flush)
        assert codes == ["0123456789"]
        assert leftover == b""


def test_parse_stream_plain_ascii_code_no_terminator_flushed():
    # The module can be configured with no end mark at all; a silent read
    # window then delimits the code and the reader flushes the trailing run.
    codes, leftover = su.parse_stream(b"0123456789", flush=True)
    assert codes == ["0123456789"]
    assert leftover == b""


def test_parse_stream_acks_interleaved_with_passive_codes():
    # GM acks (from trigger commands a passive module ignored, or a real GM
    # module answering) interleaved with plain codes: the acks are stripped,
    # including their printable CRC bytes, and both codes survive.
    buf = su.ACK + b"0123456789\r\n" + su.ACK + b"5901234123457"
    codes, leftover = su.parse_stream(buf, flush=True)
    assert codes == ["0123456789", "5901234123457"]
    assert leftover == b""


class FakeSerial:
    """A scripted pyserial stand-in. Each entry in ``reads`` is what one
    read() call returns regardless of the requested size; an exhausted script
    reads as silence (b""), which is what a timeout returns. in_waiting stays
    0 so the drain loop exits after each scripted read."""
...
def _scanner_with(fake):
    scanner = su.SerialScanner("/dev/fake", 9600)
    scanner._ser = fake
    scanner.available = True
    return scanner


def test_scan_reads_a_passive_code_with_no_ack_and_no_reply_to_the_trigger():
    # The whole SEENGREAT case in one read: the trigger is written (harmless,
    # ignored), nothing acks it, and the code still comes back.
    fake = FakeSerial(reads=[b"0123456789\r\n"])
    scanner = _scanner_with(fake)
    assert scanner.scan() == ["0123456789"]
    assert fake.writes == [su.SCAN_COMMAND]


def test_scan_completes_a_code_that_straddles_the_read_window():
    # A passive engine emits on its own clock, so a code can arrive across the
    # window boundary. The first read ends mid-code; the reader must wait one
    # more window for the rest instead of flushing half a barcode.
    fake = FakeSerial(reads=[b"01234", b"56789\r\n"])
    scanner = _scanner_with(fake)
    assert scanner.scan() == ["0123456789"]


def test_scan_returns_nothing_on_a_silent_window():
    scanner = _scanner_with(FakeSerial())
    assert scanner.scan() == []


def test_configure_holds_passive_bytes_instead_of_eating_them():
    # configure() swallows the GM write ack, but a passive engine never acks:
    # bytes read there are real barcode data and must reach the next scan.
    fake = FakeSerial(reads=[b"0123456", b"789\r\n"])
    scanner = _scanner_with(fake)
    assert scanner.configure() is True
    assert scanner.scan() == ["0123456789"]


def test_configure_ack_from_a_real_gm_module_is_still_stripped():
    fake = FakeSerial(reads=[bytes(su.ACK), b"5901234123457\r\n"])
    scanner = _scanner_with(fake)
    assert scanner.configure() is True
    assert scanner.scan() == ["5901234123457"]


def test_discard_pending_drops_the_leftover_and_never_raises():
    # While enabled but dark the daemon throws away whatever a passive engine
    # emitted, so a stale code cannot flood the next session's first read.
    fake = FakeSerial()
    scanner = _scanner_with(fake)
    scanner._leftover = b"stale"
    scanner.discard_pending()
    assert scanner._leftover == b""
    assert fake.resets == 1
    # Safe on a never-opened handle and on a port whose flush blows up.
    su.SerialScanner("/dev/fake", 9600).discard_pending()
    fake.reset_input_buffer = lambda: (_ for _ in ()).throw(OSError("gone"))
    scanner.discard_pending()
