...
def _no_off_fetch(monkeypatch):
    """A successful consume-mode scan now asks Open Food Facts for calorie
    data to build the log-as-eaten offer (FoodAssistant-4mi3). Stub that fetch
    so these tests stay offline; the offer itself is covered in
    test_calorie_offer.py."""
    from app.routers import pending as pending_router

    async def _none(barcode):
        return None

    monkeypatch.setattr(pending_router, "fetch_off_product", _none)
...
def sat_client(monkeypatch, tmp_path):
    ...
    from app.main import app
    try:
        # The caller here is the device's own screen, on the loopback address.
        # With no password set, forwarding is closed to anything else on the
        # network (see test_no_password_gate.py).
        yield TestClient(app, client=("127.0.0.1", 50000)), recorder, tmp_path
    finally:
        os.chdir(cwd)
...
def test_a_mode_set_here_primes_the_led_cache(sat_client):
    """A mode picked on this device's own screen has to reach the lit key at
    once. The LED snapshot reads the fleet mode through a short cache, so a
    satellite that just forwarded a mode must prime it; without that the key
    stayed lit on the old mode until the cache expired, and no amount of LED
    polling could catch up."""
    client, recorder, tmp_path = sat_client
    from app.services import stemma
    stemma._mode_cache.set({"mode": "inventory", "label": "Stock"})
    client.post("/pending/scanner-mode", json={"mode": "consume"})
    assert stemma._mode_cache.get() == {"mode": "consume", "label": "Use"}
