...
def client(monkeypatch, tmp_path):
    ...
    try:
        # A home-network client address: before setup finishes, the first-run
        # routes answer only a browser genuinely on the local network (see
        # test_first_run_reveal_gate.py), and this file is about what happens
        # once a request gets past that.
        yield TestClient(app, client=("192.168.1.50", 50000))
    finally:
        os.chdir(cwd)
