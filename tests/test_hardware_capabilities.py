...
def test_capable_board_2gb_supported():
    # A Pi 4 with 2 GB runs the light default stack (Grocy + Pantry Raider,
    # Mealie off) alongside the kiosk, so Pi Hosted is offered (FoodAssistant).
    assert hardware.supports_local_stack("Raspberry Pi 4 Model B", ram_mb=2048) is True
    # A 2 GB board reports a little under 2048 once firmware reserves its slice.
    assert hardware.supports_local_stack("Raspberry Pi 4 Model B", ram_mb=1850) is True


def test_capable_board_1gb_blocked():
    # A 1 GB board is still too small for the local stack plus a kiosk.
    assert hardware.supports_local_stack("Raspberry Pi 4 Model B", ram_mb=900) is False
