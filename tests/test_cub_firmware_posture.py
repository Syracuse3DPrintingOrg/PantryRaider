...
# --------------------------------------------------------------------------- #
# every id(...) a lambda reaches for must actually exist in that build
# --------------------------------------------------------------------------- #

def _compose(profile: Path) -> str:
    """The text of a build file plus every package it includes.

    Crude on purpose: ESPHome merges packages at load time, and reproducing
    that faithfully means running ESPHome. Concatenating the text is enough to
    answer the one question here, which is whether an id is defined anywhere in
    the build at all.
    """
    import re as _re
    text = profile.read_text()
    for m in _re.finditer(r"!include\s+(\S+)", text):
        inc = (_ESPHOME / m.group(1)).resolve()
        if inc.exists():
            text += "\n" + inc.read_text()
    return text


def test_no_lambda_reaches_for_an_id_the_build_does_not_define():
    """Removing a component means removing what points at it, too.

    This exact break shipped: the update entity came out of cub-base.yaml for
    FoodAssistant-11smp, but the on_boot lambda that handed the hub a callback
    still said id(cub_firmware). Every profile failed ESPHome validation with
    "Couldn't find ID 'cub_firmware'", and nothing here noticed, because the
    other checks read structure rather than whether the pieces still line up.
    """
    import re as _re
    offenders = []
    for profile in sorted(_ESPHOME.glob("cub-*.yaml")):
        if profile.name.endswith(".example.yaml"):
            continue
        text = _compose(profile)
        # Both the mapping form ("  id: name") and the list-item form that
        # globals and platform entries use ("  - id: name").
        defined = set(_re.findall(r"^\s*-?\s*id:\s*([A-Za-z_][A-Za-z0-9_]*)\s*$", text, _re.M))
        # ESPHome also accepts the inline "id: !extend name" and generated ids;
        # only bare definitions matter for what a lambda can reach.
        for ref in set(_re.findall(r"\bid\(\s*([A-Za-z_][A-Za-z0-9_]*)\s*\)", text)):
            if ref not in defined:
                offenders.append(f"{profile.name}: id({ref}) is used but never defined")
