...
def test_subnav_pills_scroll_instead_of_truncating_on_a_phone():
    # The shared page styles moved out of base.html's inline <style> into
    # static/css/base.css (cacheable across navigations); same rules, one file.
    css = (_S / "css" / "base.css").read_text()
    phone = css.split("@media (max-width: 767.98px) {", 1)[1].split("\n}", 1)[0]
