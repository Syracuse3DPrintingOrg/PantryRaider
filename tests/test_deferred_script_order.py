...
def _reads(js: str, name: str) -> bool:
    ...
    Comments are stripped first. A comment cannot read anything, and the
    first false positive was a file whose header explained, in prose, that
    its blocks read PRKioskStatus only inside DOMContentLoaded handlers: the
    explanation itself tripped the rule it was describing.
    """
    js = _COMMENT.sub("", js)
