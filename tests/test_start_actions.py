...
# ---------- shopping_add (on-screen quick-add, FoodAssistant-1a8h) ----------

def test_shopping_add_key_adds_to_first_list(monkeypatch):
    # Mealie-backed list (installs still running their recipes there).
    monkeypatch.setattr(settings, "shopping_backend", "mealie", raising=False)
    monkeypatch.setattr(settings, "streamdeck_ha_slots", [], raising=False)
    monkeypatch.setattr(settings, "streamdeck_key_overrides",
                        [{"id": "s1", "type": "shopping_add", "item": "Milk"}],
                        raising=False)
    calls = {}
    ...
    class FakeMealie:
        ...
        async def add_shopping_item(self, list_id, note, quantity=1.0):
            calls["added"] = (list_id, note, quantity)
            return {"id": "i1"}

    import app.services.mealie as mealie_mod
    monkeypatch.setattr(mealie_mod, "MealieClient", lambda: FakeMealie())
    res = asyncio.run(sa.fire_key("s1"))
    assert res["ok"] and "Milk" in res["detail"]
    assert calls["added"] == ("L1", "Milk", 1.0)


def test_shopping_add_key_adds_to_grocy_list(monkeypatch):
    # Grocy-backed list (the default): the same key lands on the Grocy list.
    monkeypatch.setattr(settings, "shopping_backend", "grocy", raising=False)
    monkeypatch.setattr(settings, "streamdeck_ha_slots", [], raising=False)
    monkeypatch.setattr(settings, "streamdeck_key_overrides",
                        [{"id": "sg", "type": "shopping_add", "item": "Milk"}],
                        raising=False)
    calls = {}

    from app.services.grocy import GrocyClient

    async def ensure_list(self):
        return 1

    async def get_lists(self):
        return [{"id": 1, "name": "Shopping list"}]

    async def add_item(self, list_id, note, amount=1.0, product_id=None):
        calls["added"] = (list_id, note, amount)
        return {"created_object_id": 5}

    async def no_product(self, name):
        return None

    monkeypatch.setattr(GrocyClient, "ensure_shopping_list", ensure_list)
    monkeypatch.setattr(GrocyClient, "get_shopping_lists", get_lists)
    monkeypatch.setattr(GrocyClient, "add_shopping_item", add_item)
    monkeypatch.setattr(GrocyClient, "product_id_by_name", no_product)
    res = asyncio.run(sa.fire_key("sg"))
    assert res["ok"] and "Milk" in res["detail"]
    assert calls["added"] == (1, "Milk", 1.0)


def test_shopping_add_key_no_list_reports_cleanly(monkeypatch):
    monkeypatch.setattr(settings, "shopping_backend", "mealie", raising=False)
    monkeypatch.setattr(settings, "streamdeck_ha_slots", [], raising=False)
    monkeypatch.setattr(settings, "streamdeck_key_overrides",
                        [{"id": "s2", "type": "shopping_add", "item": "Eggs"}],
                        raising=False)
    ...
    import app.services.mealie as mealie_mod
    monkeypatch.setattr(mealie_mod, "MealieClient", lambda: FakeMealie())
    res = asyncio.run(sa.fire_key("s2"))
    assert res["ok"] is False and "no shopping list" in res["detail"].lower()


def test_shopping_add_key_without_item_reports(monkeypatch):
    monkeypatch.setattr(settings, "streamdeck_ha_slots", [], raising=False)
    monkeypatch.setattr(settings, "streamdeck_key_overrides",
                        [{"id": "s3", "type": "shopping_add"}],
                        raising=False)
    res = asyncio.run(sa.fire_key("s3"))
    assert res["ok"] is False and "no item" in res["detail"].lower()
