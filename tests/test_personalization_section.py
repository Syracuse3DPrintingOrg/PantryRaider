...
def test_recipe_prefs_inputs_render_in_prefs_pane(client, monkeypatch):
    """After the IA reorg (FoodAssistant-s6q) recipes live together in the
    Recipes pane under Kitchen: the Mealie + external-source cards and the
    suggestion-tuning + appliances inputs, each with its own save button. The
    Cameras pane (id pane-connections) no longer holds the Mealie fields."""
    html = _render(client, monkeypatch, satellite=False)
