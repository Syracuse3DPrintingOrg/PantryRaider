...
def test_default_cidr_returns_slash24_or_none(monkeypatch):
    ...
    _local_ips() discovers addresses two ways: an outbound UDP socket
    (socket.socket) and resolving the hostname (socket.getaddrinfo). Both must
    be mocked or the CI runner's real interface addresses leak in and win the
    ranking, so the assertion sees the runner's network instead of the fixture.
    """
    import socket
    ...
    # No extra host-resolved addresses: the outbound IP is the only candidate.
...
def test_default_cidr_returns_none_on_failure(monkeypatch):
    ...
    def bad_getaddrinfo(*a, **kw):
        raise OSError("no resolver")

    # Force both discovery paths to fail so no real address can leak in.
    monkeypatch.setattr(socket, "socket", bad_socket)
    monkeypatch.setattr(socket, "getaddrinfo", bad_getaddrinfo)
