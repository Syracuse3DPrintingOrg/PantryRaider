...
def test_timers_page_has_probes_section(client):
    ...
    # The page's script moved to static/js/timers-page.js (cacheable across
    # visits); the page delivers the markup plus that file, so check both,
    # and that the page really links the file.
    assert 'src="static/js/timers-page.js?v=' in html
    js = (Path(__file__).resolve().parents[1] / "service" / "app" / "static" / "js" / "timers-page.js").read_text()
    assert "probesSection" in html + js and "gadgets/state" in html + js


# -- Accessories bus honesty (FoodAssistant-dxr22) ----------------------------
#
# Before the host reader has EVER checked in, the truthful bus state is "the
# reader is not set up", not the old optimistic default that sent people
# hunting for cable faults on a fresh device. Once a reader has checked in,
# its own report wins unchanged.


def test_i2c_status_truth_table():
    honest = (False, gadgets.I2C_NOT_SET_UP_DETAIL)
    # Reader never checked in: honest default, whatever the stored dict says.
    assert gadgets.i2c_status(None, 0.0) == honest
    assert gadgets.i2c_status({}, 0.0) == honest
    assert gadgets.i2c_status({"available": True, "detail": ""}, 0.0) == honest
    assert gadgets.i2c_status({"available": False, "detail": "x"}, 0.0) == honest
    # Reader has checked in: its own report keeps today's semantics.
    assert gadgets.i2c_status({}, 100.0) == (True, "")
    assert gadgets.i2c_status(None, 100.0) == (True, "")
    assert gadgets.i2c_status({"available": True, "detail": ""}, 100.0) == (True, "")
    assert gadgets.i2c_status({"available": False, "detail": "no bus"},
                              100.0) == (False, "no bus")


def test_get_state_i2c_defaults_honest_until_reader_seen(tmp_path, monkeypatch):
    monkeypatch.setattr(settings, "data_dir", str(tmp_path), raising=False)
    monkeypatch.setattr(settings, "gadget_devices", [], raising=False)
    # Fresh install: no reader has ever checked in.
    state = gadgets.get_state()
    assert state["reader_age_seconds"] is None
    assert state["i2c_available"] is False
    assert state["i2c_detail"] == gadgets.I2C_NOT_SET_UP_DETAIL
    # The reader's first push flips to its own report (bus fine by default).
    gadgets.ingest({"devices": []})
    state = gadgets.get_state()
    assert state["reader_age_seconds"] is not None
    assert state["i2c_available"] is True
    assert state["i2c_detail"] == ""
    # And a checked-in reader's bad-bus report still comes through verbatim.
    gadgets.ingest({"devices": [], "i2c": {"available": False,
                                           "detail": "no /dev/i2c-1"}})
    state = gadgets.get_state()
    assert state["i2c_available"] is False
    assert state["i2c_detail"] == "no /dev/i2c-1"
