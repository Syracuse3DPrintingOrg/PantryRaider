...
# -- Alarms mirrored back down to the satellite ----------------------------------------

def _warnings():
    return [e for e in ha_events.poll(0)["events"] if e["type"] == "warning"]


def _alarm(key="hygro:AA:11", started=100, level="error"):
    return {"key": key, "kind": "hygrometer", "device_id": "AA:11",
            "name": "Fridge", "location": "Kitchen fridge",
            "message": "Kitchen fridge is at 54F, above its 45F limit.",
            "level": level, "started_epoch": started}


def test_select_new_alarms_is_per_onset_and_forgets_cleared_ones():
    a, b = _alarm(), _alarm(key="contact:BB:22", started=200)
    fresh, seen = satellite_svc.select_new_alarms([a, b], {})
    assert [x["key"] for x in fresh] == ["hygro:AA:11", "contact:BB:22"]
    assert seen == {"hygro:AA:11": 100, "contact:BB:22": 200}
    # Still alarming: nothing new to say.
    fresh, seen2 = satellite_svc.select_new_alarms([a, b], seen)
    assert fresh == [] and seen2 == seen
    # One clears: it drops out of the map rather than lingering forever.
    fresh, seen3 = satellite_svc.select_new_alarms([a], seen2)
    assert fresh == [] and seen3 == {"hygro:AA:11": 100}
    # It comes back later: a genuine new onset, so it speaks up again.
    again = _alarm(key="contact:BB:22", started=900)
    fresh, _ = satellite_svc.select_new_alarms([a, again], seen3)
    assert [x["key"] for x in fresh] == ["contact:BB:22"]


def test_select_new_alarms_ignores_junk():
    fresh, seen = satellite_svc.select_new_alarms(
        [None, "junk", {"no": "key"}, {"key": "  "}], {})
    assert fresh == [] and seen == {}
    assert satellite_svc.select_new_alarms(None, "not a dict") == ([], {})


def test_alarm_toast_carries_the_servers_wording_and_weight():
    toast = satellite_svc.alarm_toast(_alarm())
    assert toast == {"message": "Kitchen fridge is at 54F, above its 45F limit.",
                     "title": "Kitchen fridge",
                     "key": "gadget-alarm:hygro:AA:11",
                     "level": "error"}
    # A humidity or gone-quiet alarm stays a warning; an unknown level clamps.
    assert satellite_svc.alarm_toast(_alarm(level="warning"))["level"] == "warning"
    assert satellite_svc.alarm_toast(_alarm(level="shouty"))["level"] == "warning"
    # An alarm with no location falls back to the sensor's name.
    quiet = dict(_alarm(), location="")
    assert satellite_svc.alarm_toast(quiet)["title"] == "Fridge"


def test_mirror_alarms_toasts_once_per_onset(satellite):
    assert satellite_svc.mirror_alarms([_alarm()]) == 1
    warned = _warnings()
    assert len(warned) == 1
    assert warned[0]["key"] == "gadget-alarm:hygro:AA:11"
    assert warned[0]["level"] == "error"
    assert "above its 45F limit" in warned[0]["message"]
    # The alarm is still live on the next push: no second toast (the server is
    # edge-triggered, and so is the mirror).
    assert satellite_svc.mirror_alarms([_alarm()]) == 0
    assert len(_warnings()) == 1
    # It clears, then a fresh breach starts: that one is worth saying.
    assert satellite_svc.mirror_alarms([]) == 0
    assert satellite_svc.mirror_alarms([_alarm(started=500)]) == 1
    assert len(_warnings()) == 2


def test_mirror_alarms_dedupes_across_the_two_paths(satellite):
    """The relay reply and the config pull can both carry the same alarm."""
    assert satellite_svc.mirror_alarms([_alarm()]) == 1
    assert satellite_svc.mirror_alarms([_alarm()]) == 0     # the other path
    assert len(_warnings()) == 1


def test_mirror_alarms_is_a_no_op_off_a_satellite(server_mode):
    assert satellite_svc.mirror_alarms([_alarm()]) == 0
    assert _warnings() == []


def test_mirror_alarms_survives_an_unwritable_data_dir(satellite, monkeypatch):
    monkeypatch.setattr(settings, "data_dir", "/nope/not/here", raising=False)
    # The toast still lands; only the "already said this" memory is lost.
    assert satellite_svc.mirror_alarms([_alarm()]) == 1
    assert len(_warnings()) == 1


def test_server_reply_carries_alarms_only_for_a_relayed_push(server_mode, monkeypatch):
    monkeypatch.setattr(settings, "hygrometers_enabled", True, raising=False)
    monkeypatch.setattr(settings, "hygrometer_devices", [
        {"id": "AA:11", "name": "Fridge", "location": "Kitchen fridge",
         "thresholds": {"max_temp_c": 7.0}, "alarm_grace_seconds": 0}],
        raising=False)
    monkeypatch.setattr(settings, "contact_devices", [], raising=False)
    monkeypatch.setattr(settings, "gadget_devices", [], raising=False)
    push = {"source": "bandit-autopi", "devices": [
        {"id": "AA:11", "kind": "hygrometer", "temp_c": 12.0}]}
    result = asyncio.run(gadgets_router.post_readings(push))
    assert [a["key"] for a in result["alarms"]] == ["hygro:AA:11"]
    assert result["alarms"][0]["level"] == "error"
    assert result["alarms"][0]["location"] == "Kitchen fridge"
    # A push from the server's own reader gets no alarm list: this app already
    # raised the toast itself.
    local = asyncio.run(gadgets_router.post_readings(
        {"devices": [{"id": "AA:11", "kind": "hygrometer", "temp_c": 12.0}]}))
    assert "alarms" not in local


def test_relay_delivery_mirrors_the_alarms_in_the_reply(satellite, monkeypatch):
    class _Resp:
        status_code = 200

        def json(self):
            return {"ok": True, "alarms": [_alarm()]}

    monkeypatch.setattr(gadgets_relay.httpx, "post", lambda *a, **k: _Resp())
    assert gadgets_relay._post({"devices": []}) is True
    assert len(_warnings()) == 1
    assert _warnings()[0]["key"] == "gadget-alarm:hygro:AA:11"


def test_relay_delivery_survives_a_reply_with_no_alarms(satellite, monkeypatch):
    class _Resp:
        status_code = 200
