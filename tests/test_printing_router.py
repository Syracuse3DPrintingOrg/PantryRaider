...
def test_label_print_offline_error_gets_clear_prompt(client, monkeypatch):
    # A powered-off Bluetooth label printer (FoodAssistant-fu62) must surface a
    # clear, distinct "turn it on" prompt, not the raw backend error text.
    client._settings.printing_enabled = True
    client._settings.label_printer_queue = "Supvan_kitchen"
    monkeypatch.setattr(printing_service, "printing_available", lambda: True)

    def _fake_print(queue, data, *, options=None):
        return printing_service.PrintResult(
            ok=False, error_kind="offline",
            error="lp: Unable to contact printer: Host is down")
    ...
    assert body["printer_offline"] is True
    assert "Host is down" not in body["error"]
    assert "turn your label printer on" in body["error"].lower()


def test_label_print_other_error_is_not_flagged_offline(client, monkeypatch):
    client._settings.printing_enabled = True
    client._settings.label_printer_queue = "Zebra"
    monkeypatch.setattr(printing_service, "printing_available", lambda: True)

    def _fake_print(queue, data, *, options=None):
        return printing_service.PrintResult(ok=False, error="Print command failed.")
    ...
    body = r.json()
    assert body["ok"] is False
    assert body["printer_offline"] is False
    assert body["error"] == "Print command failed."
...
def test_print_label_renders_recorded_source_end_to_end(client, monkeypatch):
    ...
    def _fake_render(spec, shape="rectangle"):
...
def test_label_media_opts_matches_stock_size():
    """Label prints must pin the media to the stock size, or CUPS blanks the
    label onto the wrong default media (FoodAssistant-pbr9)."""
    from app.routers.printing import _label_media_opts
    # Metric stock reads as clean whole millimetres.
    assert _label_media_opts(40 / 25.4, 30 / 25.4) == {"media": "Custom.40x30mm"}
    # Inch stock converts to millimetres (2x1in).
    assert _label_media_opts(2.0, 1.0) == {"media": "Custom.50.8x25.4mm"}
    # No/invalid size passes no media rather than a broken option.
    assert _label_media_opts(0, 0) == {}
    assert _label_media_opts(None, 1.0) == {}


def test_local_label_queue_prefers_device_hosted_printer(monkeypatch):
    """A Supvan label printer registered locally is used directly, and an
    explicit local choice resolves; a device with no label printer returns ""
    so it falls back to forwarding/fleet (FoodAssistant-eml9 follow-up)."""
    from app.services import printing
    from unittest.mock import patch
    with patch.object(printing, "list_queues",
                      return_value=[{"name": "Brother_HL"},
                                    {"name": "Supvan_T50_Series_T0148"}]):
        assert printing.local_label_queue("") == "Supvan_T50_Series_T0148"
        assert printing.local_label_queue("Brother_HL") == "Brother_HL"
    with patch.object(printing, "list_queues",
                      return_value=[{"name": "Brother_HL"}]):
        assert printing.local_label_queue("") == ""
        assert printing.local_label_queue("Missing") == ""


def test_satellite_does_not_forward_label_when_it_hosts_the_printer(monkeypatch):
    """A satellite that physically hosts the label printer prints locally
    (upstream=None for label paths); document jobs still forward."""
    import sys
    ...
    from app.services import printing
    with patch.object(printing, "list_queues",
                      return_value=[{"name": "Supvan_T50_Series_T0148"}]):
        # Label path: local printer present -> print locally (no forward).
        assert app_main._satellite_recipe_upstream("/printing/label") is None
        assert app_main._satellite_recipe_upstream("/printing/decorative") is None
        # Document path always forwards (document printer is server-side).
        assert app_main._satellite_recipe_upstream("/printing/document") == "http://srv:9284"
    with patch.object(printing, "list_queues", return_value=[]):
        # No local label printer -> label forwards to the server.
        assert app_main._satellite_recipe_upstream("/printing/label") == "http://srv:9284"
...
def _png_pixels(content: bytes):
    """A preview response decoded to (image, pixel access) in mode L."""
    img = Image.open(io.BytesIO(content)).convert("L")
    return img, img.load()


def test_label_preview_round_shape_draws_cut_circle(client):
    r = client.post("/printing/label/preview", json={
        "name": "Strawberry Jam", "best_by": "2026-08-01",
        "width_in": 2.0, "height_in": 2.0, "shape": "round"})
    assert r.status_code == 200
    img, px = _png_pixels(r.content)
    w, h = img.size
    # The corners (outside the cut circle) stay background...
    for x, y in ((0, 0), (w - 1, 0), (0, h - 1), (w - 1, h - 1)):
        assert px[x, y] == 255
    # ...and the cut-line ring shows at the circle's topmost point.
    assert px[w // 2, 0] < 128


def test_label_preview_defaults_to_the_configured_shape(client, monkeypatch):
    monkeypatch.setattr(client._settings, "label_shape", "round")
    r = client.post("/printing/label/preview", json={
        "name": "Jam", "width_in": 2.0, "height_in": 2.0})
    assert r.status_code == 200
    img, px = _png_pixels(r.content)
    assert px[img.size[0] // 2, 0] < 128  # the configured round shape applied


def test_label_preview_rect_shape_keeps_top_edge_clear(client):
    # Rectangular stock keeps its margin clear along the top edge, so the
    # ring pixel that marks a round label is absent here.
    r = client.post("/printing/label/preview", json={
        "name": "Jam", "width_in": 2.0, "height_in": 2.0, "shape": "rectangle"})
    assert r.status_code == 200
    img, px = _png_pixels(r.content)
    assert px[img.size[0] // 2, 0] == 255


def test_decorative_preview_accepts_round_shape(client):
    r = client.post("/printing/decorative/preview", json={
        "text": "Basil", "width_in": 1.5, "height_in": 1.5, "shape": "round"})
    assert r.status_code == 200
    img, px = _png_pixels(r.content)
    w, h = img.size
    assert px[0, 0] == 255 and px[w - 1, h - 1] == 255
    assert px[w // 2, 0] < 128


def test_layout_preview_round_shape(client):
    layout = {"width_in": 2.0, "height_in": 2.0, "dpi": 203,
              "elements": [{"field": "name", "x": 0.0, "y": 0.0,
                            "w": 1.0, "h": 1.0}]}
    r = client.post("/printing/label/layout/preview",
                    json={"layout": layout, "shape": "round"})
    assert r.status_code == 200
    img, px = _png_pixels(r.content)
    w, h = img.size
    for x, y in ((0, 0), (w - 1, 0), (0, h - 1), (w - 1, h - 1)):
...
def test_saved_custom_layout_composes_round_when_requested(client, monkeypatch):
    # A saved designer layout on round stock is laid out in the inscribed
    # content box and composed onto the round canvas, same as the default.
    layout = {"width_in": 2.0, "height_in": 2.0, "dpi": 203,
              "elements": [{"field": "name", "x": 0.0, "y": 0.0,
                            "w": 1.0, "h": 1.0}]}
    monkeypatch.setattr(client._settings, "label_layout_json",
                        json.dumps(layout))
    r = client.post("/printing/label/preview", json={
        "name": "Jam", "width_in": 2.0, "height_in": 2.0, "shape": "round"})
    assert r.status_code == 200
    img, px = _png_pixels(r.content)
    w, h = img.size
    for x, y in ((0, 0), (w - 1, 0), (0, h - 1), (w - 1, h - 1)):
