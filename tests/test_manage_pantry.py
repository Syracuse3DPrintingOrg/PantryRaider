...
def test_default_nav_label_is_manage_pantry():
    # Only the DEFAULT label is renamed; the key and href are identifiers and
    # must not move (old links, saved nav orders, and nav_hidden entries keep
    # working because "add" and "ui/add" are unchanged).
    from app.navigation import NAV_TABS

    tab = next(t for t in NAV_TABS if t["key"] == "add")
    assert tab["label"] == "Manage Pantry"
    assert tab["key"] == "add"
...
def test_mode_tabs_wired_to_scanner_mode_endpoint(client):
    ...
    # poll it so an externally-changed mode (deck key) moves the tab.
    html = _page(client)
    assert "pending/scanner-mode" in html
    assert "refreshMode" in html
...
def test_kiosk_adaptation_hooks_present(client):
    ...
    assert "navigator.mediaDevices" in html
    assert "kioskMode" in html


def test_shopping_tab_gated_on_mealie(client, monkeypatch):
    # Without Mealie the shopping pane shows a settings pointer instead of
    # controls that cannot work.
    with patch.object(type(settings), "mealie_configured", lambda self: False):
        html = _page(client)
    assert 'id="shopping-unconfigured"' in html
    assert 'id="shoppingNameInput"' not in html

    with patch.object(type(settings), "mealie_configured", lambda self: True):
        html = _page(client)
