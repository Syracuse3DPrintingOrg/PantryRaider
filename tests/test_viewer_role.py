...
def test_viewer_cannot_mint_an_api_key_through_pairing(client, monkeypatch):
    """Approving a pairing request mints a full-access key, so it is an admin
    action even though it lives outside /setup and /admin. Before this, a
    viewer could raise a request and approve it to come back with more access
    than the viewer password gives (security audit, Sep 2026)."""
    _configured(monkeypatch)
    _login(client, "kitchen")
    assert client.get("/api/pairing/pending").status_code == 403
    assert client.post("/api/pairing/approve",
                       json={"request_id": "x"}).status_code == 403
    assert client.post("/api/pairing/deny",
                       json={"request_id": "x"}).status_code == 403


def test_admin_still_approves_pairing(client, monkeypatch):
    _configured(monkeypatch)
    _login(client, "hunter2")
    assert client.get("/api/pairing/pending").status_code == 200
