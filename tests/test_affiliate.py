...
def test_shop_page_renders_with_disclosure(client):
    ...
    body = r.text
    # The disclosure tracks the licence (PolyForm Noncommercial), so it must
    # not claim the project is open source.
    assert "affiliate links" in body
    assert "free for personal and non-commercial use" in body
