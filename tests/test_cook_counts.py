...
def test_top_counts_ranks_and_thresholds(db):
    cc.record_cook(db, "mealie", slug="a", title="Chicken Soup")
    for _ in range(3):
        cc.record_cook(db, "mealie", slug="b", title="Tacos")
    cc.record_cook(db, "mealie", slug="c", title="Only Once")
    top = cc.top_counts(db, min_count=2)
    assert [r["title"] for r in top] == ["Tacos"]
    assert top[0]["count"] == 3
    assert top[0]["recipe_key"] == "mealie:b"


def test_top_counts_empty_when_nothing_meets_threshold(db):
    cc.record_cook(db, "mealie", slug="a", title="Once")
    assert cc.top_counts(db, min_count=2) == []


def test_top_counts_respects_limit(db):
    for i in range(5):
        for _ in range(2):
            cc.record_cook(db, "mealie", slug=f"r{i}", title=f"Recipe {i}")
    assert len(cc.top_counts(db, min_count=1, limit=3)) == 3
