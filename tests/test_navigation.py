...
def test_active_section_returns_parent_for_a_child_page():
    # Convert nests under the Guide section; being on Convert makes Guide active.
    tabs = [_tab("guide"), _tab("convert"), _tab("weather"), _tab("inventory")]
    tree = navigation.build_nav_tree(
        tabs, parents={"convert": "guide", "weather": "guide"})
    assert navigation.active_section("convert", tree) == "guide"
    assert navigation.active_section("weather", tree) == "guide"


def test_active_section_returns_parent_for_the_parents_own_page():
    # Standing on the section's own page also keeps that section expanded.
    tabs = [_tab("guide"), _tab("convert")]
    tree = navigation.build_nav_tree(tabs, parents={"convert": "guide"})
    assert navigation.active_section("guide", tree) == "guide"


def test_active_section_empty_for_a_plain_top_level_page():
    # A top-level link with no submenu has no section to keep open.
    tabs = [_tab("guide"), _tab("convert"), _tab("inventory")]
    tree = navigation.build_nav_tree(tabs, parents={"convert": "guide"})
    assert navigation.active_section("inventory", tree) == ""
    # An unknown/never-matched key is empty, not an error.
    assert navigation.active_section("nope", tree) == ""


def test_persistent_submenus_default_on_for_web():
    from app.config import persistent_submenus_default as d
    # A plain server install (or unset mode) is "the web": default on at any scale.
    assert d("server", "normal") is True
    assert d("", "large") is True


def test_persistent_submenus_default_pi_only_at_small_scale():
    from app.config import persistent_submenus_default as d
    # A Pi appliance defaults on only at the small interface scale.
    assert d("pi_hosted", "small") is True
    assert d("pi_remote", "small") is True
    assert d("pi_hosted", "normal") is False
    assert d("pi_hosted", "large") is False
    assert d("pi_remote", "xlarge") is False


def test_active_section_marked_and_floatnav_contract_intact(monkeypatch, tmp_path):
    # On a Kitchen Guide sub-page (Convert), the Guide section carries the
    # nav-section-active marker so the option can keep it expanded; the config
    # element and script are present, and the floating nav still exposes every
    # data attribute base.html's CSS and floating-nav.js read.
    import os
    cwd = os.getcwd()
    os.chdir(SERVICE)
    try:
        ...
        monkeypatch.setattr(settings, "nav_order", "", raising=False)
        monkeypatch.setattr(settings, "nav_hidden", "", raising=False)
        monkeypatch.setattr(settings, "nav_parents", {}, raising=False)  # default grouping
        monkeypatch.setattr(settings, "custom_nav_tabs", [], raising=False)
        from fastapi.testclient import TestClient
        html = TestClient(__import__("app.main", fromlist=["app"]).app).get("/ui/convert").text
        # The Guide dropdown is the active section here.
        assert "nav-section-active" in html
        assert 'id="navSub_guide"' in html
        # Server-resolved default is exposed for the per-device script to layer on.
        assert 'id="nav-submenus-config"' in html
        assert 'data-default="1"' in html
        assert "nav-submenus.js" in html
        # Floating-nav contract: every attribute the script and CSS read is intact.
        assert 'id="floatNav"' in html
        assert 'data-position=' in html
        assert 'data-autohide-streamdeck=' in html
        assert 'data-nav-hidden=' in html
        assert 'data-has-streamdeck=' in html
    finally:
        os.chdir(cwd)


def test_persistent_submenu_device_toggle_in_setup_pane(monkeypatch, tmp_path):
    # The per-device override selector lives in the Screen pane so a user can turn
    # persistent submenus off (or on) for this browser.
    import os
    from unittest.mock import patch
    cwd = os.getcwd()
    os.chdir(SERVICE)
    try:
        ...
        from fastapi.testclient import TestClient
        with patch.object(type(settings), "is_configured", lambda self: True):
            html = TestClient(__import__("app.main", fromlist=["app"]).app).get("/setup").text
        assert 'id="nav_persistent_device"' in html
        assert '_navSubmenusSetDevice' in html
    finally:
        os.chdir(cwd)
