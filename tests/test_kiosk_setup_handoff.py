...
def _loopback_client(app):
    """A TestClient whose requests look like they come from 127.0.0.1 (the
    appliance's own kiosk over host networking).

    The client address is injected into the ASGI scope by a tiny passthrough
    rather than TestClient's ``client=`` kwarg: that kwarg exists in newer
    starlette but not in the version this project pins (fastapi 0.115.0), so
    using it passes locally on a drifted env yet fails in CI and on the shipped
    image. Wrapping the app keeps the test honest against the version we ship.
    """
