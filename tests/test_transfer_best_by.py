...
def _fake_move_client(monkeypatch, entries, locations):
    ...
    async def fake_get(self, path):
        if path == "/stock/products/1/entries":
            return entries
...
def test_move_rewrites_the_date_before_the_transfer(monkeypatch):
    ...
    # It goes through /stock/entry: Grocy does not expose stock through the
    # generic /objects API, and the old /objects/stock write silently failed
    # against a real Grocy for as long as this test pinned it (fo6c).
    assert ops.index(("PUT", "/stock/entry/51")) < ops.index(
        ("POST", "/stock/products/1/transfer"))
    assert not any(p.startswith("/objects/stock") for _m, p, _b in calls)
    # The fixture entry carries no price, so the write must not invent one:
    # a default of 0 corrupts Grocy's inventory value.
    body = next(b for m, p, b in calls if p == "/stock/entry/51")
    assert "price" not in body and body["best_before_date"] == new.isoformat()
...
def test_move_leaves_dates_alone_when_nothing_is_proposed(monkeypatch):
    ...
    assert not any(p.startswith(("/objects/stock/", "/stock/entry/")) for _m, p, _b in calls)
