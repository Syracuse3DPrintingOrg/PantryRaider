...
def test_install_docker_retries_transient_apt_failure():
    """firstboot install_docker must retry rather than abort on a transient apt
    mirror-sync failure (FoodAssistant-4qz5). Source-guard: DRY_RUN skips the
    real install, so assert the retry loop is present in the script."""
    text = FIRSTBOOT.read_text()
    start = text.index("install_docker()")
    body = text[start:start + 2000]
    # Retries a few times and clears the apt lists between attempts, instead of
    # a single `sh "$tmp" || die`.
    assert "for attempt in 1 2 3" in body
    assert "/var/lib/apt/lists/*" in body
    # Only dies after the attempts are exhausted, with a re-run hint.
