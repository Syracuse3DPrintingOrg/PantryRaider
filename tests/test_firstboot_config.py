...
def test_default_enables_mealie(tmp_path):
    # A pi_hosted appliance with enough RAM (4 GB+) is a full kitchen hub, so a
    # default (non-remote) install ships Mealie on and pulls its image during
    # provisioning. FORCE_MEM_MB pins the RAM tier so the test does not depend on
...
def test_low_ram_defaults_mealie_off(tmp_path):
    # On a 2 GB board Pantry Raider + Grocy fit but Mealie does not, so the
    # default install drops Mealie and logs why (the user can add it on 4 GB+).
    rc, out = run_firstboot(
        tmp_path, "HOSTNAME=foodassistant\n", extra_env={"FORCE_MEM_MB": "2000"})
    assert rc == 0, out
    assert "MEALIE=false" in out
    assert "Compose profiles: <none>" in out
    assert "Low RAM" in out


def test_low_ram_respects_explicit_mealie_opt_in(tmp_path):
    # A low-RAM tier is only a default: an explicit ENABLE_MEALIE=true still wins.
    rc, out = run_firstboot(
        tmp_path, "ENABLE_MEALIE=true\n", extra_env={"FORCE_MEM_MB": "2000"})
    assert rc == 0, out
    assert "--profile with-mealie" in out
