...
def test_deploy_stack_waits_for_inventory_ready():
    """First boot waits for Grocy to actually answer HTTP before finishing, so
    the slow first start happens during install (with a spinner) and not in the
    web wizard (Dan, 2026-07-10). Source-guard."""
    text = FIRSTBOOT.read_text()
    ds = text[text.index("deploy_stack()"):]
    ds = ds[:ds.index("\n}\n") + 3]
    assert "Waiting for the inventory service to be ready" in ds
    assert "localhost:9383" in ds
    # Best-effort: it must not fail the install when Grocy is slow.
    assert 'pi_remote' in ds  # skipped on a satellite
