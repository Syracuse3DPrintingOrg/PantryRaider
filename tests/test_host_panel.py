...
def test_setup_ships_the_running_badge_hidden_for_post_paint_reveal(client):
    # The render no longer asks the bridge whether Mealie is up (the probe is
    # a docker compose ps, too slow for a page render, FoodAssistant-0hwez).
    # The running badge ships in a hidden data-mealie-running block that
    # panes.js reveals after paint when GET /setup/mealie/status says
    # "running"; whatever the true state, no start button is ever offered.
    import re
    html = _render_setup(client, {})
    assert "Start Mealie on this device" not in html
    tag = re.search(r"<div[^>]*data-mealie-running[^>]*>", html)
    assert tag is not None, "the hidden running block must render"
    assert "d-none" in tag.group(0)
    assert "Mealie is running on this device" in html


def test_setup_never_offers_to_start_mealie_and_render_skips_the_bridge(client):
    # Mealie is never installed/started from the UI (FoodAssistant-9mu5), and
    # since FoodAssistant-0hwez the render must not touch the bridge at all:
    # a bridge call here would blow the page up, which is the point.
    def _boom(*a, **kw):
        raise AssertionError("the settings render must not call the bridge")

    with patch("app.routers.setup.is_raspberry_pi", return_value=True), \
         patch("app.templating.is_raspberry_pi", return_value=True), \
         patch("app.routers.setup.bridge_client", side_effect=_boom), \
         patch.object(type(settings), "is_configured", lambda self: True):
        r = client.get("/setup")
    assert r.status_code == 200
