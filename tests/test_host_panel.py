...
def test_mealie_action_running_hides_start():
    assert mealie_action_state(installed=True, running=True) == "running"


def test_mealie_action_installed_but_stopped_offers_start():
    assert mealie_action_state(installed=True, running=False) == "start"


def test_mealie_action_not_installed_offers_start():
    assert mealie_action_state(installed=False, running=False) == "start"
...
def test_setup_shows_start_button_when_mealie_not_installed(client):
    html = _render_setup(client, {
        "/mealie/status": _Resp(200, {"ok": True, "state": "not-installed", "running": False}),
    })
    assert "Start Mealie on this device" in html
    assert "Mealie is running on this device" not in html


def test_setup_shows_start_button_when_bridge_unreachable(client):
    # A bridge hiccup must not blank the card: fall back to offering the start
    # button (the prior behaviour), never crash the settings page.
    class _Boom:
        async def __aenter__(self):
    ...
    with patch("app.routers.setup.is_raspberry_pi", return_value=True), \
        ...
        r = client.get("/setup")
    assert r.status_code == 200
    assert "Start Mealie on this device" in r.text
