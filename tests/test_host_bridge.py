...
# --- wake on presence (FoodAssistant-6z8c) ----------------------------------

def test_persist_wake_on_presence_roundtrip(tmp_path):
    p = tmp_path / "wake-on-presence"
    assert bridge._write_persisted_wake_on_presence("off", path=str(p)) is True
    assert bridge._read_persisted_wake_on_presence(path=str(p)) == "off"


def test_read_persisted_wake_on_presence_defaults_to_auto(tmp_path):
    assert bridge._read_persisted_wake_on_presence(path=str(tmp_path / "missing")) == "auto"
    # A garbage value on disk also falls back to auto rather than raising.
    p = tmp_path / "wake-on-presence"
    p.write_text("sometimes\n")
    assert bridge._read_persisted_wake_on_presence(path=str(p)) == "auto"


def test_presence_wake_enabled_truth_table():
    # off always disables, even with the line readable.
    assert bridge._presence_wake_enabled("off", True) is False
    assert bridge._presence_wake_enabled("off", False) is False
    # auto and on both follow the hardware: no readable line, no phantom wakes.
    assert bridge._presence_wake_enabled("auto", True) is True
    assert bridge._presence_wake_enabled("auto", False) is False
    assert bridge._presence_wake_enabled("on", True) is True
    assert bridge._presence_wake_enabled("on", False) is False


def test_held_presence_keeps_counting_as_activity_not_just_the_edge():
    """A person standing at the panel must keep the display awake.

    The loop used to wake only on the low-to-high edge, so a mmWave sensor
    reading someone who stands still produced one wake and then nothing, and
    the idle timer blanked the screen with them right in front of it. Every
    high sample has to count, or "wake on presence" only ever works for the
    first few seconds of a visit.
    """
    # Sample after sample of held presence: all of them count.
    for _ in range(5):
        assert bridge._presence_holds_display_awake(True, "auto", True) is True
    # An empty room does not hold the display awake.
    assert bridge._presence_holds_display_awake(False, "auto", True) is False
    # And the feature switched off stays off however high the line reads.
    assert bridge._presence_holds_display_awake(True, "off", True) is False


def test_presence_available_truth_table():
    # off is never available: the poll loop is not reading in that mode.
    assert bridge._presence_available("off", True, True) is False
    assert bridge._presence_available("off", False, False) is False
    # An unreadable line is never available, regardless of mode or the latch.
    assert bridge._presence_available("on", False, True) is False
    assert bridge._presence_available("auto", False, True) is False
    # "on" trusts the setting the moment the line is readable, latch or not.
    assert bridge._presence_available("on", True, False) is True
    assert bridge._presence_available("on", True, True) is True
    # "auto" needs the latch: a never-triggered sensor reads unavailable...
    assert bridge._presence_available("auto", True, False) is False
    # ...until the line has shown at least one real HIGH since boot.
    assert bridge._presence_available("auto", True, True) is True


def test_read_presence_line_falls_back_through_readers(monkeypatch):
    # gpiod unavailable, lgpio unavailable, gpioget answers: the chain still
    # resolves rather than giving up after the first failure.
    monkeypatch.setattr(bridge, "_read_presence_gpiod", lambda *a, **k: None)
    monkeypatch.setattr(bridge, "_read_presence_lgpio", lambda *a, **k: None)
    monkeypatch.setattr(bridge, "_read_presence_gpioget", lambda *a, **k: True)
    assert bridge._read_presence_line() is True


def test_read_presence_line_none_when_every_reader_fails(monkeypatch):
    monkeypatch.setattr(bridge, "_read_presence_gpiod", lambda *a, **k: None)
    monkeypatch.setattr(bridge, "_read_presence_lgpio", lambda *a, **k: None)
    monkeypatch.setattr(bridge, "_read_presence_gpioget", lambda *a, **k: None)
    assert bridge._read_presence_line() is None


def test_read_presence_line_swallows_reader_exceptions(monkeypatch):
    # A reader raising (e.g. a permission error) never bubbles out; the chain
    # just moves on to the next candidate (FoodAssistant-6z8c fail-safety).
    def _boom(*a, **k):
        raise OSError("no permission")
    monkeypatch.setattr(bridge, "_read_presence_gpiod", _boom)
    monkeypatch.setattr(bridge, "_read_presence_lgpio", _boom)
    monkeypatch.setattr(bridge, "_read_presence_gpioget", lambda *a, **k: False)
    assert bridge._read_presence_line() is False


def test_read_presence_gpiod_none_without_the_module():
    # gpiod is not installed in the test environment, so this exercises the
    # real ImportError path rather than a monkeypatched stand-in.
    assert bridge._read_presence_gpiod() is None


def test_read_presence_lgpio_none_without_the_module():
    assert bridge._read_presence_lgpio() is None


def test_read_presence_gpioget_none_without_the_binary():
    assert bridge._read_presence_gpioget(which=lambda name: None) is None


# The two libgpiod CLI majors disagree on both the argv and the output shape:
# v1 takes the chip positionally and prints "1", v2 takes it only via -c and
# prints "17=active". Trixie ships v2, Bookworm v1, so the fallback has to
# speak both or it silently reports "no presence sensor" on a current image.


class _FakeRun:
    """Stands in for subprocess.run: answers one argv shape, rejects others."""

    def __init__(self, accept, stdout):
        self.accept = accept
        self.stdout = stdout
        self.calls = []
...
def _gpioget(run, chip="gpiochip0", line=17):
    return bridge._read_presence_gpioget(
        line=line, which=lambda name: "/usr/bin/gpioget", chip=chip, run=run)


def test_gpioget_v1_positional_chip_still_works():
    run = _FakeRun(["gpiochip0", "17"], "1\n")
    assert _gpioget(run) is True
    assert len(run.calls) == 1


def test_gpioget_v2_needs_the_chip_flag_and_named_output():
    # v2 rejects the positional form, then answers "17=active" under -c.
    run = _FakeRun(["--numeric", "-c", "gpiochip0", "17"], "17=active\n")
    assert _gpioget(run) is True
    assert run.calls[0][1:] == ["gpiochip0", "17"]


def test_gpioget_v2_without_the_numeric_option_is_still_read():
    # --numeric arrived in libgpiod 2.1; 2.0 only understands bare -c.
    run = _FakeRun(["-c", "gpiochip0", "17"], "17=inactive\n")
    assert _gpioget(run) is False
    assert len(run.calls) == 3


def test_gpioget_returns_none_when_no_argv_shape_is_accepted():
    run = _FakeRun(["never"], "1\n")
    assert _gpioget(run) is None


def test_gpioget_value_parsing_covers_both_output_shapes():
    assert bridge._parse_gpioget_value("1") is True
    assert bridge._parse_gpioget_value("0") is False
    assert bridge._parse_gpioget_value("17=active") is True
    assert bridge._parse_gpioget_value("17=inactive") is False
    assert bridge._parse_gpioget_value("17=1") is True
    assert bridge._parse_gpioget_value("") is None
    assert bridge._parse_gpioget_value(None) is None
    assert bridge._parse_gpioget_value("gpioget: cannot find line") is None


def test_presence_status_reflects_activity_state(monkeypatch):
    with bridge._activity_lock:
        bridge._activity_state["wake_on_presence"] = "auto"
        bridge._activity_state["presence_readable"] = True
        bridge._activity_state["presence_ever_high"] = True
        bridge._activity_state["presence_detected"] = True
    try:
        assert bridge._presence_status() == {"available": True, "detected": True}
    finally:
        with bridge._activity_lock:
            bridge._activity_state["wake_on_presence"] = "auto"
            bridge._activity_state["presence_readable"] = False
            bridge._activity_state["presence_ever_high"] = False
            bridge._activity_state["presence_detected"] = False


def test_activity_get_reports_presence_readability():
    # GET /activity carries presence_readable and presence_ever_high alongside
    # presence_detected (FoodAssistant-99vy): the kiosk presence indicator
    # mounts only while the sensor is actually readable, so the body must let
    # it tell "no one here" apart from "no sensor here".
    with bridge._activity_lock:
        prev = {k: bridge._activity_state[k] for k in
                ("presence_readable", "presence_ever_high", "presence_detected")}
        bridge._activity_state["presence_readable"] = True
        bridge._activity_state["presence_ever_high"] = True
        bridge._activity_state["presence_detected"] = True
    sent = {}
    ...
    try:
        FakeHandler()._activity_get()
    finally:
        with bridge._activity_lock:
            bridge._activity_state.update(prev)
    assert sent["code"] == 200
    assert sent["body"]["presence_detected"] is True
    assert sent["body"]["presence_readable"] is True
    assert sent["body"]["presence_ever_high"] is True


def test_activity_get_readability_false_without_a_sensor():
    # The import-time defaults are the no-sensor case; the indicator must see
    # an explicit False, not a missing key, so an old cached kiosk script can
    # never mistake absence for presence.
    with bridge._activity_lock:
        prev = {k: bridge._activity_state[k] for k in
                ("presence_readable", "presence_ever_high", "presence_detected")}
        bridge._activity_state["presence_readable"] = False
    ...
    try:
        ...
        with bridge._activity_lock:
            bridge._activity_state.update(prev)
    assert sent["body"]["presence_readable"] is False
    assert sent["body"]["presence_ever_high"] is False
...
def test_ap_status_reports_inactive_when_flag_set_but_connected(tmp_path, monkeypatch):
    # Wire the handler's decision path end to end without HTTP: flag present
    # plus live connectivity must report inactive, WITHOUT acting. The
    # continuous watchdog loop is the single owner of standing the AP down;
    # the status endpoint only reports, so the two can never fight.
    ...
    assert stood_down == [], "status polls must not stand the AP down themselves"
    assert flag.exists(), "the flag is the watchdog loop's to clear"
...
def test_ap_status_active_when_flag_set_and_no_connectivity(tmp_path, monkeypatch):
    ...
    class FakeHandler:
        headers = {}
        _ap_status = bridge._Handler._ap_status
        _has_valid_token = bridge._Handler._has_valid_token
...
# --- Grocy stillborn-database repair guards ---------------------------------
# The repair may only ever act on a 0-byte grocy.db that is old enough to be
# provably dead, and never twice within the cooldown. A database with any
# content is untouchable by construction.


def test_grocy_repair_acts_only_on_an_old_empty_stub():
    act, reason = bridge._grocy_repair_decision(True, 0, 900.0, None)
    assert act is True
    assert reason


def test_grocy_repair_refuses_a_missing_database():
    act, reason = bridge._grocy_repair_decision(False, -1, -1.0, None)
    assert act is False
    assert "nothing to repair" in reason


def test_grocy_repair_never_touches_a_database_with_content():
    act, reason = bridge._grocy_repair_decision(True, 4096, 900.0, None)
    assert act is False
    assert "left untouched" in reason
    # Even a single byte counts as content.
    assert bridge._grocy_repair_decision(True, 1, 900.0, None)[0] is False


def test_grocy_repair_waits_out_a_freshly_created_file():
    # A 0-byte file can be legitimate for a short window (Grocy opened it and
    # is about to write the schema); only an old one is provably stillborn.
    act, reason = bridge._grocy_repair_decision(True, 0, 30.0, None)
    assert act is False
    assert "too new" in reason
    # At the threshold the file qualifies.
    assert bridge._grocy_repair_decision(True, 0, 120.0, None)[0] is True


def test_grocy_repair_honors_the_cooldown():
    assert bridge._grocy_repair_decision(True, 0, 900.0, 60.0)[0] is False
    assert bridge._grocy_repair_decision(True, 0, 900.0, 599.0)[0] is False
    assert bridge._grocy_repair_decision(True, 0, 900.0, 3600.0)[0] is True


def test_grocy_repair_every_refusal_has_a_reason():
    cases = [(False, -1, -1.0, None), (True, 10, 900.0, None),
             (True, 0, 5.0, None), (True, 0, 900.0, 1.0)]
    for case in cases:
        act, reason = bridge._grocy_repair_decision(*case)
        assert act is False
        assert reason.strip()


def test_grocy_stub_aside_path_is_alongside_and_timestamped():
    aside = bridge._grocy_stub_aside_path("/opt/foodassistant/grocy/data/grocy.db",
                                          1752680000.7)
    assert aside == ("/opt/foodassistant/grocy/data/"
                     "grocy.db.stillborn-1752680000")


def test_read_epoch_marker_missing_and_garbage(tmp_path):
    assert bridge._read_epoch_marker(str(tmp_path / "nope")) is None
    bad = tmp_path / "bad"
    bad.write_text("not a number\n")
    assert bridge._read_epoch_marker(str(bad)) is None


def test_read_epoch_marker_reads_epoch(tmp_path):
    p = tmp_path / "grocy-repair-last"
    p.write_text("1752680000\n")
    assert bridge._read_epoch_marker(str(p)) == 1752680000.0
...
def test_token_gate_no_expected_token_fails_closed():
    # The bridge could not write its token file: root actions are denied
    # rather than left open to any local process (FoodAssistant-ubpu).
    assert bridge._token_gate("POST", "/reboot", "", "", False) == "deny"
    assert bridge._token_gate("POST", "/reboot", "stale", "", False) == "deny"
    assert bridge._token_gate("POST", "/reboot", "", "", True) == "deny"
    # Read-only exempt GETs keep working so status pages stay alive.
    assert bridge._token_gate("GET", "/ap/status", "", "", False) == "ok"
...
def test_load_or_create_token_creates_dir_and_file(tmp_path):
    ...
    # Not world-readable (FoodAssistant-ow4f, FoodAssistant-56qf): 0o640 when
    # the primary group resolves (root owner + app/deck user read via group),
    # else 0o600 root-only. Never anything a third local account can read.
    mode = path.stat().st_mode & 0o777
    assert mode in (0o640, 0o600)
    assert not mode & 0o007  # never world-readable
...
def test_authorized_grace_allows_and_warns(monkeypatch, capsys):
    ...
    monkeypatch.setattr(bridge, "_token_seen", False)
    # The bounded window is open (FoodAssistant-ubpu).
    monkeypatch.setattr(bridge, "_grace_started", _time.time())
...
# --- Bounded grace window + fail-closed token store (FoodAssistant-ubpu) ---

def test_grace_active_only_inside_the_window(monkeypatch):
    monkeypatch.setattr(bridge, "_token_seen", False)
    monkeypatch.setattr(bridge, "_grace_started", 1000.0)
    assert bridge._grace_active(True, now=1000.0 + 60) is True
    assert bridge._grace_active(True, now=1000.0 + bridge.GRACE_WINDOW_SECONDS + 1) is False
    assert bridge._grace_active(False, now=1000.0 + 60) is False


def test_grace_active_unknown_window_start_fails_closed(monkeypatch):
    monkeypatch.setattr(bridge, "_token_seen", False)
    monkeypatch.setattr(bridge, "_grace_started", 0.0)
    assert bridge._grace_active(True) is False


def test_grace_active_off_once_a_token_was_seen(monkeypatch):
    import time as _time
    monkeypatch.setattr(bridge, "_token_seen", True)
    monkeypatch.setattr(bridge, "_grace_started", _time.time())
    assert bridge._grace_active(True) is False


def test_grace_window_marker_persists_across_restarts(tmp_path):
    marker = tmp_path / "grace-started"
    first = bridge._load_or_start_grace_window(str(marker), now=5000.0)
    assert first == 5000.0
    # A later "restart" resumes the same window instead of restarting it.
    assert bridge._load_or_start_grace_window(str(marker), now=9999.0) == 5000.0


def test_grace_window_marker_unwritable_fails_closed(tmp_path):
    missing_dir = tmp_path / "not-there" / "grace-started"
    assert bridge._load_or_start_grace_window(str(missing_dir), now=5000.0) == 0.0


def test_grace_window_marker_garbage_fails_closed(tmp_path):
    marker = tmp_path / "grace-started"
    marker.write_text("not-a-number\n")
    assert bridge._load_or_start_grace_window(str(marker), now=5000.0) == 0.0


def test_retry_token_init_is_rate_limited(monkeypatch):
    calls = []
    monkeypatch.setattr(bridge, "_load_or_create_token", lambda: calls.append(1) or "")
    monkeypatch.setattr(bridge, "_token_retry_at", 0.0)
    assert bridge._retry_token_init(now=100.0) == ""
    assert bridge._retry_token_init(now=101.0) == ""  # inside the 30s limit
    assert len(calls) == 1
    bridge._retry_token_init(now=200.0)
    assert len(calls) == 2


def test_authorized_tokenless_denied_after_window(monkeypatch):
    monkeypatch.setattr(bridge, "_EXPECTED_TOKEN", "secret")
    monkeypatch.setattr(bridge, "GRACE_MODE", True)
    monkeypatch.setattr(bridge, "_token_seen", False)
    monkeypatch.setattr(bridge, "_grace_started", 1.0)  # long expired
...
# --- Fallback-AP credentials for the attached display (FoodAssistant-fgh3) --

def test_parse_hostapd_conf_reads_ssid_and_passphrase():
    text = ("interface=wlan0\nssid=FoodAssistant\nwpa=2\n"
            "wpa_passphrase=abcd-efgh-jkmn\nrsn_pairwise=CCMP\n")
    assert bridge._parse_hostapd_conf(text) == ("FoodAssistant", "abcd-efgh-jkmn")


def test_parse_hostapd_conf_missing_keys_are_empty():
    assert bridge._parse_hostapd_conf("interface=wlan0\n") == ("", "")
    assert bridge._parse_hostapd_conf("") == ("", "")
    assert bridge._parse_hostapd_conf(None) == ("", "")


def test_ap_credentials_unreadable_conf_is_empty(tmp_path):
    assert bridge._ap_credentials(str(tmp_path / "missing.conf")) == ("", "")


def _ap_status_dummy(headers):
    class Dummy:
...
def test_ap_status_hides_passphrase_without_token(monkeypatch, tmp_path):
    flag = tmp_path / "ap-active"
    flag.write_text("")
    monkeypatch.setattr(bridge, "_AP_FLAG", str(flag))
    monkeypatch.setattr(bridge, "_EXPECTED_TOKEN", "secret")
    monkeypatch.setattr(bridge, "_has_lan_connectivity", lambda: False)
    monkeypatch.setattr(bridge, "_ap_credentials",
                        lambda *a, **k: ("FoodAssistant", "pw-abcdef"))
    d = _ap_status_dummy({})
    d._ap_status()
    assert d.sent[1]["active"] is True
    assert "passphrase" not in d.sent[1] and "ssid" not in d.sent[1]


def test_ap_status_reveals_passphrase_with_valid_token(monkeypatch, tmp_path):
    flag = tmp_path / "ap-active"
    flag.write_text("")
    monkeypatch.setattr(bridge, "_AP_FLAG", str(flag))
    monkeypatch.setattr(bridge, "_EXPECTED_TOKEN", "secret")
    ...
    monkeypatch.setattr(bridge, "_ap_credentials",
                        lambda *a, **k: ("FoodAssistant", "pw-abcdef"))
    d = _ap_status_dummy({"X-Bridge-Token": "secret"})
    d._ap_status()
    assert d.sent[1]["passphrase"] == "pw-abcdef"
    assert d.sent[1]["ssid"] == "FoodAssistant"


def test_ap_status_wrong_token_hides_passphrase(monkeypatch, tmp_path):
    flag = tmp_path / "ap-active"
    flag.write_text("")
    monkeypatch.setattr(bridge, "_AP_FLAG", str(flag))
    monkeypatch.setattr(bridge, "_EXPECTED_TOKEN", "secret")
    ...
    monkeypatch.setattr(bridge, "_ap_credentials",
                        lambda *a, **k: ("FoodAssistant", "pw-abcdef"))
    d = _ap_status_dummy({"X-Bridge-Token": "nope"})
    d._ap_status()
    assert "passphrase" not in d.sent[1]


# --- Recovery hotspot config: settable SSID and password --------------------
# GET/POST /ap/config let the app change the hotspot's broadcast identity.
# Validation truth tables first (pure helpers), then the write path and the
# handlers.


def test_validate_ap_ssid_truth_table():
    cases = [
        ("FoodAssistant", True),          # the default is always valid
        ("Kitchen Pi", True),             # spaces inside are fine
        ("a", True),                      # 1 byte, the minimum
        ("x" * 32, True),                 # 32 bytes, the maximum
        ("x" * 33, False),                # one byte over
        ("café" + "x" * 27, True),   # 31 bytes: multibyte counts as bytes
        ("café" + "x" * 29, False),  # 33 bytes via a 2-byte character
        ("", False),                      # empty
        ("   ", False),                   # whitespace only
        (" edge", False),                 # leading space
        ("edge ", False),                 # trailing space
        ("two\nlines", False),            # newline: hostapd.conf injection
        ("tab\tname", False),             # control character
        (None, False),                    # not a string
    ]
    for ssid, want in cases:
        ok, err = bridge._validate_ap_ssid(ssid)
        assert ok is want, (ssid, err)
        assert (err == "") is want, (ssid, err)


def test_validate_ap_passphrase_truth_table():
    cases = [
        ("goodpass", True),               # 8 chars, the minimum
        ("x" * 63, True),                 # 63 chars, the maximum
        ("with spaces ok", True),         # spaces are legal WPA2
        ("punct!@#$%^&*()", True),
        ("x" * 7, False),                 # too short
        ("x" * 64, False),                # too long
        ("", False),
        ("café-password", False),    # non-ASCII is outside WPA2's range
        ("line\nbreak-pw", False),        # control character
        (None, False),                    # not a string
    ]
    for pw, want in cases:
        ok, err = bridge._validate_ap_passphrase(pw)
        assert ok is want, (pw, err)


def test_app_and_bridge_ap_validation_agree():
    """The app's proxy validators (client-facing wording) and the bridge's
    must accept and reject the same values, or a value could pass one side
    and be refused by the other."""
    import sys
    _service = str(REPO / "service")
    if _service not in sys.path:
        sys.path.insert(0, _service)
    from app.routers.setup import validate_ap_passphrase, validate_ap_ssid

    for ssid in ("FoodAssistant", "Kitchen Pi", "x" * 32, "x" * 33, "",
                 " edge", "two\nlines", "café" + "x" * 29):
        app_ok = validate_ap_ssid(ssid) == ""
        bridge_ok = bridge._validate_ap_ssid(ssid)[0]
        assert app_ok == bridge_ok, ssid
    for pw in ("goodpass", "x" * 63, "x" * 7, "x" * 64, "",
               "café-password", "with spaces ok"):
        app_ok = validate_ap_passphrase(pw) == ""
        bridge_ok = bridge._validate_ap_passphrase(pw)[0]
        assert app_ok == bridge_ok, pw


def test_gen_ap_passphrase_shape_and_detection():
    for _ in range(5):
        pw = bridge._gen_ap_passphrase()
        assert len(pw) == 14 and pw[4] == "-" and pw[9] == "-"
        assert all(c in bridge._AP_GEN_ALPHABET + "-" for c in pw)
        assert bridge._ap_passphrase_generated(pw) is True
    assert bridge._gen_ap_passphrase() != bridge._gen_ap_passphrase()
    # Owner-chosen passwords read as custom, including near misses.
    for pw in ("owner-picked-99", "hunter2hunter2", "", "abcd-efgh-ijkl"):
        assert bridge._ap_passphrase_generated(pw) is False, pw


def test_render_hostapd_conf_roundtrips_and_keeps_wpa2_aes():
    text = bridge._render_hostapd_conf("Kitchen Pi", "pw-123456")
    assert bridge._parse_hostapd_conf(text) == ("Kitchen Pi", "pw-123456")
    assert "rsn_pairwise=CCMP" in text
    assert "TKIP" not in text
    assert "interface=wlan0" in text


def test_ap_current_config_prefers_hostapd_then_files(tmp_path):
    conf = tmp_path / "hostapd.conf"
    ssid_f = tmp_path / "ap-ssid"
    pass_f = tmp_path / "ap-passphrase"
    # Nothing anywhere: the default name and no passphrase.
    assert bridge._ap_current_config(str(conf), str(ssid_f), str(pass_f)) \
        == ("FoodAssistant", "")
    # Stored files fill in when hostapd.conf is missing (un-provisioned AP).
    ssid_f.write_text("Kitchen Pi\n")
    pass_f.write_text("stored-pass\n")
    assert bridge._ap_current_config(str(conf), str(ssid_f), str(pass_f)) \
        == ("Kitchen Pi", "stored-pass")
    # hostapd.conf wins: it is what actually broadcasts.
    conf.write_text(bridge._render_hostapd_conf("OnAir", "live-pass1"))
    assert bridge._ap_current_config(str(conf), str(ssid_f), str(pass_f)) \
        == ("OnAir", "live-pass1")


def test_ap_config_write_lands_everywhere_with_tight_modes(tmp_path):
    conf = tmp_path / "hostapd.conf"
    ssid_f = tmp_path / "ap-ssid"
    pass_f = tmp_path / "ap-passphrase"
    boot = tmp_path / "boot"
    boot.mkdir()
    bridge._ap_config_write("Kitchen Pi", "pw-123456", str(conf), str(ssid_f),
                            str(pass_f), [str(boot)])
    assert bridge._parse_hostapd_conf(conf.read_text()) == ("Kitchen Pi", "pw-123456")
    assert (conf.stat().st_mode & 0o777) == 0o600
    assert pass_f.read_text() == "pw-123456\n"
    assert (pass_f.stat().st_mode & 0o777) == 0o600
    assert ssid_f.read_text() == "Kitchen Pi\n"
    mirror = (boot / "pantry-raider-hotspot.txt").read_text()
    assert "SSID: Kitchen Pi" in mirror and "Password: pw-123456" in mirror


def test_ap_config_write_missing_boot_dir_is_not_fatal(tmp_path):
    conf = tmp_path / "hostapd.conf"
    bridge._ap_config_write("Name", "pw-123456", str(conf),
                            str(tmp_path / "ssid"), str(tmp_path / "pass"),
                            [str(tmp_path / "nonexistent-boot")])
    assert conf.exists()


def _ap_config_dummy(headers, body=None):
    class Dummy:
        ...
        _has_valid_token = bridge._Handler._has_valid_token
        _ap_config_get = bridge._Handler._ap_config_get
        _ap_config_set = bridge._Handler._ap_config_set
    ...
    return Dummy(headers, body)


def _ap_paths(monkeypatch, tmp_path, active=False):
    """Point every AP path constant into tmp and seed a current identity."""
    conf = tmp_path / "hostapd.conf"
    conf.write_text(bridge._render_hostapd_conf("FoodAssistant", "old-pass-1"))
    boot = tmp_path / "boot"
    boot.mkdir(exist_ok=True)
    flag = tmp_path / "ap-active"
    if active:
        flag.write_text("")
    monkeypatch.setattr(bridge, "_HOSTAPD_CONF", str(conf))
    monkeypatch.setattr(bridge, "_AP_SSID_FILE", str(tmp_path / "ap-ssid"))
    monkeypatch.setattr(bridge, "_AP_PASSPHRASE_FILE", str(tmp_path / "ap-passphrase"))
    monkeypatch.setattr(bridge, "_AP_BOOT_DIRS", (str(boot),))
    monkeypatch.setattr(bridge, "_AP_FLAG", str(flag))
    return conf, boot, flag


def test_ap_config_get_hides_passphrase_without_token(monkeypatch, tmp_path):
    _ap_paths(monkeypatch, tmp_path)
    monkeypatch.setattr(bridge, "_EXPECTED_TOKEN", "secret")
    d = _ap_config_dummy({})
    d._ap_config_get()
    code, body = d.sent
    assert code == 200 and body["ok"] is True
    assert body["ssid"] == "FoodAssistant"
    assert body["active"] is False
    assert "passphrase" not in body


def test_ap_config_get_reveals_passphrase_with_token(monkeypatch, tmp_path):
    _ap_paths(monkeypatch, tmp_path)
    monkeypatch.setattr(bridge, "_EXPECTED_TOKEN", "secret")
    d = _ap_config_dummy({"X-Bridge-Token": "secret"})
    d._ap_config_get()
    _, body = d.sent
    assert body["passphrase"] == "old-pass-1"
    assert body["generated"] is False


def test_ap_config_set_rejects_bad_ssid_and_passphrase(monkeypatch, tmp_path):
    conf, _, _ = _ap_paths(monkeypatch, tmp_path)
    before = conf.read_text()
    for payload in ({"ssid": "x" * 33}, {"ssid": "two\nlines"},
                    {"passphrase": "short"}, {"passphrase": "x" * 64}):
        d = _ap_config_dummy({"X-Bridge-Token": "secret"}, payload)
        monkeypatch.setattr(bridge, "_EXPECTED_TOKEN", "secret")
        d._ap_config_set()
        assert d.sent[0] == 400, payload
        assert conf.read_text() == before, "a rejected save must change nothing"


def test_ap_config_set_updates_identity_without_restart_when_inactive(monkeypatch, tmp_path):
    conf, boot, _ = _ap_paths(monkeypatch, tmp_path, active=False)
    monkeypatch.setattr(bridge, "_EXPECTED_TOKEN", "secret")
    calls = []
    monkeypatch.setattr(bridge.subprocess, "run",
                        lambda *a, **k: calls.append(a[0]))
    d = _ap_config_dummy({"X-Bridge-Token": "secret"},
                         {"ssid": "Kitchen Pi", "passphrase": "new-pass-99"})
    d._ap_config_set()
    code, body = d.sent
    assert code == 200 and body["ok"] is True
    assert body["ssid"] == "Kitchen Pi" and body["passphrase"] == "new-pass-99"
    assert bridge._parse_hostapd_conf(conf.read_text()) == ("Kitchen Pi", "new-pass-99")
    mirror = (boot / "pantry-raider-hotspot.txt").read_text()
    assert "SSID: Kitchen Pi" in mirror and "new-pass-99" in mirror
    assert calls == [], "hostapd must not restart while the AP is down"


def test_ap_config_set_restarts_hostapd_only_when_broadcasting(monkeypatch, tmp_path):
    _ap_paths(monkeypatch, tmp_path, active=True)
    monkeypatch.setattr(bridge, "_EXPECTED_TOKEN", "secret")
    calls = []
    monkeypatch.setattr(bridge.subprocess, "run",
                        lambda *a, **k: calls.append(a[0]))
    d = _ap_config_dummy({"X-Bridge-Token": "secret"}, {"passphrase": "new-pass-99"})
    d._ap_config_set()
    assert d.sent[0] == 200 and d.sent[1]["active"] is True
    assert ["systemctl", "restart", "hostapd"] in calls


def test_ap_config_set_empty_ssid_resets_to_default(monkeypatch, tmp_path):
    conf, _, _ = _ap_paths(monkeypatch, tmp_path)
    # Start from a custom name so the reset is observable.
    conf.write_text(bridge._render_hostapd_conf("Kitchen Pi", "old-pass-1"))
    monkeypatch.setattr(bridge, "_EXPECTED_TOKEN", "secret")
    d = _ap_config_dummy({"X-Bridge-Token": "secret"}, {"ssid": ""})
    d._ap_config_set()
    assert d.sent[1]["ssid"] == "FoodAssistant"
    assert bridge._parse_hostapd_conf(conf.read_text())[0] == "FoodAssistant"
    # The untouched passphrase survives the rename.
    assert bridge._parse_hostapd_conf(conf.read_text())[1] == "old-pass-1"


def test_ap_config_set_regenerate_mints_a_generated_password(monkeypatch, tmp_path):
    conf, _, _ = _ap_paths(monkeypatch, tmp_path)
    monkeypatch.setattr(bridge, "_EXPECTED_TOKEN", "secret")
    d = _ap_config_dummy({"X-Bridge-Token": "secret"}, {"regenerate": True})
    d._ap_config_set()
    _, body = d.sent
    assert body["generated"] is True
    new_pass = bridge._parse_hostapd_conf(conf.read_text())[1]
    assert new_pass != "old-pass-1"
    assert bridge._ap_passphrase_generated(new_pass)
...
# The Printing pane's "Set up" button posts {"bluetooth": true}; a plain,
# no-body /print-setup call (the existing "Install now" button) must behave
# exactly as before this feature was added.


def test_print_setup_run_args_no_bluetooth_matches_original_behavior():
    assert bridge._print_setup_run_args({}) == ({}, 600)
    assert bridge._print_setup_run_args(None) == ({}, 600)
    assert bridge._print_setup_run_args({"bluetooth": False}) == ({}, 600)


def test_print_setup_run_args_bluetooth_true_gets_env_and_long_timeout():
    env, timeout = bridge._print_setup_run_args({"bluetooth": True})
    assert env == {"ENABLE_BLUETOOTH_PRINTING": "1"}
    assert timeout > 600


def test_print_setup_status_reports_supvan_active(monkeypatch):
    monkeypatch.setattr(bridge, "_service_active", lambda unit: unit == bridge._SUPVAN_UNIT)
    ...
    class FakeHandler:
        ...
        _print_setup_status = bridge._Handler._print_setup_status

    h = FakeHandler()
    h._print_setup_status()
    assert h.sent == (200, {"ok": True, "supvan_active": True})


def test_print_setup_status_reports_inactive(monkeypatch):
    monkeypatch.setattr(bridge, "_service_active", lambda unit: False)
    ...
    h = FakeHandler()
    h._print_setup_status()
    assert h.sent == (200, {"ok": True, "supvan_active": False})


# -- Token gate + auto-graduation (FoodAssistant-ow4f) ------------------------

def test_token_gate_exempt_get_always_ok():
    assert bridge._token_gate("GET", "/hostname", "", "TOK", True) == "ok"
    assert bridge._token_gate("GET", "/system/health", "", "TOK", False) == "ok"


def test_token_gate_valid_token_ok_wrong_denied():
    assert bridge._token_gate("POST", "/reboot", "TOK", "TOK", False) == "ok"
    assert bridge._token_gate("POST", "/reboot", "WRONG", "TOK", True) == "deny"


def test_token_gate_missing_token_follows_grace():
    # Tokenless: allowed only while grace is active.
    assert bridge._token_gate("POST", "/reboot", "", "TOK", True) == "grace"
    assert bridge._token_gate("POST", "/reboot", "", "TOK", False) == "deny"


def test_grace_active_ends_after_a_token_is_seen(monkeypatch):
    import time as _time
    monkeypatch.setattr(bridge, "_token_seen", False)
    # The bounded window must be open for grace to hold (FoodAssistant-ubpu).
    monkeypatch.setattr(bridge, "_grace_started", _time.time())
    assert bridge._grace_active(True) is True
    monkeypatch.setattr(bridge, "_token_seen", True)
    # Once graduated, grace is off even with the static flag on.
    assert bridge._grace_active(True) is False


def test_graduation_persists_and_reloads(tmp_path, monkeypatch):
    marker = tmp_path / "bridge-token-enforced"
    monkeypatch.setattr(bridge, "_token_seen", False)
    assert bridge._load_graduation(str(marker)) is False
    bridge._mark_graduated(str(marker))
    assert bridge._token_seen is True
    assert marker.exists()
    # A fresh run re-latches enforcement from the marker.
    assert bridge._load_graduation(str(marker)) is True


def test_harden_token_perms_falls_back_when_no_group(tmp_path, monkeypatch):
    f = tmp_path / "bridge-token"
    f.write_text("secret\n")
    monkeypatch.setattr(bridge, "_primary_user_gid", lambda: None)
    bridge._harden_token_perms(str(f))
    import stat
    mode = stat.S_IMODE(os.stat(f).st_mode)
    # No resolvable primary group means no non-root reader exists, so the
    # root-equivalent token stays root-only (FoodAssistant-56qf).
    assert mode == 0o600


# --- POST /grocy/repair: the handler itself ---------------------------------
# The pure decision matrix above proves the guards; these prove the HANDLER
# obeys them: a refusal must never reach docker, the act path must follow the
# exact stop / re-check / rename / start order, and the post-stop re-check
# must abort the rename when the database gained content at the last moment.


class _RepairHarness:
    """A _grocy_repair invocation with everything outside the handler faked."""

    def __init__(self, bridge_mod, monkeypatch, tmp_path, *,
                 db_bytes=b"", db_age=600.0, db_missing=False,
                 marker_age=None, services=("grocy",),
                 stop_side_effect=None):
        self.sent = []
        self.calls = []
        self.db = tmp_path / "grocy.db"
        if not db_missing:
            self.db.write_bytes(db_bytes)
            old = os.path.getmtime(self.db) - db_age
            os.utime(self.db, (old, old))
        marker = tmp_path / "grocy-repair-last"
        if marker_age is not None:
            import time as _t
            marker.write_text("%d\n" % int(_t.time() - marker_age))
        monkeypatch.setattr(bridge_mod, "_GROCY_DB_PATH", str(self.db))
        monkeypatch.setattr(bridge_mod, "_GROCY_REPAIR_MARKER", str(marker))
        monkeypatch.setattr(bridge_mod, "_read_stack_compose", lambda path=None: "")
        monkeypatch.setattr(bridge_mod, "_compose_backend_services",
                            lambda text: list(services))
        monkeypatch.setattr(bridge_mod, "_stack_stop_cmd",
                            lambda svcs: ["STOP"] + list(svcs))
        monkeypatch.setattr(bridge_mod, "_stack_up_cmd", lambda: ["UP"])
        monkeypatch.setattr(bridge_mod, "_compose_env", lambda: {})

        harness = self
        stop_hook = stop_side_effect
        ...
        def fake_run(cmd, **kw):
            ...
            if cmd[0] == "STOP" and stop_hook:
                stop_hook()
            return _Done()

        monkeypatch.setattr(bridge_mod.subprocess, "run", fake_run)

        self.handler = object.__new__(bridge_mod._Handler)
        self.handler._send = lambda code, obj: self.sent.append((code, obj))

    def run(self):
        self.handler._grocy_repair()
        assert len(self.sent) == 1
        return self.sent[0]


def test_grocy_repair_handler_repairs_an_old_empty_stub(tmp_path, monkeypatch):
    h = _RepairHarness(bridge, monkeypatch, tmp_path)
    code, body = h.run()
    assert code == 200 and body["ok"] is True and body["action"] == "repaired"
    # Exact order: stop the container, then start it again. Nothing else.
    assert h.calls == [["STOP", "grocy"], ["UP", "grocy"]]
    # The stub was renamed aside (never deleted) and the original is gone.
    assert not h.db.exists()
    aside = Path(body["moved"])
    assert aside.exists() and aside.name.startswith("grocy.db.stillborn-")
    # The cooldown marker was written at the attempt.
    assert bridge._read_epoch_marker(bridge._GROCY_REPAIR_MARKER) is not None


def test_grocy_repair_handler_refusal_never_reaches_docker(tmp_path, monkeypatch):
    h = _RepairHarness(bridge, monkeypatch, tmp_path, db_bytes=b"real data")
    code, body = h.run()
    assert code == 200 and body["ok"] is False and body["action"] == "refused"
    assert h.calls == []                      # docker was never invoked
    assert h.db.read_bytes() == b"real data"  # and the database is untouched


def test_grocy_repair_handler_cooldown_refuses_without_docker(tmp_path, monkeypatch):
    h = _RepairHarness(bridge, monkeypatch, tmp_path, marker_age=60.0)
    code, body = h.run()
    assert body["action"] == "refused" and h.calls == []
    assert h.db.exists()


def test_grocy_repair_handler_post_stop_recheck_aborts(tmp_path, monkeypatch):
    """The database gained content while the container was stopping: the
    handler must put the container back and keep its hands off the file."""
    written = {}

    def grow_db():
        # Simulated last-moment schema write landing during the stop.
        h.db.write_bytes(b"schema arrived")
        written["yes"] = True

    h = _RepairHarness(bridge, monkeypatch, tmp_path, stop_side_effect=grow_db)
    code, body = h.run()
    assert written and body["action"] == "refused" and body["ok"] is False
    assert h.db.read_bytes() == b"schema arrived"     # untouched
    assert h.calls == [["STOP", "grocy"], ["UP", "grocy"]]  # restarted


def test_grocy_repair_handler_400_without_a_grocy_service(tmp_path, monkeypatch):
    h = _RepairHarness(bridge, monkeypatch, tmp_path, services=())
    code, body = h.run()
    assert code == 400 and body["action"] == "refused"
    assert h.calls == []


# -- Concurrent request handling (FoodAssistant-0hwez) ------------------------
#
# The bridge used to run on the single-threaded http.server.HTTPServer, so one
# slow handler (a docker compose ps on a busy SD card) made every other caller
# queue behind it: kiosk activity polls and settings-page renders each waited
# out a stranger's request. It now serves on ThreadingHTTPServer.


def test_bridge_main_binds_a_threading_server():
    import re
    src = BRIDGE.read_text()
    assert "http.server.ThreadingHTTPServer(" in src
    # No plain single-threaded HTTPServer construction remains.
    assert not re.search(r"(?<!Threading)HTTPServer\(", src), \
        "a single-threaded HTTPServer construction crept back in"


def test_threaded_bridge_answers_while_a_slow_request_runs(monkeypatch):
    """Smoke: two concurrent requests are handled in parallel. The FIRST
    request blocks until the SECOND one arrives and frees it, which can only
    end well when requests get their own threads; on the old single-threaded
    server the second request would never start and the first would time out.
    """
    import http.server
    ...
    def _hostname(self):
        with order_lock:
            ...
            first = len(calls) == 1
        if first:
            freed = release.wait(4.0)  # freed by the second request arriving
            self._send(200, {"ok": bool(freed), "hostname": "first"})
        else:
            release.set()
            self._send(200, {"ok": True, "hostname": "second"})

    monkeypatch.setattr(bridge._Handler, "_get_hostname", _hostname)
    monkeypatch.setattr(bridge._Handler, "log_message",
                        lambda self, *a, **kw: None)
    srv = http.server.ThreadingHTTPServer(("127.0.0.1", 0), bridge._Handler)
    ...
    try:
        ...
        def hit():
            with urllib.request.urlopen(
                    f"http://127.0.0.1:{port}/hostname", timeout=6) as r:
        ...
        for t in threads:
            t.join(timeout=8)
        assert len(bodies) == 2
        assert all(b.get("ok") is True for b in bodies), bodies
    finally:
        srv.shutdown()
        srv.server_close()


# -- Grocy config.php repair (the Grocy 4.7 AUTH_CLASS move) -------------------

def test_last_json_line_reads_the_helper_summary():
    out = "[grocy-repair] Backed up\n[grocy-repair] done\n{\"ok\": true, \"action\": \"repaired\"}\n"
    assert bridge._last_json_line(out) == {"ok": True, "action": "repaired"}
    assert bridge._last_json_line("just a log line") == {}
    assert bridge._last_json_line("") == {}
    assert bridge._last_json_line(None) == {}
    assert bridge._last_json_line("[1, 2]") == {}


class _ConfigRepairHarness:
    def __init__(self, bridge_mod, monkeypatch, services=("grocy",), helper="/usr/local/bin/foodassistant-grocy-repair",
                 stdout="", stderr="", returncode=0, raise_exc=None):
        self.sent = []
        self.calls = []
        monkeypatch.setattr(bridge_mod, "_read_stack_compose", lambda path=None: "")
        monkeypatch.setattr(bridge_mod, "_compose_backend_services", lambda text: list(services))
        monkeypatch.setattr(bridge_mod, "_ensure_helper", lambda name, **kw: helper)
        monkeypatch.setattr(bridge_mod, "_compose_env", lambda: {})
        harness = self
        ...
        def fake_run(cmd, **kw):
            harness.calls.append((list(cmd), kw))
            if raise_exc:
                raise raise_exc
            d = _Done()
            d.stdout, d.stderr, d.returncode = stdout, stderr, returncode
            return d
    ...
    def run(self):
        self.handler._grocy_repair_config()
        assert len(self.sent) == 1
        return self.sent[0]


def test_grocy_repair_config_relays_the_helper_summary(monkeypatch):
    h = _ConfigRepairHarness(
        bridge, monkeypatch,
        stdout="[grocy-repair] Backed up\n{\"ok\": true, \"action\": \"repaired\", \"reason\": \"Changed AUTH_CLASS\", \"backup\": \"/config/data/config.php.bak-1\"}\n")
    code, body = h.run()
    assert code == 200 and body["ok"] is True and body["action"] == "repaired"
    assert body["backup"].endswith(".bak-1")
    assert body["log"].startswith("[grocy-repair] Backed up")
    cmd, kw = h.calls[0]
    assert cmd == ["/usr/local/bin/foodassistant-grocy-repair"]
    assert kw["timeout"] == bridge._GROCY_CONFIG_REPAIR_TIMEOUT
    assert kw["cwd"] == bridge.INSTALL_DIR


def test_grocy_repair_config_skipped_is_still_ok(monkeypatch):
    h = _ConfigRepairHarness(
        bridge, monkeypatch,
        stdout="{\"ok\": true, \"action\": \"skipped\", \"reason\": \"nothing to repair\"}\n")
    code, body = h.run()
    assert code == 200 and body["action"] == "skipped"


def test_grocy_repair_config_failure_is_a_500_with_the_reason(monkeypatch):
    h = _ConfigRepairHarness(
        bridge, monkeypatch, returncode=1,
        stdout="{\"ok\": false, \"action\": \"failed\", \"reason\": \"Could not write the backup\"}\n")
    code, body = h.run()
    assert code == 500 and body["ok"] is False
    assert "backup" in body["reason"]


def test_grocy_repair_config_refuses_without_a_local_grocy(monkeypatch):
    h = _ConfigRepairHarness(bridge, monkeypatch, services=())
    code, body = h.run()
    assert code == 400 and body["action"] == "refused"
    assert h.calls == []


def test_grocy_repair_config_missing_helper_points_at_update(monkeypatch):
    h = _ConfigRepairHarness(bridge, monkeypatch, helper=None)
    code, body = h.run()
    assert code == 500 and "Update" in body["reason"]
    assert h.calls == []


def test_grocy_repair_config_no_summary_line_is_a_failure(monkeypatch):
    h = _ConfigRepairHarness(bridge, monkeypatch, stdout="bash: something broke\n", stderr="", returncode=2)
    code, body = h.run()
    assert code == 500 and body["ok"] is False
    assert "something broke" in body["reason"]


def test_grocy_repair_config_timeout(monkeypatch):
    import subprocess as _sp
    h = _ConfigRepairHarness(bridge, monkeypatch, raise_exc=_sp.TimeoutExpired("x", 1))
    code, body = h.run()
    assert code == 500 and "timed out" in body["reason"]


def test_grocy_repair_config_is_a_token_gated_post():
    # Every POST outside the short exempt-GET list needs the token.
    assert bridge._token_gate("POST", "/grocy/repair-config", "", "TOK", False) == "deny"
    assert bridge._token_gate("POST", "/grocy/repair-config", "TOK", "TOK", False) == "ok"


# --- WireGuard config values (wg-quick runs this file as root) ---------------

_KEY = "K" * 43 + "="


def _wg(**over):
    args = dict(private_key=_KEY, address="10.99.4.7",
                server_public_key=_KEY, endpoint="vps.example:51820",
                allowed_ips="10.99.0.1/32")
    args.update(over)
    return bridge._render_wg_config(**args)


def test_wg_render_accepts_the_shapes_the_cloud_sends():
    text = _wg(dns="10.99.0.1", keepalive=30)
    assert "Address = 10.99.4.7/32" in text
    assert "DNS = 10.99.0.1" in text
    assert "PersistentKeepalive = 30" in text
    assert "Endpoint = [2001:db8::1]:51820" in _wg(endpoint="[2001:db8::1]:51820")
    assert "AllowedIPs = 10.99.0.1/32, fd00::1/128" in _wg(
        allowed_ips="10.99.0.1/32, fd00::1/128")


@pytest.mark.parametrize("field", ["address", "server_public_key", "endpoint",
                                   "allowed_ips", "dns"])
def test_wg_render_refuses_a_smuggled_line(field):
    # wg-quick runs PostUp as root, so a newline in a cloud-supplied value
    # would be a command rather than data.
    with pytest.raises(ValueError):
        _wg(**{field: "10.99.0.1/32\nPostUp = touch /tmp/pwned"})
    with pytest.raises(ValueError):
        _wg(**{field: "10.99.0.1/32\rPostUp = touch /tmp/pwned"})


def test_wg_render_refuses_junk_per_field():
    with pytest.raises(ValueError):
        _wg(address="not-an-address")
    with pytest.raises(ValueError):
        _wg(endpoint="vps.example")           # no port
    with pytest.raises(ValueError):
        _wg(endpoint="vps.example:noport")
    with pytest.raises(ValueError):
        _wg(private_key="short")
    with pytest.raises(ValueError):
        _wg(server_public_key="")
    with pytest.raises(ValueError):
        _wg(allowed_ips="10.0.0.1/32, junk")


def test_wg_render_never_echoes_the_value_it_rejected():
    secret = "S" * 43 + "=\nPostUp = id"
    try:
        _wg(private_key=secret)
    except ValueError as e:
        assert str(e) == "private_key"
        assert "PostUp" not in str(e)
    else:
        raise AssertionError("a private key with a smuggled line was accepted")


def test_wg_render_clamps_a_nonsense_keepalive():
    assert "PersistentKeepalive = 25" in _wg(keepalive="30\nPostUp = id")
    assert "PersistentKeepalive = 25" in _wg(keepalive=-1)
    assert "PersistentKeepalive = 25" in _wg(keepalive=99999)


# --- Presence pin: which gpiochip is the 40-pin header -----------------------

def test_presence_chip_resolved_by_line_name():
    # Pi 5: the header is not chip 0, and only the line NAME says which chip
    # (and which offset) carries GPIO17.
    def info(path, name):
        assert name == "GPIO17"
        return {"/dev/gpiochip0": ("pinctrl-rp1-south", None),
                "/dev/gpiochip4": ("pinctrl-rp1", 17)}.get(path)

    assert bridge._resolve_presence_chip(
        paths=["/dev/gpiochip0", "/dev/gpiochip4"], info=info) == ("/dev/gpiochip4", 17)


def test_presence_chip_uses_the_named_offset_when_a_bank_is_split():
    def info(path, name):
        return {"/dev/gpiochip0": ("pinctrl-rp1-south", None),
                "/dev/gpiochip1": ("pinctrl-rp1", 3)}.get(path)

    assert bridge._resolve_presence_chip(
        paths=["/dev/gpiochip0", "/dev/gpiochip1"], info=info) == ("/dev/gpiochip1", 3)


def test_presence_chip_falls_back_to_the_pin_controller_label():
    # A kernel that publishes no line names still identifies its pin
    # controller, which is enough to pick the header chip.
    def info(path, name):
        return {"/dev/gpiochip0": ("gpio-brcmstb", None),
                "/dev/gpiochip4": ("pinctrl-bcm2711", None)}.get(path)

    assert bridge._resolve_presence_chip(
        paths=["/dev/gpiochip0", "/dev/gpiochip4"], info=info) == ("/dev/gpiochip4", 17)


def test_presence_chip_falls_back_to_chip_zero():
    # Nothing readable (no gpiod, or an unusual kernel): the pre-Pi 5 answer,
    # which is what every older board wants anyway.
    assert bridge._resolve_presence_chip(
        paths=[], info=lambda p, n: None) == ("/dev/gpiochip0", 17)
    assert bridge._resolve_presence_chip(
        paths=["/dev/gpiochip0"], info=lambda p, n: None) == ("/dev/gpiochip0", 17)


def test_gpiochip_paths_sort_numerically():
    listed = ["gpiochip10", "gpiochip4", "gpiochip0", "tty", "null"]
    assert bridge._gpiochip_paths(listdir=lambda d: listed) == [
        "/dev/gpiochip0", "/dev/gpiochip4", "/dev/gpiochip10"]


def test_presence_chip_is_cached_then_reprobed_after_a_failure():
    bridge._forget_presence_chip()
    calls = []

    def resolve():
        calls.append(1)
        return ("/dev/gpiochip4", 17)

    assert bridge._presence_chip(resolve=resolve) == ("/dev/gpiochip4", 17)
    bridge._presence_chip(resolve=resolve)
    assert len(calls) == 1, "the probe opens every chip; it must not run per sample"
    # A chip that enumerates late (or a sensor moved to another pin) is picked
    # up on the next sample, not at the next reboot.
    bridge._forget_presence_chip()
    bridge._presence_chip(resolve=resolve)
    assert len(calls) == 2
    bridge._forget_presence_chip()
...
def test_unreadable_pin_drops_the_cached_chip(monkeypatch):
    bridge._forget_presence_chip()
    bridge._presence_chip(resolve=lambda: ("/dev/gpiochip4", 17))
    monkeypatch.setattr(bridge, "_read_presence_gpiod", lambda: None)
    monkeypatch.setattr(bridge, "_read_presence_lgpio", lambda: None)
    monkeypatch.setattr(bridge, "_read_presence_gpioget", lambda: None)
    assert bridge._read_presence_line() is None
    assert bridge._presence_chip_cache is None


# --- Running python out of the shared venv ----------------------------------

def test_venv_python_runs_directly_when_root_owns_it():
    assert bridge._venv_python_argv(["-m", "foodassistant_streamdeck"], owner="root") == [
        "/opt/foodassistant/venv/bin/python", "-m", "foodassistant_streamdeck"]


def test_venv_python_drops_privileges_when_someone_else_owns_it():
    # A device flashed before the venv was handed to root still has one the
    # desktop account can rewrite; root must not execute that interpreter.
    assert bridge._venv_python_argv(["-m", "x"], owner="pi") == [
        "runuser", "-u", "pi", "--", "/opt/foodassistant/venv/bin/python", "-m", "x"]
