...
def test_load_or_create_token_creates_dir_and_file(tmp_path):
    ...
    # Not world-readable (FoodAssistant-ow4f): 0o640 when the primary group
    # resolves (root owner + app/deck user read via group), else a fail-safe
    # 0o644 so a reader is never locked out. Never 0o600 (that would break the
    # non-root venv app and deck).
    mode = path.stat().st_mode & 0o777
    assert mode in (0o640, 0o644)
    assert mode & 0o040  # group can always read
...
def test_harden_token_perms_falls_back_when_no_group(tmp_path, monkeypatch):
    ...
    # No resolvable group: fail-safe to the old 0o644, never a lockout.
    assert mode == 0o644
