...
def test_sidebar_and_overview_labels_match_for_reconciled_panes():
    """The three panes whose side-menu and Overview-card labels had drifted now
    read the same in both places. The Start Page label is the pi variant, since
    that is the only place the Stream Deck wording appears."""
    sidebar = _norm((TEMPLATES / "setup.html").read_text())
    overview = _norm((TEMPLATES / "setup" / "_pane_overview.html").read_text())
    for label in ("Start Page & Stream Deck", "Fleet & Remote Access",
                  "Backups & Updates"):
