...
def test_page_ships_the_defensive_json_reader(client):
    ...
    assert "function readJson(" in html
    assert "function friendlyHttpError(" in html
    assert "await readJson(r)" in html
    # The old fragile pattern that threw a raw SyntaxError on a non-JSON body
    # must be gone.
    assert "(await r.json()).detail" not in html


def test_page_has_human_messages_for_non_json_replies(client):
    _app, c = client
    html = _page(c)
    assert "session has expired" in html          # 401 over the tunnel
    assert "too large" in html                     # 413 from a proxy
    assert "did not respond in time" in html       # 502/503/504
