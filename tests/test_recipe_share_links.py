...
def test_share_link_uploads_image_bytes_to_forager(client, monkeypatch):
    # FIX A: the private share uploads the photo's bytes so Forager hosts its
    # own copy, keyed by the returned share token.
    monkeypatch.setattr(settings, "cloud_instance_token", "prc_secret")
    _stub_detail(monkeypatch, _detail())
    _stub_create_share(monkeypatch, {
        "status": 201, "url": "https://forager.test/r/t1", "token": "t1",
        "error": None})

    from app.routers import mealie as mealie_router

    async def fake_image_bytes(db, slug, native, mealie_id=None):
        return (b"\xff\xd8\xff photo", "image/jpeg")
    monkeypatch.setattr(mealie_router, "_share_image_bytes", fake_image_bytes)

    seen = {}

    async def fake_upload(path, image, transport=None):
        seen["path"] = path
        seen["ctype"] = image[1]
        return {"status": 200, "image_url": None, "error": None}
    monkeypatch.setattr(recipes_forager, "upload_recipe_image", fake_upload)

    r = client.post("/mealie/recipes/share-link",
                    json={"slug": "garlic-bread", "attribution": "Dan"})
    assert r.status_code == 200
    # Uploaded to the share's own image endpoint, keyed by the returned token.
    assert seen["path"] == "/v1/recipes/shares/t1/image"
    assert seen["ctype"] == "image/jpeg"


def test_share_link_no_photo_skips_upload(client, monkeypatch):
    # No stored photo: the share still succeeds and no upload is attempted.
    monkeypatch.setattr(settings, "cloud_instance_token", "prc_secret")
    _stub_detail(monkeypatch, _detail())
    _stub_create_share(monkeypatch, {
        "status": 201, "url": "https://forager.test/r/t1", "token": "t1",
    ...
    async def fake_upload(path, image, transport=None):  # must never run
        raise AssertionError("no image upload when the recipe has no photo")
    monkeypatch.setattr(recipes_forager, "upload_recipe_image", fake_upload)

    # _share_image_bytes finds no real recipe row, so it returns None.
    r = client.post("/mealie/recipes/share-link",
                    json={"slug": "garlic-bread", "attribution": "Dan"})
    assert r.status_code == 200
