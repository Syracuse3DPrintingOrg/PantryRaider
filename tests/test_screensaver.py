...
def test_timers_page_has_clear_all_and_screensaver_buttons(client):
    ...
    with patch.object(type(settings), "is_configured", lambda self: True):
        ...
        # The page's behaviour moved to static/js/timers-page.js (cacheable
        # across visits). The page delivers the markup plus that file, so the
        # script-side checks read both, and the page must really link it.
        assert 'src="static/js/timers-page.js?v=' in r.text
        js = (Path(__file__).resolve().parents[1] / "service" / "app" / "static" / "js" / "timers-page.js").read_text()
        delivered = r.text + js
        assert 'fetch("timers", { method: "DELETE" })' in delivered
        assert "window.confirm(msg)" in delivered
        assert "window.__screensaverTest" in delivered
