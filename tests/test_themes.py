...
def test_every_theme_is_well_formed():
    for name, info in THEMES.items():
        ...
        # A theme normally uses at most one of stylesheet/overlay (the native
        # dark/light and the settings-driven custom theme use neither). The one
        # exception is a vendored Bootswatch stylesheet paired with a small fix
        # overlay: base.html loads the overlay after the stylesheet, so a theme
        # like Cyborg can ship cyborg.min.css plus a targeted readability fix.
        _fix_overlay_ok = {"cyborg"}
        if name not in _fix_overlay_ok:
