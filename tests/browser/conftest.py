...
# CI sets REQUIRE_BROWSER=1. There Chromium is installed on purpose, so an
# unavailable browser means the job is broken, not that the suite should quietly
# skip itself: the auto-skip above is why these tests never actually ran in CI.
# Local runs leave the variable unset and keep skipping.
if not BROWSER_AVAILABLE and os.environ.get("REQUIRE_BROWSER") == "1":
    raise RuntimeError(
        "REQUIRE_BROWSER=1 but the browser suite cannot run: " + SKIP_REASON)
...
_EXTRA_ENV = {
    ...
    # The timer-key tests exercise the arranged layout below, so pin Custom
    # mode: the default home is the Glance preset, which renders the top-level
    # nav pages instead of these keys.
    "START_PAGE_MODE": "custom",
    "START_PAGE_KEYS": "6",
