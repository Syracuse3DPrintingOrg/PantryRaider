...
def test_active_count_counts_live_events():
    # Empty ring reads zero (the Glance "Alerts" pill's all-clear state).
    assert ha_events.active_count() == 0
    ha_events.add_notification("one")
    ha_events.add_camera(name="Door", src="ui/camera/0/snapshot")
    assert ha_events.active_count() == 2


def test_active_count_drops_expired_events(monkeypatch):
    # An event older than the TTL is pruned, so it does not inflate the count.
    ha_events.add_notification("stale")
    assert ha_events.active_count() == 1
    real_time = ha_events.time.time
    monkeypatch.setattr(ha_events.time, "time",
                        lambda: real_time() + ha_events._TTL_SECONDS + 5)
    assert ha_events.active_count() == 0
...
# -- detection-driven camera pop-up (FoodAssistant-akd0) --------------------

def test_camera_detect_pops_up_when_type_enabled(client, monkeypatch):
    monkeypatch.setattr(settings, "streamdeck_cameras", [
        {"name": "Front Door", "ha_entity": "camera.front", "popup_types": ["person"]},
    ], raising=False)
    r = client.post("/events/camera-detect",
                    json={"camera": "Front Door", "detection_type": "person"}).json()
    assert r["ok"] is True and r["popped"] is True and r["camera"] == "Front Door"
    ev = client.get("/events/poll?since=0").json()["events"][-1]
    assert ev["type"] == "camera" and ev["src"] == "ui/camera/0/snapshot"


def test_camera_detect_no_popup_when_type_not_enabled(client, monkeypatch):
    monkeypatch.setattr(settings, "streamdeck_cameras", [
        {"name": "Front Door", "ha_entity": "camera.front", "popup_types": ["animal"]},
    ], raising=False)
    r = client.post("/events/camera-detect",
                    json={"camera": "Front Door", "detection_type": "person"}).json()
    assert r["ok"] is True and r["popped"] is False
    assert client.get("/events/poll?since=0").json()["events"] == []


def test_camera_detect_no_popup_when_camera_has_no_types_configured(client, monkeypatch):
    monkeypatch.setattr(settings, "streamdeck_cameras", [
...
def test_camera_detect_unknown_camera(client, monkeypatch):
    monkeypatch.setattr(settings, "streamdeck_cameras", [], raising=False)
    r = client.post("/events/camera-detect", json={"detection_type": "person"}).json()
...
def test_ha_events_js_declares_every_constant_it_uses():
    """Renaming a timing constant once left a stray reference behind, and that
    one undefined name threw as the script started: the poll loop never began,
    so the whole on-screen event channel went dead (no toasts, no camera
    pop-ups, and a NeoKey press moved nothing). Nothing else caught it, because
    the file still parsed. This is the cheap guard for that class of break."""
    src = HAEVENTS_JS.read_text()
    # Strip comments and string literals first, so prose ("HTTP", "NeoKey") and
    # string constants ('POST') cannot be mistaken for identifier references.
    code = re.sub(r"/\*.*?\*/", " ", src, flags=re.S)
    code = re.sub(r"//[^\n]*", " ", code)
    code = re.sub(r"'[^'\n]*'|\"[^\"\n]*\"", "''", code)
    declared = set(re.findall(r"\bvar\s+([A-Z][A-Z0-9_]{2,})\b", code))
    used = set(re.findall(r"\b([A-Z][A-Z0-9_]{2,})\b", code))
    missing = sorted(used - declared)
    assert not missing, (
        f"ha-events.js references undeclared constants {missing}: a stray "
        "reference throws as the script loads and kills the event channel "
        "outright, so physical controls stop moving the screen.")
...
def test_ha_events_js_warning_toast_deep_links_to_pane():
    # A warning toast that carries a pane (FoodAssistant-44f6) is clickable and
    # opens /setup on that pane, in place if already on /setup.
    js = HAEVENTS_JS.read_text()
    assert "function goToPane(pane)" in js
    assert "openSettingsPane" in js
    assert "/setup#" in js
...
def test_add_warning_store_shape():
    ...
    # The settings pane the fix lives on (FoodAssistant-44f6), so the client
    # can deep-link the toast instead of the generic /setup landing page.
    assert ev["pane"] == "pane-network"


def test_add_warning_pane_defaults_to_empty():
    # A warning with no pane (e.g. a caller that hasn't been updated) still
    # stores cleanly; the client just skips the deep-link affordance.
    ha_events.reset()
    ha_events.add_warning("hot")
    assert ha_events.poll(0)["events"][-1]["pane"] == ""
