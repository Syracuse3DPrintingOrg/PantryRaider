...
# A WireGuard key is 43 base64 characters plus the '=' pad; both renderers
# refuse anything else, so the fixtures use keys of the real shape.
DEVICE_KEY = "D" * 43 + "="
SERVER_KEY = "S" * 43 + "="
...
def test_wg_config_renders_split_tunnel():
    text = bridge._render_wg_config(
        private_key=DEVICE_KEY, address="10.99.4.7",
        server_public_key=SERVER_KEY, endpoint="vps.example:51820",
        allowed_ips="10.99.0.1/32", dns="", keepalive=25,
    )
    assert "[Interface]" in text and "[Peer]" in text
    assert f"PrivateKey = {DEVICE_KEY}" in text
    # A bare tunnel IP is normalized to a /32 host address.
    assert "Address = 10.99.4.7/32" in text
    assert f"PublicKey = {SERVER_KEY}" in text
...
def test_wg_config_keeps_prefix_and_adds_dns_when_given():
    text = bridge._render_wg_config(
        private_key=DEVICE_KEY, address="10.99.4.7/32",
        server_public_key=SERVER_KEY, endpoint="h:51820",
        allowed_ips="10.99.0.1/32", dns="10.99.0.1",
    )
    assert "Address = 10.99.4.7/32" in text
...
def test_local_config_routes_a_list_of_networks():
    text = tunnel_local.render_config(
        private_key=DEVICE_KEY, address="10.99.4.7",
        server_public_key=SERVER_KEY, endpoint="vps.example:51820",
        allowed_ips="10.0.0.1/32, 10.0.0.2/32")
    assert "AllowedIPs = 10.0.0.1/32, 10.0.0.2/32" in text


def _params(**overrides):
    base = {"private_key": DEVICE_KEY, "address": "10.99.4.7",
            "server_public_key": SERVER_KEY, "endpoint": "vps.example:51820",
            "allowed_ips": "10.99.0.1/32"}
...
def test_a_newline_in_any_tunnel_parameter_is_refused():
    """wg-quick's config is line based and its PostUp runs as root.

    These values come from the cloud over the wire, so a newline inside one
    would let it append a command of its own to the interface config.
    """
    for field in ("address", "endpoint", "allowed_ips", "server_public_key",
                  "private_key"):
        smuggled = "10.99.4.7\nPostUp = /bin/sh -c id"
        with pytest.raises(ValueError):
            tunnel_local.render_config(**_params(**{field: smuggled}))
        with pytest.raises(ValueError):
            tunnel_local.render_config(**_params(**{field: "x\rPostUp = id"}))


def test_tunnel_parameters_must_match_their_shape():
    with pytest.raises(ValueError):
        tunnel_local.render_config(**_params(address="not-an-ip"))
    with pytest.raises(ValueError):
        tunnel_local.render_config(**_params(address="10.99.4.7/99"))
    with pytest.raises(ValueError):
        tunnel_local.render_config(**_params(endpoint="vps.example"))
    with pytest.raises(ValueError):
        tunnel_local.render_config(**_params(endpoint="vps.example:notaport"))
    with pytest.raises(ValueError):
        tunnel_local.render_config(**_params(allowed_ips="10.0.0.1/32, junk"))
    with pytest.raises(ValueError):
        tunnel_local.render_config(**_params(allowed_ips=""))
    with pytest.raises(ValueError):
        tunnel_local.render_config(**_params(private_key="short"))
    with pytest.raises(ValueError):
        tunnel_local.render_config(**_params(server_public_key=""))


def test_a_trailing_tail_on_a_valid_shape_is_refused():
    # A value that starts out well formed and then carries extra characters
    # must not slip through: wg-quick would run whatever ends up in the file.
    with pytest.raises(ValueError) as err:
        tunnel_local.render_config(**_params(private_key=DEVICE_KEY + "ZZZZ"))
    assert str(err.value) == "invalid WireGuard private key"
    assert "ZZZZ" not in str(err.value)

    with pytest.raises(ValueError) as err:
        tunnel_local.render_config(**_params(server_public_key=SERVER_KEY + "junk"))
    assert str(err.value) == "invalid WireGuard peer key"

    with pytest.raises(ValueError) as err:
        tunnel_local.render_config(**_params(endpoint="1.2.3.4:51820junk"))
    assert str(err.value) == "invalid WireGuard endpoint"
    assert "51820junk" not in str(err.value)


def test_a_rejected_parameter_never_shows_the_value():
    # The same config carries the device private key, so the message names the
    # field and nothing else.
    secret = "SUPERSECRETVALUE\nPostUp = id"
    with pytest.raises(ValueError) as err:
        tunnel_local.render_config(**_params(private_key=secret))
    assert "SUPERSECRET" not in str(err.value)


def test_a_junk_keepalive_falls_back_to_the_default():
    text = tunnel_local.render_config(**_params(), keepalive="25\nPostUp = id")
    assert "PersistentKeepalive = 25" in text
