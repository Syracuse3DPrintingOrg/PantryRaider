...
def test_start_page_enabled_renders_grid(client, monkeypatch):
    ...
    # The corner buttons render in Custom mode too: Exit top-right, Settings
    # top-left, both padded by the panel safe-area insets.
    assert 'class="btn btn-outline-secondary btn-sm start-exit"' in r.text
    assert 'href="setup" class="btn btn-outline-secondary btn-sm start-settings"' in r.text


def test_start_page_glance_renders_the_landing_page(client, monkeypatch):
    # Glance renders as the welcome landing page (Dan, l697 follow-on): a hero
    # in the website's visual language plus one option card per top-level nav
    # page, still fed by the FoodAssistant-7598 preset layout, so nav edits
    # keep reshaping the page and Custom still seeds from this arrangement.
    monkeypatch.setattr(settings, "start_page_enabled", True)
    monkeypatch.setattr(settings, "start_page_mode", "glance")
    r = client.get("/ui/start")
    assert r.status_code == 200
    assert "glance-landing" in r.text and "glance-hero" in r.text
    # A top-level page renders as a plain navigating card (an anchor).
    assert 'class="glance-card" href="ui/inventory"' in r.text
    # Every default top-level page gets a card.
    for href in ("ui/add", "ui/pending", "ui/cook", "ui/shopping",
                 "ui/kitchen-guide", "ui/timers", "ui/weather"):
    ...
    # Corner buttons: Exit (top-right) escapes to the app, Settings (top-left)
    # opens setup; both sit inside the panel safe-area insets so a clipping
    # kiosk edge never swallows them (FoodAssistant-2e0f mechanism).
    assert 'href="ui/inventory" class="btn btn-outline-secondary btn-sm start-exit"' in r.text
    assert 'href="setup" class="btn btn-outline-secondary btn-sm start-settings"' in r.text
    assert "var(--kiosk-inset-right, 0px)" in r.text
    assert "var(--kiosk-inset-left, 0px)" in r.text
