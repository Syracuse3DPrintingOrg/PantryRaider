...
# --- The database goes in as a consistent snapshot ----------------------------
...
def _sqlite_db(path: Path, rows: int = 3) -> None:
    ...
    con.executemany("INSERT INTO items (name) VALUES (?)",
                    [(f"row {i}",) for i in range(rows)])
    con.commit()
    con.close()


def test_backup_carries_a_readable_database_snapshot(data_dir):
    _write_settings(data_dir, {"grocy_base_url": "http://x"})
    _sqlite_db(data_dir / "foodassistant.db")
    members = _members(admin._build_zip(include_secrets=True)[0])
    assert "foodassistant-data/foodassistant.db" in members

    # The archived copy opens and holds the rows, which a torn byte-for-byte
    # copy of a database being written to cannot promise.
    import sqlite3
    zf = zipfile.ZipFile(io.BytesIO(admin._build_zip(include_secrets=True)[0]))
    out = data_dir.parent / "restored.db"
    out.write_bytes(zf.read("foodassistant-data/foodassistant.db"))
    con = sqlite3.connect(str(out))
    assert con.execute("SELECT count(*) FROM items").fetchone()[0] == 3
    con.close()


def test_backup_falls_back_to_a_raw_copy_when_the_snapshot_fails(data_dir):
    """A database that will not open for backup must not cost the whole
    backup: the file goes in as-is and everything else is unaffected."""
    _write_settings(data_dir, {"grocy_base_url": "http://x"})
    (data_dir / "foodassistant.db").write_bytes(b"not a database at all")
    members = _members(admin._build_zip(include_secrets=True)[0])
    assert "foodassistant-data/foodassistant.db" in members
    assert "foodassistant-data/settings.json" in members


def test_backup_never_carries_a_journal_wal_or_partial_file(data_dir):
    """A journal or WAL captured out of step with its database can roll a
    restored database back, and a .part file is half of something."""
    _write_settings(data_dir, {"grocy_base_url": "http://x"})
    _sqlite_db(data_dir / "foodassistant.db")
    for name in ("foodassistant.db-journal", "foodassistant.db-wal",
                 "foodassistant.db-shm", "backup.zip.part", "scratch.tmp"):
        (data_dir / name).write_bytes(b"x")
    members = _members(admin._build_zip(include_secrets=True)[0])
    assert not [m for m in members if m.endswith(
        ("-journal", "-wal", "-shm", ".part", ".tmp"))]


def test_restore_never_lays_down_a_stale_journal(data_dir):
    """Even from an older archive that still carries one."""
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w") as zf:
        zf.writestr("foodassistant-data/settings.json", json.dumps({"staple_items": "miso"}))
        zf.writestr("foodassistant-data/foodassistant.db-journal", "stale")
        zf.writestr("foodassistant-data/foodassistant.db-wal", "stale")
    admin._restore_zip(buf.getvalue())
    assert not (data_dir / "foodassistant.db-journal").exists()
    assert not (data_dir / "foodassistant.db-wal").exists()
