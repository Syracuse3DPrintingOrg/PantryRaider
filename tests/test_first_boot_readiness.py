...
Grocy does, so navigation is steered to /ui/getting-ready until the inventory is
CONNECTED (an API key is saved). These tests pin the whole matrix: the gate
engages on a pi_hosted install with no inventory key, keyed on the key rather
than is_configured() (the appliance seeds its own Grocy URL, so a wizard
password makes is_configured() true while the inventory is still empty, n2b4);
it holds while provisioning is still working (including after Grocy's first HTTP
answer, which is the middle of the job, not the end), hands off when the key
lands or when provisioning reports it has stopped trying, honours the user's
dismissal, obeys the backstop deadline, and can never reappear once released.
...
def test_gate_holds_until_the_inventory_key_lands(monkeypatch, tmp_path):
    _fresh_pi(monkeypatch, tmp_path)
    ...
    assert readiness.gate_possible() is False
    # A wizard password alone does NOT end it. The appliance seeds its own Grocy
    # URL, so is_configured() flips true while the inventory (the API key) is
    # still empty; releasing there is exactly the "Grocy not configured after
    # setup" bug (Dan, 2026-07-16, FoodAssistant-n2b4). The gate must hold until
    # the key actually lands.
    monkeypatch.setattr(settings, "grocy_api_key", "", raising=False)
...
@pytest.mark.anyio
async def test_startup_giveup_does_not_release_the_gate(monkeypatch, tmp_path):
    """A bounded first-run task exhausting its attempts must NOT release the
    gate. The appliance's Grocy is co-hosted and WILL come up, and the
    getting-ready page keeps re-driving provisioning; releasing on give-up
    dropped users into a Grocy-less app while Grocy was still initialising
    (FoodAssistant-n2b4). Only a connect, a definite hands-off, a dismissal, or
    the backstop deadline may open it."""
    _fresh_pi(monkeypatch, tmp_path)

    async def _unreachable(base=""):
        return {"ok": False, "configured": False, "retryable": True}
    monkeypatch.setattr(first_run, "provision_grocy", _unreachable)
    # Nothing to do for Mealie, so the loop only exercises the Grocy give-up.
    ...
    assert readiness._state["provisioned"] is False
    monkeypatch.setattr(readiness, "grocy_answering", _never_answers)
    assert await readiness.gate_active() is True
...
def test_gate_holds_after_setup_before_the_key_lands(client, monkeypatch):
    """The exact n2b4 case: setup looks complete (is_configured() true via the
    appliance's seeded Grocy URL) but first-run provisioning has not connected
    Grocy yet (no API key). Navigation must still be held on the progress page,
    never dropped into a Grocy-less app, even though the middleware's old
    is_configured() guard would have let it straight through."""
    monkeypatch.setattr(settings, "auth_required", False, raising=False)
    monkeypatch.setattr(readiness, "grocy_answering", _never_answers)
    assert settings.is_configured() is True and settings.grocy_api_key == ""
    # An app path is steered to the getting-ready progress page...
    r = client.get("/ui/", follow_redirects=True)
    assert r.status_code == 200
    assert "getting ready" in r.text.lower()
    # ...and /setup itself lands there too, not the wizard.
    r = client.get("/setup", follow_redirects=False)
    assert r.status_code in (302, 303, 307)
    assert "/ui/getting-ready" in r.headers["location"]
    # Once the key lands, the gate is gone and the app serves normally.
    monkeypatch.setattr(settings, "grocy_api_key", "k", raising=False)
    r = client.get("/setup", follow_redirects=False)
    assert r.status_code == 200
