...
def test_recipe_split_between_menus(client, monkeypatch):
    """Recipes are consolidated in one pane under Kitchen (FoodAssistant-s6q):
    the Mealie + external-source cards and the suggestion tuning, each saved by
    its own button. The Cameras pane no longer carries the Mealie fields."""
    html = _render(client, monkeypatch, mode="server", is_pi=False)
    ...
    for field in ("mealie_base_url", "mealie_api_key", "recipe_source",
                  ...
                  "staple_items", "cook_ai_context", "kitchen-appliances",
                  "perishable_days", "expiring_soon_days", "suggest_per_tier"):
        assert field in prefs, f"{field} not in Recipes pane"
    assert 'onclick="savePaneRecipes(this)"' in prefs
