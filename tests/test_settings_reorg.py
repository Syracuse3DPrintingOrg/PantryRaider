...
def test_old_pane_hashes_have_aliases(client, monkeypatch):
    ...
    # Scope the checks to that map: menu.js also carries the lazy-script map
    # (PANE_LAZY_SCRIPTS, FoodAssistant-0hwez), which legitimately names live
    # panes like pane-start-page.
    full_menu_js = client.get("static/js/setup/menu.js").text
    menu_js = full_menu_js[full_menu_js.index("PANE_HASH_ALIASES = {"):]
    menu_js = menu_js[:menu_js.index("};")]
    for old, new in {
...
def test_start_deck_pane_sub_toggle(client, monkeypatch):
    ...
    # One deck entry point (FoodAssistant-gn6g): the pill is plain Start Page
    # and the deck opens through the Devices entry's openStreamDeckSettings().
    assert "Start Page &amp; Stream Deck" not in pi
    assert "openStreamDeckSettings()" in pi
...
def test_mealie_disclosure_open_only_when_in_use(client, monkeypatch):
    """The Advanced Mealie disclosure stays collapsed unless Mealie is
    genuinely in use (FoodAssistant-gn6g). On a fresh pi_hosted the wizard
    persists the suggested localhost address with no token, and a Mealie
    container merely existing must not expand it either; only a full
    connection (address AND token) or Mealie serving recipes or the
    shopping list opens it."""
    def _details_open(html: str) -> bool:
        m = re.search(r'<details id="mealie-connection-details"[^>]*>', html)
        assert m, "mealie disclosure missing"
        return " open" in m.group(0)

    # Fresh appliance: address saved by the wizard, no token -> collapsed.
    monkeypatch.setattr(settings, "mealie_base_url", "http://localhost:9285")
    monkeypatch.setattr(settings, "mealie_api_key", "")
    monkeypatch.setattr(settings, "recipes_backend", "native", raising=False)
    monkeypatch.setattr(settings, "shopping_backend", "", raising=False)
    assert not _details_open(_render(client, monkeypatch, mode="pi_hosted", is_pi=True))

    # A working connection (address AND token) opens it.
    monkeypatch.setattr(settings, "mealie_api_key", "tok")
    assert _details_open(_render(client, monkeypatch, mode="pi_hosted", is_pi=True))

    # The shopping list living in Mealie opens it even with no token.
    monkeypatch.setattr(settings, "mealie_api_key", "")
    monkeypatch.setattr(settings, "shopping_backend", "mealie", raising=False)
    assert _details_open(_render(client, monkeypatch, mode="pi_hosted", is_pi=True))

    # Recipes served from Mealie open it too.
    monkeypatch.setattr(settings, "shopping_backend", "", raising=False)
    monkeypatch.setattr(settings, "recipes_backend", "mealie", raising=False)
    assert _details_open(_render(client, monkeypatch, mode="pi_hosted", is_pi=True))


def test_pane_hash_updates_keep_the_settings_path():
    """A bare "#pane-x" in replaceState is a relative URL, so it resolves
    against the page's <base href="<ingress>/"> and rewrites the address to the
    site root. The saves that reload (theme, display, background) then came back
    on the home page instead of the pane being edited (FoodAssistant-6upk).
    Every hash update must spell the current path out."""
    import pathlib
    js_dir = pathlib.Path(__file__).resolve().parents[1] / "service/app/static/js/setup"
    for name in ("menu.js", "deck-editor.js"):
        src = (js_dir / name).read_text()
        for line in src.splitlines():
            if "replaceState" not in line or line.strip().startswith("//"):
                continue
            assert "window.location.pathname" in line or "here +" in line, (
                f"{name}: replaceState with a bare hash resolves against <base>: {line.strip()}")
