...
GET /ha/state, POST /ha/settings, POST /ha/display, POST /ha/connect, plus the
services/discovery.py mDNS service-info builder.
...
def test_pi_hosted_mode_includes_fleet_fields(client, monkeypatch):
    ...
    # A connected inventory, or the request is held on the first-boot gate
    # instead of reaching /ha/state (FoodAssistant-6v9q).
...
def test_timers_next_none_when_nothing_running(client, monkeypatch):
    ...
    resp = client.get("/ha/state")
    timers = resp.json()["timers"]
    assert timers == {"running": 0, "next": None, "active": []}


def test_timers_active_capped_at_20(client, monkeypatch):
    from app.services import timers as timers_svc

    def fake_list_timers():
        return [
            {"label": f"Timer {i}", "remaining_seconds": float(i), "expired": False}
            for i in range(30)
        ]
    monkeypatch.setattr(timers_svc, "list_timers", fake_list_timers)
    _quiet_backends(monkeypatch)
    ...
    resp = client.get("/ha/state")
    timers = resp.json()["timers"]
    assert timers["running"] == 30
    assert len(timers["active"]) == 20
    assert timers["active"][0]["label"] == "Timer 0"
    assert timers["active"][-1]["label"] == "Timer 19"


def test_pi_remote_still_omits_timers(client, monkeypatch):
    monkeypatch.setattr(settings, "deployment_mode", "pi_remote", raising=False)
    monkeypatch.setattr(settings, "remote_server_url", "http://server.test", raising=False)
    monkeypatch.setattr(settings, "upstream_api_key", "upstream-key", raising=False)
    _quiet_backends(monkeypatch)
    ...
    resp = client.get("/ha/state")
    assert "timers" not in resp.json()
...
class _FakeBridgeClient:
    """Stand-in for services.bridge.bridge_client's async context manager."""

    def __init__(self, responses):
        self._responses = responses  # {path: (status_code, payload)}
    ...
    async def post(self, url, *a, **kw):
        for path, (code, payload) in self._responses.items():
            if url.endswith(path):
                return _FakeBridgeResponse(code, payload)
        raise AssertionError(f"unexpected bridge call: {url}")


def test_display_sleep_proxies_to_bridge(client, monkeypatch):
    monkeypatch.setattr("app.routers.ha.is_raspberry_pi", lambda: True)
    monkeypatch.setattr(
        "app.routers.ha.bridge_client",
        lambda **kw: _FakeBridgeClient({"/display/blank": (200, {"ok": True})}))

    resp = client.post("/ha/display", json={"action": "sleep"})
    assert resp.status_code == 200
    assert resp.json() == {"ok": True}


def test_display_wake_proxies_to_bridge(client, monkeypatch):
    monkeypatch.setattr("app.routers.ha.is_raspberry_pi", lambda: True)
    monkeypatch.setattr(
        "app.routers.ha.bridge_client",
        lambda **kw: _FakeBridgeClient({"/display/wake": (200, {"ok": True})}))

    resp = client.post("/ha/display", json={"action": "wake"})
    assert resp.status_code == 200
    assert resp.json() == {"ok": True}


def test_display_off_pi_returns_ok_false(client, monkeypatch):
    monkeypatch.setattr("app.routers.ha.is_raspberry_pi", lambda: False)
...
def test_display_unknown_action_rejected(client, monkeypatch):
    monkeypatch.setattr("app.routers.ha.is_raspberry_pi", lambda: True)

    resp = client.post("/ha/display", json={"action": "dance"})
    assert resp.status_code == 422


def test_display_bridge_failure_is_fail_soft(client, monkeypatch):
    monkeypatch.setattr("app.routers.ha.is_raspberry_pi", lambda: True)

    def _boom(**kw):
        raise ConnectionError("bridge unreachable")
    monkeypatch.setattr("app.routers.ha.bridge_client", _boom)
...
def test_display_bridge_non_200_is_fail_soft(client, monkeypatch):
    monkeypatch.setattr("app.routers.ha.is_raspberry_pi", lambda: True)
    monkeypatch.setattr(
        "app.routers.ha.bridge_client",
        lambda **kw: _FakeBridgeClient({"/display/blank": (500, {"ok": False, "error": "power"})}))
...
class _FakeHttpxClient:
    ...
    async def get(self, url, headers=None):
        ...
        return _FakeHttpxResponse(self._status_code)


def test_connect_saves_settings_and_reports_verified_true(client, monkeypatch, tmp_path):
    monkeypatch.setattr(settings, "data_dir", str(tmp_path), raising=False)
    # The probe now goes through the SSRF-guarded client (FoodAssistant-0h8i);
    # stub the guard's resolution check and its client so no DNS or socket runs.
    monkeypatch.setattr("app.services.egress.is_safe_public_url",
                        lambda url, **kw: True)
    monkeypatch.setattr("app.services.egress.guarded_async_client",
                        lambda **kw: _FakeHttpxClient(status_code=200))

    resp = client.post("/ha/connect", json={
        "base_url": "http://homeassistant.local:8123/",
        "token": "long-lived-token",
    })
    assert resp.status_code == 200
    body = resp.json()
    assert body == {"ok": True, "verified": True}
    assert settings.streamdeck_ha_base_url == "http://homeassistant.local:8123"
    assert settings.streamdeck_ha_token == "long-lived-token"


def test_connect_saves_even_when_verification_fails(client, monkeypatch, tmp_path):
    monkeypatch.setattr(settings, "data_dir", str(tmp_path), raising=False)
    monkeypatch.setattr("app.services.egress.is_safe_public_url",
                        lambda url, **kw: True)
    monkeypatch.setattr("app.services.egress.guarded_async_client",
                        lambda **kw: _FakeHttpxClient(raise_exc=ConnectionError("no route")))
    ...
    assert resp.status_code == 200
    body = resp.json()
    assert body == {"ok": True, "verified": False}
    # Saved regardless: HA's internal URL may be unreachable from here while
    # still correct on the LAN.
    assert settings.streamdeck_ha_base_url == "http://192.168.1.50:8123"
    assert settings.streamdeck_ha_token == "some-token"


def test_connect_rejects_bad_base_url(client):
    resp = client.post("/ha/connect", json={
        "base_url": "not-a-url", "token": "tok"})
    assert resp.status_code == 422


def test_connect_rejects_empty_token(client):
    resp = client.post("/ha/connect", json={
        "base_url": "http://homeassistant.local:8123", "token": ""})
    assert resp.status_code == 422


def test_connect_strips_trailing_slash(client, monkeypatch, tmp_path):
    monkeypatch.setattr(settings, "data_dir", str(tmp_path), raising=False)
    monkeypatch.setattr("app.services.egress.is_safe_public_url",
                        lambda url, **kw: True)
    monkeypatch.setattr("app.services.egress.guarded_async_client",
    ...
    resp = client.post("/ha/connect", json={
        "base_url": "https://ha.example.com///", "token": "tok"})
    assert resp.status_code == 200
    assert settings.streamdeck_ha_base_url == "https://ha.example.com"


# ---------------------------------------------------------------------------
# services/discovery.py: mDNS service-info builder (pure logic, no network)
# ---------------------------------------------------------------------------

def test_discovery_resolve_port_defaults_by_mode(monkeypatch):
    from app.services import discovery
    monkeypatch.delenv("PORT", raising=False)
    monkeypatch.delenv("UVICORN_PORT", raising=False)

    assert discovery.resolve_port("pi_remote") == 80
    assert discovery.resolve_port("server") == 9284
    assert discovery.resolve_port("pi_hosted") == 9284
    assert discovery.resolve_port("") == 9284


def test_discovery_resolve_port_env_override(monkeypatch):
    from app.services import discovery
    monkeypatch.setenv("PORT", "12345")

    assert discovery.resolve_port("pi_remote") == 12345
    assert discovery.resolve_port("server") == 12345


def test_discovery_build_service_info_shape():
    from app.services import discovery

    info = discovery.build_service_info(
        hostname="kitchen-pi", mode="pi_hosted", version="0.18.21",
        device_id="abc123", port=9284)

    assert info["name"] == "Pantry Raider kitchen-pi._pantry-raider._tcp.local."
    assert info["port"] == 9284
    assert info["properties"] == {
        "device_id": "abc123", "mode": "pi_hosted", "version": "0.18.21",
        "hostname": "kitchen-pi",
    }


def test_discovery_build_service_info_blank_hostname_falls_back():
    from app.services import discovery

    info = discovery.build_service_info(
        hostname="", mode="server", version="0.1.0", device_id="", port=9284)
    assert info["properties"]["hostname"] == "pantryraider"
    assert "pantryraider" in info["name"]


def test_discovery_start_constructs_a_registrable_service(monkeypatch):
    """start() must build ServiceInfo from builder keys that actually exist.

    The original code read spec['hostname'] (which lives under properties),
    and the fail-soft except swallowed the KeyError, so the advertisement
    silently never registered (found live on the Bandit). Drive start() with
    zeroconf stubbed and assert registration is reached.
    """
    import asyncio
    from app.services import discovery
    ...
    monkeypatch.setattr(zasync, "AsyncZeroconf", FakeAZC)
    asyncio.run(discovery.start("autopi", "pi_remote", "1.0", "dev-1"))
    assert len(registered) == 1
    info = registered[0]
    assert info.server == "autopi.local."
    assert info.port == 80
    asyncio.run(discovery.stop())
