...
def _scan_enriched(client, monkeypatch, barcode, lookup):
    """Scan a barcode and drive its background enrichment to completion, then
    return the enriched pending row.

    Fast-ack (FoodAssistant-x61t) queues a placeholder immediately and looks up
    the name/date in the background, so the looked-up provenance lands on the
    row only after enrich_pending_item runs. The scheduler is stubbed so the
    only enrichment is the deterministic one this helper drives."""
    import asyncio
    from app.routers import pending as pending_router
    monkeypatch.setattr(pending_router, "lookup_barcode", lookup)
    monkeypatch.setattr(pending_router, "_spawn_enrichment", lambda *a: None)
    posted = client.post("pending/scan", json={"barcode": barcode})
    assert posted.status_code == 200
    item_id = posted.json()["item"]["id"]
    asyncio.run(pending_router.enrich_pending_item(item_id, barcode))
...
def test_scan_carries_llm_source_and_commit_records_it(client, monkeypatch):
    ...
    item = _scan_enriched(client, monkeypatch, "078000082401",
                          _fake_llm_lookup(best_by))
...
def test_committed_scan_badges_on_printed_label(client, monkeypatch):
    ...
    def _spy_render(spec, shape="rectangle"):
        captured["source"] = spec.best_by_source
        return real_render(spec, shape)
