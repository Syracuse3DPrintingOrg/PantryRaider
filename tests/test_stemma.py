...
def test_keys_rest_in_brand_pink_away_from_the_manage_screen():
    ...
    assert set(palette) == set(SCANNER_MODES)
    assert set(palette.values()) == {stemma.BRAND_PINK}
...
def test_the_mode_colors_come_in_on_the_manage_screen():
    ...
    assert palette == dict(stemma.MODE_COLORS)
    # The four modes stay visually distinct there, or the tiles cannot match.
    assert len(set(palette.values())) == len(SCANNER_MODES)
...
def test_outputs_carry_the_resting_palette_until_manage_is_seen():
    ...
    assert set(map(tuple, resting["mode_colors"].values())) == {stemma.BRAND_PINK}
