...
def _no_network(monkeypatch):
    ...
    # The OTA image has its own fetch path (_fetch_release_ota), added with the
    # published-.ota.bin support. It must be blocked too, or a test meaning to
    # stay offline silently downloads the real firmware from GitHub.
...
def _no_release(monkeypatch):
    ...
    # _fetch_release_ota returns bytes|None (not a tuple); None = nothing
    # published. Without this the manifest downloads the real .ota.bin.
