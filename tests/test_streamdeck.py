...
async def _noop_coro(*args, **kwargs):
    """An awaitable no-op, for stubbing a controller's async helpers in tests."""
    return None
...
def test_shopping_check_press_confirms_and_names_item():
    # Checking an item off the quick-check page rebuilds the page but does not
    # change the kitchen display, so it confirms the check-off on screen
    # (FoodAssistant-rdlo), naming the item.
    ctrl, deck, loop = _make_controller()
    ctrl.client = _FakeClient(
        put_map={"/mealie/shopping/items/a": _Resp(200, {"ok": True})}
    )
    ctrl.shopping_check_items = {"shopping_check_0": {"id": "a", "note": "Milk"}}
    ctrl._refresh_shopping_check = _noop_coro
    ...
    assert confirms and confirms[0][2]["message"] == "Checked off Milk"
    loop.close()


def test_shopping_check_press_failure_does_not_confirm():
    # A failed check-off (no matching PUT route -> "Failed") posts no toast.
    ctrl, deck, loop = _make_controller()
    ctrl.client = _FakeClient()
    ctrl.shopping_check_items = {"shopping_check_0": {"id": "a", "note": "Milk"}}
    ctrl._refresh_shopping_check = _noop_coro
...
def test_press_running_preset_adds_a_minute_not_restart():
    # FoodAssistant-p9vd: pressing a RUNNING preset timer extends the remaining
    # countdown by 60s (via the shared /extend endpoint) instead of resetting
    # it to the full preset. The moved deadline rebinds the key, so the web UI
    # and satellites see the same extension.
    import time as _time
    ctrl, deck, loop = _make_controller()
    now = _time.time()
    new_deadline = now + 260  # ~200s left, ~260s after +60 (NOT the 360 preset)
    ctrl.client = _FakeClient(post_map={"/timers/11/extend": _Resp(
        200, {"timer": {"id": 11, "label": "Timer Eggs",
                        "deadline_epoch": new_deadline}}
    )})
    t = ctrl.timers["timer_eggs"] = actions.TimerState()
    t.bind({"id": 11, "deadline_epoch": now + 200})
    loop.run_until_complete(ctrl._timer_press("timer_eggs", long_press=False))
    # Extended on the server; no cancel, and no fresh /timers create.
    assert ("POST", ctrl.config.base_url.rstrip("/") + "/timers/11/extend") in ctrl.client.calls
    assert not any(method == "DELETE" for method, _ in ctrl.client.calls)
    body = next(b for b in ctrl.client.bodies if b[1].endswith("/timers/11/extend"))
    assert body[2]["seconds"] == 60
    # Key rebinds to the moved deadline (~260s left, not reset to the 360 preset).
    assert t.timer_id == 11
    assert 258 <= t.remaining_seconds() <= 260
    loop.close()


def test_press_running_preset_offline_extends_local_preserving_elapsed():
    # Offline (no reachable server): the deck-local countdown still gains the
    # minute, keeping the already-elapsed time rather than restarting.
    ctrl, deck, loop = _make_controller()  # no client
    t = ctrl.timers["timer_eggs"] = actions.TimerState()
    t.start_local(200)  # 200s left on a local run
    before = t.remaining_seconds()
    loop.run_until_complete(ctrl._timer_press("timer_eggs", long_press=False))
    assert t.timer_id is None
    # Roughly +60 over what was left, not reset to the 360s preset.
    assert before + 59 <= t.remaining_seconds() <= before + 61
    loop.close()


def test_press_running_preset_confirms_added_minute_on_screen():
    # The screen does not change on a timer press, so the extension confirms on
    # the kiosk with a "+1 min" toast (FoodAssistant-rdlo).
    import time as _time
    ctrl, deck, loop = _make_controller()
    now = _time.time()
    ctrl.client = _FakeClient(post_map={"/timers/11/extend": _Resp(
        200, {"timer": {"id": 11, "deadline_epoch": now + 260}}
    )})
    t = ctrl.timers["timer_eggs"] = actions.TimerState()
    t.bind({"id": 11, "deadline_epoch": now + 200})
    loop.run_until_complete(ctrl._timer_press("timer_eggs", long_press=False))
    confirms = [b for b in ctrl.client.bodies if b[1].endswith("/events/confirm")]
    assert confirms, "adding a minute should confirm on screen"
    assert "+1 min" in confirms[0][2]["message"]
    loop.close()


def test_extend_server_timer_posts_and_returns_updated():
    import time as _time
    deadline = _time.time() + 260
    client = _FakeClient(post_map={"/timers/7/extend": _Resp(
        200, {"timer": {"id": 7, "deadline_epoch": deadline}}
    )})
    timer = asyncio.run(actions.extend_server_timer(client, "http://x", 7, 60))
    assert timer["id"] == 7
    body = next(b for b in client.bodies if b[1].endswith("/timers/7/extend"))
    assert body[2]["seconds"] == 60


def test_extend_server_timer_tolerates_failure_and_bad_id():
    # A network failure (or an expired timer that 404s) returns None so the
    # press can fall back to a deck-local extension; a bad id never posts.
    assert asyncio.run(actions.extend_server_timer(_FakeClient(), "http://x", 7, 60)) is None
    assert asyncio.run(actions.extend_server_timer(_FakeClient(), "http://x", None, 60)) is None


def test_extend_local_preserves_elapsed_and_ignores_idle():
    t = actions.TimerState()
    # An idle key is untouched: a press there starts, it does not extend.
    t.extend_local(60)
    assert t.deadline_epoch == 0.0
    # A running key moves out by 60 from its existing deadline (elapsed kept).
    t.start_local(120, now=1000.0)
    t.extend_local(60, now=1005.0)
    assert t.deadline_epoch == 1180.0
    assert t.remaining_seconds(now=1005.0) == 175
