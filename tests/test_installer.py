...
def test_mealie_defaults_on_for_pi_hosted():
    ...
    # now resolve to on for a full local stack with enough RAM (4 GB+).
    # FORCE_MEM_MB pins the RAM tier so the test does not depend on the runner.
    p = plan({"FORCE_PI": "1", "FORCE_MEM_MB": "4000"})
    assert p["mealie"] == "true"


def test_mealie_defaults_on_for_server():
    # Same rule as firstboot: any non-remote mode ships Mealie on by default.
    p = plan({"FORCE_MEM_MB": "4000"})
    assert p["mode"] == "server"
    assert p["mealie"] == "true"


def test_mealie_defaults_off_on_low_ram_pi():
    # FoodAssistant-xkyy: a 2 GB Pi runs Pantry Raider + Grocy but not Mealie, so
    # an unset Mealie resolves to off there. The user can add it later on 4 GB+.
    p = plan({"FORCE_PI": "1", "FORCE_MEM_MB": "2000"})
    assert p["mealie"] == "false"


def test_low_ram_respects_explicit_mealie_opt_in():
    # The RAM tier only sets a default; an explicit opt-in still wins.
    p = plan({"FORCE_PI": "1", "FORCE_MEM_MB": "2000", "ENABLE_MEALIE": "true"})
    assert p["mealie"] == "true"
