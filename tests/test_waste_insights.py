...
def test_load_waste_reads_the_log_and_the_names_at_once(monkeypatch):
    """The two reads do not depend on each other, so the Expiring page waits
    for one round trip rather than two (FoodAssistant-ydj2a)."""
    ...
    async def fake_get(self, path):
        return await _tracked(LOG)

    async def fake_products(self):
        return await _tracked(
            [{"id": int(pid), "name": name} for pid, name in PRODUCTS.items()])

    monkeypatch.setattr(GrocyClient, "_get", fake_get)
    monkeypatch.setattr(GrocyClient, "get_products", fake_products)
    assert asyncio.run(load_waste(GrocyClient())) != []
    assert inflight["max"] == 2   # sequential awaits would never exceed 1


def test_load_waste_degrades_to_empty_when_only_the_names_fail(monkeypatch):
    async def fake_get(self, path):
        return LOG

    async def broken_products(self):
        raise GrocyError("Grocy is not reachable. Inventory will return when it is.")

    monkeypatch.setattr(GrocyClient, "_get", fake_get)
    monkeypatch.setattr(GrocyClient, "get_products", broken_products)
    assert asyncio.run(load_waste(GrocyClient())) == []
