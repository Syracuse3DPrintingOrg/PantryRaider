...
def test_upload_sends_a_scrubbed_zip(client, monkeypatch):
    # The cloud upload shares the local backup builder, so what it stores must
    # be scrubbed the same way: no key from the settings file, from the spare
    # copies saving one leaves behind, or from the captured log.
    secret = "sk-live-abcdef0123456789"
    data = Path(settings.data_dir)
    (data / "settings.json").write_text(json.dumps({"gemini_api_key": secret}))
    (data / "settings.json.bak").write_text(json.dumps({"gemini_api_key": secret}))
    (data / "logs").mkdir()
    (data / "logs" / "foodassistant.log").write_text(f"provider key={secret}\n")
    ...
    fake = _FakeAsyncClient(_resp(200, body={
        ...
        "created_at": "2026-07-07T12:00:00+00:00"}))
    with _patch(fake):
        client.post("/setup/cloud/backup/upload")

    blob = fake.post_kwargs["files"]["file"][1]
    zf = zipfile.ZipFile(io.BytesIO(blob))
    assert not [n for n in zf.namelist() if secret.encode() in zf.read(n)]
    assert not [n for n in zf.namelist()
                if n.startswith("foodassistant-data/settings.json.")]
