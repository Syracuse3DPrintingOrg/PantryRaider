...
def test_published_ota_asset_name_and_url():
    assert cub_svc.ota_asset_name("touch7", "0.9.0") == "bandit-cub-touch7-0.9.0.ota.bin"
    assert cub_svc.ota_release_download_url("Owner/Repo", "0.9.0", "touch7") == (
        "https://github.com/Owner/Repo/releases/download/v0.9.0/"
        "bandit-cub-touch7-0.9.0.ota.bin")


def test_valid_ota_image_checks_the_app_magic():
    assert cub_svc.valid_ota_image(b"\xe9payload") == b"\xe9payload"
    assert cub_svc.valid_ota_image(b"nope") is None
    assert cub_svc.valid_ota_image(b"") is None
    assert cub_svc.valid_ota_image(None) is None


def test_ota_endpoint_prefers_the_published_image_over_slicing(client):
    """A release that carries its own .ota.bin is served as published: no
    slice, no assumption about where the app sits in flash."""
    from app.config import APP_VERSION
    published = b"\xe9published"
    cached_ota = cub_svc.cached_ota_path(settings.data_dir, "tdisplay", APP_VERSION)
    cached_ota.parent.mkdir(parents=True, exist_ok=True)
    cached_ota.write_bytes(published)
    # A factory image is there too; the published app image still wins.
    cub_svc.cached_firmware_path(settings.data_dir, "tdisplay",
                                 APP_VERSION).write_bytes(_factory())

    r = client.get("/cub/firmware/tdisplay.ota.bin")
    assert r.status_code == 200
    assert r.content == published


def test_ota_endpoint_falls_back_to_the_slice_for_an_older_release(client, monkeypatch):
    """A release published before CI attached the app image keeps working."""
    from app.config import APP_VERSION
    from app.routers import cub as cub_router

    async def no_published_ota(profile, version):
        return None
    monkeypatch.setattr(cub_router, "_fetch_release_ota", no_published_ota)
    cached = cub_svc.cached_firmware_path(settings.data_dir, "tdisplay", APP_VERSION)
    cached.parent.mkdir(parents=True, exist_ok=True)
    cached.write_bytes(_factory())
...
def test_firmware_is_reachable_from_the_lan_without_a_key(client, monkeypatch):
    ...
    # The published .ota.bin has its own fetch path; block it too or the
    # ".ota.bin is 404 when nothing is published" assertion below instead
    # downloads the real firmware from GitHub and gets a 200.
    ...
    # The LAN gate is now the stricter request-based check (FoodAssistant-gbs8),
    # which also rejects proxied/tunneled requests. Make this request read as a
    # genuine direct LAN client.
