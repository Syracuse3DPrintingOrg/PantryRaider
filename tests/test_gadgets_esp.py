...
# -- egress guard on the poller's own client ---------------------------------

def test_poll_builds_a_guarded_client(monkeypatch, tmp_path):
    """The ESP host comes from saved settings, so the poll goes through the
    shared egress guard: LAN allowed, loopback and the metadata address not."""
    monkeypatch.setattr(settings, "data_dir", str(tmp_path), raising=False)
    monkeypatch.setattr(settings, "gadget_esp_enabled", True, raising=False)
    monkeypatch.setattr(settings, "gadget_esp_devices",
                        [{"host": "192.168.1.50", "sensor": "fridge"}], raising=False)
    ...
    class _NoopClient:
        async def get(self, *a, **kw):
            raise AssertionError("the fake client never fetches")
    ...
    monkeypatch.setattr(egress, "guarded_async_client", fake_guarded)
    monkeypatch.setattr(gadgets_esp.httpx, "AsyncClient", lambda **kw: (_ for _ in ()).throw(
        AssertionError("the poller must not build an unguarded client")))
    asyncio.run(gadgets_esp.poll_once(now=NOW))
    assert built["kwargs"]["allow_private"] is True
