...
def _render(client, monkeypatch, *, mode: str, is_pi: bool) -> str:
    ...
    # settings.features() calls app.config.is_raspberry_pi() (lru_cached for the
    # process), so patch that copy too, otherwise the mode gating uses whatever
    # value a prior test cached and this flakes in full-suite ordering.
    with patch.object(type(settings), "is_configured", lambda self: True), \
         patch("app.config.is_raspberry_pi", return_value=is_pi), \
...
# Recipe suggestions (pane-recipe-tuning) is the taste tuning split off from
# the Recipes connection settings (FoodAssistant-ysj1); it shows in every mode
# (read-only on a satellite, like the Recipes card).
_ALWAYS_CARDS = {
    "pane-personalization-recipes", "pane-recipe-tuning",
    "pane-scanning", "pane-screen",
    ...
    "pane-resources", "pane-printing", "pane-gadgets",
    # Settings reorg 2026-07-15: Status moved into the System group (it is
    # diagnostics) and gained an Overview card alongside the other System panes.
    "pane-status",
}
...
def test_overview_is_the_landing(client, monkeypatch):
    ...
    for mode, is_pi in (("server", False), ("pi_hosted", True), ("pi_remote", True)):
        ...
        # The group headings frame the cards, mirroring the side menu (Devices
        # added in the 2026-07-15 settings reorg).
        region = _overview_region(html)
        for heading in ("Kitchen", "Devices", "This Device", "Connections", "System"):
