...
def test_forecast_insight_buckets_are_non_empty():
    cases = [
        {"temp": "95", "unit": "F", "desc": "Sunny"},          # hot
        {"temp": "72", "unit": "F", "desc": "Partly cloudy"},  # pleasant
        {"temp": "25", "unit": "F", "desc": "Clear"},          # freezing
        {"temp": "72", "unit": "F", "desc": "Light rain"},     # rain_warm
        {"temp": "45", "unit": "F", "desc": "Rain"},           # rain
        {"temp": "30", "unit": "F", "desc": "Heavy snow"},     # snowy
        {"temp": "75", "unit": "F", "desc": "Thunderstorm"},   # stormy
        {"temp": "70", "unit": "F", "desc": "Clear", "wind": "25"},  # windy
    ]
    for cur in cases:
        assert weather.forecast_insight(cur, day=1)  # non-empty for every bucket


def test_forecast_insight_hot_line_is_hot_themed():
    line = weather.forecast_insight({"temp": "95", "unit": "F", "desc": "Sunny"}, day=0)
    assert any(w in line.lower() for w in ("fry an egg", "scorcher", "griddle", "oven", "hot"))


def test_forecast_insight_rain_line_is_rain_themed():
    line = weather.forecast_insight({"temp": "72", "unit": "F", "desc": "Light rain"}, day=0)
    assert any(w in line.lower() for w in ("grill", "wet", "rain", "umbrella"))


def test_forecast_insight_cold_line_is_soup_themed():
    line = weather.forecast_insight({"temp": "20", "unit": "F", "desc": "Clear"}, day=0)
    assert any(w in line.lower() for w in ("soup", "stew"))


def test_forecast_insight_is_deterministic_per_day():
    cur = {"temp": "95", "unit": "F", "desc": "Sunny"}
    assert weather.forecast_insight(cur, day=7) == weather.forecast_insight(cur, day=7)


def test_forecast_insight_rotates_across_days():
    cur = {"temp": "95", "unit": "F", "desc": "Sunny"}
    seen = {weather.forecast_insight(cur, day=d) for d in range(len(weather._INSIGHT_LINES["hot"]))}
    assert len(seen) == len(weather._INSIGHT_LINES["hot"])  # every candidate is reachable


def test_forecast_insight_mild_day_still_gets_a_note():
    # A valid but unremarkable reading (a mild, dry 50F) used to go blank, which
    # read as broken on ordinary days; it now falls back to the "mild" bucket so
    # the kitchen always gets a line (FoodAssistant-2e0f).
    line = weather.forecast_insight({"temp": "50", "unit": "F", "desc": "Overcast"}, day=0)
    assert line and line in weather._INSIGHT_LINES["mild"]


def test_forecast_insight_missing_temp_is_blank():
    # Only a genuinely missing/unparseable reading yields no note.
    assert weather.forecast_insight(None) == ""
    assert weather.forecast_insight({}) == ""
    assert weather.forecast_insight({"temp": "?", "unit": "F", "desc": ""}) == ""


def test_forecast_insight_celsius_hits_hot_band():
    # 35C is about 95F, so the Celsius reading should land in the hot bucket.
    line = weather.forecast_insight({"temp": "35", "unit": "C", "desc": "Clear"}, day=0)
    assert line and line in weather._INSIGHT_LINES["hot"]


def test_no_insight_line_uses_an_em_dash():
    for lines in weather._INSIGHT_LINES.values():
        for line in lines:
...
# --------------------------------------------------------------------------- #
# CC BY 4.0: the credit has to travel with the data, on every surface
# --------------------------------------------------------------------------- #

def test_every_surface_showing_forecast_data_credits_open_meteo():
    """Open-Meteo publishes under CC BY 4.0, which asks for a visible credit
    wherever the data appears. That is easy to satisfy on the weather page and
    easy to forget on a new surface, so check every template that renders
    forecast values rather than a fixed list of two.

    The Glance page was exactly this miss: its live keys show temperature and
    forecast pulled from the same feed, with no credit anywhere on the page.
    """
    templates = (SERVICE / "app" / "templates")
    # Surfaces that render live values from the weather feed, as opposed to
    # merely linking to the weather page or naming a setting.
    # Match on fetching or painting the feed, not on carrying a units setting:
    # the Stream Deck pane holds streamdeck_weather_units in a hidden input and
    # shows no forecast at all, so it owes no credit.
    SHOWS_DATA = ("ui/weather/data", "wx.forecast", "wx.weather")
    renders_forecast = [f for f in sorted(templates.rglob("*.html"))
                        if any(s in f.read_text() for s in SHOWS_DATA)]

    assert renders_forecast, "no weather surface found: has the feed moved?"
    missing = [f.relative_to(SERVICE.parent).as_posix()
               for f in renders_forecast if "open-meteo.com" not in f.read_text().lower()]
    assert not missing, (
        "these show Open-Meteo data with no CC BY credit: " + ", ".join(missing))
