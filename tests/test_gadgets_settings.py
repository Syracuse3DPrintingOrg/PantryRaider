...
def test_stemma_devices_round_trip_through_the_setup_payload():
    """Regression for t0vp5: POST /setup/save answered ok and silently
    dropped stemma_devices because the payload never declared it."""
    from app.routers.setup import SetupPayload
    from app.config import _SAVEABLE

    assert "stemma_devices" in SetupPayload.model_fields
    assert "stemma_devices" in _SAVEABLE
    pad = [{"id": "i2c:1:0x30", "kind": "neokey", "name": "Pad"}]
    assert SetupPayload(stemma_devices=pad).model_dump(exclude_unset=True) == {
        "stemma_devices": pad}
    # Absent means "keep what is stored", so it never rides along uninvited.
    assert "stemma_devices" not in SetupPayload().model_dump(exclude_unset=True)


def test_stemma_devices_save_cleans_entries_and_rejects_junk(client, monkeypatch):
    monkeypatch.setattr(settings, "stemma_devices", [], raising=False)
    monkeypatch.setattr(settings, "stemma_enabled", True, raising=False)
    with patch.object(type(settings), "is_configured", lambda self: True):
        r = client.post("/setup/save", json={"stemma_devices": [
            {"id": "I2C:1:0X30", "kind": "NeoKey", "name": "Pad",
             "options": {"brightness": 300, "keymap": ["consume"]}},
            {"id": "i2c:1:0x30", "kind": "neokey", "name": "duplicate"},
            {"id": "not-a-bus-address", "kind": "neokey"},
            {"id": "i2c:1:0x38", "kind": "toaster"},
            "junk", 7, None,
        ]})
        assert r.status_code == 200 and r.json()["ok"] is True
        saved = settings.stemma_devices
        assert [d["id"] for d in saved] == ["i2c:1:0x30"]
        assert saved[0]["kind"] == "neokey" and saved[0]["name"] == "Pad"
        # Options go through the same cleaning the /gadgets/stemma route uses.
        assert saved[0]["options"]["brightness"] == 100
        assert saved[0]["options"]["keymap"][0] == "consume"
        assert len(saved[0]["options"]["keymap"]) == 4
        # Not a list: the stored registry stays.
        r = client.post("/setup/save", json={"stemma_devices": "nope"})
        assert r.status_code in (200, 422)
        assert [d["id"] for d in settings.stemma_devices] == ["i2c:1:0x30"]
        # An empty list is a legitimate "remove them all".
        client.post("/setup/save", json={"stemma_devices": []})
...
def test_setup_frees_the_scanner_uart_from_bluetooth():
    """A Pi has two UARTs and the Bluetooth chip holds the one wired to
    GPIO14/15 by default, which is exactly where the scan engine is. Freeing
    the serial console is not enough: on real hardware this came out as
    Bluetooth owning the scanner's port and the scanner read zero bytes at every
    baud until it was moved (FoodAssistant-x82m). The installer must move
    Bluetooth to the mini-UART, and must NOT reach for disable-bt, because the
    thermometers, fridge sensors and shelf buttons all need the radio.
    """
    src = (REPO / "scripts" / "image-build" / "foodassistant-gadgets-setup").read_text()
    assert "dtoverlay=miniuart-bt" in src, \
        "the installer no longer moves Bluetooth off the scanner's UART"
    # The directive itself, not the word: the script's own comment explains why
    # disable-bt is the wrong tool, and that prose must not fail the check.
    assert "dtoverlay=disable-bt" not in src, \
        "disable-bt would kill the Bluetooth sensors; miniuart-bt keeps both"
    # Only written where Bluetooth actually sits on a UART.
    assert "device-tree/soc/serial@" in src


def test_setup_points_the_reader_at_this_devices_app_port():
    """The reader package defaults to port 9284 (the Docker stack), but a Pi
    Remote serves the app on port 80, so a satellite's reader sat unable to
    reach an app that was running the whole time (FoodAssistant-3alc). The
    installer must work out which port actually answers instead of assuming.
    """
    src = (REPO / "scripts" / "image-build" / "foodassistant-gadgets-setup").read_text()
    assert "FOODASSISTANT_BASE_URL" in src, \
        "the installer no longer tells the reader where the app is"
    assert "127.0.0.1:80" in src and "127.0.0.1:9284" in src, \
        "both deployment ports must be probed, not just the Docker one"
