...
def test_unconfigured_first_run_grocy_answers_json_not_html(monkeypatch, tmp_path):
    ...
    # A home-network client address, because the first-run routes now insist on
    # one while there is still no login (see test_first_run_reveal_gate.py).
    cwd = os.getcwd()
    os.chdir(SERVICE)
    monkeypatch.setattr(settings, "data_dir", str(tmp_path), raising=False)
    from fastapi.testclient import TestClient
...
def test_session_cookie_secure_over_https_only(client, monkeypatch):
    # The session cookie carries Secure when the request itself arrived over
    # TLS, and stays plain over LAN HTTP where Secure would break login
    # (FoodAssistant-g6ai).
    from fastapi.testclient import TestClient
    from app.main import app as _app
    _configured(monkeypatch, password="hunter2")
    for scheme, want_secure in (("https", True), ("http", False)):
        c = TestClient(_app, base_url=f"{scheme}://testserver")
        r = c.post("/ui/login", data={"password": "hunter2"},
                   follow_redirects=False)
        assert r.status_code == 303
        sc = "; ".join(v for k, v in r.headers.items()
                       if k.lower() == "set-cookie")
        assert "session=" in sc
        assert ("Secure" in sc) is want_secure, (scheme, sc)
