...
def test_kiosk_consumers_subscribe_to_the_shared_loop():
    """Every steady kiosk poller now prefers the consolidated loop (with a
    legacy direct-fetch fallback for a page or cached copy without it)."""
    for name in ("timer-chips.js", "ha-events.js", "presence-indicator.js"):
