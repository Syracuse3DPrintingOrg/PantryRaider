...
def test_supvan_setup_brings_the_bluetooth_adapter_up():
    """Bluetooth printing needs the radio actually up, not just bluetooth.service
    enabled: a fresh Pi comes up rfkill-blocked / hci DOWN so RFCOMM to the
    printer fails and it never appears (Dan's Bandit, 2026-07-11). The Supvan
    install must unblock + up + power on the adapter, and the unit must re-do it
    on every boot. Source-guard."""
    src = (Path(__file__).resolve().parents[1] / "scripts" / "image-build"
           / "foodassistant-print-setup").read_text()
    # A dedicated helper that unblocks, ups the interface, and powers on.
    assert "bluetooth_adapter_up()" in src
    assert "rfkill unblock bluetooth" in src
    assert "hciconfig hci0 up" in src
    assert "bluetoothctl power on" in src
    # install_supvan_bridge calls it, and the unit re-asserts on boot.
    isb = src[src.index("install_supvan_bridge()"):]
    assert "bluetooth_adapter_up" in isb[:400]
    assert "ExecStartPre=" in src and "rfkill unblock bluetooth" in \
        src[src.index("ExecStartPre="):src.index("ExecStartPre=") + 300]


def test_floating_nav_autohides_with_streamdeck_by_default():
    """A device with a Stream Deck should not also show the on-screen nav by
    default (Dan 2026-07-11); it only hides when a deck is actually present."""
    import sys
