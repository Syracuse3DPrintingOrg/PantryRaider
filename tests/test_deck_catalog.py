...
def test_guide_is_a_navigate_action_to_the_kitchen_guide():
    """The Kitchen Guide gets a deck key wired like the other page-opening
    keys (FoodAssistant-ioa8), so a press lands the display on the guide."""
    from foodassistant_streamdeck.actions import ACTIONS
    spec = ACTIONS["guide"]
    assert spec.kind == "nav"
    assert spec.target_path == "ui/kitchen-guide"
    # The Start Page opens the same page for the on-screen key.
    assert start_page.ACTION_HREF["guide"] == "ui/kitchen-guide"
    # It reaches the editors' palette (the catalog), grouped with the other
    # navigation keys, wearing the same book glyph as the web UI's nav tab.
    entry = next(a for a in _bundled() if a["name"] == "guide")
    assert entry["group"] == "Navigation"
...
def test_action_href_covers_new_navigation_keys():
    ...
    # the historic aliases kept for saved layouts ("guide" left this set when
    # it became a real catalog action, FoodAssistant-ioa8).
    names = {a["name"] for a in _bundled()}
    dangling = {k for k in hrefs if k not in names}
    assert dangling <= {"nutrition", "audit", "shop", "settings"}, (
