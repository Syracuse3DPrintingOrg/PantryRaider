...
# ── cook_time (FoodAssistant-v7gj) ─────────────────────────────────────────────

def test_cook_time_round_trips_through_create(db):
    parsed = dict(PARSED, cook_time="20 minutes")
    saved = recipe_store.create_from_parsed(db, parsed)
    assert saved["cookTime"] == "20 minutes"
    again = recipe_store.detail(db, saved["slug"])
    assert again["cookTime"] == "20 minutes"


def test_cook_time_defaults_blank_and_feeds_current_recipe(db):
    saved = recipe_store.create_from_parsed(db, PARSED)  # no cook_time given
    assert saved["cookTime"] == ""
    from app.services import current_recipe
    normalized = current_recipe.from_mealie_detail(
        recipe_store.detail(db, saved["slug"]), saved["slug"])
    assert normalized["cook_time"] == ""


def test_cook_time_round_trips_through_update(db):
    saved = recipe_store.create_from_parsed(db, PARSED, source="url")
    updated = recipe_store.update_from_parsed(
        db, saved["slug"],
        {"name": "Weeknight Chili", "ingredients": ["a"], "instructions": ["b"],
         "cook_time": "25 minutes"})
    assert updated["cookTime"] == "25 minutes"
    # And current_recipe's normalizer picks it up through the same "cookTime" key
    # the native store now writes, per the comment in from_mealie_detail.
    from app.services import current_recipe
    normalized = current_recipe.from_mealie_detail(updated, saved["slug"])
    assert normalized["cook_time"] == "25 minutes"

    # An update that never mentions the time fields keeps them (sz4h): only a
    # caller that sends a time changes it, so a partial PUT cannot blank one.
    kept = recipe_store.update_from_parsed(
        db, saved["slug"],
        {"name": "Weeknight Chili", "ingredients": ["a"], "instructions": ["b"]})
    assert kept["cookTime"] == "25 minutes"

    # Clearing it with an explicit blank on a later edit is respected too.
    cleared = recipe_store.update_from_parsed(
        db, saved["slug"],
        {"name": "Weeknight Chili", "ingredients": ["a"], "instructions": ["b"],
         "cook_time": ""})
    assert cleared["cookTime"] == ""


# ── Image attach / replace / remove via the store, and edit-preserves-image ────

def test_attach_replace_and_remove_image(db, tmp_path):
    saved = recipe_store.create_from_parsed(db, PARSED)
    r = recipe_store.get_by_slug(db, saved["slug"])
    ...
    assert url == f"/recipes/images/{r.id}"
    assert recipe_store.detail(db, saved["slug"])["image"] == url
    png_path = tmp_path / "recipe-images" / f"{r.id}.png"
    assert png_path.read_bytes() == b"png-bytes"

    # Replace with a different type: the old file is gone, the new one is there.
    recipe_store.attach_image(db, saved["slug"], b"jpg-bytes", "image/jpeg")
    assert not png_path.exists()
    jpg_path = tmp_path / "recipe-images" / f"{r.id}.jpg"
    assert jpg_path.read_bytes() == b"jpg-bytes"

    # Remove: the file and the record both go, the recipe itself stays.
    assert recipe_store.remove_image(db, saved["slug"]) is True
    assert not jpg_path.exists()
    assert recipe_store.detail(db, saved["slug"])["image"] is None
    assert recipe_store.get_by_slug(db, saved["slug"]) is not None

    # Removing again (nothing to remove), or on an unknown recipe: both False.
    assert recipe_store.remove_image(db, saved["slug"]) is False
    assert recipe_store.remove_image(db, "no-such") is False


def test_edit_without_image_change_keeps_existing_image(db):
    """An edit that never touches the image (the common case: the editor only
    calls attach_image/remove_image when the user actually changes the photo)
    must leave a previously attached image exactly as it was."""
    saved = recipe_store.create_from_parsed(db, PARSED)
    url = recipe_store.attach_image(db, saved["slug"], b"png-bytes", "image/png")
    updated = recipe_store.update_from_parsed(
        db, saved["slug"],
        {"name": "Weeknight Chili, Revised", "ingredients": ["a"], "instructions": ["b"]})
    assert updated["image"] == url
    assert recipe_store.detail(db, saved["slug"])["image"] == url


# ── Ingredient sections (FoodAssistant-zq7k) ──────────────────────────────────

def test_split_ingredient_sections_forms():
    """The store's normalizer accepts a flat list (with an optional parallel
    sections list) or the grouped object form, and always returns aligned,
    empty-filtered (lines, sections)."""
    from app.services.recipe_store import split_ingredient_sections

    # Flat, no sections at all: every heading is None.
    lines, secs = split_ingredient_sections({"ingredients": ["a", "b"]})
    assert lines == ["a", "b"]
    assert secs == [None, None]

    # Flat with a parallel sections list; a blank line drops from both, keeping
    # alignment by original position.
    lines, secs = split_ingredient_sections(
        {"ingredients": ["a", "", "b"], "ingredient_sections": ["Sauce", "Sauce", ""]})
    assert lines == ["a", "b"]
    assert secs == ["Sauce", None]

    # Grouped object form: items expand in order, each carrying its heading.
    lines, secs = split_ingredient_sections({"ingredients": [
        {"section": "Sauce", "items": ["tomato", "basil"]},
        {"section": "", "items": ["salt"]}]})
    assert lines == ["tomato", "basil", "salt"]
    assert secs == ["Sauce", "Sauce", None]
...
_SECTIONED = {
    ...
    "instructions": ["Brown the beef.", "Assemble and bake."],
}


def _titles(wire):
    """The section title on each recipeIngredient entry (None when absent), so a
    test can assert the run-start mechanism directly."""
    return [entry.get("title") for entry in wire]


def test_sections_round_trip_through_create(db):
    saved = recipe_store.create_from_parsed(db, _SECTIONED, source="photo")
    wire = saved["recipeIngredient"]
    assert [i["display"] for i in wire] == _SECTIONED["ingredients"]
    # A title marks the FIRST entry of each section run; the rest carry none,
    # exactly like Mealie's own grouping.
    assert _titles(wire) == ["Meat sauce", None, "Topping"]
    # The heading is denormalized onto every stored row.
    from app.models.db_models import RecipeIngredient
    rows = (db.query(RecipeIngredient)
            .filter(RecipeIngredient.recipe_id == saved["native_id"])
            .order_by(RecipeIngredient.position).all())
    assert [r.section for r in rows] == ["Meat sauce", "Meat sauce", "Topping"]
    # Detail rebuilds the same grouped wire.
    again = recipe_store.detail(db, saved["slug"])
    assert _titles(again["recipeIngredient"]) == ["Meat sauce", None, "Topping"]


def test_sections_from_grouped_object_form(db):
    grouped = {
        "name": "Grouped Bake",
        "ingredients": [
    ...
    saved = recipe_store.create_from_parsed(db, grouped, source="photo")
    wire = saved["recipeIngredient"]
    assert [i["display"] for i in wire] == \
        ["1 lb ground beef", "2 cups tomatoes", "1 cup breadcrumbs"]
    assert _titles(wire) == ["Meat sauce", None, "Topping"]


def test_sections_survive_update_and_can_flatten(db):
    saved = recipe_store.create_from_parsed(db, _SECTIONED, source="photo")
    # Rename a section and keep the grouping.
    renamed = recipe_store.update_from_parsed(db, saved["slug"], {
        "name": "Layered Lasagna",
        "ingredients": ["1 lb ground beef", "2 cups tomatoes", "1 cup breadcrumbs"],
        "ingredient_sections": ["Bolognese", "Bolognese", "Topping"],
        "instructions": ["Brown the beef.", "Assemble and bake."],
    })
    assert _titles(renamed["recipeIngredient"]) == ["Bolognese", None, "Topping"]
    # Flatten: drop every heading and the recipe becomes a plain ungrouped list.
    flat = recipe_store.update_from_parsed(db, saved["slug"], {
        "name": "Layered Lasagna",
        "ingredients": ["1 lb ground beef", "2 cups tomatoes", "1 cup breadcrumbs"],
        "instructions": ["Brown the beef.", "Assemble and bake."],
    })
    assert _titles(flat["recipeIngredient"]) == [None, None, None]


def test_flat_recipe_wire_is_unchanged_by_sections(db):
    """Backward-compat pin: a recipe with no sections adds no ``title`` key to any
    ingredient entry, so its wire is byte-for-byte what it was before zq7k."""
    saved = recipe_store.create_from_parsed(db, PARSED)
    for entry in saved["recipeIngredient"]:
        assert "title" not in entry
    for entry in recipe_store.detail(db, saved["slug"])["recipeIngredient"]:
        assert "title" not in entry


# ── Times default blank, never a fake value (FoodAssistant-u65k) ──────────────

def test_unstated_times_persist_blank(db):
    """A recipe imported without servings or times (a cocktail has no cook time)
    stores and returns those fields blank, so nothing fake ever leaks in."""
    parsed = {"name": "Negroni", "ingredients": ["1 oz gin", "1 oz Campari"],
              "instructions": ["Stir with ice."]}
    saved = recipe_store.create_from_parsed(db, parsed)
    for key in ("recipeYield", "prepTime", "cookTime", "totalTime"):
        assert saved[key] == ""
    d = recipe_store.detail(db, saved["slug"])
    for key in ("recipeYield", "prepTime", "cookTime", "totalTime"):
        assert d[key] == ""
    # An edit that leaves them blank keeps them blank (no default reappears).
    updated = recipe_store.update_from_parsed(db, saved["slug"], parsed)
    for key in ("recipeYield", "prepTime", "cookTime", "totalTime"):
        assert updated[key] == ""
