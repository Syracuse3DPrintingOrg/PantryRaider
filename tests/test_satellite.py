...
def test_config_endpoint_serves_shareable_fields(client, monkeypatch):
    ...
    # The server's live sensor alarms ride along so a satellite can show them
    # on its own screen (FoodAssistant-me3t).
    assert isinstance(body["gadget_alarms"], list)
...
def _signed_get(payload: dict, key: str = "upstream-secret"):
    """A httpx.get side_effect that signs ``payload['config']`` with the nonce the
    satellite sent (FoodAssistant-619i), so a sync applies its credential-bearing
    fields. Mirrors what the real server does: HMAC over nonce+config with the
    shared API key, echoing the nonce back."""
    from app.services.satellite import config_signature

    def _fake(url, headers=None, timeout=None, **kw):
        nonce = (headers or {}).get("X-Config-Nonce", "")
        signed = dict(payload)
        signed["config_nonce"] = nonce
        signed["config_signature"] = config_signature(
            key, nonce, payload.get("config", {}))
...
def test_sync_happy_path_applies_fields_and_invalidates_providers(satellite_mode):
    ...
    with patch.object(sat.httpx, "get", side_effect=_signed_get(payload)) as mock_get, \
...
def test_sync_picks_up_mealie_started_after_setup(satellite_mode):
    ...
    with patch.object(sat.httpx, "get", side_effect=_signed_get(payload)), \
...
def test_fleet_printer_defaults_are_pulled_but_local_queue_is_not():
    """The fleet printer DEFAULT the main server chooses is pulled, but a
    device's own queue choice is NOT, so inheritance never clobbers a local
    printer (FoodAssistant-7u7z)."""
    assert "fleet_label_printer_queue" in SATELLITE_PULL_FIELDS
    assert "fleet_document_printer_queue" in SATELLITE_PULL_FIELDS
    # The device-local choices stay device-local: they must not ride the pull.
    assert "label_printer_queue" not in SATELLITE_PULL_FIELDS
    assert "document_printer_queue" not in SATELLITE_PULL_FIELDS
    # printing_enabled is a device-local master switch, never fleet-pulled.
    assert "printing_enabled" not in SATELLITE_PULL_FIELDS


def test_apply_config_inherits_fleet_default_without_clobbering_local(monkeypatch, tmp_path):
    """A satellite that has picked its own label printer keeps it; the pulled
    fleet default becomes the inherited default only, and the resolver prefers
    the local override (FoodAssistant-7u7z)."""
    from app.services.satellite import _apply_config
    from app.services.printing import resolve_effective_queue
    monkeypatch.setattr(settings, "data_dir", str(tmp_path), raising=False)
    # This device already chose its own label printer; it must survive the pull.
    monkeypatch.setattr(settings, "label_printer_queue", "MyLocalZebra", raising=False)
    ...
    applied = _apply_config({
        "fleet_label_printer_queue": "FleetZebra",
        "fleet_document_printer_queue": "FleetLaser",
    })
    assert "fleet_label_printer_queue" in applied
    assert "fleet_document_printer_queue" in applied
    # The local override is untouched by the pull.
    assert settings.label_printer_queue == "MyLocalZebra"
    # The inherited default landed.
    assert settings.fleet_label_printer_queue == "FleetZebra"
    assert settings.fleet_document_printer_queue == "FleetLaser"
    # Resolution: local wins for labels, inherited default fills in for documents.
    assert resolve_effective_queue(
        settings.label_printer_queue, settings.fleet_label_printer_queue) == "MyLocalZebra"
    assert resolve_effective_queue(
        settings.document_printer_queue, settings.fleet_document_printer_queue) == "FleetLaser"
...
def test_clock_format_is_synced_to_satellites():
    """The 12/24-hour clock reading is pulled and overlaid into the deck config
    so a satellite's clock key follows the server (FoodAssistant-ylax)."""
    from app.services import satellite as sat
    assert "clock_format" in SATELLITE_PULL_FIELDS
    assert "clock_format" in sat._STREAMDECK_SYNCED_FIELDS
    merged = sat._merge_streamdeck_settings(
        {"rotation": 0}, "B", "f", "dark", clock_format="12")
    assert merged["clock_format"] == "12"
    # Absent/empty falls back to "auto" (the controller reads that as 24-hour).
    merged_default = sat._merge_streamdeck_settings({"rotation": 0}, "B", "f", "dark")
    assert merged_default["clock_format"] == "auto"


def test_streamdeck_config_push_stamps_clock_format(client, monkeypatch):
    """POST /setup/streamdeck/config stamps the server's clock_format into the
    deck config so the deck's clock key follows the app setting."""
    from app.routers import setup as setup_mod

    monkeypatch.setattr(setup_mod, "_HOST_BRIDGE", "http://127.0.0.1:9299")
    monkeypatch.setattr(settings, "clock_format", "12")
    ...
    monkeypatch.setattr(setup_mod, "bridge_client", lambda *a, **k: _FakePost())
    monkeypatch.setattr(settings, "auth_required", False)
    with patch.object(type(settings), "is_configured", lambda self: True):
        r = client.post("/setup/streamdeck/config", json={"config": {"keys": ["clock"]}})
    assert r.status_code == 200
    assert captured["json"]["config"]["clock_format"] == "12"
...
def test_sync_mirrors_the_servers_alarms(satellite_mode):
    """The slow backstop path (FoodAssistant-me3t): a satellite with no sensor
    reader of its own never POSTs readings, so its only way to hear about the
    fridge alarm is the config pull it already makes."""
    from app.services import satellite as sat

    alarm = {"key": "hygro:AA:11", "kind": "hygrometer", "device_id": "AA:11",
             "name": "Fridge", "location": "Kitchen fridge",
             "message": "Kitchen fridge is at 54F, above its 45F limit.",
             "level": "error", "started_epoch": 100}
    payload = {"ok": True, "config": {}, "expiry_defaults": [],
               "gadget_alarms": [alarm], "command": None}
    mirrored = []
    with patch.object(sat.httpx, "get", return_value=_FakeResponse(200, payload)), \
            patch.object(sat, "_apply_defaults", return_value=0), \
            patch.object(sat, "mirror_alarms", side_effect=mirrored.append):
        out = sat.sync_from_upstream()
    assert out["ok"] is True
    assert mirrored == [[alarm]]


def test_sync_from_an_older_server_without_alarms_is_fine(satellite_mode):
    from app.services import satellite as sat
