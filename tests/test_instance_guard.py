...
def test_heartbeat_refreshes_well_inside_the_freshness_window():
    """The claim is rewritten on a fixed tick whether or not anything changed,
    so the tick is as slow as the freshness window allows: at most half of it,
    which still leaves a full missed beat before a live claim looks stale."""
    import app.services.instance_guard as ig
    assert ig.HEARTBEAT_SECONDS * 2 <= ig.FRESH_SECONDS
    assert ig.HEARTBEAT_SECONDS >= 30
