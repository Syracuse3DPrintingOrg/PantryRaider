...
@pytest.mark.anyio
async def test_fetch_reolink_ai_state_signs_in_and_returns_json(monkeypatch):
    calls = {"login": 0, "ai": 0}
    ...
    def handler(request):
        ...
        if "cmd=Login" in q:
            calls["login"] += 1
            return _login_ok()
        if "cmd=GetAiState" in q:
            calls["ai"] += 1
            assert "token=TOK123" in q and "channel=" in q
            return httpx.Response(200, json={"value": {"people": {"alarm_state": 1}}})
        return httpx.Response(404)

    _mock_httpx(monkeypatch, handler)
    data = await cameras.fetch_reolink_ai_state(_REOLINK)
    assert data == {"value": {"people": {"alarm_state": 1}}}
    assert calls == {"login": 1, "ai": 1}


@pytest.mark.anyio
async def test_fetch_reolink_ai_state_unreachable_returns_empty(monkeypatch):
    def handler(request):
        raise httpx.ConnectError("refused", request=request)

    _mock_httpx(monkeypatch, handler)
    assert await cameras.fetch_reolink_ai_state(_REOLINK) == {}


@pytest.mark.anyio
async def test_fetch_reolink_dev_info_signs_in_and_returns_json(monkeypatch):
    def handler(request):
        q = request.url.query.decode()
        if "cmd=Login" in q:
            return _login_ok()
        if "cmd=GetDevInfo" in q:
            # GetDevInfo is not channel-scoped.
            assert "channel=" not in q
            return httpx.Response(200, json={"value": {"DevInfo": {"model": "RLC-810A"}}})
        return httpx.Response(404)

    _mock_httpx(monkeypatch, handler)
    data = await cameras.fetch_reolink_dev_info(_REOLINK)
    assert data == {"value": {"DevInfo": {"model": "RLC-810A"}}}
...
async def test_add_reolink_camera_verify_ok_on_good_login(monkeypatch):
    ...
    async def fake_dev_info(entry, timeout=6.0):
        return {}  # a plain camera with no AI/doorbell info exposed

    monkeypatch.setattr(cameras, "fetch_reolink_snapshot", fake_fetch)
    monkeypatch.setattr(cameras, "fetch_reolink_dev_info", fake_dev_info)
    saved = {}
    ...
    assert saved["streamdeck_cameras"][0]["device_type"] == "camera"
    assert saved["streamdeck_cameras"][0]["two_way_talk"] is False


@pytest.mark.anyio
async def test_add_reolink_camera_flags_doorbell_and_two_way_talk(monkeypatch):
    # FoodAssistant-qft4: a Reolink doorbell's GetDevInfo names the model, which
    # flags the stored entry so the setup pane can show "Doorbell" /
    # "Two-way talk capable" and the visitor detection (a "visitor" AI state
    # category, see camera_detect.reolink_ai_detections) can pop up like any
    # other detection.
    from app.routers import setup as setup_router
    ...
    async def fake_dev_info(entry, timeout=6.0):
        return {"value": {"DevInfo": {"type": "wifi_doorbell", "model": "Reolink Video Doorbell WiFi"}}}
    ...
    monkeypatch.setattr(settings, "streamdeck_cameras", [], raising=False)
    payload = setup_router.ReolinkAddPayload(
        name="Front Doorbell", host="192.168.1.61", username="admin", password="s3cret")
    result = await setup_router.add_reolink_camera(payload)
    assert result["ok"] is True
    assert result["camera"]["device_type"] == "doorbell"
    assert result["camera"]["two_way_talk"] is True
    assert saved["streamdeck_cameras"][0]["device_type"] == "doorbell"


@pytest.mark.anyio
async def test_add_reolink_camera_capability_probe_failure_never_blocks_add(monkeypatch):
    # A best-effort probe: an unreachable/erroring GetDevInfo must never stop the
    # camera from being added.
    from app.routers import setup as setup_router
    ...
    monkeypatch.setattr(settings, "streamdeck_cameras", [], raising=False)
    payload = setup_router.ReolinkAddPayload(
        host="192.168.1.62", username="admin", password="s3cret")
    result = await setup_router.add_reolink_camera(payload)
    assert result["ok"] is True
    assert "device_type" not in result["camera"]
...
def test_reolink_panel_exposes_preview_control_wired_to_endpoint():
    # The Add a Reolink camera panel offers a Preview button, and the JS wires it
    # to the credential-safe preview endpoint via a POST body (FoodAssistant-26mf).
    from pathlib import Path
    root = Path(__file__).resolve().parent.parent / "service" / "app"
    html = (root / "templates" / "setup" / "_pane_connections.html").read_text()
    assert "_camPreviewReolink()" in html
    js = (root / "static" / "js" / "setup" / "cameras-ha.js").read_text()
    assert "function _camPreviewReolink" in js
    # The preview posts to the endpoint; the login never rides a query URL.
    assert "setup/cameras/reolink/preview" in js
    assert "postBody: body" in js


@pytest.mark.anyio
async def test_preview_reolink_camera_returns_image_on_good_login(monkeypatch):
    # Preview signs in with the token flow and streams the still bytes back,
    # without saving anything (FoodAssistant-26mf).
    from app.routers import setup as setup_router
    ...
    payload = setup_router.ReolinkAddPayload(
        name="Doorbell", host="192.168.1.60", username="admin", password="s3cret")
    resp = await setup_router.preview_reolink_camera(payload)
    assert resp.status_code == 200
    assert resp.body == _JPEG
    assert resp.media_type.startswith("image/")
    # Nothing is saved by a preview, and the password never appears in the reply.
    assert saved == {}
    assert b"s3cret" not in resp.body


@pytest.mark.anyio
async def test_preview_reolink_camera_rejects_bad_login(monkeypatch):
    from app.routers import setup as setup_router
    ...
    assert "rejected that username or password" in body
    # The friendly failure never echoes the login back.
    assert "wrong" not in body


@pytest.mark.anyio
async def test_preview_reolink_camera_reports_unreachable(monkeypatch):
    from app.routers import setup as setup_router
    ...
    monkeypatch.setattr(cameras, "fetch_reolink_snapshot", fake_fetch)
    payload = setup_router.ReolinkAddPayload(
        host="192.168.1.60", username="admin", password="s3cret")
    resp = await setup_router.preview_reolink_camera(payload)
    assert resp.status_code == 502
    body = resp.body.decode()
    assert "Could not reach the camera" in body
    assert "s3cret" not in body


@pytest.mark.anyio
async def test_preview_reolink_camera_reports_no_snapshot(monkeypatch):
    from app.routers import setup as setup_router
    ...
    resp = await setup_router.preview_reolink_camera(payload)
    assert resp.status_code == 502
    assert "did not return a snapshot" in resp.body.decode()
