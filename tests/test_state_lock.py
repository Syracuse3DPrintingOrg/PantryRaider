...
def test_accumulating_state_writers_hold_the_cross_process_lock():
    """Every service that READ-MODIFY-WRITES a shared state file must hold the
    cross-process lock, not just its own in-process one.

    A server runs several uvicorn workers, so an in-process lock coordinates
    nothing between them: two workers each load the file, each apply their own
    change, and the second write silently drops the first. os.replace keeps a
    reader from seeing a torn file but does nothing about a lost update
    (FoodAssistant-dxl0). Services that replace a file wholesale, like
    community_expiry's feed refresh, are safe on os.replace alone and are
    deliberately not listed here.
    """
    services = Path(__file__).parent.parent / "service" / "app" / "services"
    accumulating = (
        "timers.py", "current_recipe.py", "scanner_mode.py", "audit.py",
        "ha_events.py", "gadgets.py", "scan_session.py",
        "best_by_provenance.py", "expiry_learning.py",
    )
    missing = [n for n in accumulating
               if "state_write_lock" not in (services / n).read_text()]
    assert not missing, (
        f"these accumulate shared state without the cross-process lock: {missing}. "
        "Across uvicorn workers that silently loses one worker's update.")
