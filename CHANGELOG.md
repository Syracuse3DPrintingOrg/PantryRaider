...
- **A satellite can ask to join, no key-copying needed.** Setting up a Pi Remote used to mean creating an API key on the main server and pasting it into the new device. Now the wizard's Main Server step has a Request access button: the new device shows a short code, the same code pops up on the server, and you approve the match under Settings, Devices. The server issues the device its own named key automatically, so it can still be renamed or revoked one device at a time. Requests only work from your own network, expire after five minutes, and the whole feature can be turned off in Settings, Security & Access.
...
- **The device helper stops trusting unsigned local requests once it can.** The root helper that applies updates, reboots, and restores on a Pi used to allow those actions from any local process without the shared token, as a safety net for older software. It now switches to requiring the token automatically the moment it sees the app present it, so an up-to-date device closes that gap on its own with no risk to the update path, and its token file is no longer readable by other local accounts.

- **A Content-Security-Policy now guards every page.** The app tells the browser to only run scripts and styles it serves itself, blocking whole classes of injection attacks. Verified against every page in the same browser engine the kiosk runs, with no breakage; external images for the photo screensaver keep working.
- **The sign-in cookie is marked Secure over HTTPS.** When you reach the app over a secure connection, the browser is now told never to resend the session cookie over plain HTTP. Sign-in on the local network over HTTP keeps working exactly as before.
- **New installs no longer carry a master API key.** The Security & Access page shows the legacy primary key only on installs that already have one; everything new uses per-device named keys, so revoking one device never disturbs the others.
...
- **The inventory service starts during installation, not during setup.** Installation now waits for the inventory service to be ready (with a progress line), so on a slow connection the one-time download happens up front instead of leaving the setup wizard stuck. If it is still finishing when you open setup, the wizard shows clear live progress and there is no button to guess about, it completes on its own.
...
- **Mealie is tucked under Advanced in the setup wizard.** Recipes, the meal plan, and the shopping list are built into Pantry Raider, so the Optional Integrations step no longer leads with a Mealie card. Already run Mealie? It is still right there on the same step under "Advanced: connect a Mealie you already run", and in Settings any time.

### Fixed

- **Touch scrolling truly works on kiosk pages now.** The earlier scroll fix helped scaled displays but missed the real culprit: a style rule meant to stop scroll-bouncing made the kiosk browser swallow every vertical flick outright. Found by driving the page with simulated touches in the same browser engine the kiosk runs; a flick now scrolls every page at every display scale.
- **No more splash screen between menus on a kiosk.** Switching pages from the Stream Deck could flash white, then black, then replay the boot animation. Two causes fixed: the boot animation could mistake the first navigation for the first boot (it now plays only at actual startup), and the kiosk browser painted white while each page loaded (it now paints the app's dark background instead). Already-installed devices pick the browser fix up with their next update.
- **The kiosk display no longer cuts off the right side at larger UI scales.** With the display scale set above Normal, pages laid out wider than the screen and the right edge was clipped out of reach. The page now lays out to fit the panel exactly at every scale, so nothing hangs off-screen.
- **Touch scrolling works on every kiosk page.** A finger flick on pages like Timers or Inventory could do nothing at all on a scaled kiosk display, while the settings screen scrolled fine. The same fix that keeps pages inside the screen restores normal touch scrolling, and taps respond a touch quicker too.
- **The Timers page fits a kitchen glance.** Running timers and thermometer readings now sit at the top of the page, with the start-a-timer buttons below them, so what is cooking is the first thing you see. On a kiosk touchscreen the preset buttons are more compact and the big readouts a bit smaller, so timers and temperatures fit a small panel without scrolling.
- **Finishing setup no longer loops back to the first page.** Completing the wizard while the inventory service was still getting its connection key sent you back to page 1 and started over. Setup now completes as soon as your password and choices are saved; the inventory connects itself in the background moments later, and the summary says so instead of asking you to add a key.
- **The setup QR code loads before setup is complete.** The code on the kiosk splash was requested before the app considered itself set up, so it rendered as a broken image. It now always answers.
- **Turning on remote access during setup no longer errors.** The Forager remote-access step could show "Unexpected token '<'" when used from the wizard; it now returns a proper answer.
- **"Continue with Google" now shows during first-time setup.** The Google sign-in button on the wizard's Forager step could stay hidden even when your Forager account offered it, because an unrelated hiccup earlier on the page stopped the check that reveals it. The button now appears whenever Google sign-in is available.
...
- **No more on-screen keyboard on your computer.** On a Pi, the touch keyboard belongs to the device's own screen, yet a phone or computer browsing the same device could end up with it popping over every text field, most annoyingly during setup. Kiosk mode now stays on the appliance's attached display; other browsers shed it automatically on their next visit. Turning kiosk mode on from the menu on purpose still works anywhere.
- **The kiosk screen waits for the install to finish.** On a brand-new device the attached screen used to show the setup page while the installation was still downloading and building in the background, which made setup look ready when it was not. The screen now comes up only after installation completes and the app is answering, and on later boots it appears as soon as the app is up, with no added delay.
