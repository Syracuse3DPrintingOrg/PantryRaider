...
# A keymap slot can also hold a "nav:<page>" or "timer:<op>" action string
# (FoodAssistant-bo9v): a press then opens a kiosk page or runs one timer
# operation instead of selecting a mode. These are the app's tables
# (service/app/services/stemma.py NAV_PAGES / TIMER_KEY_ACTIONS), mirrored so
# both sides normalize a pulled keymap identically; tests/test_stemma.py
# asserts the pair agree. The agent never resolves what an action means, it
# only validates the string and posts it, so the key set is all it needs.
NAV_PREFIX = "nav:"
TIMER_PREFIX = "timer:"
NAV_PAGES = ("start", "inventory", "expiring", "add", "pending", "cook",
             "recipes", "mealplan", "shopping", "guide", "timers", "weather",
             "camera")
TIMER_ACTIONS = ("start5", "add5", "dismiss")

# What a mapped-but-inactive key shows: its color at this fraction of the
# brightness, matching the app's IDLE_LED_SCALE.
IDLE_LED_SCALE = 0.12

# What a key mapped to a page or timer action wears: a white glow at the idle
# scale, so the key is visibly mapped without joining the mode colors. The
# app owns this too (stemma.ACTION_GLOW, shipped in the outputs palette under
# "action"); this is the fallback for when the app has not answered yet.
ACTION_GLOW = (255, 255, 255)
...
def normalize_key_action(value) -> str:
    ...
    Mirrors the app's stemma.normalize_key_action exactly (the tests pin the
    pair): a bare scanner mode passes unchanged, "nav:<page>" and
    "timer:<op>" pass when the page or op is in the mirrored tables, and
    anything else becomes "" so a typo never makes a key do a real thing.
    """
    action = str(value or "").strip().lower()
    if action in SCANNER_MODES:
        return action
    if action.startswith(NAV_PREFIX) and action[len(NAV_PREFIX):] in NAV_PAGES:
        return action
    if (action.startswith(TIMER_PREFIX)
            and action[len(TIMER_PREFIX):] in TIMER_ACTIONS):
        return action
    return ""
...
def led_colors(keymap, active_mode: str, brightness: int = 40,
    ...
    in the dark, unmapped keys off. A key mapped to a page or timer action
    wears the dim white ACTION_GLOW: momentary actions have no "active"
    state, but a mapped key must still be findable in the dark. ``palette``
    lets the outputs poll hand in the app's colors (the action glow rides it
    under "action"); without it the built-in tables are used.
    """
    ...
    for action in keys:
        if not action:
            out.append((0, 0, 0))
            continue
        if action not in SCANNER_MODES:
            base = tuple(colors.get("action") or ACTION_GLOW)
            out.append(scale_color(base, level * IDLE_LED_SCALE))
            continue
        base = tuple(colors.get(action) or MODE_COLORS.get(action) or (0, 0, 0))
        out.append(scale_color(base, level if action == active
