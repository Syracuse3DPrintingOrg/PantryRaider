...
class I2CModule:
    ...
    def _press(self, dev_id: str, dev: dict, key: int, now: float) -> None:
        """Turn one key-down into the slot's action.

        A slot holds an action string: a scanner mode (the original and
        common case), a "nav:<page>" jump, or a "timer:<op>" operation. The
        agent does not interpret the new forms, it hands the string to the
        daemon to post; only a mode press repaints the LEDs optimistically,
        because only a mode has a lit-key state to move.
        """
        keymap = neokey.normalize_keymap(
            (dev.get("options") or {}).get("keymap"))
        action = keymap[key] if key < len(keymap) else ""
        if not action:
            log.debug("NeoKey %s key %s is mapped to nothing; ignoring",
                      dev_id, key + 1)
            return
        if action in neokey.SCANNER_MODES:
            # Optimistic: the LEDs move now, not when the server answers, so
            # the key feels instant even if the app is briefly slow.
            with self._lock:
            ...
            log.info("NeoKey %s key %s fires %s", dev_id, key + 1, action)
        self._dispatch(self._on_press, action)
...
def press_request(action: str) -> tuple[str, dict]:
    """Which app endpoint a pressed key's action string posts to, and the
    JSON body. Pure, so the routing is a testable truth table.

    A plain scanner mode posts to /pending/scanner-mode exactly as it always
    has (backward compatible on the wire in both directions: an older app
    never sees the new endpoint for a mode, and an older keymap never
    produces anything else). Every other action string goes to
    /gadgets/key-action verbatim: the agent stays dumb about what the string
    means, the app decides.
    """
    action = str(action or "").strip().lower()
    if action in neokey.SCANNER_MODES:
        return "/pending/scanner-mode", {"mode": action, "source": "neokey"}
    return "/gadgets/key-action", {"action": action, "source": "neokey"}
