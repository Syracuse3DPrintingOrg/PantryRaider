...
class I2CModule:
    ...
    def apply_outputs(self, data) -> None:
        ...
        with self._lock:
            ...
            # A press we just made wins over an outputs answer that predates
            # it: the poll may well have been in flight when the key went
            # down, and snapping the LED back would look like a dropped
            # press. The next poll agrees with us or corrects us.
            if mode and time.time() - self._mode_from_press > 3.0:
                self._mode = mode
