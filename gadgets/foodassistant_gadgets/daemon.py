...
# (FoodAssistant-kh1m). A key press repaints them immediately, so this only has
# to catch a mode changed on ANOTHER surface, and that turned out to be very
# visible: tapping a mode tile on the screen left the lit key stale for up to a
# couple of seconds, which reads as the pad ignoring you. /gadgets/outputs is a
# cheap local read (~35ms measured on a Pi 4), so half a second keeps the lit key
# in step with the screen while still costing almost nothing.
_OUTPUTS_POLL_SECONDS = 0.5
...
# How long the same barcode is ignored after it was last seen. Both engines
# re-read a code that sits in view (command-mode polling on the GM engine,
# continuous auto-sense on the SEENGREAT), so the window slides: every repeat
# sighting refreshes it, and a code parked in front of the scanner posts once,
# not once per window. Pull the code away for this long and it counts again.
...
class Daemon:
    def __init__(self, cfg: Config):
        ...
        # The UART barcode scanner (FoodAssistant-x61t): a scan engine (a GM
        # 3.3V module, or a level-shifted 5V SEENGREAT in passive mode) wired
        # to the Pi's GPIO UART, read here and POSTed to /pending/scan.
    ...
    def _on_stemma_press(self, action: str) -> None:
        """A NeoKey key was pressed: post its action string, now.
        ...
        buttons make. Where it lands is press_request's pure decision: a
        scanner mode goes to the scanner-mode API exactly as it always has
        (state file synced across workers, satellite forwarding for free),
        and any other action string goes verbatim to /gadgets/key-action for
        the app to interpret. The agent never knows what a nav or timer
        action means, which is what keeps it dumb and forward compatible.
        """
        from .i2c.module import press_request
        path, payload = press_request(action)
        ...
        async def _send() -> None:
            try:
                ...
                log.warning("Could not deliver a key press to the app: %s", exc)
        ...
        try:
            while True:
                ...
                if not self._scan_active:
                    # Enabled but no session: keep it dark, send nothing. A
                    # passive engine keeps emitting regardless; throw those
                    # bytes away so a code scanned an hour ago cannot flood
                    # the next session's first read.
                    await asyncio.to_thread(scanner.discard_pending)
                ...
                for code in codes:
                    ...
                    # repeats before they hit the app. Refreshing the
                    # timestamp on every sighting makes the window sliding: a
                    # parked code posts once, and again only after it has
