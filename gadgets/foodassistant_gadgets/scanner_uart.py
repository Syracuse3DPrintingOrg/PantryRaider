...
A scan engine wired to the Pi's GPIO UART (TX/RX on GPIO14/15, physical pins
8/10). Two engine families are supported:

* The Waveshare/GM engine: 3.3V supply and 3.3V logic, wired directly, and it
  speaks the GM serial command protocol below, so the agent can hold it dark
  in command mode and trigger each scan itself.
* The SEENGREAT engine (seengreat.com wiki 109): 5V supply and 5V-TTL serial,
  so its TX must be level-shifted down before the Pi's RX (the Pi is not 5V
  tolerant). Its serial trigger protocol is undocumented and it ignores the
  GM frames, so it runs passively: the vendor's config barcodes put it in
  serial-output plus continuous (auto-sense) mode, it emits each decoded
  barcode as plain ASCII on its own, and the agent just reads.

Either way, the agent reads decoded barcodes off the serial port and POSTs
them to the app on ``/pending/scan``, the same endpoint the kiosk's on-screen
scanner uses, so a scan lands in the Review queue no matter which surface
caught it.

A GM engine stays dark and idle. On startup the agent puts it in command mode
...
and the module goes dark again: no stray scans, no light in a dark kitchen. A
passive SEENGREAT engine keeps scanning on its own instead; the daemon reads
it only while a session is active and discards what it emitted in between.
...
# How many extra read-timeout windows scan() may wait for a code that is still
# arriving. A passive engine emits on its own clock, so a code can straddle the
# read window; each extension is one read_timeout window (0.2s by default), and
# the cap keeps a gapless stream from pinning the reader thread here (16
# windows is about 3 seconds).
_MAX_WINDOW_EXTENSIONS = 16
...
class SerialScanner:
    ...
    def configure(self) -> bool:
        ...
        try:
            ...
            self._ser.flush()
            # On a GM engine this read swallows the 7-byte write ack so it
            # does not lead the first scan. A passive engine never acks, so
            # anything read here is real barcode data: hold it for the next
            # scan's parse instead of discarding it (a real ack is stripped
            # there anyway).
            self._leftover += self._ser.read(len(ACK))
    ...
    def discard_pending(self) -> None:
        """Throw away anything received while nobody was listening.

        A passive engine in continuous mode keeps decoding whether or not a
        scan session is open; without this, a code scanned while no session
        was active would sit in the OS serial buffer and flood the next
        session's first read. The daemon calls it while enabled but dark.
        Never raises: a dead port surfaces on the next scan instead.
        """
        self._leftover = b""
        ...
        try:
            self._ser.reset_input_buffer()
        except Exception:  # noqa: BLE001 - best effort; scan() reports a dead port
            pass

    def scan(self) -> list[str]:
        """Ask for a scan and return any decoded barcodes.

        Sends the GM trigger command, then reads whatever the module emits
        within the read window. A passive engine (SEENGREAT) ignores the
        trigger and emits codes on its own schedule; sending it anyway costs
        nine bytes and saves a which-engine setting nobody should have to
        set. The read never depends on a reply: it is a plain timeout window,
        extended while the buffer still ends mid-code so a code arriving
        across the window boundary is never cut in half. Raises
        ``ScannerUnavailable`` if the port dies; the daemon catches that and
        ...
        try:
            ...
            # While the buffer ends inside a printable run (a code with no
            # terminator behind it yet), give the module one more timeout
            # window to finish it; a window with no bytes at all is a real
            # gap, and the code is complete. Bounded so a gapless stream
            # cannot pin the worker thread here.
            for _ in range(_MAX_WINDOW_EXTENSIONS):
                if not buf or not 0x20 <= buf[-1] <= 0x7E:
                    break
        ...
        # A silent window delimits a code even without an end mark, so flush
        # the trailing run rather than holding it for a read that may never
        # come.
        codes, self._leftover = parse_stream(buf, flush=True)
        return codes
