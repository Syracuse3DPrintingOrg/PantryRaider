...
A Cub built from these files keeps its own firmware current. It checks the
server it paired with, at the address it actually found, and installs a newer
firmware when the screen is not busy (no ringing timer, no alarm, no pairing
code). The server's **Automatic Cub updates** setting decides whether it may,
fleet-wide or per Cub, so there is nothing to configure here.

Two pieces make it work, and a custom YAML needs both or neither:

```yaml
update:
  - platform: http_request
    id: cub_firmware
    source: http://pr.local:9284/cub/firmware/manifest.json  # replaced at runtime
```

and, at boot, handing the hub a way to retarget that entity:
...
The hub never names the update entity itself, so leaving both out is fine:
`cub-custom.example.yaml` does exactly that and builds. See
`packages/cub-base.yaml` for the install gate that goes with them.

To check a fixed address instead of whichever server the Cub found, set the
hub's `ota_manifest_url` option (the `ota_manifest_url` substitution on the
stock builds). Empty, the default, means follow the server.
