...
void PantryRaiderHub::setup() {
  ...
  if (this->transport_ == CUB_TRANSPORT_BLE) {
    // Receive-only, and enforced rather than merely intended. Setting the
    // pairing state was not enough on its own: load_key_() and load_server_()
    // have already run by this point, so a Cub reflashed from LAN mode still
    // carried a key and a remembered address in flash, stayed paired() (which
    // is only "we hold some key"), and still let the firmware check follow
    // that remembered server. Clearing both here is what actually makes the
    // sentence above true, on a device with a past life as well as a new one.
    this->api_key_.clear();
    this->server_.clear();
    this->server_is_explicit_ = false;
    this->pairing_state_ = PAIRING_IDLE;
  }
#endif
  // A pinned or remembered server means we already know where firmware comes
  // from, so the first check after boot can go to the right place. A
  // receive-only Cub has no server by construction, so this pushes nothing
  // and no firmware check ever leaves the device.
  this->push_ota_url_();
}
