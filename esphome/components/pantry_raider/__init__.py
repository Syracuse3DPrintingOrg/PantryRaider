...
# Part of Pantry Raider's Bandit Cub firmware; see ../../NOTICE.md for
# why this folder is GPL-3.0 rather than the repository's PolyForm license.
...
_BASE_SCHEMA = cv.Schema(
    {
        ...
        # Where firmware updates are checked. Empty (the default) means
        # follow the server this Cub found, which is the only address that
        # can be right on a LAN where the server is a Docker container with
        # no mDNS name. Set a URL to pin the check somewhere fixed.
        cv.Optional(CONF_OTA_MANIFEST_URL, default=""): cv.Any(
            cv.one_of(""), cv.url
        ),
    }
...
async def to_code(config):
    ...
    if config[CONF_OTA_MANIFEST_URL]:
        cg.add(var.set_ota_manifest_url(config[CONF_OTA_MANIFEST_URL]))
