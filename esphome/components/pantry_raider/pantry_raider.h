...
// Part of Pantry Raider's Bandit Cub firmware. Built with ESPHome,
// whose C++ runtime is GPL-3.0, so this file is GPL-3.0-or-later and
// not under the repository's PolyForm license. See ../../NOTICE.md.
...
struct CubSettings {
  ...
  // The server's cub_auto_update: may this Cub install new firmware by
  // itself? Defaults on, matching the server, so a summary from an older
  // server (which sends no such key) still keeps Cubs current.
...
#endif
{
 public:
  ...
  // A fixed firmware manifest URL from YAML. Empty (the default) means follow
  // whichever server this Cub is actually talking to.
  void set_ota_manifest_url(const std::string &url) { this->ota_manifest_url_ = url; }
#ifdef PR_USE_BLE_RELAY
  ...
  //
  // The update entity is optional: a hand-written YAML may leave it out, and
  // a BLE-only Cub never has a server to fetch from. So the hub never names
  // that component. It hands out a URL through a callback instead, and does
  // nothing at all when nobody set one. packages/cub-base.yaml wires the
  // callback to the stock http_request update entity at boot.
  //
  // The callback is handed the manifest URL for the server this Cub found,
  // and is called again whenever that address changes. Setting one pushes the
  // current URL straight away, if there is one to push.
  void set_ota_url_callback(std::function<void(const std::string &)> cb) {
    this->ota_url_cb_ = std::move(cb);
    this->push_ota_url_();
  }
  // Whether the server says this Cub may update itself.
  bool ota_auto_update() const { return this->state_.settings.auto_update; }
  // Nobody is being interrupted right now: no timer ringing, no alarm on
  // screen, no pairing code waiting to be typed in.
  bool ota_quiet() const;
  // The whole decision for the install gate in cub-base.yaml: a live server
  // that allows updates, a version worth moving to, and a quiet kitchen.
  // False simply means "not now": the gate asks again a few minutes later.
  bool ota_install_ready(const std::string &latest_version) const;
  // The gate says so right before it flashes. A successful install reboots
  // into the new firmware and none of this matters; an install that fails
  // leaves the update on offer, and without this the Cub would retry it every
  // few minutes forever. One try per version until the cooldown is up, and a
  // newer version published in the meantime is never made to wait.
  void ota_mark_attempt(const std::string &version);

 protected:
  std::string base_url_() const;
  // The manifest URL to check: the YAML override when set, else the server
  // this Cub is talking to. Empty when there is no server yet.
  std::string ota_url_() const;
  // Hand the current URL to the callback, if it changed since the last one.
  void push_ota_url_();
  ...
  std::string ota_manifest_url_;  // YAML override; empty means follow the server
  std::string ota_url_sent_;      // last URL handed to the callback
  std::function<void(const std::string &)> ota_url_cb_{};
  std::string ota_tried_version_;  // last version handed to the flasher
  uint32_t ota_tried_ms_{0};       // millis() when it was tried
