...
- **Kitchen kiosk and Stream Deck.** A dedicated countertop control surface
  with a Glance home screen that builds itself from your pages, plus an
  optional custom Start Page that works like an on-screen Stream Deck.
- **Home Assistant integration.** A native HACS integration that pairs from the
  kitchen screen and exposes your food counts, timers, thermometer readings, and
  screen controls as entities, with on-screen notifications and camera pop-ups.
...
- **Label and document printing.** Print food and spice labels with a
  drag-and-drop label designer, set up a Bluetooth label printer right from
  Settings, and send a recipe or document to a regular printer.
- **Bluetooth kitchen thermometers.** Read meat and probe thermometers,
  including the ThermoPro TempSpike and Govee grill thermometers, locally,
  with doneness presets, a ready-in estimate, and target alerts on the
  Timers page.

## Language and units

The interface is English only for now, and measurements default to US units
(Fahrenheit for weather and thermometers). The app starts on US Eastern time.
Units and the timezone are both yours to change in Settings, and a Raspberry
Pi appliance picks up the timezone from the device during setup.
...
- [Reducing waste: extending and adjusting best-by dates](reducing-waste.md):
  the sniff test on the Expiring list, tossing what went bad and watching the
  Waste summary, marking a package opened, and how moving food to the freezer
  shifts its date for you.
...
- [Personalization and on-screen features](personalization.md): themes,
  background images, the navigation editor, the Glance home and Start Page,
  the screensaver (including your own photos), shared kitchen timers, weather,
  and on-screen Home Assistant events.
- [Remote access](remote-access.md): reaching your kitchen from outside the
  house, what a Forager plan includes, and how the tunnel works.
- [Using your phone's camera](phone-camera.md): the QR handoff, scanning
  barcodes, and photographing groceries and receipts.
