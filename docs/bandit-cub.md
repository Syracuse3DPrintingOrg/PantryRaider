...
Updates travel one way: the Cub pulls firmware from the server it is paired
with, and nothing on the network can push firmware to it. That is worth saying
plainly because the usual ESPHome setup allows the push, and a push nobody has
to authorise is a way to run any code at all on the device. Cubs flashed from
this page do not offer it.
