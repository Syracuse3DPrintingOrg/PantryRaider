...
- **Server found, waiting for you**: the 4 digit pairing code, which stays
  up until you approve it in Settings, Devices.
...
- It is **off by default** on both ends. The broadcast is the
  `cub_ble_advertise` setting (a Settings toggle is coming; until then set
  it in `settings.json` or the environment), and the Cub must be flashed
  from the Bluetooth-receiver build (`cub-tdisplay-ble.yaml` in the
  `esphome/` folder); the standard builds do not listen.
...
buttons near a Cub appear in Settings, Thermometers on the server, in the
