...
- **Just the app** (single container): you already run Grocy, or you want to
  install Grocy from Community Applications yourself and point Pantry Raider
  at it. This is the simplest route and the one most people want.
- **The whole set** (Compose stack): install Pantry Raider and Grocy together
  in one step with the Docker Compose Manager plugin (an optional Mealie can
  ride along for people who already use it).

Either way, Pantry Raider uses port **9284** and Grocy uses **9383** (an
optional Mealie would use **9285**).
...
3. Fill in the fields:
   ...
   - **Grocy Address**: the address of your Grocy, for example
     `http://192.168.1.50:9383`. No trailing slash. If you do not have Grocy
   ...
   - **UI Password**: set one here, or leave it blank and set it during the
     on-screen setup.
   - The advanced fields (timezone, API key, and a Mealie address and key if
     you already use Mealie) are optional. Show them with the **Advanced
     View** toggle.
...
Use this if you want Pantry Raider to bring its own Grocy along, all managed
together.
...
4. Paste in the compose file from the project:
   ...
   It stores all data under `/mnt/user/appdata` and uses the same pinned
   backend versions the project ships. Recipes and meal plans are built in;
   the optional Mealie block starts out commented, for people who already use
   Mealie.
5. Save, then click **Compose Up**.
6. Open `http://YOUR-UNRAID-IP:9284/setup`. Enter the Grocy address
   `http://YOUR-UNRAID-IP:9383` (and `http://YOUR-UNRAID-IP:9285` if you
   enabled the optional Mealie).

## Which Grocy?

Grocy has its own Community Applications template already, so you can install
it from the **Apps** tab and just point Pantry Raider at it. The Compose stack
above is only for people who would rather run everything from one place.
...
- **Compose stack**: pull the new images and run **Compose Up** again, or pin a
  version by setting `PANTRYRAIDER_TAG` (or the older `FOODASSISTANT_TAG`) in a
  `.env` next to the compose file.

### Grocy updated and inventory stopped loading

If Grocy updated itself (the Community Applications Grocy follows `:latest`)
and Pantry Raider's inventory banner says Grocy "reported a problem with its
own setup" about `AUTH_CLASS`, Grocy 4.7 moved a class that your saved
`config.php` still points at. Back up `config.php` in Grocy's data folder
(`/mnt/user/appdata/grocy/data` for the CA template), change its `AUTH_CLASS`
line to `Grocy\Middleware\Auth\DefaultAuthMiddleware`, and press Refresh; no
restart needed. The full write-up, with a one-line command, is under
[Grocy updated and inventory stopped loading](platforms.md#grocy-updated-and-inventory-stopped-loading).
The compose stack above can fix this on its own next time: step 4b in the
compose file drops a start-up script into Grocy that repairs the line.
