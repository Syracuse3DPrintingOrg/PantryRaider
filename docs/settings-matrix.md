...
Mealie is optional. If you run one and it still answers to its stock sign-in,
Pantry Raider can set it up for you: it creates its own API token, secures
the account with a generated password (stored in `mealie_admin_password`,
revealed from the Connections pane), and adds a Groceries shopping list.
...
| `recipes_backend` | | Device-local | Device-local | Device-local |
