...
`kiosk_auto_home_enabled` is "Return to home when idle" (Settings, Screen):
...
Bluetooth reader, with their own section in Settings, Thermometers and their
...
| `cub_ble_advertise` | | Saved (no UI yet) | Saved (no UI yet) | Not shown |
...
displays in radio range. It only takes effect where a Bluetooth radio and the
reader exist, in practice a Pi appliance; a Docker server saves the setting
but has nothing to broadcast with. The settings-pane toggle lands with the
Devices pane rework.
