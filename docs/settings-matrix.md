...
signed-in user confirms the codes match under Settings, Devices. Requests are
...
instead. `label_width_in`, `label_height_in`, `label_dpi`, `label_layout_json`
(the saved design), `label_layout_presets` (named saved designs),
`label_show_logo`, `label_shape` (rectangle, square, or round stock), and the
`document_page_size` / `document_color_mode` /
`document_duplex` document-printer options all describe the printer's label
...
designer sections entirely on a Pi Remote, which prints through the server's
choices.
...
| `label_width_in` | | Editable | Editable | Not shown (server-only) |
| `label_height_in` | | Editable | Editable | Not shown (server-only) |
| `label_dpi` | | Editable | Editable | Not shown (server-only) |
| `label_shape` | | Editable | Editable | Not shown (server-only) |
| `label_layout_json` | | Editable | Editable | Not shown (server-only) |
| `label_layout_presets` | | Editable | Editable | Not shown (server-only) |
| `label_show_logo` | | Editable | Editable | Not shown (server-only) |
