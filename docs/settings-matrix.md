...
| `kiosk_split_enabled` | | Editable | Editable | Device-local |
| `kiosk_split_primary` | | Editable | Editable | Device-local |
| `kiosk_split_secondary` | | Editable | Editable | Device-local |
| `kiosk_split_lock_primary` | | Editable | Editable | Device-local |
...
The Recovery hotspot card (Settings, Network) is Pi-only and its two values
(the hotspot name and password) live on the device host, not in the settings
store: the app writes them through the host bridge (`POST /ap/config`), which
lands them in `/etc/hostapd/hostapd.conf`, `/etc/foodassistant/ap-ssid`,
`/etc/foodassistant/ap-passphrase`, and the boot-partition
`pantry-raider-hotspot.txt` mirror. They never sync between devices; each
appliance's hotspot is its own.
...
`kiosk_split_enabled` is the split screen (Settings, Display & Sleep): the
display shows two app pages at once, side by side on a wide panel and stacked
on a tall one, made for ultrawide and tall bar panels (a Waveshare 480x1920,
for example). `kiosk_split_primary` and `kiosk_split_secondary` pick the page
each pane starts on, and `kiosk_split_lock_primary` keeps the first pane on
its page (Glance as an always-on status panel is the natural pairing) while
the second pane browses the whole app normally; navigation inside a pane
stays inside that pane. When the split is on, a display that starts in kiosk
mode and lands on Glance opens the split view automatically, and it is always
available directly at `/ui/split`. All four stay device-local: the split
exists because of the panel's shape, and every panel differs.
...
- **The Bluetooth reader.** The radio belongs to the host, not the app
  ...
  one click (Set up for me, through the host bridge, offered from the Probes
  and Accessories sections alike); it also installs itself at provision time
  when a plug-in accessory is detected (`ENABLE_GADGETS=auto`, the default;
  force with `true`/`false`) or by hand
...
its four key assignments, each a scanner mode, a page to open, or a timer
action, plus its LED brightness). Both are device-local
...
`cub_auto_update` (on by default) changes nothing for a Cub flashed from this
release. Cubs are display-only for the beta and update by being plugged in and
flashed, so no Cub asks this question. The setting stays because a build using
the older network transport does ask it before installing anything, and because
automatic updates come back for everyone once firmware images are signed.
Read the rest of this paragraph as describing that path, not today's. Per-Cub overrides work here like the rest of this section, so one
...
`cub_ble_relay` (off by default, and not available for the beta) is the
beacon's mirror image: instead of a
...
`cub_ble_advertise` (off by default) turns on the Bluetooth status beacon,
which is how every Cub gets its state now: a device running the thermometer
reader broadcasts the Cub summary's numbers (counts, soonest timer, one probe
temperature; never names) for any display in radio range. Leave it off and a
Cub has nothing to show. It takes a Bluetooth radio and the reader to
...
`main` (every change) or `stable` (releases only); `main` is the default, and
`stable` is the recommended choice for an everyday kitchen. `check_for_updates`
is device-local, unlike `auto_update`: it controls only
