...
printer keeps printing off no matter what the server does. `label_printer_queue`
and `document_printer_queue` are this device's own printer choices; when set they
always win. `label_width_in`, `label_height_in`, and `label_dpi` describe the
label stock (a 2x1 inch thermal label at 203 dpi by default), and stay
device-local so each device can match its own stock.

The main server picks a fleet default with `fleet_label_printer_queue` and
`fleet_document_printer_queue`. A Pi Remote inherits those as its default: it
prints to the fleet default unless it has set its own local queue, which then
wins for that device only. Inheriting never overwrites a device's own choice.
On the server and Pi Hosted the Printing pane shows the fleet-default selectors
(also what that device prints to); on a Pi Remote it shows the inherited fleet
default plus a local override.

| Setting | Secret | Server | Pi Hosted | Pi Remote |
| --- | --- | --- | --- | --- |
| `printing_enabled` | | Device-local | Device-local | Device-local |
| `label_printer_queue` | | Device-local | Device-local | Device-local (overrides fleet default) |
| `document_printer_queue` | | Device-local | Device-local | Device-local (overrides fleet default) |
...
Bluetooth kitchen thermometers (Inkbird, ThermoPro, Combustion, ThermoWorks
BlueDOT) show live probe temperatures on the Timers page and raise on-screen
alerts when a probe reaches its target. Settings, Thermometers is the
management surface in every mode (server, Pi Hosted, Pi Remote): the feature
toggle, the reader status, adding and removing thermometers, and the Home
Assistant source all live there; day-to-day temperatures and targets stay on
the Timers page.
...
- **The Bluetooth reader.** The radio belongs to the host, not the app
  ...
  reports to the app. On a Pi appliance the Thermometers pane sets it up in
...
added thermometers and their per-probe targets. `gadget_ha_enabled` turns the
Home Assistant source on (adding an entity does it for you) and
`gadget_ha_entities` lists the entity ids read as probes. All are
device-local: a thermometer lives near one device's radio, and the Home
Assistant entities are polled by the device that lists them.
