...
The Settings page is a single menu grouped under plain-language headings:
Kitchen, Devices, This Device, Connections, and System. Nothing is hidden
behind a toggle, and a search box above the menu filters it as you type and
jumps to the match. This page covers the appearance and on-screen features:
Appearance, Display & Sleep, the Start Page and Stream Deck editors, and
Recipe suggestions. Each section below says where its setting lives.
...
Settings, Appearance holds the tab editor; the on-screen
navigation bar lives under Display & Sleep.
...
Settings, Start Page. The home screen is served
...
Settings, Display & Sleep gathers everything about the screen:
...
- **Split screen** shows two app pages at once, each in its own pane: side by
  side on a wide panel, top and bottom on a tall one, a natural fit for
  bar-shaped panels like the Waveshare 480x1920. Pick the page each pane
  starts on, and lock the first pane to its page to keep it as an always-on
  status panel (Glance on one side, the full app on the other) while the
  second pane browses normally; anything you open there stays in that pane.
  A display that starts in kiosk mode and lands on Glance opens the split
  automatically once the option is on, and the screensaver still covers the
  whole screen, above both panes. A per-device choice, like the rest of this
  section.
...
brings the page right back. It is configured per device in Display & Sleep:
