...
A "Check for updates automatically" switch in Settings (Backups & Updates)
controls the background check above; it is on by default. Turn it off and the
app itself stops contacting GitHub on its own. The auto-update switch is
separate and only controls whether a found update is applied automatically.
Pressing the Check for updates button always runs a live, one-off check
regardless of either switch. If your install has no internet route, a check
simply fails quietly and everything else works.

**On a server install, one more thing checks on its own.** The Docker Compose
stack ships a small Watchtower container that keeps a plain server up to date,
because a server has no host bridge to run the one-button update a Raspberry Pi
appliance uses. Watchtower asks the GitHub container registry once a day
whether a newer Pantry Raider image exists, and pulls it when there is one. It
looks only at the Pantry Raider container, never at Grocy, Mealie, or Ollama.
That daily check runs on its own schedule and is not affected by the settings
switches above, which cover the app's own update notice. The registry sees the
same thing GitHub does: your network's public address and a generic client
signature, with no install id or account attached.

To stop it, either stop that one container:

```bash
docker compose stop watchtower
```

or pin a version in your `.env` so nothing newer is ever picked up:

```
PANTRYRAIDER_TAG=0.18.101
```

Raspberry Pi appliances do not run Watchtower at all.
