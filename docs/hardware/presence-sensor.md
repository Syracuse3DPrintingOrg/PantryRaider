...
(physical pin 11), and that pin is fixed: the appliance always reads presence
there, and no setting moves it. If another board on your Pi also uses GPIO17
(the SparkFun Qwiic pHAT v2.0's onboard button does), pick one or the other;
the [pin map](pinout-and-compatibility.md) covers the workarounds.
