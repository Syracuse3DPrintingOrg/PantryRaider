...
On a Raspberry Pi appliance you can also wire a scan engine straight to the serial
pins, no USB port used. A 3.3V Waveshare-style engine wires directly and stays dark
until a scan page is open. The **SEENGREAT Barcode Scanner Reader** is also supported,
with two caveats: it is a 5V module, so its data-out wire needs a level shifter or a
two-resistor divider before the Pi (the Pi's serial input is not 5V tolerant), and it
ships acting as a USB keyboard, so you must scan its "serial port output" and
"continuous scanning" configuration barcodes once before the serial pins say anything.
The [wired UART barcode scanner guide](uart-barcode-scanner.md) has the wiring and the
one-time setup for both.
