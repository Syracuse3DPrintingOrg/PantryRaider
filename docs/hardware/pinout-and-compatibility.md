...
| 2 | 5V | mmWave presence sensor VCC (LD2410C or LD2410D; a 3.3V-only LD2420 takes pin 1 instead) | Yes |
...
| 8 | GPIO14 (UART TX) | A wired UART barcode scanner's RX, if you fit one | Yes |
| 9 | GND | Ground for a STEMMA QT / Qwiic device wired by cable | Yes |
| 10 | GPIO15 (UART RX) | A wired UART barcode scanner's TX, if you fit one | Yes |
| 11 | GPIO17 | mmWave presence sensor OUT (wakes the display) | Yes |
...
block (physical pins 19, 21, 23, 24, and 26). And GPIO14 (pin 8) is both the UART
transmit pin, where a [wired UART barcode scanner](uart-barcode-scanner.md) takes
its data in, and where the official Pi 4 case fan expects its control wire, so a
wired scanner and that fan want the same pin; run the fan off the Pi 5 fan header
instead, or use a USB scanner.
...
Four routes, in the order most builds should consider them. The brand names mix
freely here: Adafruit lists SparkFun's Qwiic boards with "STEMMA QT" in the
title, and every board below carries either brand's devices, so shop whichever
catalog you are in.
...
**A plug-on QT socket: the Adafruit Pi Stemma QT breakout.** Adafruit's own board
(product 6365, a couple of dollars) is a 2x3 socket that plugs straight down onto
the six pins at the pin 1 corner (pins 1 through 6) and gives you one STEMMA QT
port, with no soldering. It brings its own 3.3V regulator, so it does not lean on
the Pi's small 3.3V rail, and a green LED confirms it is powered. Because it is a
socket rather than a thin wedge, a HAT cannot seat on the header while it is
fitted (that is the SHIM's trick, next), though jumper wires on pins 7 through 40
still can, and it pulls off in a second when you want the corner back. Like the
SHIM, it takes the corner that a header-powered DSI display's power and the
presence sensor's power and ground also want; see the matrix below.

**A Qwiic / STEMMA QT SHIM.** A thin board that friction-fits onto pins 1 through 6
and gives you one Qwiic port, with no soldering. Its trick is that it is thin
enough that a HAT still seats fully on the header on top of it, so a PoE HAT and a
Qwiic port can coexist. It draws from 5V and regulates its own 3.3V, so it does not
lean on the Pi's small 3.3V rail. The catch is mechanical: it occupies pins 1
through 6, which is where a header-powered DSI display's power jumpers and the
presence sensor's power and ground also want to be (a pogo-pin DSI panel does not
care; see [DSI displays](#dsi-displays) below). It is a SparkFun board (DEV-15794)
that Adafruit carried as the "Qwiic or STEMMA QT SHIM" (product 4463) but no
longer stocks; the Pi Stemma QT socket above is Adafruit's current answer for the
same job when nothing needs to stack on top. See the matrix below.

**A Qwiic / STEMMA QT HAT or pHAT.** A board that gives you four ports at once. It
is the tidiest option if the Pi has nothing else on its header, and the least
flexible if it does. Before you buy one, read the button note below.

### The Qwiic pHAT button and GPIO17

This is the note every other GPIO17 mention on this page points back to. The
four-port board currently sold is the SparkFun **Qwiic pHAT v2.0** (DEV-15945;
Adafruit carried it dual-branded as "STEMMA QT / Qwiic", product 5142, but no
longer stocks it), and it has a small onboard button that SparkFun wires to
**GPIO17, physical pin 11**. That is the pin Pantry Raider
reads the mmWave presence sensor on, and it is fixed: there is no setting to move the presence sensor to
another pin. So this particular pHAT and the presence sensor cannot share a Pi;
the button and the sensor's output land on the same line, and one of them has to
go. If you want four Qwiic ports and presence wake together, reach the bus
another way (the plain cable, the Pi Stemma QT socket, or the SHIM above), or
choose a board without a button. If you are not using the presence sensor, the
button is harmless: Pantry Raider ignores it. The older button-free SparkFun
Qwiic / STEMMA QT HAT (Adafruit 4688) is discontinued, so "the same board without
the button" is not something you can buy new.
...
A mmWave presence sensor wakes the display as you walk up. Three wires:
...
**The LD2410C is the part to buy**, and the table above is its wiring. The project
reads a single presence pin rather than any sensor's serial protocol, so any module
with a plain 3.3V presence output can sit on GPIO17: the **LD2410D** works the same
way if the board you receive exposes that pin, and the **LD2420** is not recommended
because its presence pin and its serial pin swap places between firmware versions.
The full guide, including which sensors work, per-part wiring, mounting, and turning
it on, is the [mmWave presence sensor](presence-sensor.md) page. The compatibility
facts to carry into a build:

- **GPIO17 (pin 11) is claimed** whenever this sensor is fitted, and the pin is
  fixed: there is no setting to move the sensor elsewhere. Nothing else in the
  project uses GPIO17, but some third-party boards do, most notably the SparkFun
  Qwiic pHAT v2.0's onboard button. That board and this sensor cannot share a
  Pi; see [The Qwiic pHAT button and GPIO17](#the-qwiic-phat-button-and-gpio17) above.
- **The LD2410C is 5V-powered with a 3.3V-safe output**, so OUT connects straight to
  GPIO17 with nothing in between. No sensor on the guide's list outputs 5V logic, so
  none of them needs a divider. The LD2420 is the exception on power rather than
  signal: it is a 3.3V part, so it would take pin 1 instead of pin 2.
...
A DSI touch panel's video and touch both travel up the DSI ribbon on their own
buses, so the display puts no signal on the 40-pin header at all. The only
header question a DSI panel raises is power, and different panels answer it in
two different ways. Check which way yours works before planning the header,
because the answer decides whether the display takes the pin 2 corner or
nothing at all.

**Pogo pins underneath the Pi: no header pins used.** Many current panels mount
the Pi on the back of the display and carry power through spring-loaded pogo
pins that press against contacts on the underside of the board. The Hosyond
7 inch works this way: screw the Pi to the panel's standoffs, check that the
panel's spring pins line up with their contacts under the Pi and make full
contact, and power flows with nothing plugged into the top of the header.
Mounted like this, the display leaves the entire 40-pin header free for a HAT,
a sensor, or jumper wires. The same panel ships a two-wire jumper cable for
builds where the Pi is not mounted to the display, which turns it into the
second kind below.

**5V and ground from the header: two pins.** Other panels take their power from
the header. The original Raspberry Pi 7 inch touchscreen uses separate 5V and
ground jumper wires, which can land on any 5V and any ground pin, so they move
out of the way of whatever else wants the corner. The current Raspberry Pi
Touch Display 2 instead ships a power cable ending in a one-piece three-pin
plug that seats on pins 2, 4, and 6 (5V on pin 2, ground on pin 6). That plug
uses two pins electrically but covers three, and the covered pin 4 is the
header's only other 5V pin, so nothing else can take 5V from the header while
it is fitted.

Either way, the display draws its power through the Pi, so count it in the
power supply budget at the top of this page. And if a listing does not say
which method a panel uses, look for "pogo pins" or "spring pins" in its manual,
or plan around the header method as the safe assumption.

What this means for the rest of this page: a pogo-pin panel conflicts with
nothing on the header, full stop. A header-powered panel wants the pin 2 / pin
4 / pin 6 corner, which is the same corner the presence sensor and the Qwiic
SHIM want, and the matrix below carries that as "check" cells rather than hard
conflicts because separate jumper wires can usually move. The one stubborn case
is a one-piece plug like the Touch Display 2's, which takes both 5V pins at
once; anything else that needs 5V from the header then has to get it elsewhere,
such as a PoE HAT's own 5V header.
...
| | PoE HAT (passthrough) | PoE HAT (no passthrough) | PoE HAT (6-pin corner) | Qwiic / STEMMA QT pHAT | Qwiic / STEMMA QT SHIM | Pi Stemma QT socket | QT cable to pins 1/3/5/9 | mmWave on GPIO17 | DSI display | I2S amp | USB speaker | GPIO fan | Stream Deck | Printers |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **PoE HAT (passthrough)** | | | | Check: clearance above the PoE HAT is unpublished | Works: SHIM fits under the HAT | Works: the socket plugs onto the passthrough pins on top | Works | Works | Works | Works | Works | Check: Pi 5 fan header is free; a Pi 4 GPIO fan needs a reachable pin | Works | Works |
| **PoE HAT (no passthrough)** | | | | Conflicts: header covered | Works: SHIM fits under the HAT | Conflicts: header covered, and a socket cannot slide underneath the way the SHIM can | Conflicts: header covered | Conflicts: header covered | Check: a pogo-pin panel needs no header pins; a header-powered one has none reachable (see DSI displays) | Conflicts: header covered | Works | Check: Pi 5 fan header is free; Pi 4 has no reachable pin | Works | Works |
| **PoE HAT (6-pin corner)** | | | | Conflicts: the pHAT's 40-pin connector covers pins 1 to 6, where the PoE HAT has to seat | Check: both want pins 1 to 6; the SHIM is built for a HAT to seat on top of it, but on a 6-pin block that is unverified | Check: both seat on pins 1 to 6; the socket can only go on top, and only if the block passes the pins through | Check: pins 1, 3, and 5 are under the block, and the header has no second SDA/SCL; depends whether the block passes through | Check: pin 11 is not claimed, but verify you can reach it under the board, and take the sensor's 5V from the HAT's own header | Check: a pogo-pin panel needs no header pins; a header-powered one takes its 5V and ground from the HAT's own header (see DSI displays) | Check: pins 12/35/40 are not claimed; take the amp's 5V and ground from the HAT's header | Works: with the isolated model | Works: the HAT brings its own PWM fan header, at 0x2f on I2C-1 | Works: with the isolated model | Works: with the isolated model |
| **Qwiic / STEMMA QT pHAT** | Check: clearance above the PoE HAT is unpublished | Conflicts: header covered | Conflicts: the pHAT's 40-pin connector covers pins 1 to 6, where the PoE HAT has to seat | | Works: same bus, no need for both | Works: same bus, no need for both | Works: same bus | Check: the SparkFun Qwiic pHAT v2.0's button is wired to GPIO17, the sensor's fixed pin; that board and the sensor cannot share a Pi (see the Qwiic pHAT note) | Check: a pogo-pin panel is unaffected; a header-powered panel's power plug has nowhere to land under the pHAT (see DSI displays) | Check: pHAT covers pins 12/35/40 | Works | Check: pHAT covers the header | Works | Works |
| **Qwiic / STEMMA QT SHIM** | Works | Works | Check: both want pins 1 to 6; the SHIM is built for a HAT to seat on top of it, but on a 6-pin block that is unverified | Works: same bus, no need for both | | Works: same bus, no need for both | Works: same bus | Conflicts: SHIM occupies pins 1 to 6, the sensor needs 2 and 6 | Check: a pogo-pin panel is unaffected; a header-powered panel wants pins the SHIM occupies (see DSI displays) | Works | Works | Works | Works | Works |
| **Pi Stemma QT socket** | Works: the socket plugs onto the passthrough pins on top | Conflicts: header covered, and a socket cannot slide underneath the way the SHIM can | Check: both seat on pins 1 to 6; the socket can only go on top, and only if the block passes the pins through | Works: same bus, no need for both | Works: same bus, no need for both | | Works: same bus | Conflicts: the socket caps pins 1 to 6, the sensor needs 2 and 6 | Check: a pogo-pin panel is unaffected; a header-powered panel wants pins the socket caps (see DSI displays) | Check: pins 12/35/40 are free, but the socket caps both 5V pins; the amp's 5V has to come from somewhere else | Works | Check: a Pi 5 fan has its own header; a Pi 4 case fan powers from pin 4, which the socket caps | Works | Works |
| **QT cable to pins 1/3/5/9** | Works | Conflicts: header covered | Check: pins 1, 3, and 5 are under the block, and the header has no second SDA/SCL; depends whether the block passes through | Works: same bus | Works: same bus | Works: same bus | | Works | Works: display avoids pins 1/3/5/9 | Works: I2S and I2C are different pins | Works | Works | Works | Works |
| **mmWave on GPIO17** | Works | Conflicts: header covered | Check: pin 11 is not claimed, but verify you can reach it under the board, and take the sensor's 5V from the HAT's own header | Check: the SparkFun Qwiic pHAT v2.0's button is wired to GPIO17, the sensor's fixed pin; that board and the sensor cannot share a Pi (see the Qwiic pHAT note) | Conflicts: SHIM occupies pins 1 to 6, the sensor needs 2 and 6 | Conflicts: the socket caps pins 1 to 6, the sensor needs 2 and 6 | Works | | Check: a pogo-pin panel leaves the corner free; separate power jumpers share it (move the sensor's 5V to pin 4), but the Touch Display 2's one-piece plug covers both 5V pins (see DSI displays) | Works | Works | Works | Works | Works |
| **DSI display** | Works | Check: a pogo-pin panel needs no header pins; a header-powered one has none reachable (see DSI displays) | Check: a pogo-pin panel needs no header pins; a header-powered one takes its 5V and ground from the HAT's own header (see DSI displays) | Check: a pogo-pin panel is unaffected; a header-powered panel's power plug has nowhere to land under the pHAT (see DSI displays) | Check: a pogo-pin panel is unaffected; a header-powered panel wants pins the SHIM occupies (see DSI displays) | Check: a pogo-pin panel is unaffected; a header-powered panel wants pins the socket caps (see DSI displays) | Works: display avoids pins 1/3/5/9 | Check: a pogo-pin panel leaves the corner free; separate power jumpers share it (move the sensor's 5V to pin 4), but the Touch Display 2's one-piece plug covers both 5V pins (see DSI displays) | | Works | Works: DSI panels have no speakers | Works | Works | Works |
| **I2S amp** | Works | Conflicts: header covered | Check: pins 12/35/40 are not claimed; take the amp's 5V and ground from the HAT's header | Check: pHAT covers pins 12/35/40 | Works | Check: pins 12/35/40 are free, but the socket caps both 5V pins; the amp's 5V has to come from somewhere else | Works: I2S and I2C are different pins | Works | Works | | Works: pick one as the default output | Check: a Pi 4 fan on GPIO18 collides with the I2S clock; use GPIO14 | Works | Works |
| **USB speaker** | Works | Works | Works: with the isolated model | Works | Works | Works | Works | Works | Works | Works: pick one as the default output | | Works | Works: uses a USB port | Works: uses a USB port |
| **GPIO fan** | Check: Pi 5 fan header is free; a Pi 4 GPIO fan needs a reachable pin | Check: Pi 5 fan header is free; Pi 4 has no reachable pin | Works: the HAT brings its own PWM fan header, at 0x2f on I2C-1 | Check: pHAT covers the header | Works | Check: a Pi 5 fan has its own header; a Pi 4 case fan powers from pin 4, which the socket caps | Works | Works | Works | Check: a Pi 4 fan on GPIO18 collides with the I2S clock; use GPIO14 | Works | | Works | Works |
| **Stream Deck** | Works | Works | Works: with the isolated model | Works | Works | Works | Works | Works | Works | Works | Works | Works | | Works: watch the USB current budget |
| **Printers** | Works | Works | Works: with the isolated model | Works | Works | Works | Works | Works | Works | Works | Works | Works | Works: watch the USB current budget | |
...
1. **A HAT that covers the 40-pin header ends the conversation** for everything
   ...
   underneath; the Pi Stemma QT socket is not, because it needs the top of the
   pins, so a covered header locks it out too.
2. **A header-powered DSI display and the presence sensor both want the pin 2 /
   pin 6 corner.** Because 5V and ground are duplicated elsewhere on the header,
   that is usually a move-a-wire problem, not a pick-one problem. The exceptions
   are a one-piece display power plug that takes both 5V pins (the Touch
   Display 2's does) and the Qwiic SHIM, which wants all six corner pins
   physically. A pogo-pin DSI panel sits the whole argument out, because it
   takes nothing from the header (see [DSI displays](#dsi-displays)).
3. **Everything that fights over pins 1 to 6 is fighting over the same corner.** The
   Qwiic SHIM, the Pi Stemma QT socket, a header-powered DSI display's power
   jumpers, the presence sensor's power and ground, and a corner-landing PoE HAT
   all want that block. This is why the
...
- A DSI touch panel. A pogo-pin panel (the Pi mounted on the back of the
  display) takes no header pins, so it changes nothing else in this list. If
  yours powers from the header instead, move the presence sensor's 5V to pin 4
  and its ground to pin 14 so the display's power wires can have the pin 2 and
  pin 6 corner, and first check that the panel uses separate power wires rather
  than a one-piece plug (see [DSI displays](#dsi-displays)).
...
- A Qwiic SHIM or an Adafruit Pi Stemma QT socket on pins 1 to 6 for a solder-free
  QT port, or the plain cable to pins 1, 3, 5, and 9 if you also want the presence
  sensor or a header-powered DSI display (the SHIM and the socket each want that
  whole corner to themselves; a pogo-pin panel does not care).
