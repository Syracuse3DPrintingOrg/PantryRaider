...
The **Bluetooth label printer** panel in **Settings, Printing** walks you
through one step at a time, rather than a single button that quietly runs a
several-minute install:

1. Press **Set up**. The panel shows its progress and a status of Not set up,
   Setting up, Ready, or Failed; the first run can take several minutes while
   it prepares the printer software, and later runs are quick.
2. Charge the T50M and load a roll of labels.
3. Once the panel says **Ready**, turn the printer on and make sure it is not
   already paired to a phone (most of these printers only talk to one device
   at a time), then press **Find my printer**, which jumps straight to the
   search.
4. The T50M appears there like any network printer, including a T50M Pro
   that announces itself as a bare serial number instead of a friendly name;
   add it and pick it as your label printer.

Re-running setup later is a small link rather than the main button, so a
device that is already Ready does not look stuck. On a satellite (a thin
display pointed at your main server), the panel says plainly that the
printer works there and is shared up to your main server, since printers are
chosen on the server; see [Printing labels and documents](../printing.md#one-label-printer-and-one-document-printer-for-the-whole-kitchen).
...
only advertises over Bluetooth while it is awake. If the printer has
auto-powered off after sitting idle, a print job tells you to turn it back
on and try again rather than failing silently.

Set your **label size** (Settings, Printing, Label size) to match your
stock, or pull the sizes the printer itself advertises and pick one; a label
sent with the wrong size can come out blank.
