...
A Grocy release can rename a class that Grocy's saved `config.php` points at
(Grocy 4.7 moved `AUTH_CLASS`), after which Grocy answers every request with
an error page and the inventory banner says Grocy "reported a problem with
its own setup". An appliance repairs this itself: every update checks the
running Grocy container and fixes that line, backup first, and the banner
offers a **Repair Grocy config** button that does the same on demand. Grocy
also carries the repair as a start-up script, so a container recreated on a
newer image fixes itself as it comes up. Nothing to do on your side beyond
pressing the button if you see it; the backup sits next to `config.php` as
`config.php.bak-<timestamp>`.
