...
1. Open Settings, Thermometers & Sensors, and find the Shelf buttons section.
...
- **A button near a Bandit works too.** A Bandit satellite with the
  Bluetooth reader relays presses to your main server, so a shelf button in
  another room (or next to a radio-less server's satellite) fires the
  mapping configured on the server, exactly once.
