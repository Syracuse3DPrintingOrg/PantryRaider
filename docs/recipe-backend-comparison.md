...
- **Recipes.** Your own library, stored in Pantry Raider. Import from a web
  ...
  community, generate a recipe with AI, or write one by hand. A recipe in
  your library can be opened and edited later too: change the name,
  description, servings, prep, cook, and total time, ingredients, steps, and
  the photo. Pictures ride along and everything is covered by the normal app
  backup. (A recipe kept in Mealie is still edited there.) A recipe written
  in parts (for the pasta, for the sauce, for the topping) keeps its
  ingredient sections everywhere: through a photo or file import, in the
  editor where you can rename or merge them, through the Optimize button, on
  the printout, and on the On the Line page, where each heading sits above
  its ingredients the way the recipe was written.
...
- **Shopping list.** Kept in Grocy, right next to your inventory, so "things
  ...
  scanner in shopping mode, or the Stream Deck. When the groceries come
  home, the Shopping page's **Add receipt** button (shown when an AI
  provider is set up) reads the prices you actually paid off a photo of the
  receipt and records each one on the matching product's newest stock entry,
  so Grocy's price history and inventory value reflect real prices. You
  review the pairings first and untick anything that looks wrong; a line the
  receipt cannot answer for is skipped, never guessed. Reading the photo
  takes a minute or two and runs in the background, so you are free to leave
  the page: when it finishes, the pairings are waiting on the Shopping page,
  with a note in your Review inbox that links straight to them.
...
When a recipe names an amount and the product has unit conversions defined
in Grocy, the match checks the amount too: a bottle known to hold 500 g
honestly covers, or fails to cover, a recipe's 200 g, instead of the name
alone deciding. Products without a stored conversion behave exactly as
before.
