...
| Recipes and meal plan (Mealie) | Yes | Nothing external; the Mealie container runs on your box | Mealie stores your recipes, meal plan, and shopping list locally | Nothing to do; it is local by design |
...
| Recipe search from public catalogs | No | TheMealDB and, if you add a key, Spoonacular | Optional discovery of new recipes beyond your own library | Skip external recipe search; your own Mealie library and suggestions stay local |
| Community recipes (Forager) | No | Forager (`forager.pantryraider.app`) | Browse, download, and share recipes with other Pantry Raider kitchens; included free with any Forager account | It is opt-in; leave Forager unlinked and use your own Mealie library and external recipe search |
