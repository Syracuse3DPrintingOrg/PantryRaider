...
| Recipe share links and sending recipes to people (Forager) | No | Forager (`forager.pantryraider.app`) | Creating a public link, emailing a recipe, or sending one to another Forager user is hosted by Forager so recipients outside your home can open it | It is opt-in; without Forager you can still download a recipe file or email the recipe text from the share panel |
...
| Software updates | No | GitHub (source) and the GitHub container registry (`ghcr.io`) | To pull new app code and container images | Updates run when you trigger one or on a schedule; on a server install the bundled Watchtower container also checks the registry once a day, see the note below |
...
**A server install checks for a new image once a day.** The Docker Compose
stack includes a small Watchtower container that keeps a plain server current,
since a server has no host bridge to run the one-button update a Raspberry Pi
appliance uses. Once a day it asks the GitHub container registry whether a newer
Pantry Raider image exists and pulls it when there is one, looking only at the
Pantry Raider container and never at Grocy, Mealie, or Ollama. That daily check
runs on its own schedule, separate from the update switches in Settings. If you
want a server that never reaches out on its own, stop that container with
`docker compose stop watchtower`, or pin a version in your `.env`
(`PANTRYRAIDER_TAG=0.18.101`) so nothing newer is picked up. Raspberry Pi
appliances do not run Watchtower.
