...
Looking for a small ESP32 **screen** instead, one that shows expiring items,
timers, and probes? That is a [Bandit Cub](bandit-cub.md), with prebuilt
firmware you flash from the browser, no YAML needed.
...
binary_sensor:
  - platform: gpio
    ...
    on_press:
      then:
        - http_request.post:
            url: "http://192.168.1.50:9284/gadgets/esp-action"
            headers:
