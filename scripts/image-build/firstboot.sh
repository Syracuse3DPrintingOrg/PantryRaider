...
install_docker() {
  ...
  # which now and then serves a half-synced index ("File has unexpected size...
  # Mirror sync in progress?"). That is transient, so retry a few times rather
  # than aborting the whole first boot on it: clear the apt lists between
  # attempts (forces a fresh index fetch instead of reusing the cached partial)
  # and back off a little to give the mirror time to finish (FoodAssistant-4qz5).
  local attempt rc=1
  for attempt in 1 2 3; do
    if sh "$tmp"; then rc=0; break; fi
    if [ "$attempt" -lt 3 ]; then
      warn "Docker install attempt $attempt failed (often a transient package-mirror sync); clearing apt lists and retrying"
  ...
  rm -f "$tmp"
  [ "$rc" -eq 0 ] || die "Docker install failed after 3 attempts. This is usually a temporary package-mirror hiccup; wait a minute and re-run the installer."
