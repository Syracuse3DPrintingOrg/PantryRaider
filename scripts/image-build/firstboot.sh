...
deploy_stack() {
  ...
  # Wait here for Grocy (the inventory engine) to actually answer HTTP, not just
  # for its container to start. On a slow link the image pull plus Grocy's own
  # first-run database setup take a few minutes, and doing that wait HERE (with a
  # spinner, during install) keeps it out of the web setup wizard, where it used
  # to leave the user staring at a half-finished step (Dan, 2026-07-10). The app
  # then auto-configures the now-ready Grocy in the background, so setup has
  # nothing to do. Best-effort: if it is still slow we do not fail the install
  # (the background first-run task keeps retrying after boot). Grocy only runs on
  # a hosted appliance, never a satellite.
  if [ "$DEPLOYMENT_MODE" != "pi_remote" ] && [ "$DRY_RUN" != "1" ]; then
    spin_run "Waiting for the inventory service to be ready" \
      sh -c 'for i in $(seq 1 90); do curl -fsS -o /dev/null "http://localhost:9383/" 2>/dev/null && exit 0; sleep 5; done; exit 0'
  fi
}
