...
# The RAM floor (in MB) below which Mealie is left OFF by default. A 4 GB Pi
# reports roughly 3800-3900 MB of MemTotal and a 2 GB Pi about 1900 MB, so 3000
# cleanly splits the two tiers. Only used when ENABLE_MEALIE is unset; an
# explicit choice (config.env, installer, or the web wizard) always wins.
MEALIE_MIN_MB="${MEALIE_MIN_MB:-3000}"
...
load_config() {
  ...
  # satellite (pi_remote) runs no local backend, so it stays off there. On a
  # low-RAM board (a 2 GB Pi) Mealie is left OFF: Pantry Raider plus Grocy fit
  # comfortably in 2 GB, but adding Mealie pushes the practical floor to 4 GB.
  # The user can turn it on later once they know their board can carry it. An
  # explicit ENABLE_MEALIE (config.env, installer, or the wizard) always wins.
  if [ -z "$ENABLE_MEALIE" ]; then
    if is_remote_mode; then
      ENABLE_MEALIE="false"
    else
      local _ram_mb
      _ram_mb="$(detect_ram_mb)"
      if [ "$_ram_mb" -gt 0 ] 2>/dev/null && [ "$_ram_mb" -lt "$MEALIE_MIN_MB" ]; then
        ENABLE_MEALIE="false"
        log "Low RAM (${_ram_mb} MB < ${MEALIE_MIN_MB} MB): defaulting Mealie OFF. Enable it later on a 4 GB+ board from the Settings page or the with-mealie compose profile."
      else
        ENABLE_MEALIE="true"
      fi
    fi
