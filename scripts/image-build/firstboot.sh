...
load_config() {
  ...
  # Bluetooth kitchen thermometers (FoodAssistant-6ivl). Strictly opt-in (no
  # auto: there is no attached hardware to detect before a thermometer is
  # paired), so a plain image never grows a Bluetooth daemon uninvited.
  ENABLE_GADGETS="${ENABLE_GADGETS:-false}"
...
# Step: Bluetooth thermometer reader (opt-in via ENABLE_GADGETS=true)
configure_gadgets() {
  if ! is_true "$ENABLE_GADGETS"; then
    log "Bluetooth thermometers not enabled (ENABLE_GADGETS=$ENABLE_GADGETS); skipping"
    return 0
  fi
...
# already, so the mirror adds no exposure a stolen card did not.) Prints the
# passphrase on stdout; returns non-zero when none could be established.
...
configure_wifi_ap_fallback() {
  ...
  local ap_pass
  ap_pass="$(ap_fallback_passphrase)" || ap_pass=""
  if [ -z "$ap_pass" ]; then
...
EOF
  ...
  log "Recovery hotspot: SSID FoodAssistant with a password unique to this device."
