...
# mdns, docker, stack, rotation, kiosk, streamdeck, gadgets, hostbridge, quiet_boot, boot_splash, fb_splash.
...
# Where the detailed output of the quiet, spinner-wrapped steps is kept, so the
# console stays clean but nothing is lost (the tail is shown if a step fails).
STEP_LOG="${STEP_LOG:-/var/log/foodassistant-install.log}"

# spin_run "Doing the thing" cmd args...: run a long, chatty command behind a
# single status line instead of letting it scroll pages of output (Docker image
# pulls and builds are the worst offenders). Its stdout+stderr go to STEP_LOG; on
# failure the tail is printed so an error is never hidden. When stdout is not a
# terminal (piped or logged) it degrades to a plain begin/end line with no
# animation, and DRY_RUN just announces. Returns the command's exit status.
spin_run() {
  local msg="$1"; shift
  if [ "$DRY_RUN" = "1" ]; then
    log "DRY_RUN would run ($msg): $*"
    return 0
  fi
  ...
  if [ ! -t 1 ]; then
    ...
    warn "$msg: FAILED (see $STEP_LOG)"
    tail -n 20 "$STEP_LOG" >&2 2>/dev/null || true
    return 1
  fi
  "$@" >>"$STEP_LOG" 2>&1 &
  local pid=$! rc=0 i=0
...
# Architecture / OS detection (exported for steps + tests). FORCE_ARCH and
# FORCE_LONG_BIT let a test exercise the 32-bit path without 32-bit hardware.
detect_arch() { printf '%s' "${FORCE_ARCH:-$(uname -m)}"; }
detect_long_bit() { printf '%s' "${FORCE_LONG_BIT:-$(getconf LONG_BIT 2>/dev/null || echo 64)}"; }

# The published container image has no 32-bit manifest, so the full stack needs
# a 64-bit OS. A satellite pulls no image (the app runs from a Python venv),
# which is what keeps the 32-bit Pi Zero 2 W thin client supported. Warns and
# continues either way: this device is already flashed, and stopping here would
# also skip the kiosk, Stream Deck, and network steps that do work.
check_arch_64bit() {
  local arch
  arch="$(detect_arch)"
  case "$arch" in
    armv6l|armv7l|armhf|i386|i686) ;;
    *) [ "$(detect_long_bit)" = "32" ] || return 0 ;;
  esac
  if is_remote_mode; then
    warn "32-bit OS detected ($arch). Satellite mode works here; the full stack needs Raspberry Pi OS Lite (64-bit)."
    return 0
  fi
  warn "32-bit OS detected ($arch), and the Pantry Raider container image is 64-bit only."
  warn "The stack will not start on this OS. Reflash with Raspberry Pi OS Lite (64-bit), then run first boot again."
  return 0
}

# Total system RAM in whole MB. Reads MemTotal (kB) from /proc/meminfo. Tests
# and manual runs can override the reading with FORCE_MEM_MB so the RAM-tiered
# decisions below can be exercised without the target hardware. Returns 0 when
# the value cannot be read, which the callers treat as "unknown, be cautious".
detect_ram_mb() {
  if [ -n "${FORCE_MEM_MB:-}" ]; then
    printf '%s' "$FORCE_MEM_MB"
    return 0
  fi
  local kb=""
...
load_config() {
  local f found=""
  # An explicit choice from the installer (install.sh) or the environment arrives
  # as FA_HOSTNAME, not HOSTNAME: bash auto-populates $HOSTNAME with the running
  # host's name, so we cannot tell an intended value from the shell's default.
  # Capture FA_HOSTNAME first, then clear HOSTNAME so the config file, that
  # explicit choice, or our brand default wins (in that order) below.
  local requested_hostname="${FA_HOSTNAME:-}"
  ...
  # Hostname precedence: a HOSTNAME= line in the config file, then an explicit
  # installer/env choice (FA_HOSTNAME), then the brand default "pr" (so the box
  # answers at pr.local). The device-local identifiers (systemd units, /opt and
  # /etc paths, the /health app field) intentionally stay "foodassistant"; only
  # the user-facing mDNS name is branded here (FoodAssistant-a8fn).
  HOSTNAME="${HOSTNAME:-$requested_hostname}"
  HOSTNAME="${HOSTNAME:-pr}"
  TZ="${TZ:-$(os_timezone)}"
  # Mealie is OFF by default: recipes, the meal plan, and the shopping list
  # are built into Pantry Raider. It stays available as an opt-in for people
  # who already use it (ENABLE_MEALIE=true in config.env or the environment).
  ENABLE_MEALIE="${ENABLE_MEALIE:-false}"
  ENABLE_OLLAMA="${ENABLE_OLLAMA:-false}"
  ENABLE_KIOSK="${ENABLE_KIOSK:-auto}"
  ENABLE_STREAMDECK="${ENABLE_STREAMDECK:-auto}"
  # Bluetooth kitchen thermometers and plug-in accessories (FoodAssistant-6ivl,
  # FoodAssistant-dxr22). Default "auto": the reader installs when a plug-in
  # STEMMA QT accessory (a NeoKey pad, say) answers on the I2C bus at first
  # boot, the same detect-and-decide the Stream Deck uses. Bluetooth sensors
  # alone cannot be detected before one is paired, so a box with no accessory
  # plugged in resolves to off and Settings, Thermometers & Sensors offers the
  # one-click setup instead. Force with "true"/"false".
  ENABLE_GADGETS="${ENABLE_GADGETS:-auto}"
  # Boot splash (FoodAssistant-y8vj): a Plymouth theme that hides the boot
  # console and shows the Pantry Raider logo until the kiosk paints. OFF by
  # default: the Plymouth-to-cage handoff is display/driver specific, so it is
  # opt-in (ENABLE_BOOT_SPLASH=true) and verified on a device before becoming
  # the default. The install is fail-safe either way (see configure_boot_splash):
  # any failure leaves the existing quiet boot untouched, never a broken boot.
  ENABLE_BOOT_SPLASH="${ENABLE_BOOT_SPLASH:-false}"
  # Framebuffer boot splash (FoodAssistant-y8vj): a tiny systemd oneshot that
  # paints the Pantry Raider mark straight to /dev/fb0 right after the root
  # filesystem mounts, hiding the remaining boot text until the kiosk takes
  # the display. No Plymouth, no initramfs change, no new packages (python3
  # ships with Pi OS). ON by default for kiosk devices because every failure
  # path skips silently (see the unit's Condition lines and the writer's
  # catch-all): the worst case is the plain quiet boot. ENABLE_FB_SPLASH=false
  # turns it off; the opt-in Plymouth theme above wins when enabled (the unit
  # steps aside when "splash" is on the kernel cmdline).
  ENABLE_FB_SPLASH="${ENABLE_FB_SPLASH:-true}"
  ...
  # A satellite (pi_remote) runs no local backend, so Mealie is forced off
  # there regardless of how the flag arrived.
  if is_remote_mode; then
    ENABLE_MEALIE="false"
  fi
...
# Step: pick a hostname that is not already taken on the LAN, so two Pantry
# Raider devices (a host plus a satellite, say) do not both answer to the same
# <name>.local (FoodAssistant-a8fn). Probes the desired name over mDNS and
# appends -2, -3, ... until one is free. A name that resolves to our OWN IP is
# treated as free, so re-running keeps the same name instead of climbing the
# suffix every time. Fail-safe: any uncertainty (no network, no avahi, a
# timeout) keeps the base name rather than renaming; DRY_RUN is a no-op.
resolve_unique_hostname() {
  [ "$DRY_RUN" = "1" ] && return 0
  if ! command -v avahi-resolve-host-name >/dev/null 2>&1; then
    apt_install avahi-daemon avahi-utils >/dev/null 2>&1 || true
  fi
  command -v avahi-resolve-host-name >/dev/null 2>&1 || return 0
  # The daemon must be up to answer queries; it publishes the CURRENT (old)
  # hostname for now, which is not our desired name, so probing is not self.
  systemctl start avahi-daemon >/dev/null 2>&1 || true
  local our_ips base cand ip n
  ...
  if [ "$cand" != "$HOSTNAME" ]; then
    log "Hostname '$HOSTNAME.local' is already used on this network; using '$cand.local' instead"
    HOSTNAME="$cand"
  fi
}
...
configure_mdns() {
  ...
  # If collision resolution or the hostname step changed the name after avahi was
  # already started (during the mDNS probe), restart it so it republishes the
  # final <hostname>.local rather than the stale one.
  [ "$DRY_RUN" = "1" ] || systemctl try-restart avahi-daemon >/dev/null 2>&1 || true
  # WireGuard tools power Forager remote access: the host bridge runs wg and
  # wg-quick to bring the tunnel up on the host. Without them, turning on
  # remote access fails with "could not set up remote access". The kernel
  # module ships with Pi OS; only the userspace tools need installing.
  if ! command -v wg >/dev/null 2>&1; then
    log "Installing wireguard-tools for Forager remote access"
    apt_install wireguard-tools || warn "wireguard-tools install failed; Forager remote access will be unavailable until it is installed"
  fi
...
# Make sure the Docker Compose v2 plugin (`docker compose`) is available. The
# stack's compose files need v2, and how you get it varies by install path:
#   - get.docker.com installs it as docker-compose-plugin (from Docker's repo);
#   - Debian/Ubuntu ship it as docker-compose-v2 (a distro package);
#   - neither name is portable, and Docker's repo can be down (FoodAssistant-4qz5),
# so when both package routes miss we drop in the official static plugin binary
# from GitHub, which is Docker's documented manual method and does not depend on
# download.docker.com. Returns success only when `docker compose` actually runs.
install_compose_v2() {
  command -v docker >/dev/null 2>&1 || return 1
  docker compose version >/dev/null 2>&1 && return 0
  if [ "$DRY_RUN" = "1" ]; then
    log "DRY_RUN would ensure the Docker Compose v2 plugin"
    return 0
  fi
  # Try the two package names first (whichever the distro/repo offers).
  apt_install docker-compose-v2 >/dev/null 2>&1 || true
  docker compose version >/dev/null 2>&1 && return 0
  apt_install docker-compose-plugin >/dev/null 2>&1 || true
  docker compose version >/dev/null 2>&1 && return 0
  # Neither package is available: install the official static plugin binary.
  local arch dest ver url
  case "$(uname -m)" in
  ...
  log "Installing Docker Compose v2 plugin ${ver} (${arch}) from GitHub"
  mkdir -p "$dest" 2>/dev/null || true
  if command -v curl >/dev/null 2>&1; then
    curl -fsSL "$url" -o "$dest/docker-compose" || true
  elif command -v wget >/dev/null 2>&1; then
    wget -qO "$dest/docker-compose" "$url" || true
  fi
  verify_compose_binary "$ver" "$arch" "$dest/docker-compose" || return 1
  chmod +x "$dest/docker-compose" 2>/dev/null || true
  docker compose version >/dev/null 2>&1
}

# Check a downloaded compose plugin against the checksums file the same release
# publishes. A mismatch removes the binary and fails (both callers only warn).
# An unreachable or unparsable checksums file warns and keeps the TLS-fetched
# binary: this path is already the last resort after both apt routes missed, and
# there is no third source to fall back to.
verify_compose_binary() {
  local ver="$1" arch="$2" bin="$3"
  [ -s "$bin" ] || return 1
  local sums="${COMPOSE_SUMS_FILE:-}" fetched=0 sumurl want got
  if [ -z "$sums" ]; then
    sums="$(mktemp 2>/dev/null || printf '/tmp/fa-compose-sums.%s' "$$")"
    fetched=1
    sumurl="https://github.com/docker/compose/releases/download/${ver}/checksums.txt"
    if command -v curl >/dev/null 2>&1; then
      curl -fsSL "$sumurl" -o "$sums" 2>/dev/null || true
    elif command -v wget >/dev/null 2>&1; then
      wget -qO "$sums" "$sumurl" 2>/dev/null || true
    fi
  fi
  # Lines read "<sha256>  docker-compose-linux-<arch>"; the name may carry a
  # leading "*" from a binary-mode checksum.
  want="$(awk -v f="docker-compose-linux-${arch}" \
    '{ n=$2; sub(/^\*/, "", n); if (n == f) { print $1; exit } }' "$sums" 2>/dev/null || true)"
  if [ "$fetched" = "1" ]; then
    rm -f "$sums" 2>/dev/null || true
  fi
  if [ -z "$want" ] || ! command -v sha256sum >/dev/null 2>&1; then
    warn "Could not verify the Docker Compose ${ver} checksum; keeping the TLS-fetched binary"
    return 0
  fi
  got="$(sha256sum "$bin" 2>/dev/null | awk '{print $1}')"
  if [ "$got" != "$want" ]; then
    warn "Docker Compose ${ver} checksum mismatch; discarding the download"
    rm -f "$bin" 2>/dev/null || true
    return 1
  fi
  log "Verified the Docker Compose ${ver} checksum"
  return 0
}
...
install_docker() {
  ...
  if command -v docker >/dev/null 2>&1; then
    warn "Docker present but Compose v2 missing; installing the compose plugin"
    install_compose_v2 || warn "Could not install Docker Compose v2; the stack may not start"
    return 0
  fi
  ...
  # The get.docker.com script runs its own apt-get update against Docker's repo.
  # download.docker.com sits behind a CDN (CloudFront) that intermittently serves
  # a stale Release/Packages pair, so apt reports "File has unexpected size ...
  # Mirror sync in progress?" and the whole first boot used to abort. Clearing the
  # local lists does not help: the skew is on the CDN edge. Ask apt to revalidate
  # (No-Cache) and retry a few times. Note: this repo does not publish by-hash, so
  # By-Hash must NOT be forced here (its /by-hash paths 404). (FoodAssistant-4qz5)
  local aptconf="/etc/apt/apt.conf.d/99-foodassistant-docker-fetch"
  cat > "$aptconf" 2>/dev/null <<'APTCONF' || true
...
APTCONF
  ...
  for attempt in 1 2 3; do
    if sh "$tmp"; then rc=0; break; fi
    warn "Docker install attempt $attempt via get.docker.com failed; clearing apt lists and retrying"
    rm -rf /var/lib/apt/lists/* 2>/dev/null || true
    apt-get update -y >/dev/null 2>&1 || true
    [ "$attempt" -lt 3 ] && sleep $((attempt * 15))
  done
  rm -f "$aptconf" 2>/dev/null || true
  rm -f "$tmp"

  # Fallback: if Docker's own repo stayed broken, install Docker from the distro
  # archive instead (docker.io plus the docker-compose-v2 plugin, both in Debian
  # trixie and current Ubuntu). This path never touches download.docker.com, so a
  # Docker CDN outage can no longer block first boot. Drop the repo the
  # convenience script added so its broken index does not fail the distro update.
  if [ "$rc" -ne 0 ]; then
    warn "get.docker.com kept failing; falling back to the distro Docker packages (docker.io)"
    rm -f /etc/apt/sources.list.d/docker.list 2>/dev/null || true
    rm -rf /var/lib/apt/lists/* 2>/dev/null || true
    apt-get update -y >/dev/null 2>&1 || true
    if DEBIAN_FRONTEND=noninteractive apt-get install -y docker.io docker-compose-v2; then
      rc=0
    elif DEBIAN_FRONTEND=noninteractive apt-get install -y docker.io; then
      rc=0
    fi
  fi

  [ "$rc" -eq 0 ] || die "Docker install failed: get.docker.com and the distro docker.io package both failed. Check internet access and re-run the installer."
  # The distro docker.io does not pull the compose v2 plugin on its own, and the
  # convenience script's docker-compose-plugin is unavailable once its repo is
  # dropped; ensure `docker compose` works (falling back to the official static
  # plugin binary) since the stack's compose files need it.
  install_compose_v2 || warn "Docker installed but the Compose v2 plugin is missing; the stack may not start"
  systemctl enable --now docker || warn "Could not enable docker service"
}

# Write CONTENT to PATH atomically (temp file in the same directory, fsync, then
# rename), so a power cut during provisioning never leaves a half-written config
# on the SD card: the file is either the whole old content or the whole new one.
# Skips the write under DRY_RUN.
atomic_write() {
  local path="$1" content="$2"
  if [ "$DRY_RUN" = "1" ]; then
    log "DRY_RUN would atomically write $path"
    return 0
  fi
  local dir tmp
  dir="$(dirname "$path")"
  ...
  tmp="$(mktemp "$dir/.$(basename "$path").XXXXXX" 2>/dev/null)" || {
    warn "Could not create a temp file next to $path; writing directly"
    printf '%s' "$content" > "$path"
    return $?
  }
  printf '%s' "$content" > "$tmp"
  # Flush the temp file's data to the card before the rename so the rename can
  # never publish an empty or partial file after a power loss.
  sync "$tmp" 2>/dev/null || sync
  mv -f "$tmp" "$path"
  # Flush the directory entry (the rename) too.
  sync
}

# Step: SD-card longevity + power-loss resilience.
# SD cards wear out and corrupt from constant small writes and from power cuts
# mid-write. This reduces both:
#   - journald logs to RAM (Storage=volatile), so routine logging stops
#     hammering the card. The one-time firstboot log under /var/log stays on
#     disk for troubleshooting.
#   - the root filesystem mounts with noatime (no write on every read) and
#     commit=120 (batch writeback every 2 minutes instead of every 5 seconds),
#     shrinking the window where a power cut can catch an in-flight write.
# Reversible: delete the journald drop-in and remove noatime,commit=120 from
# /etc/fstab (a .bak is left beside it), then reboot.
configure_sd_resilience() {
  if ! is_raspberry_pi; then
    log "Not a Raspberry Pi; skipping SD-card resilience tuning."
    return 0
  fi
  # journald to RAM.
  local jdir="/etc/systemd/journald.conf.d"
  local jconf="$jdir/foodassistant-volatile.conf"
  atomic_write "$jconf" "# Managed by FoodAssistant first-boot provisioner (SD-card longevity).
# Keep the systemd journal in RAM (/run/log/journal) instead of writing it to
# the SD card. Remove this file and reboot to restore on-disk journals.
[Journal]
Storage=volatile
RuntimeMaxUse=32M
"
  if [ "$DRY_RUN" != "1" ]; then
    systemctl restart systemd-journald 2>/dev/null || warn "Could not restart systemd-journald (journald.conf applies on next boot)."
  fi
  log "journald set to volatile (logs in RAM); wrote $jconf"

  # noatime + commit on the root filesystem.
  local fstab="/etc/fstab"
  if [ "$DRY_RUN" = "1" ]; then
    log "DRY_RUN would add noatime,commit=120 to the root mount in $fstab"
    return 0
  fi
  if [ ! -f "$fstab" ]; then
    warn "$fstab not found; skipping mount-option hardening."
    return 0
  fi
  if awk '$2 == "/" {print $4}' "$fstab" | grep -q 'noatime'; then
    log "Root filesystem already mounts noatime; leaving $fstab untouched."
    return 0
  fi
  # Rewrite only the root ( / ) line, appending noatime and commit=120 to its
  # options field. A timestamped backup is kept so the change is reversible.
  cp -f "$fstab" "${fstab}.foodassistant.bak"
  local new
  new="$(awk 'BEGIN{OFS="\t"}
    /^[[:space:]]*#/ {print; next}
    $2 == "/" && NF >= 4 {
    ...
    {print}' "$fstab")"
  atomic_write "$fstab" "$new
"
  log "Added noatime,commit=120 to the root mount in $fstab (backup at ${fstab}.foodassistant.bak; applies on next boot)."
}

# Step: swap sizing for low-RAM boards (zram instead of SD swap).
# Swapping to the SD card is both slow and a major source of write wear and
# power-loss corruption. zram gives compressed swap in RAM instead: on a 2 GB
# board that buys real headroom (Grocy, the app, and a browser) without ever
# touching the card. dphys-swapfile (Pi OS's default on-card swap) is disabled.
# Reversible: 'sudo systemctl disable --now zramswap', re-enable dphys-swapfile.
configure_zram() {
  if ! is_raspberry_pi; then
    log "Not a Raspberry Pi; skipping zram swap setup."
    return 0
  fi
  # Turn off swap-on-SD first so the two do not fight over swap priority.
  if [ "$DRY_RUN" = "1" ]; then
    log "DRY_RUN would disable dphys-swapfile and install/configure zram-tools (PERCENT=50, zstd)."
    return 0
  fi
  if systemctl list-unit-files dphys-swapfile.service >/dev/null 2>&1; then
    systemctl disable --now dphys-swapfile 2>/dev/null || warn "Could not disable dphys-swapfile."
    # Remove the on-card swap file it created, if any.
    [ -f /var/swap ] && rm -f /var/swap 2>/dev/null || true
    log "Disabled dphys-swapfile (no more swap on the SD card)."
  fi
  if ! dpkg -s zram-tools >/dev/null 2>&1; then
    apt_install zram-tools || { warn "zram-tools install failed; leaving swap as-is."; return 0; }
  fi
  # Size zram to half of RAM (it compresses ~2-3x, so this is safe headroom) and
  # prefer it over any other swap. zstd gives the best ratio for the CPU cost.
  atomic_write /etc/default/zramswap "# Managed by FoodAssistant first-boot provisioner (low-RAM headroom).
# Compressed swap in RAM instead of on the SD card. Adjust PERCENT to taste, or
# 'sudo systemctl disable --now zramswap' to turn it off.
ALGO=zstd
PERCENT=50
PRIORITY=100
"
  systemctl enable --now zramswap 2>/dev/null || warn "Could not start zramswap (applies on next boot)."
  log "Configured zram swap (zstd, 50% of RAM, priority 100)."
}

# Step: reachable on both interfaces when Ethernet and Wi-Fi are connected at
# once (FoodAssistant-0s5q). With both NICs on the same home subnet, Linux's
# default ARP behaviour answers a request on whichever interface has the
# lowest metric, not necessarily the one it arrived on, and strict reverse-path
# filtering then drops the reply as if it were spoofed. From the browser's
# side that looks like "the Ethernet address works, the Wi-Fi address (or
# vice versa) just times out" even though the Pi is up and the app is
# listening on both. arp_ignore=1 makes each interface answer ARP only for its
# own address, arp_announce=2 stops it announcing an address that belongs to
# another local interface, and rp_filter=2 (loose mode) accepts a reply route
# through any local interface instead of only the one the kernel would have
# used to send it, so a reply going out the "wrong" NIC is no longer dropped.
# Reversible: remove the drop-in and reboot (or 'sysctl --system').
configure_multihome_sysctl() {
  local sconf="/etc/sysctl.d/85-foodassistant-multihome.conf"
  atomic_write "$sconf" "# Managed by FoodAssistant first-boot provisioner (FoodAssistant-0s5q).
# Keeps the device reachable on both its Ethernet and Wi-Fi address when both
# are connected at once. Without this, Linux's default ARP behaviour and
# strict reverse-path filtering can answer on the wrong interface and drop
# the reply, so one address works and the other silently times out.
# Remove this file and reboot (or run 'sysctl --system') to restore defaults.

# Each interface answers ARP requests only for its own address, instead of
# whichever local interface a lookup happens to prefer.
net.ipv4.conf.all.arp_ignore = 1
net.ipv4.conf.default.arp_ignore = 1

# Never announce an address that belongs to a different local interface;
# always use the address of the interface actually sending the ARP request.
net.ipv4.conf.all.arp_announce = 2
net.ipv4.conf.default.arp_announce = 2

# Loose reverse-path filtering: accept a packet if it is routable through ANY
# local interface, not only the one the kernel would use to reach the source.
# Strict (1, the Debian/Pi OS default) drops the asymmetric replies this
# dual-NIC setup produces; loose (2) keeps spoof protection without that.
net.ipv4.conf.all.rp_filter = 2
net.ipv4.conf.default.rp_filter = 2
"
  if [ "$DRY_RUN" = "1" ]; then
    log "DRY_RUN would apply $sconf with sysctl --system"
    return 0
  fi
  sysctl --system >/dev/null 2>&1 || warn "sysctl --system failed; multihome settings apply on next boot."
  log "Configured dual-NIC reachability (arp_ignore, arp_announce, loose rp_filter); wrote $sconf"
}
...
write_env_file() {
  ...
  atomic_write "$env_path" "# Generated by FoodAssistant first-boot provisioner. Edit and re-run
...
deploy_stack() {
  ...
  # The pull/build/up run behind a single spinner line with Docker's own
  # per-layer progress quieted (--quiet / --progress=quiet / --quiet-pull), so
  # the console shows one clean status per phase instead of scrolling layer
  # output; the full detail still lands in STEP_LOG (FoodAssistant-tq2j).
  export REPO_DIR
  # Pull the Grocy image in the background while the app image downloads: the
  # two used to fetch serially (this pull, then `up -d --quiet-pull` for the
  # rest), which put the whole Grocy download AFTER the app was already
  # serving and stretched the first-boot inventory wait (FoodAssistant-0m61).
  # Best-effort: a failure here just means `up -d` pulls it like before.
  local grocy_pull_pid=""
  if [ "$DEPLOYMENT_MODE" != "pi_remote" ]; then
    # shellcheck disable=SC2086
    ( cd "$INSTALL_DIR" && docker compose $profiles pull --quiet grocy ) \
      >>"$STEP_LOG" 2>&1 &
    grocy_pull_pid=$!
  fi
  # shellcheck disable=SC2086
  if ! spin_run "Fetching the app image" \
        sh -c "cd \"$INSTALL_DIR\" && docker compose $profiles pull --quiet service"; then
    ...
    # shellcheck disable=SC2086
    spin_run "Building the app image (a few minutes)" \
      sh -c "cd \"$INSTALL_DIR\" && docker compose $profiles build --progress=quiet service" \
      || die "Local build also failed. See $STEP_LOG (and $REPO_DIR/service)."
  fi
  # Collect the parallel Grocy pull before `up -d`, so its result (or failure)
  # is settled rather than racing the stack start.
  if [ -n "$grocy_pull_pid" ]; then
    wait "$grocy_pull_pid" 2>/dev/null || true
  fi
  # shellcheck disable=SC2086
  spin_run "Starting the stack" \
    sh -c "cd \"$INSTALL_DIR\" && docker compose $profiles up -d --quiet-pull" \
    || die "Could not start the stack. See $STEP_LOG."

  # Wait briefly for the APP to answer /health so the kiosk (whose start gate
  # watches the same URL) paints as soon as provisioning finishes. The long
  # wait-for-Grocy that used to sit here is gone (FoodAssistant-0m61): while
  # Grocy runs its slow first start, the app now shows a live getting-ready
  # page (/ui/getting-ready) and hands the browser to the setup wizard the
  # moment the inventory answers, so holding first boot hostage for many
  # minutes here only delayed that page. Best-effort, never fails the install.
  if [ "$DEPLOYMENT_MODE" != "pi_remote" ] && [ "$DRY_RUN" != "1" ]; then
    spin_run "Waiting for the app to come up" \
      sh -c 'for i in $(seq 1 60); do curl -fsS -o /dev/null "http://localhost:9284/health" 2>/dev/null && exit 0; sleep 2; done; exit 0'
    # Ask Grocy for its home page once it serves: that page is the ONLY place
    # Grocy builds and migrates its database, and a hands-off first boot has no
    # browser to open it (the app's probes stick to /login and /api paths).
    # Without this, a fresh install's database stays an empty 0-byte stub and
    # every sign-in answers HTTP 500 (FoodAssistant-n2b4 field failure). The
    # 0.18.44 readiness work removed the old wait-for-Grocy curl loop that had
    # been triggering the migration by accident; this puts the trigger back on
    # purpose. Ignores redirects on purpose (Grocy 302s its very first request
    # back to /), fetching twice so the second request lands on the real page.
    # Backgrounded: Grocy's first start can trail the app by minutes and first
    # boot must not wait on it (the app self-heals later if this misses).
    sh -c 'for i in $(seq 1 120); do
             if curl -fsS -o /dev/null "http://localhost:9383/" 2>/dev/null; then
               sleep 2
               curl -fsS -o /dev/null "http://localhost:9383/" 2>/dev/null || true
               exit 0
             fi
             sleep 5
           done' >/dev/null 2>&1 &
...
install_accel_rotation() {
  ...
  # Same bounded pin gadgets/requirements.txt carries, so the accelerometer
  # helper and the reader never land on different smbus2 majors.
  pip3 install --quiet --break-system-packages "smbus2>=0.4,<1" 2>/dev/null || warn "smbus2 install failed; accelerometer rotation may not work"
...
# Returns 0 if something answers on the plug-in accessories bus right now: a
# STEMMA QT board (the NeoKey pad and friends) shows up as an I2C address once
# configure_i2c has turned the bus on, which is the only hardware signal a
# fresh flash has for ENABLE_GADGETS=auto (FoodAssistant-dxr22). Anything
# short of a positive answer (no bus device, no i2cdetect, an empty sweep)
# resolves to "not present": auto then leaves the reader off, and Settings,
# Thermometers & Sensors offers the one-click setup instead.
# Test hooks: FORCE_I2C=1 forces present, FORCE_I2C=0/false forces absent;
# I2C_DEV points the bus-node check somewhere hermetic.
has_i2c_device() {
  case "${FORCE_I2C:-}" in
    0|false|FALSE|no) return 1 ;;   # test hook: force absent
    ?*) return 0 ;;                 # test hook: force present
  esac
  [ -e "${I2C_DEV:-/dev/i2c-1}" ] || return 1
  command -v i2cdetect >/dev/null 2>&1 || return 1
  # Any non-"--" cell after the header row is a device acknowledging.
  i2cdetect -y 1 2>/dev/null \
    | awk 'NR > 1 { for (i = 2; i <= NF; i++) if ($i != "--") found = 1 } END { exit !found }'
}
...
_write_touch_calibration() {
  local name_glob="$1"     # udev ATTRS{name} glob, e.g. "ADS7846*", or the
                           # sentinel "id-touchscreen" to match ANY touchscreen
                           # by its udev property (USB HID panels report vendor
                           # names, not "*Touchscreen*", so a name glob misses
                           # them and the matrix never applies; FoodAssistant-ly82).
  ...
  local match
  if [ "$name_glob" = "id-touchscreen" ]; then
    match='ENV{ID_INPUT_TOUCHSCREEN}=="1"'
  else
    match="ATTRS{name}==\"${name_glob}\""
  fi
...
SUBSYSTEM=="input", KERNEL=="event*", ${match}, ENV{LIBINPUT_CALIBRATION_MATRIX}="${matrix}"
EOF
...
configure_touch() {
  ...
  if [ "$driver" = "usb" ]; then
    ...
    # overlay, but the coordinate axes are sometimes mirrored or rotated. Match
    # by the ID_INPUT_TOUCHSCREEN udev property rather than a device-name glob:
    # these panels report vendor names (not "*Touchscreen*"), so a name match
    # misses them and the rotation matrix never reaches the panel, leaving touch
    # in native orientation while the display rotates (FoodAssistant-ly82). The
    # matrix follows DISPLAY_ROTATION; set TOUCH_CALIBRATION_MATRIX in config.env
    # if a specific panel is still mis-mapped.
    _write_touch_calibration "id-touchscreen"
  fi
}
...
configure_kiosk() {
  ...
  # transcode iPhone HEIC/HEIF photos on a USB drive to JPEG for the
  # screensaver slideshow (browsers cannot render HEIC natively). Best-effort:
  # without it HEIC photos are simply skipped, other formats still show.
  apt_install --no-install-recommends libheif-examples \
    || warn "libheif-examples install failed; HEIC/HEIF photos will be skipped in the slideshow"
  ...
  # What the unit's app wait probes: the cheap unauthenticated /health endpoint
  # on the same scheme://host[:port] the kiosk opens. Falls back to KIOSK_URL
  # itself if the URL does not parse (a custom KIOSK_URL still gets a wait).
  local kiosk_wait_url
  kiosk_wait_url="$(printf '%s' "$KIOSK_URL" | sed -nE 's#^(https?://[^/]+).*#\1/health#p')"
  [ -n "$kiosk_wait_url" ] || kiosk_wait_url="$KIOSK_URL"
  ...
  # Escape hatch from external pages (FoodAssistant-wn8w). A tapped external
  # link (an Amazon listing from the Shop page) fills the display with a site
  # that has no way back: the kiosk browser is chromeless. Install a tiny local
  # extension whose content script overlays a floating return button and a
  # top-edge swipe-down on every page that is NOT the app itself, both
  # navigating back to KIOSK_URL. The app URL is baked into the INSTALLED copy
  # (the repo copy keeps a placeholder and the script fails closed on it), and
  # the browser flags are only added once the files verifiably landed, so a
  # failed copy can never hold up the kiosk.
  local ext_flags="" ext_src="" ext_dir="$INSTALL_DIR/kiosk-exit-extension"
  for candidate in "$ASSET_DIR/kiosk-exit-extension" \
                   "$REPO_DIR/scripts/image-build/kiosk-exit-extension"; do
    [ -f "$candidate/manifest.json" ] && { ext_src="$candidate"; break; }
  done
  if [ -n "$ext_src" ]; then
    if mkdir -p "$ext_dir" && cp -f "$ext_src"/* "$ext_dir"/ 2>/dev/null \
        && [ -f "$ext_dir/origin.js" ]; then
      # sed replacement escaping: &, \ and the | delimiter are the only
      # characters special on the right-hand side.
      local esc_url
      esc_url="$(printf '%s' "$KIOSK_URL" | sed -e 's/[\\&|]/\\&/g')"
      sed -i "s|__PR_KIOSK_HOME_URL__|$esc_url|" "$ext_dir/origin.js" || true
      if ! grep -q "__PR_KIOSK_HOME_URL__" "$ext_dir/origin.js"; then
        ext_flags="--load-extension=$ext_dir --disable-extensions-except=$ext_dir"
        log "Installed the kiosk return extension for $KIOSK_URL"
      fi
    fi
  fi
  [ -n "$ext_flags" ] || warn "kiosk return extension not installed; external pages keep no exit"
...
# On the very first boot the wait can cover the remaining provisioning steps
# and the stack pull/build, so the budget is generous; a normal boot passes
# the wait in a single probe.
TimeoutStartSec=900
User=$kuser
PAMName=login
TTYPath=/dev/tty1
...
}# Wait (bounded, best-effort) for provisioning to finish and the app to answer
# before launching the browser. Chromium renders a connection-refused page and
# never retries (FoodAssistant-kyl2), and on the very first boot the setup
# screen used to appear while the stack was still pulling and building, long
# before the install was actually usable. So: while the first-boot provisioner
# is still running, keep waiting; once it is done (or was never involved),
# poll the app's /health until it answers. On a normal boot the marker exists
# and the app is already up, so the first probe passes with zero delay. Always
# exits 0 so a missing curl or an app that stays down never blocks the kiosk.
ExecStartPre=-/bin/sh -c 'command -v curl >/dev/null 2>&1 || exit 0; for i in \$\$(seq 1 420); do if [ -e "$DONE_MARKER" ] || [ "\$\$(systemctl is-active foodassistant-firstboot.service 2>/dev/null)" != "activating" ]; then curl -sf -o /dev/null --max-time 2 "$kiosk_wait_url" && exit 0; fi; sleep 2; done; exit 0'
# --default-background-color: Chromium paints white while a page loads, which
# flashed on every Stream Deck navigation (a full page load) before the dark
# app painted (FoodAssistant-9dhh). Match the app's dark background instead.
ExecStart=$cage_bin -- $chromium_bin --kiosk --noerrdialogs \\
  ...
  --default-background-color=212529 $ext_flags \\
...
EOF
  ...
  # --no-block: the unit's ExecStartPre waits for provisioning to finish, and
  # provisioning is THIS script, so a blocking start would deadlock until the
  # start timeout. Queue the start and move on; the kiosk paints once the
  # remaining steps are done and the app answers.
  systemctl start --no-block foodassistant-kiosk.service || warn "kiosk start failed (will retry on boot)"
...
# Step: boot splash (FoodAssistant-y8vj). Install the Pantry Raider Plymouth
# theme so boot shows the logo instead of console text. Opt-in (default off) and
# FAIL-SAFE at every step: a missing package, missing theme asset, or a tool
# error logs a warning and returns without touching the boot path, so the worst
# case is the existing quiet boot. Only the "splash" cmdline param and the theme
# selection are changed, both reversible. Requires the kiosk (a headless box has
# no screen to splash).
configure_boot_splash() {
  if ! is_true "$ENABLE_BOOT_SPLASH"; then
    log "Boot splash not enabled (ENABLE_BOOT_SPLASH=$ENABLE_BOOT_SPLASH); leaving the plain quiet boot"
    return 0
  fi
  if ! flag_enabled "$ENABLE_KIOSK" has_display; then
    log "Boot splash skipped (no kiosk/display on this device)"
    return 0
  fi
  # Resolve the theme source (boot payload beside this script, or the checkout).
  local theme_src=""
  for candidate in "$ASSET_DIR/plymouth-theme/pantryraider" \
                   "$REPO_DIR/scripts/image-build/plymouth-theme/pantryraider"; do
    [ -d "$candidate" ] && theme_src="$candidate" && break
  done
  if [ -z "$theme_src" ]; then
    warn "Boot splash: theme assets not found; leaving the plain quiet boot"
    return 0
  fi
  if [ "$DRY_RUN" = "1" ]; then
    log "DRY_RUN would install the Pantry Raider Plymouth boot splash from $theme_src"
    return 0
  fi
  # Plymouth is not on Pi OS Lite by default; install best-effort.
  if ! command -v plymouth-set-default-theme >/dev/null 2>&1; then
    apt_install plymouth plymouth-themes || {
      warn "Boot splash: could not install plymouth; leaving the plain quiet boot"
      return 0
    }
  fi
  command -v plymouth-set-default-theme >/dev/null 2>&1 || {
    warn "Boot splash: plymouth tools still missing after install; skipping"
    return 0
  }
  local dst="/usr/share/plymouth/themes/pantryraider"
  mkdir -p "$dst" 2>/dev/null || { warn "Boot splash: could not create $dst; skipping"; return 0; }
  cp -f "$theme_src/pantryraider.plymouth" "$theme_src/pantryraider.script" \
        "$theme_src/logo.png" "$dst/" 2>/dev/null || {
    warn "Boot splash: could not copy theme files; skipping"
    return 0
  }
  # Select the theme. -R rebuilds the initramfs so the splash is available
  # early; if that fails, undo the selection so a half-applied theme cannot
  # leave a black screen, and fall back to quiet boot.
  if ! plymouth-set-default-theme -R pantryraider >/dev/null 2>&1; then
    warn "Boot splash: theme rebuild failed; reverting to the default theme"
    plymouth-set-default-theme -R $(plymouth-set-default-theme --list 2>/dev/null | head -1) >/dev/null 2>&1 || true
    return 0
  fi
  # Add the "splash" cmdline param (quiet is already added by configure_quiet_boot).
  local cmdline="" f
  for f in $CMDLINE_CANDIDATES; do [ -f "$f" ] && cmdline="$f" && break; done
  if [ -n "$cmdline" ]; then
  ...
  log "Boot splash installed: the Pantry Raider logo shows during boot (takes effect on the next reboot)"
}

# Step: framebuffer boot splash (FoodAssistant-y8vj). Installs a oneshot unit
# that paints the Pantry Raider mark to /dev/fb0 as soon as the root fs is up,
# so the display shows the brand instead of boot text until the kiosk starts
# (the compositor simply takes the framebuffer over). Lightweight on purpose:
# no Plymouth, no initramfs rebuild, no new packages. FAIL-SAFE at every step:
# missing sources, a failed install, or no framebuffer at boot all skip
# silently, so the worst case is the existing quiet boot.
configure_fb_splash() {
  if ! is_true "$ENABLE_FB_SPLASH"; then
    log "Framebuffer splash not enabled (ENABLE_FB_SPLASH=$ENABLE_FB_SPLASH); leaving the plain quiet boot"
    return 0
  fi
  if ! flag_enabled "$ENABLE_KIOSK" has_display; then
    log "Framebuffer splash skipped (no kiosk/display on this device)"
    return 0
  fi
  # Resolve the three pieces from the boot payload or the checkout.
  local writer="" unit="" asset="" candidate
  for candidate in "$ASSET_DIR/foodassistant-boot-splash" \
                   "$REPO_DIR/scripts/image-build/foodassistant-boot-splash"; do
    [ -f "$candidate" ] && writer="$candidate" && break
  done
  for candidate in "$ASSET_DIR/foodassistant-boot-splash.service" \
                   "$REPO_DIR/scripts/image-build/foodassistant-boot-splash.service"; do
    [ -f "$candidate" ] && unit="$candidate" && break
  done
  for candidate in "$ASSET_DIR/boot-splash/splash.ppm.gz" \
                   "$REPO_DIR/scripts/image-build/boot-splash/splash.ppm.gz"; do
    [ -f "$candidate" ] && asset="$candidate" && break
  done
  if [ -z "$writer" ] || [ -z "$unit" ] || [ -z "$asset" ]; then
    warn "Framebuffer splash sources not found; leaving the plain quiet boot"
    return 0
  fi
  if [ "$DRY_RUN" = "1" ]; then
    log "DRY_RUN would install the framebuffer boot splash (writer, unit, splash image)"
    return 0
  fi
  install -m 755 "$writer" /usr/local/bin/foodassistant-boot-splash 2>/dev/null || {
    warn "Framebuffer splash: could not install the writer; leaving the plain quiet boot"
    return 0
  }
  # Fixed path: the unit's ConditionPathExists points here.
  mkdir -p /opt/foodassistant/boot-splash 2>/dev/null || true
  install -m 644 "$asset" /opt/foodassistant/boot-splash/splash.ppm.gz 2>/dev/null || {
    warn "Framebuffer splash: could not install the splash image; leaving the plain quiet boot"
    return 0
  }
  install -m 644 "$unit" /etc/systemd/system/foodassistant-boot-splash.service 2>/dev/null || {
    warn "Framebuffer splash: could not install the unit; leaving the plain quiet boot"
    return 0
  }
  systemctl daemon-reload 2>/dev/null || true
  systemctl enable foodassistant-boot-splash.service 2>/dev/null \
    || warn "could not enable foodassistant-boot-splash.service"
  log "Framebuffer splash installed: the Pantry Raider mark shows while the device starts (from the next reboot)"
}

# Step: I2C bus for plug-in accessories (FoodAssistant-dxr22). Unconditional,
# NOT gated on ENABLE_GADGETS: the bus has to be on before anything can detect
# an accessory, and gating it on the reader opt-in is exactly how a fresh
# flash shipped with a dead NeoKey (the pane then blamed the cable). All three
# writes are idempotent and harmless on a box with nothing plugged in; the
# same commands live in foodassistant-gadgets-setup for manual installs.
configure_i2c() {
  if [ "$DRY_RUN" = "1" ]; then
    log "DRY_RUN would enable the I2C bus (dtparam=i2c_arm=on, i2c-dev module, i2c-tools)"
    return 0
  fi
  ...
  # Load the module now so first boot works without a reboot, and on boot.
  ...
  # i2cdetect is how ENABLE_GADGETS=auto looks for a plugged-in accessory
  # (and the standard debug tool when one misbehaves). Best-effort: without
  # it, auto simply resolves to off.
  command -v i2cdetect >/dev/null 2>&1 \
    || apt_install i2c-tools >/dev/null 2>&1 \
    || warn "could not install i2c-tools; ENABLE_GADGETS=auto will resolve to off"
  return 0
}

# Step: Bluetooth thermometer and accessory reader. Default "auto": installs
# when a plug-in accessory answers on the I2C bus (mirrors the Stream Deck's
# detect-and-decide); force with ENABLE_GADGETS=true/false.
configure_gadgets() {
  if ! flag_enabled "$ENABLE_GADGETS" has_i2c_device; then
    log "Sensor reader not enabled (ENABLE_GADGETS=$ENABLE_GADGETS, no plug-in accessory detected); skipping"
    return 0
  fi
  local setup="$REPO_DIR/scripts/image-build/foodassistant-gadgets-setup"
  if [ ! -f "$setup" ]; then
    warn "foodassistant-gadgets-setup not found in the checkout; skipping"
    return 0
  fi
  if [ "$DRY_RUN" = "1" ]; then
    log "DRY_RUN would run foodassistant-gadgets-setup"
    return 0
  fi
  log "Installing the Bluetooth thermometer reader"
  REPO_DIR="$REPO_DIR" bash "$setup" || warn "gadgets setup failed (re-run with STEPS=gadgets)"
}
...
configure_streamdeck() {
  ...
  # streamdeck/requirements.txt is the one place these pins live. Firstboot can
  # also run from a boot payload with no repo checked out, so an inline copy
  # stays as the fallback; keep the two in step.
  local sd_req=""
  for candidate in "$ASSET_DIR/streamdeck-requirements.txt" \
                   "$REPO_DIR/streamdeck/requirements.txt"; do
    [ -f "$candidate" ] && sd_req="$candidate" && break
  done
  log "Installing Python dependencies into venv"
  run "$venv_dir/bin/pip" install --quiet --upgrade pip
  if [ -n "$sd_req" ]; then
    log "Using Stream Deck requirements from $sd_req"
    run "$venv_dir/bin/pip" install --quiet -r "$sd_req"
  else
    warn "streamdeck/requirements.txt not found; installing the inline pin list"
  ...
  # The venv and the package stay owned by root and world-readable. Root runs
  # python out of this venv (the host bridge asks the controller for its key
  # layout and its action list), so anything the desktop account could write
  # here would be code executed as root the next time that happens. The
  # controller only ever reads these paths; its writable state comes from the
  # unit's StateDirectory below. This also re-secures a device provisioned
  # before the ownership was tightened.
  if [ "$DRY_RUN" != "1" ]; then
    chown -R root:root "$sd_dst" "$venv_dir" 2>/dev/null || true
    chmod -R a+rX "$sd_dst" "$venv_dir" 2>/dev/null || true
  fi
...
RestartSec=5
User=$sd_user
# The program directory and the venv are read-only to this service (they are
# root-owned); anything it needs to write goes here, created and handed over
# by systemd at start.
StateDirectory=foodassistant-streamdeck
${sd_base_env}
...
# activates a fallback hotspot (default SSID: FoodAssistant) whenever the
# device has no network: the watchdog is a continuous loop, so the hotspot
# rises when connectivity is lost at ANY point in the uptime, not just at
# boot, and stands itself down when the network returns. The AP is NOT
# started at provisioning; it only activates on failure, so a device that
# connects normally is unaffected.
#
# The hotspot passphrase is PER DEVICE (FoodAssistant-fgh3). Every image used
# to share one passphrase that also sat in the public repo, which made the
# recovery hotspot effectively open: anyone in radio range could join a
# stranded (or brand-new, not-yet-configured) appliance and reach its setup
# pages. Each device now gets its own random passphrase at provisioning time.
# The broadcast SSID is user-settable (Settings, Network, or an AP_SSID seed
# in config.env); the DEFAULT stays the internal identifier FoodAssistant.

# Where the per-device hotspot passphrase is persisted (root-only). A re-run
# of this step reuses it, so the password an owner wrote down keeps working.
AP_PASSPHRASE_FILE="${AP_PASSPHRASE_FILE:-/etc/foodassistant/ap-passphrase}"
# Where a user-chosen hotspot SSID is persisted (the app's settings page
# writes it through the host bridge). Absent means the default name.
AP_SSID_FILE="${AP_SSID_FILE:-/etc/foodassistant/ap-ssid}"

# Generate a random hotspot passphrase: 12 characters in xxxx-xxxx-xxxx form,
# from a lowercase alphabet with the easily-confused characters (i, l, o, 0, 1)
# removed, so it is easy to type on a phone. Prints it on stdout; non-zero if
# /dev/urandom somehow yields nothing usable.
gen_ap_passphrase() {
  local raw="" tries=0
  while [ ${#raw} -lt 12 ] && [ $tries -lt 10 ]; do
    raw="$raw$(head -c 256 /dev/urandom | tr -dc 'abcdefghjkmnpqrstuvwxyz23456789' || true)"
    tries=$((tries + 1))
  done
  [ ${#raw} -ge 12 ] || return 1
...
# Whether a candidate hotspot SSID is usable: 1..32 bytes (the 802.11 limit
# is bytes, not characters) with no control characters, so it can never
# smuggle extra lines into the line-based hostapd config.
ap_ssid_ok() {
  local s="$1" n
  n=$(printf '%s' "$s" | LC_ALL=C wc -c)
  [ "$n" -ge 1 ] && [ "$n" -le 32 ] || return 1
  if printf '%s' "$s" | LC_ALL=C grep -q '[[:cntrl:]]'; then return 1; fi
  return 0
}

# Resolve the hotspot SSID for this device and persist it, mirroring the
# passphrase precedence: an owner-chosen AP_SSID from config.env, then an SSID
# a user set from the app's settings page (stored at AP_SSID_FILE, so it
# survives a re-provision), then the default FoodAssistant. An invalid value
# warns and falls through rather than leaving the hotspot unnamed. Prints the
# SSID on stdout; always succeeds (the default is always available).
ap_fallback_ssid() {
  local ssid="${AP_SSID:-}"
  if [ -n "$ssid" ] && ! ap_ssid_ok "$ssid"; then
    warn "AP_SSID must be 1..32 bytes of printable text; ignoring it and using the stored or default hotspot name"
    ssid=""
  fi
  if [ -z "$ssid" ] && [ -s "$AP_SSID_FILE" ]; then
    ssid="$(head -n 1 "$AP_SSID_FILE" 2>/dev/null | tr -d '\r' || true)"
    ap_ssid_ok "$ssid" || ssid=""
  fi
  [ -n "$ssid" ] || ssid="FoodAssistant"
  mkdir -p "$(dirname "$AP_SSID_FILE")"
  printf '%s\n' "$ssid" > "$AP_SSID_FILE" 2>/dev/null || true
  printf '%s\n' "$ssid"
}

# Resolve the hotspot passphrase for this device and persist it. Order: an
# owner-chosen AP_PASSPHRASE from config.env, then a passphrase from an
# earlier provisioning run, then a freshly generated one. It is stored
# root-only at AP_PASSPHRASE_FILE and mirrored to pantry-raider-hotspot.txt on
# the boot partition, so a headless owner can recover it by putting the SD
# card in any computer. (Anyone holding the card physically owns the device
# already, so the mirror adds no exposure a stolen card did not.) The optional
# first argument is the SSID the mirror should show (default FoodAssistant).
# Prints the passphrase on stdout; returns non-zero when none could be
# established.
ap_fallback_passphrase() {
  local mirror_ssid="${1:-FoodAssistant}"
  local pass="${AP_PASSPHRASE:-}"
  # An owner-supplied passphrase must be a valid WPA2 length. If it is not,
  # warn and fall through to a stored/generated one rather than leaving a
  # headless device with NO recovery hotspot at all (fail safe, not fail
  # dark). WPA2 requires 8..63 characters.
  if [ -n "$pass" ]; then
    local on=${#pass}
    if [ "$on" -lt 8 ] || [ "$on" -gt 63 ]; then
      warn "AP_PASSPHRASE must be 8..63 characters; ignoring it and using a generated hotspot password"
      pass=""
    fi
  fi
  if [ -z "$pass" ] && [ -s "$AP_PASSPHRASE_FILE" ]; then
    # Strip any CR so a hand-edited (CRLF) file never desyncs from what the
    # owner types on their phone.
    pass="$(head -n 1 "$AP_PASSPHRASE_FILE" 2>/dev/null | tr -d '\r' || true)"
  fi
  if [ -z "$pass" ]; then
    pass="$(gen_ap_passphrase)" || pass=""
  fi
  local n=${#pass}
  if [ "$n" -lt 8 ] || [ "$n" -gt 63 ]; then
    return 1
  fi
  mkdir -p "$(dirname "$AP_PASSPHRASE_FILE")"
  (umask 077; printf '%s\n' "$pass" > "$AP_PASSPHRASE_FILE") || return 1
  # Tighten unconditionally: the redirection above preserves the mode of a
  # pre-existing file, so re-assert root-only even if it was created loosely.
  chmod 600 "$AP_PASSPHRASE_FILE" 2>/dev/null || true
  local bootdir
  # shellcheck disable=SC2086  # word splitting of the candidate list is intended
  for bootdir in ${AP_BOOTDIR_CANDIDATES:-/boot/firmware /boot}; do
    if [ -d "$bootdir" ]; then
      # Only stop at a candidate we could actually write, so a read-only
      # boot mount does not swallow the recovery mirror silently.
      if printf 'Pantry Raider recovery hotspot\nSSID: %s\nPassword: %s\n' "$mirror_ssid" "$pass" \
        > "$bootdir/pantry-raider-hotspot.txt" 2>/dev/null; then
        break
      fi
    fi
  done
...
configure_wifi_ap_fallback() {
  ...
  if [ "$DRY_RUN" = "1" ]; then
    log "DRY_RUN would install hostapd dnsmasq, set a per-device AP passphrase, and write watchdog service"
    return 0
  fi
  ...
  local ap_ssid ap_pass
  ap_ssid="$(ap_fallback_ssid)" || ap_ssid="FoodAssistant"
  ap_pass="$(ap_fallback_passphrase "$ap_ssid")" || ap_pass=""
  if [ -z "$ap_pass" ]; then
    warn "could not establish a per-device AP passphrase; leaving the fallback AP unconfigured"
    return 0
  fi

  # hostapd configuration: WPA2 personal on wlan0, channel 6, AES/CCMP only
  # (no TKIP), per-device passphrase. Root-only: the file holds the passphrase.
  mkdir -p /etc/hostapd
...
EOF
  ...
  # Watchdog script: a continuous loop that only falls back to the setup
  ...
  # starting the AP (FoodAssistant-8ah5), debounced over three consecutive
  # failed checks so a rebooting router does not raise it. The script itself
  # lives in the repo (scripts/image-build/foodassistant-ap-watchdog) so OTA
  # updates can refresh it; it stays installed at /usr/local/sbin because
  # every deployed unit file points there (FoodAssistant-fuat).
  local wd_src=""
  ...
  # Watchdog service: the unit ships in the repo next to the script (it is a
  # plain always-restarted service now that the script loops), the same
  # two-candidate pattern the host bridge unit uses.
  local wu_src=""
  for candidate in "$ASSET_DIR/foodassistant-ap-watchdog.service" \
                   "$REPO_DIR/scripts/image-build/foodassistant-ap-watchdog.service"; do
    [ -f "$candidate" ] && wu_src="$candidate" && break
  done
  if [ -z "$wu_src" ]; then
    warn "foodassistant-ap-watchdog.service not found; Wi-Fi fallback AP unavailable"
    return 0
  fi
  install -m 644 "$wu_src" /etc/systemd/system/foodassistant-ap-watchdog.service

  systemctl daemon-reload
  systemctl enable foodassistant-ap-watchdog.service \
    || warn "ap-watchdog service enable failed"
  log "Wi-Fi fallback AP watchdog installed (raises the hotspot whenever the device has no network, any time after boot)"
  log "Recovery hotspot: SSID $ap_ssid with a password unique to this device."
  log "The password is in pantry-raider-hotspot.txt on the SD card's boot partition and in $AP_PASSPHRASE_FILE (root-only)."
}
...
seed_app_settings() {
  ...
  # name like "grocy" or an mDNS host like pr.local does not resolve
  ...
  atomic_write "$settings_file" "{\"deployment_mode\": \"$mode\", \"has_streamdeck\": $sd_val${extra}}
"
...
deploy_remote_service() {
  ...
  if [ "$DRY_RUN" = "1" ]; then
    ...
    # Nothing here turns the password off. A second screen in the kitchen sits
    # on the same network as everything else, so it asks for a password like
    # every other install and parks on the setup wizard until one is set.
    # Pinning the requirement off here used to leave these devices open to
    # anyone on the network, and an env var beats anything the wizard saves
    # later, so the wizard could not have fixed it.
    cat > "$env_file" <<EOF
...
EOF
    # Only pin REMOTE_SERVER_URL when we actually have one. env_ignore_empty
    # in the app's settings config is what makes a blank env var harmless, so
    # a blank line here would no longer shadow the value the web wizard saved
    # to settings.json; this stays conditional as belt and braces for a device
    # whose app predates that setting. When blank, settings.json is the single
    # source.
  ...
  # The service runs as the primary (non-root) user. It needs to READ the app
  # and the venv, and to WRITE only its data dir (settings.json is saved from
  # the wizard) and the Stream Deck config. The venv stays owned by root: the
  # host bridge runs python from it as root, so a venv the service user could
  # write to would be a way to run code as root. The chown is deliberately
  # narrow for that reason, and the chown to root also re-secures a device
  # provisioned before this change.
  if [ "$DRY_RUN" != "1" ] && [ -n "$sd_user" ]; then
    run mkdir -p "$INSTALL_DIR/data"
    run chown -R root:root "$venv_dir" 2>/dev/null || true
    run chmod -R a+rX "$venv_dir" "$app_dir" 2>/dev/null || true
    run chown -R "$sd_user":"$sd_user" "$INSTALL_DIR/data" 2>/dev/null \
      || chown -R "$sd_user" "$INSTALL_DIR/data" 2>/dev/null \
      || warn "could not chown $INSTALL_DIR/data to $sd_user; service may not be able to save settings"
    if [ -f "$INSTALL_DIR/config.toml" ]; then
      chown "$sd_user" "$INSTALL_DIR/config.toml" 2>/dev/null || true
    fi
...
# --no-access-log: an idle kiosk generates ~120 requests/minute of polling,
# and every one became a journald line, which is the single biggest steady
# writer on the SD card (FoodAssistant-7dt9). Errors and app logging still
# land in the journal, and the in-app debug logging (Settings, Support) gives
# full request detail on demand when troubleshooting.
ExecStart=$exec_uvicorn app.main:app --host 0.0.0.0 --port 80 --timeout-graceful-shutdown 5 --no-access-log
Restart=on-failure
RestartSec=5
# A stop or restart (every OTA update runs one) must not hang waiting for the
# kiosk's long-poll/SSE connections (timers, events, camera snapshot) to drain.
# The uvicorn flag above caps its graceful drain at 5s; this gives systemd a
# hard 10s stop timeout as a backstop, so an update restarts promptly instead
# of sitting ~90s in 'deactivating' (FoodAssistant-wtjw).
TimeoutStopSec=10
KillMode=mixed
...
main() {
  ...
  # After load_config: the check depends on the resolved deployment mode.
  check_arch_64bit
  ...
  _step_requested "hostname"    && { resolve_unique_hostname; configure_hostname; }
  ...
  if is_remote_mode; then
    ...
    _step_requested "sd_resilience"    && configure_sd_resilience
    _step_requested "zram"             && configure_zram
    _step_requested "multihome"        && configure_multihome_sysctl
    ...
    _step_requested "i2c"            && configure_i2c
    _step_requested "gadgets"        && configure_gadgets
    _step_requested "quiet_boot"     && configure_quiet_boot
    _step_requested "boot_splash"    && configure_boot_splash
    _step_requested "fb_splash"      && configure_fb_splash
    [ -z "$STEPS" ] && mark_done
...
# FIRSTBOOT_SOURCE_ONLY=1 loads the helpers without provisioning, which is how
# the tests exercise one function directly. Nothing on a device ever sets it.
if [ "${FIRSTBOOT_SOURCE_ONLY:-0}" != "1" ]; then
  main "$@"
fi
