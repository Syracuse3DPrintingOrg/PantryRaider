...
$SCRIPT_DIR/foodassistant-boot-splash
$SCRIPT_DIR/foodassistant-boot-splash.service
"
...
install_payload() {
  ...
  # The boot splash image lives in a subdir so firstboot's ASSET_DIR lookup
  # (boot-splash/splash.ppm.gz) matches the checkout layout.
  mkdir -p "$boot/foodassistant-setup/boot-splash"
  cp -f "$SCRIPT_DIR/boot-splash/splash.ppm.gz" "$boot/foodassistant-setup/boot-splash/"
